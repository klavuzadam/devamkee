...
class SelectCharacterWindow(ui.Window):
	...
	HAIR_DATA = (
		[ 2, 2, 3, 4, 5, 21, 22, 23, 24, 31, 32, 33, 34, 41, 42, 43, 44, 45], ## Warrior
		[ 1, 2, 3, 4, 5, 21, 22, 23, 24, 31, 32, 33, 34, 41, 42, 43, 44, 45], ## Assassin
		[ 1, 2, 3, 4, 5, 21, 22, 23, 24, 31, 32, 33, 34, 41, 42, 43, 44, 45], ## Sura
		[ 1, 2, 3, 4, 5, 21, 22, 23, 24, 31, 32, 33, 34, 41, 42, 43, 44, 45], ## Shaman
	)
	
	HAIR_ICON = (
		"icon/item/73001.tga", "icon/item/73251.tga", "icon/item/73501.tga", "icon/item/73751.tga"
	)
	...
	FACE_IMAGE_DICT_3 = {
						...
						#playerSettingModule.RACE_WOLFMAN_M : "icon/face/wolf_w.tga",
					}
	...
	def Open(self):
		...
		empire_w = 0
		if net.GetEmpireID() == 2:
			empire_w = 3
		else:
			empire_w = net.GetEmpireID()

		self.SetEmpire(empire_w)
		app.ShowCursor()
		self.onClickKeyDict = {}
		self.onClickKeyDict[app.DIK_RETURN] = lambda : self.StartGame()
		self.onClickKeyDict[app.DIK_LEFT] = lambda : self.ChangeSlot(self.slot-1)
		self.onClickKeyDict[app.DIK_RIGHT] = lambda : self.ChangeSlot(self.slot+1)
		self.SetFocus()

	def ChangeSlot(self, arg):
		if arg <0:
			self.SelectSlot(4)
		elif arg > 3:
			self.SelectSlot(0)
	...
	def __LoadBoardDialog(self, fileName):
		...
		try:
			...
			self.hairBoard = GetObject("character_selecthairboard")
			self.hairPrev = GetObject("hair_prev_button")
			self.hairNext = GetObject("hair_next_button")
			self.hairIcon = GetObject("HairIcon")
		...
		self.hairPrev.SetEvent(ui.__mem_func__(self.PrevHair))
		self.hairNext.SetEvent(ui.__mem_func__(self.NextHair))
		...
		GetObject("hair_prev_button").ShowToolTip = lambda arg=10: self.OverInButton(arg)
		GetObject("hair_prev_button").HideToolTip = lambda arg=10: self.OverOutButton()
		GetObject("hair_next_button").ShowToolTip = lambda arg=11: self.OverInButton(arg)
		GetObject("hair_next_button").HideToolTip = lambda arg=11: self.OverOutButton()
	...
	def StartGame(self):
		...
		#new select save face
		if constInfo.LoginAndFace != "":
			for i in xrange(4):
				if not os.path.exists("lib/account_"+str(i+1)+".cfg"):
					continue
			
				lines = open("lib/account_"+str(i+1)+".cfg", "r").readlines()
				try:
					user = lines[0].replace("\n","")
					pwd = lines[1].replace("\n","")
				except:
					user = ""
				if user+"/"+pwd == constInfo.LoginAndFace.replace("\n",""):
					race=net.GetAccountCharacterSlotDataInteger(self.slot, net.ACCOUNT_CHARACTER_SLOT_RACE)
					# job = chr.RaceToJob(race)
					f = open("lib/account_" + str(i+1) + ".cfg", "w")
					f.write(user +"\n"+pwd+"\n"+str(race))
					f.close()
					
		self.dlgBoard.GetChild("character_board").Hide()
		self.dlgBoard.GetChild("select_thinboard2").Hide()
		self.dlgBoard.GetChild("character_discriptionboard").Hide()
		self.dlgBoard.GetChild("SelectJobSlot").Hide()
		self.btnStart.Hide()
		self.btnCreate.Hide()
		self.btnDelete.Hide()
		self.btnExit.Hide()
		...
		windows = self.NameList + self.GaugeList
		for item in windows:
			item.Hide()
		...
		self.CharacterSlot_3.Hide()
		self.CharacterSlot_4.Hide()
		self.dlgBoard.GetChild("prev_button").Hide()
		self.dlgBoard.GetChild("next_button").Hide()
	...
	def SelectSlot(self, index):
		...
		self.hairIndex = -1
		if hair in self.HAIR_DATA[chr.RaceToJob(race)]:
			for hairIndex in self.HAIR_DATA[chr.RaceToJob(race)]:
				if hairIndex == hair:
					self.hairIndex = hair
					break
					
		self.hairIcon.LoadImage(self.HAIR_ICON[chr.RaceToJob(race)])
	...
	def NextHair(self):
		...
		net.SendChangeCharacterHairPacket(self.slot, self.HAIR_DATA[chr.RaceToJob(chr.GetRace())][self.hairIndex])
	...
	def OverInButton(self, stat):
		if stat == 1:
			...
			# self.toolTip.AutoAppendNewTextLine(uiScriptLocale.SELECT_PLAYTIME % (int(playTime)), grp.GenerateColor(1.0, 1.0, 0.0, 1.0))
			self.toolTip.Show()
		elif stat == 10:
			self.toolTip.ClearToolTip()
			self.toolTip.AlignHorizonalCenter()
