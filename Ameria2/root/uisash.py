...
class CombineWindow(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		self.btnCombineAll = ui.MakeButton(self, 17, 265, False, "d:/ymir work/ui/public/", "xlarge_button_01.sub", "xlarge_button_02.sub", "xlarge_button_03.sub")
		self.btnCombineAll.SetText("Automatic Combination")
		self.btnCombineAll.SetEvent(ui.__mem_func__(self.OnCombineFast))

	def OnCombineFast(self):
		(isHere, iCell) = sash.GetAttachedItem(0)
		(isHere, iCell2) = sash.GetAttachedItem(1)
		iVnum = player.GetItemIndex(iCell)
		
		if iVnum == 85004 or iVnum == 85008 or iVnum == 85014 or iVnum == 85018:
			chat.AppendChat(1, "You cant do this for this type of acces.")
			return
		
		if iCell == 0 and iCell2 == 0:
			chat.AppendChat(1, "You must add sashes in slot to perform this command.")
			return
		...
		for slotNumber in xrange(180):
			...
			if iPos1 != -1 and iPos2 != -1:
				sash.Add(player.INVENTORY, iPos1, 0)
				sash.Add(player.INVENTORY, iPos2, 1)
