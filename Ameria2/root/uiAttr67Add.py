...
class Attr67AddWindow(ui.ScriptWindow):
	...
	def SelectEmptyRegistSlot(self, slot_index):
		...
		if app.USE_BL_67_ATTR_RENEWED:
			MaterialVnum = self.GetMaterialVnum(item.GetItemType(), item.GetItemSubType())
		else:
			level_limit = 0

			#if app.ENABLE_YOHARA_SYSTEM:
			conqueror_level_limit = 0

			for i in range(item.LIMIT_MAX_NUM):
				(limitType, limitValue) = item.GetLimit(i)
				if item.LIMIT_LEVEL == limitType:
					if app.ENABLE_YOHARA_SYSTEM and level_limit == 0:
						level_limit = limitValue

					if not app.ENABLE_YOHARA_SYSTEM:
						break
				elif app.ENABLE_YOHARA_SYSTEM and limitType == item.LIMIT_CONQUEROR_LEVEL:
					conqueror_level_limit = limitValue

			MaterialVnum = self.GetMaterialVnum(level_limit, conqueror_level_limit)
	...
	if app.USE_BL_67_ATTR_RENEWED:
		def GetMaterialVnum(self, itemType, itemSubType):
			if itemType == item.ITEM_TYPE_WEAPON:
				allowedSubTypesTupple = (\
					item.WEAPON_SWORD,
					item.WEAPON_DAGGER,
					item.WEAPON_BOW,
				...
				if app.ENABLE_WOLFMAN_CHARACTER:
					allowedSubTypesTupple += (item.WEAPON_CLAW,)

				if itemSubType in allowedSubTypesTupple:
					return 39071
			elif itemType == item.ITEM_TYPE_ARMOR:
				if itemSubType == item.ARMOR_BODY:
					return 39072
				elif itemSubType == item.ARMOR_HEAD:
					return 39074
				elif itemSubType == item.ARMOR_SHIELD:
					return 39075
				elif itemSubType == item.ARMOR_FOOTS:
					return 39076
				elif app.ENABLE_YOHARA_SYSTEM and\
					itemSubType == item.ARMOR_GLOVE:
					return 39081
				else:
					allowedSubTypesTupple = (\
						item.ARMOR_WRIST,
		...
		#app.ENABLE_YOHARA_SYSTEM: conqueror_level_limit
		def GetMaterialVnum(self, item_level, conqueror_level_limit):
			if conqueror_level_limit > 0:
				return 39081
