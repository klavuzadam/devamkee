...
class DragonSoulWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		if app.ENABLE_DS_SET_RENEWAL:
			self.RefreshDSSet()
			self.bnsSetNo.SAFE_SetStringEvent("MOUSE_OVER_IN", self.OnBnsSetIn)
			self.bnsSetNo.SAFE_SetStringEvent("MOUSE_OVER_OUT", self.OnBnsSetOut)
			self.bnsSetYes.SAFE_SetStringEvent("MOUSE_OVER_IN", self.OnBnsSetIn)
			self.bnsSetYes.SAFE_SetStringEvent("MOUSE_OVER_OUT", self.OnBnsSetOut)
	...
	if app.ENABLE_DS_SET_RENEWAL:
		...
		def SetDSSet(self, idx):
			self.setStat = idx
			self.RefreshDSSet()
			#chat.AppendChat(1, "sloboz set %d" % idx)
	...
	def ShowActivateButtonTooltip(self):
		if self.toolTip:
			arglen = len(localeInfo.USE_CONSUMABLE_TOOLTIP)
			self.toolTip.ClearToolTip()
			self.toolTip.SetThinBoardSize(4 * arglen)
			self.toolTip.AppendTextLine(localeInfo.USE_DS_PAGE)
			self.toolTip.AppendSpace(5)
			self.toolTip.AppendTextLine(localeInfo.USE_DS_PAGE2)
			self.toolTip.AppendSpace(5)
			self.toolTip.AppendTextLine(localeInfo.USE_CONSUMABLE_TOOLTIP)
			self.toolTip.AppendSpace(5)
			self.toolTip.Show()

	def OnUpdate(self):
		if self.toolTip:
			if self.activateButton.IsIn():
				self.ShowActivateButtonTooltip()
			else:
				if self.toolTip:
					self.toolTip.Hide()
	...
	def ShowToolTip(self, window_type, slotIndex):
		if None != self.tooltipItem:
			if player.INVENTORY == window_type:
				...
				if app.ENABLE_IKASHOP_RENEWAL:
					self.AppendIkashopSellToolTip(player.DRAGON_SOUL_INVENTORY, slotIndex)
	...
	def UseItemSlot(self, slotIndex):
		...
		if app.ENABLE_IKASHOP_RENEWAL:
			board = ikashop.GetBusinessBoard()
			if board:
				item.SelectItem(player.GetItemIndex(player.DRAGON_SOUL_INVENTORY, slotIndex))
				if not item.IsAntiFlag(item.ANTIFLAG_GIVE) and not item.IsAntiFlag(item.ANTIFLAG_MYSHOP):
					if board.IsCreatingAuction():
						ikashop.AddItemOnAuction(player.DRAGON_SOUL_INVENTORY, slotIndex)
						return
					elif board.IsShow():
						ikashop.AddItemOnShop(player.DRAGON_SOUL_INVENTORY, slotIndex)
						return

		if app.IsPressed(app.DIK_LALT):
			link = player.GetItemLink(player.DRAGON_SOUL_INVENTORY, slotIndex)
			ime.PasteString(link)
			return
	...
	def ActivateSelectedDeck(self, newPage):
		self.deckPageIndex = newPage
...
class DragonSoulRefineWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.doOpenHelperButton = ui.MakeButton(self, 188-90, 200, False, "d:/ymir work/ui/dragonsoul/", "l_button01.tga", "l_button02.tga", "l_button03.tga")
			self.doOpenHelperButton.SetText("Fast Refine")
		...
		self.doOpenHelperButton.SetEvent(self.OpenHelperWindow)
	...
	def OpenHelperWindow(self):
		if not self.wndHelper:
			self.wndHelper = uiCommon.DSAssistmentWindow()
			
		if self.wndHelper.IsShow():
			self.wndHelper.Hide()
		else:
			self.wndHelper.Show()
