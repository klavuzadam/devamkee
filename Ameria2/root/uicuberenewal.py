		...
		for i in xrange(1,6):
			if self.GetVnumMaterial(i) != 0:
				...
				if app.ENABLE_SPECIAL_STORAGE:
					...
					if player.GetSpecialChestItemCount(self.GetVnumMaterial(i)) >= self.GetCountMaterial(i):
						number = player.GetSpecialChestItemCount(self.GetVnumMaterial(i)) / (self.GetCountMaterial(i))
						count_items.append(number)
...
class CubeRenewalWindows(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.categoria = [
			...
			[12,"BPVM",localeInfo.cube_12],
			[13,"BPVP",localeInfo.cube_13],
			[14,"PREMIUM",localeInfo.cube_14],
			[15,"STONES",localeInfo.cube_15],
			[16,"REINFORCEMENTS",localeInfo.cube_16],
			[17,"ENERGYCRISTAL",localeInfo.cube_17],
			[18,"6AND7",localeInfo.cube_18],
			[19,"BOOSTER",localeInfo.cube_19],
			[20,"AURA",localeInfo.cube_20],
			[21,"WEPOWNR",localeInfo.cube_21],
			[22,"COSTUMER",localeInfo.cube_22],
			[23,"COSTUMEH",localeInfo.cube_23],
			[24,"COSTUMESASH",localeInfo.cube_24],
			[25,"COSTUMEMOUNT",localeInfo.cube_25],
			[26,"COSTUMEEFFECT",localeInfo.cube_26],
			[27,"GLOVES",localeInfo.cube_27],
			[28,"SERPENTW",localeInfo.cube_28],
			[29,"SERPENTA",localeInfo.cube_29],
			[30,"SERPENTH",localeInfo.cube_30],
			[31,"SERPENTAC",localeInfo.cube_31],
			[32,"SERPENTGL",localeInfo.cube_32],
			[33,"PMOUNTPET",localeInfo.cube_33],
			[34,"ZODIACW",localeInfo.cube_34],
			[35,"ZODIACA",localeInfo.cube_35],
			[36,"YOHARAUPGRADE",localeInfo.cube_36],
			[37,"UPPGRADE",localeInfo.cube_37],
	...
	def LoadWindow(self):
		...
		self.icons_items.SetSelectItemSlotEvent(ui.__mem_func__(self.UseMaterialSearch))
		...
		for i in xrange(1,6):
			if self.selectedItem.GetVnumMaterial(i) != 0:
				if app.ENABLE_SPECIAL_STORAGE:
					...
					elif  player.GetSpecialChestItemCount(self.selectedItem.GetVnumMaterial(i)) >= (self.selectedItem.GetCountMaterial(i)*count):
						self.cube_elements["count_elements_%d"%i].SetPackedFontColor(grp.GenerateColor(0.5411, 0.7254, 0.5568, 1.0))
				...
				if app.ENABLE_SPECIAL_STORAGE:
					...
					elif player.GetSpecialChestItemCount(self.selectedItem.GetVnumMaterial(i)) > 0:
						self.cube_elements["count_elements_%d"%i].SetText("%d/%d"%(player.GetSpecialChestItemCount(self.selectedItem.GetVnumMaterial(i)),self.selectedItem.GetCountMaterial(i)*count))
					else:
	...
	def UseMaterialSearch(self, index):
		if self.selectedItem:
			if index == 0:
		...
		if app.IsPressed(app.DIK_LALT) and app.ENABLE_IKASHOP_RENEWAL:
			...
			if app.USE_OFFLINESHOP_SEARCH_V2:
				if self.interface:
					self.interface.ShopSearchFastItem(vnum, item.GetItemName())
			else:
	...
	def OverInItem(self, index):
		...
		self.toolTip.SearchShopKeyBind()
