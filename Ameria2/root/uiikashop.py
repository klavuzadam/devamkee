...
if app.USE_IKASHOP_UI_RENEWED:
	TELEPORT_TO_SHOP_COST = 50000
else:
...
def GetAttributeSettings():
	...
	for apply, text in attributeDict.items():
		try:
			...
			dbg.TraceError("ERROR WHILE FORMATTING ATTRIBUTE APPLY CODE: %d"%(apply, str(text)))
...
def ExtractInputPrice(dialog):
	...
	if app.USE_IKASHOP_UI_RENEWED:
		value = min(999999999999, long(value))
...
SPECIAL_INVENTORY_SLOTS = (
	...
	player.SLOT_TYPE_CHEST_INVENTORY,
)
...
class IkarusShopWindow(ui.Window):
	...
	def Destroy(self):
		...
		if app.USE_IKASHOP_UI_RENEWED and hasattr(self, "inputPriceDialog"):
			SelfDestroyObject(self.inputPriceDialog)
			del self.inputPriceDialog
	...
	def OpenMoneyInputDialog(self, acceptEvent, denyEvent = None, value = 0, cheque = 0, nocheque = 0):
		...
		self.moneyInputDialog.SetMaxLength(12)
	...
	if app.USE_IKASHOP_UI_RENEWED:
		def OpenPriceInputDialog(self, itemWindow, itemPos, acceptEvent, denyEvent = None, value = 0, cheque = 0, nocheque = 0):
			if hasattr(self, "moneyInputDialog"):
			...
			if getattr(self, "inputPriceDialog", None) == None:
				self.inputPriceDialog = uicommon.PriceInputDialog()
				self._RegisterDialog(self.inputPriceDialog)

			if ENABLE_CHEQUE_SYSTEM:
				self.inputPriceDialog.HideCheque() if nocheque \
					else self.inputPriceDialog.ShowCheque()

			self.inputPriceDialog.Open(itemWindow, itemPos)
			self.inputPriceDialog.SetAcceptEvent(acceptEvent)
			self.inputPriceDialog.SetCancelEvent(denyEvent if denyEvent else self.inputPriceDialog.Hide)
			self.inputPriceDialog.SetValue(value if value != 0 else "")

			if ENABLE_CHEQUE_SYSTEM:
				self.inputPriceDialog.SetCheque(cheque if cheque != 0 else "")
...
class IkarusShopSafeboxBoard(IkashopBoardWithTitleBar):
	...
	def _LoadIkarusShopSafeboxBoard(self):
		# making slot box
		self.slotBox = self.CreateWidget(ui.ImageBox, pos = (8, 33))
		self.slotBox.LoadImage("ikashop/lite/safebox/bg_safebox.png")

		self.slotWindow = self.CreateWidget(ui.GridSlotWindow, parent = self.slotBox, pos = (9,13))
		self.slotWindow.ArrangeSlot(0, self.GRID_COLUMNS, self.GRID_ROWS, SLOT_SIZE, SLOT_SIZE, 0, 0)
		self.slotWindow.SetSlotBaseImage("d:/ymir work/ui/public/Slot_Base.sub", 1.0, 1.0, 1.0, 1.0)
		...
		if not app.USE_IKASHOP_UI_RENEWED:
			# making money box
			self.moneyBox = self.CreateWidget(ui.ImageBox, pos = (15, 440))
			self.moneyBox.LoadImage("ikashop/lite/safebox/money_box.png")
			mx, my = self.moneyBox.GetLocalPosition()

			self.moneyText = self.CreateWidget(ui.TextLine, parent = self.moneyBox, pos = (32, 7))
			...
			self.moneyClaimButton = self.CreateWidget(ui.Button, pos = (210 , 440))
			#self.moneyClaimButton = self.CreateWidget(ui.Button, pos = (mx + self.moneyBox.GetWidth() + 2, my))
			self.moneyClaimButton.SetUpVisual("ikashop/lite/safebox/buttons/claim_money/button_yang_0.png")
			self.moneyClaimButton.SetOverVisual("ikashop/lite/safebox/buttons/claim_money/button_yang_1.png")
			self.moneyClaimButton.SetDownVisual("ikashop/lite/safebox/buttons/claim_money/button_yang_2.png")
			self.moneyClaimButton.SetText(locale.IKASHOP_SAFEBOX_CLAIM_MONEY_BUTTON_TEXT)
		...
		for i in xrange(self.PAGE_COUNT):
			selectPageButton = self.CreateWidget(ui.Button, pos = (25 + i*60, 410))

			selectPageButton.SetUpVisual("ikashop/lite/safebox/buttons/select_page/default.png")
			selectPageButton.SetDownVisual("ikashop/lite/safebox/buttons/select_page/disabled.png")
		...
		self.itemsClaimButton = self.CreateWidget(ui.Button, pos = (210 , 409))
		self.itemsClaimButton.SetUpVisual("ikashop/lite/safebox/buttons/claim_items/clam_item_0.png")
		self.itemsClaimButton.SetDownVisual("ikashop/lite/safebox/buttons/claim_items/clam_item_2.png")
		self.itemsClaimButton.SetOverVisual("ikashop/lite/safebox/buttons/claim_items/clam_item_1.png")
		self.itemsClaimButton.SetText(locale.IKASHOP_SAFEBOX_CLAIM_ITEMS_BUTTON_TEXT)
	...
	def _ShowToolTipOnItemSlot(self, slot):
		...
		if slot in self.items:
			...
			tooltip.AddItemData(data['vnum'], data['sockets'], data['attrs'], newname = data.get('varname', ''))
...
class IkarusCreateAuctionDialog(IkashopBoardWithTitleBar):
	...
	def _OnClickEmptySlot(self, slot):
		...
		if mouse.mouseController.isAttached():
			...
			if attachedType in (player.SLOT_TYPE_DRAGON_SOUL_INVENTORY, player.SLOT_TYPE_INVENTORY) or attachedType in SPECIAL_INVENTORY_SLOTS:
...
class IkarusShopAuctionOwnerBoard(IkashopBoardWithTitleBar):
	...
	def _LoadIkarusShopAuctionOwnerBoard(self):
		...
		self.infoBox.SetMouseWheelScrollEvent(self._MouseWheelScrollOffers)
	...
	def SetDuration(self, duration):
		self.durationText.SetText(locale.ShopSecondsToDHM(duration*60))
	...
	def Setup(self, data):
		...
		self.durationText.SetText(locale.ShopSecondsToDHM(seconds))
...
class IkarusShopListBoard(IkashopBoardWithTitleBar):
	...
	def _LoadIkarusShopListBoard(self):
		...
		self.boardBox.SetMouseWheelScrollEvent(self._MouseWheelScrollElements)
...
class IkarusShopOfferView(IkarusShopWindow):
	...
	def _LoadIkarusShopOfferView(self):
		...
		self.acceptOfferButton.SetUpVisual("ikashop/buttons/small/yes_button_0.png")
		self.acceptOfferButton.SetDownVisual("ikashop/buttons/small/yes_button_2.png")
		self.acceptOfferButton.SetOverVisual("ikashop/buttons/small/yes_button_1.png")
		...
		self.cancelOfferButton.SetUpVisual("ikashop/buttons/small/no_button_0.png")
		self.cancelOfferButton.SetDownVisual("ikashop/buttons/small/no_button_2.png")
		self.cancelOfferButton.SetOverVisual("ikashop/buttons/small/no_button_1.png")
	...
	def _OverInItem(self, slot):
		tooltip = self.GetToolTip()
		tooltip.ClearToolTip()
		tooltip.AddItemData(self.itemData['vnum'], self.itemData['sockets'], self.itemData['attrs'], newname = self.itemData.get('varname', ''))
		tooltip.ShowToolTip()
...
class IkarusShopBusinessBoard(IkashopBoardWithTitleBar):
	...
	def _SettingUpBoard(self):
		self.SetSize(355 if app.USE_IKASHOP_UI_RENEWED else 510, 440)
	...
	def _LoadShopBusinessWindow(self):
		...
		if app.USE_IKASHOP_UI_RENEWED:
			self.totalBG = self.CreateWidget(ui.ExpandedImageBox, pos = (10, 35))
			self.totalBG.LoadImage("ikashop/renewed_ui/common/total_bg.png")
			self.totalBG.SetScale(1.08, 1.0)

			self.yangIcoBG = self.CreateWidget(ui.ExpandedImageBox, parent = self.totalBG, pos = (self.totalBG.GetWidth() - 26, 0))
			self.yangIcoBG.LoadImage("ikashop/renewed_ui/common/yang_ico_bg.png")

			self.totalText = self.CreateWidget(ui.TextLine, parent = self.totalBG, pos = (8, 12))
			self.totalText.SetVerticalAlignCenter()

		# making slot box
		self.slotBox = self.CreateWidget(ui.ImageBox, pos = (10 if app.USE_IKASHOP_UI_RENEWED else 5, 70 if app.USE_IKASHOP_UI_RENEWED else 30))
		#"self.slotBox.LoadImage("ikashop/lite/business/slot_box.png")
		self.slotBox.LoadImage("ikashop/renewed_ui/shop_owner/bg.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/shop_create.png")

		if app.EXTEND_IKASHOP_ULTIMATE:
			self.slotBox.SetMouseWheelScrollEvent(self._OnScrollMouseWheel)

		if app.EXTEND_IKASHOP_ULTIMATE:
			self.selectPageButtons = []
			for i in xrange(self.PAGE_COUNT):
				selectPageButton = self.CreateWidget(ui.Button, pos = (((10 + 8) if app.USE_IKASHOP_UI_RENEWED else 55 - 25) + i * (56 if app.USE_IKASHOP_UI_RENEWED else 60), 353 if app.USE_IKASHOP_UI_RENEWED else 370))
				selectPageButton.SetUpVisual("ikashop/renewed_ui/common/page_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/default.png")
				selectPageButton.SetDownVisual("ikashop/renewed_ui/common/page_button_hover.png"  if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/disabled.png")
				selectPageButton.SetOverVisual("ikashop/renewed_ui/common/page_button_down.png"  if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/hover.png")
				selectPageButton.SetDisableVisual("ikashop/renewed_ui/common/page_button_disable.png"  if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/disabled.png")
				selectPageButton.SetText(RomeNumber(i + 1))
				selectPageButton.SAFE_SetEvent(self._OnSelectPage, i)
				self.selectPageButtons.append(selectPageButton)

		self.slotWindow = self.CreateWidget(ui.GridSlotWindow, parent = self.slotBox, pos = (9, 8))
		self.slotWindow.ArrangeSlot(0, self.GRID_COLUMNS, self.GRID_ROWS, SLOT_SIZE, SLOT_SIZE, 0, 0)
		self.slotWindow.SetSlotBaseImage("d:/ymir work/ui/public/Slot_Base.sub", 1.0, 1.0, 1.0, 1.0)
		...
		# making duration box
		self.durationBox = self.CreateWidget(ui.ImageBox, pos = ((10 + 8) if app.USE_IKASHOP_UI_RENEWED else 25, 390 if app.USE_IKASHOP_UI_RENEWED else 400))
		self.durationBox.LoadImage("ikashop/renewed_ui/common/time_bg.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/duration_box.png")

		self.durationText = self.CreateWidget(ui.TextLine, parent = self.durationBox, pos = (self.durationBox.GetWidth() / 2 if app.EXTEND_IKASHOP_ULTIMATE else 30, 11 if app.EXTEND_IKASHOP_ULTIMATE else 7))

		if app.USE_IKASHOP_UI_RENEWED:
			self.durationText.SetHorizontalAlignCenter()
			self.durationText.SetVerticalAlignCenter()
		else:
			self.durationText.SetPackedFontColor(0xFFFFFFFF)
			self.durationText.SetText("11d 22h 18m")

		if not app.USE_IKASHOP_UI_RENEWED:
			self.durationExpiredIcon = self.CreateWidget(ui.ImageBox, pos = (177 - 50, 3), parent = self.durationBox)
			self.durationExpiredIcon.LoadImage("ikashop/lite/business/expired_icon.png")

		if app.USE_IKASHOP_UI_RENEWED:
			(x, y) = self.durationBox.GetLocalPosition()
			self.durationIncreaseButton = self.CreateWidget(ui.Button, pos = (x + self.durationBox.GetWidth(), y))
		else:
			self.durationIncreaseButton = self.CreateWidget(ui.Button, parent = self.durationBox, pos = (148, 1))

		self.durationIncreaseButton.SetUpVisual("ikashop/renewed_ui/shop_owner/time_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/duration_up/time_btn_normal.png")
		self.durationIncreaseButton.SetOverVisual("ikashop/renewed_ui/shop_owner/time_button_hover.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/duration_up/time_btn_down.png")
		self.durationIncreaseButton.SetDownVisual("ikashop/renewed_ui/shop_owner/time_button_down.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/duration_up/time_btn_hover.png")
		...
		##CRIAR LEILÃO
		#self.privateOfferOpenButton = self.CreateWidget(ui.Button, pos = (380, 400))
		#self.privateOfferOpenButton.SetUpVisual("ikashop/lite/business/buttons/offer/offer_normal.png")
		#self.privateOfferOpenButton.SetDownVisual("ikashop/lite/business/buttons/offer/offer_down.png")
		#self.privateOfferOpenButton.SetOverVisual("ikashop/lite/business/buttons/offer/offer_hover.png")
		...
		##VER LEILÕES
		#self.auctionOpenButton = self.CreateWidget(ui.Button, pos = (380, 370))
		#self.auctionOpenButton.SetUpVisual("ikashop/lite/business/buttons/auction/auction_normal.png")
		#self.auctionOpenButton.SetDownVisual("ikashop/lite/business/buttons/auction/auction_down.png")
		#self.auctionOpenButton.SetOverVisual("ikashop/lite/business/buttons/auction/auction_hover.png")
		...
		if app.EXTEND_IKASHOP_PRO:
			if app.USE_IKASHOP_UI_RENEWED:
				(x, y) = self.totalBG.GetLocalPosition()
				self.notificationOpenButton = self.CreateWidget(ui.Button, pos = (x + self.totalBG.GetWidth() + 5, y))
			else:
				self.notificationOpenButton = self.CreateWidget(ui.Button, pos = (280, 385))

			self.notificationOpenButton.SetUpVisual("ikashop/renewed_ui/shop_owner/history_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/pro/business/buttons/notification/btn_3_normal.png")
			self.notificationOpenButton.SetOverVisual("ikashop/renewed_ui/shop_owner/history_button_hover.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/pro/business/buttons/notification/btn_3_down.png")
			self.notificationOpenButton.SetDownVisual("ikashop/renewed_ui/shop_owner/history_button_down.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/pro/business/buttons/notification/btn_3_hover.png")
			...
			self.notificationCounter = self.CreateWidget(IkarusShopCounter, pos = (13 if app.USE_IKASHOP_UI_RENEWED else 25, 5 if app.USE_IKASHOP_UI_RENEWED else 0), parent = self.notificationOpenButton)

		if app.USE_IKASHOP_UI_RENEWED:
			if app.USE_IKASHOP_UI_RENEWED:
				if app.EXTEND_IKASHOP_PRO:
					(x, y) = self.notificationOpenButton.GetLocalPosition()
					self.safeboxOpenButton = self.CreateWidget(ui.Button, pos = (x + self.notificationOpenButton.GetWidth() + 5, y))
				else:
					(x, y) = self.totalBG.GetLocalPosition()
					self.safeboxOpenButton = self.CreateWidget(ui.Button, pos = (x + self.totalBG.GetWidth() + 5, y))
		else:
			self.safeboxOpenButton = self.CreateWidget(ui.Button, pos = (330, 385))

		self.safeboxOpenButton.SetUpVisual("ikashop/renewed_ui/shop_owner/safebox_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/safebox/safebox_0.png")
		self.safeboxOpenButton.SetOverVisual("ikashop/renewed_ui/shop_owner/safebox_button_hover.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/safebox/safebox_2.png")
		self.safeboxOpenButton.SetDownVisual("ikashop/renewed_ui/shop_owner/safebox_button_down.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/safebox/safebox_1.png")
		...
		self.safeboxItemCounter = self.CreateWidget(IkarusShopCounter, pos = (13 if app.USE_IKASHOP_UI_RENEWED else 25, 5 if app.USE_IKASHOP_UI_RENEWED else 0), parent = self.safeboxOpenButton)

		if app.EXTEND_IKASHOP_PRO:
			if app.USE_IKASHOP_UI_RENEWED:
				(x, y) = self.safeboxOpenButton.GetLocalPosition()
				self.moveShopButton = self.CreateWidget(ui.Button, pos = (x + self.safeboxOpenButton.GetWidth() + 5, y))
			else:
				self.moveShopButton = self.CreateWidget(ui.Button, pos = (380 , 385))

			self.moveShopButton.SetUpVisual("ikashop/renewed_ui/common/teleport_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/move_shop/btn_teleport_normal.png")
			self.moveShopButton.SetDownVisual("ikashop/renewed_ui/common/teleport_button_hover.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/move_shop/btn_teleport_down.png")
			self.moveShopButton.SetOverVisual("ikashop/renewed_ui/common/teleport_button_down.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/move_shop/btn_teleport_hover.png")
			self.moveShopButton.SetToolTipText(locale.IKASHOP_PRO_BUSINESS_WARP_SHOP_BUTTON_TEXT)
			self.moveShopButton.SAFE_SetEvent(self._OnClickMoveShopButton)
			self.moveShopButton.ToolTipText.SetOutline(1)

		if app.USE_IKASHOP_UI_RENEWED:
			if app.EXTEND_IKASHOP_PRO:
				(x, y) = self.moveShopButton.GetLocalPosition()
				self.cancelShopButton = self.CreateWidget(ui.Button, pos = (x + self.moveShopButton.GetWidth() + 5, y))
			else:
				(x, y) = self.safeboxOpenButton.GetLocalPosition()
				self.cancelShopButton = self.CreateWidget(ui.Button, pos = (x + self.safeboxOpenButton.GetWidth() + 5, y))
		else:
			self.cancelShopButton = self.CreateWidget(ui.Button, pos = (430, 385))

		self.cancelShopButton.SetUpVisual("ikashop/renewed_ui/shop_owner/close_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/cancel_shop/btn_2_normal.png")
		self.cancelShopButton.SetOverVisual("ikashop/renewed_ui/shop_owner/close_button_hover.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/cancel_shop/btn_2_down.png")
		self.cancelShopButton.SetDownVisual("ikashop/renewed_ui/shop_owner/close_button_down.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/buttons/cancel_shop/btn_2_hover.png")
		...
		if app.EXTEND_IKASHOP_ULTIMATE:
			...
			for i in xrange(self.PAGE_SIZE):
				...
				lock.LoadImage("ikashop/ultimate/lock_0.tga")
				self.lockedCells.append(lock)

		if app.USE_IKASHOP_UI_RENEWED:
			self.safeboxTotalBG = self.CreateWidget(ui.ExpandedImageBox, pos = (self.durationIncreaseButton.GetLocalPosition()[0] + self.durationIncreaseButton.GetWidth() + 13, 353))
			self.safeboxTotalBG.LoadImage("ikashop/renewed_ui/shop_owner/safebox_total_bg.png")

			self.safeboxTotalText = self.CreateWidget(ui.TextLine, parent = self.safeboxTotalBG, pos = (8, 12))
			self.safeboxTotalText.SetVerticalAlignCenter()
			self.safeboxTotalText.SetText(locale.DottedNumber("0"))
			self.safeboxTotalText.SetPackedFontColor(0xFF00FF00)

			self.withdrawButton = self.CreateWidget(ui.Button, pos = (self.safeboxTotalBG.GetLocalPosition()[0], 390))
			self.withdrawButton.SetUpVisual("ikashop/renewed_ui/shop_owner/withdraw_button_normal.png")
			self.withdrawButton.SetOverVisual("ikashop/renewed_ui/shop_owner/withdraw_button_hover.png")
			self.withdrawButton.SetDownVisual("ikashop/renewed_ui/shop_owner/withdraw_button_down.png")
			self.withdrawButton.SetText(locale.IKASHOP_BUTTON_WITHDRAW)
			self.withdrawButton.SetEvent(self._OnClickClaimMoneyButton)
	...
	def InsertItemInSlot(self, win, pos, slot = None):
		...
		if itemIndex != 0:
			...
			if app.EXTEND_IKASHOP_PRO:
				...
				if app.USE_IKASHOP_UI_RENEWED:
					self.OpenPriceInputDialog(win, pos, self._OnAcceptInsertingItemPrice, **kargs)
				else:
					self.OpenMoneyInputDialog(self._OnAcceptInsertingItemPrice, **kargs)
			else:
				if app.USE_IKASHOP_UI_RENEWED:
					self.OpenPriceInputDialog(win, pos, self._OnAcceptInsertingItemPrice)
				else:
					self.OpenMoneyInputDialog(self._OnAcceptInsertingItemPrice)

			if app.EXTEND_IKASHOP_ULTIMATE:
				if app.USE_IKASHOP_UI_RENEWED:
					self.inputPriceDialog.SetPriceAverage(-1)
				else:
					self.moneyInputDialog.SetPriceAverage(-1)
	...
	def _OnAcceptInsertingItemPrice(self):
		if app.USE_IKASHOP_UI_RENEWED:
			self.inputPriceDialog.Close()
			ok, price = ExtractInputPrice(self.inputPriceDialog)
		else:
			self.moneyInputDialog.SoftClose()
			...
			if self.insertingPriceAllSimilar:
				for checkData in self.shopInfo['items'].values():
					if data['hash'] == checkData['hash'] and data['count'] == checkData['count'] and data['vnum'] == checkData['vnum']:
						if price[0] != checkData['price']:
							if not ENABLE_CHEQUE_SYSTEM or price[1] != checkData['cheque']:
	...
	if app.EXTEND_IKASHOP_PRO:
		...
		def _OnClickMoveShopButton(self):
			actPrice = TELEPORT_TO_SHOP_COST if app.USE_IKASHOP_UI_RENEWED else MOVE_SHOP_ENTITY_COST

			# checking money
			if player.GetElk() < actPrice:
				self.OpenPopupDialog(locale.IKASHOP_PRO_BUSINESS_MOVE_SHOP_NO_MONEY)
				return

			# sending question
			if app.USE_IKASHOP_UI_RENEWED:
				message = locale.IKASHOP_PRO_BUSINESS_TELEPORT_TO_SHOP_QUESTION.format(locale.NumberToMoneyString(actPrice))
			else:
				message = locale.IKASHOP_PRO_BUSINESS_MOVE_SHOP_QUESTION.format(locale.NumberToMoneyString(actPrice))
		...
		def _OnAcceptMoveShopQuestion(self):
			...
			if app.USE_IKASHOP_UI_RENEWED:
				ikashop.SendTeleportToShopEntity(0)
			else:
			...
			if "items" in self.shopInfo:
				for item in self.shopInfo["items"].values():
					if item.has_key("price"):
						try:
							totalYang += long(item["price"])
						except:
							pass

			self.totalText.SetText(locale.IKASHOP_TOTAL_IN_SELL_CURRENCY % (locale.DottedNumber(totalYang)))
	...
	# BINARY CALLS
	#if app.USE_IKASHOP_UI_RENEWED: @yang
	def OpenShopOwner(self, data, yang = 0):
		self.wasOpened = True

		if app.USE_IKASHOP_UI_RENEWED:
			self.safeboxTotalText.SetText(locale.DottedNumber(yang))

		# refreshing duration
		if data['duration'] > 0:
			self.durationText.SetText(locale.ShopSecondsToDHM(data['duration'] * 60))
			self.durationText.SetPackedFontColor(0xFF00FF00)
			...
			self.durationText.SetPackedFontColor(0xFFFF0000)
		...
		if app.EXTEND_IKASHOP_ULTIMATE and not app.USE_IKASHOP_UI_RENEWED:
			self.decorationBox.Show() if data['decoration'] != 0\
				else self.decorationBox.Hide()

			self.decorationTime.SetText(locale.ShopSecondsToDHM(data['decoration_time']*60))
	...
	#if app.USE_IKASHOP_UI_RENEWED: @yang
	def OpenShopOwnerEmpty(self, yang = 0):
		self.wasOpened = True
		...
		if app.USE_IKASHOP_UI_RENEWED:
			self.totalText.SetText(locale.IKASHOP_TOTAL_IN_SELL_CURRENCY % (locale.DottedNumber(0)))
	...
	def ShopOwnerRemoveItem(self, itemid):
		if self.wasOpened and 'items' in self.shopInfo:
	...
	def SetupSafebox(self, yang, items):
		if app.USE_IKASHOP_UI_RENEWED:
			self.safeboxTotalText.SetText(locale.DottedNumber(yang))
			self.safeboxBoard.Setup(items)
		else:
			self.safeboxBoard.Setup(yang, items)

	if app.USE_IKASHOP_UI_RENEWED:
		def RefreshSafeboxValutes(self, yang):
			self.safeboxTotalText.SetText(locale.DottedNumber(yang))
	...
	if app.EXTEND_IKASHOP_PRO:
		...
		if app.USE_IKASHOP_UI_RENEWED:
			def SetNotificationCounterFull(self, count):
				if not self.inventoryInst:
					import constInfo
					interfaceInst = constInfo.GetInterfaceInstance()
					if interfaceInst:
						from _weakref import proxy
						self.inventoryInst = proxy(interfaceInst.wndInventory)

				if self.inventoryInst:
					self.inventoryInst.SetShopNotificationCounter(count)
	...
	if app.EXTEND_IKASHOP_ULTIMATE:
		def SetPriceAverage(self, price):
			...
			if app.USE_IKASHOP_UI_RENEWED and hasattr(self, "inputPriceDialog"):
				self.inputPriceDialog.SetPriceAverage(price)
...
class IkarusShopGuestBoard(IkashopBoardWithTitleBar):
	...
	def _SettingUpBoard(self):
		self.SetSize(355 if app.USE_IKASHOP_UI_RENEWED else 508, 407 if app.USE_IKASHOP_UI_RENEWED else 404)
		self.SetCloseEvent(self.Close)
		self.SetCenterPosition()
		self.AddFlag("movable")
		self.AddFlag("float")
	...
	if app.USE_IKASHOP_UI_RENEWED:
		def _OnAcceptTeleportToShopQuestion(self):
			self.questionDialog.Hide()

			ikashop.SendTeleportToShopEntity(self.shopInfo['id'])

		def _OnClickTeleportToShopButton(self):
			actPrice = TELEPORT_TO_SHOP_COST
			...
			# sending question
			message = locale.IKASHOP_PRO_BUSINESS_TELEPORT_TO_SHOP_QUESTION.format(locale.NumberToMoneyString(actPrice))
			self.OpenQuestionDialog(message, self._OnAcceptTeleportToShopQuestion)

	def _LoadIkarusShopGuestBoard(self):
		if app.USE_IKASHOP_UI_RENEWED:
			self.totalBG = self.CreateWidget(ui.ExpandedImageBox, pos = (10, 35))
			self.totalBG.LoadImage("ikashop/renewed_ui/common/total_bg.png")
			self.totalBG.SetScale(1.77, 1.0)
			...
			if app.USE_IKASHOP_UI_RENEWED:
				(x, y) = self.totalBG.GetLocalPosition()
				self.warpShopButton = self.CreateWidget(ui.Button, pos = (x + self.totalBG.GetWidth() + 6, y))
				self.warpShopButton.SetUpVisual("ikashop/renewed_ui/common/teleport_button_normal.png")
				self.warpShopButton.SetOverVisual("ikashop/renewed_ui/common/teleport_button_hover.png")
				self.warpShopButton.SetDownVisual("ikashop/renewed_ui/common/teleport_button_down.png")
				self.warpShopButton.SetToolTipText(locale.IKASHOP_PRO_BUSINESS_WARP_SHOP_BUTTON_TEXT)
				self.warpShopButton.SAFE_SetEvent(self._OnClickTeleportToShopButton)
				self.warpShopButton.ToolTipText.SetOutline(1)

		# making slot box
		self.slotBox = self.CreateWidget(ui.ImageBox, pos = (10 if app.USE_IKASHOP_UI_RENEWED else 5, 70 if app.USE_IKASHOP_UI_RENEWED else 35 + 27))
		self.slotBox.LoadImage("ikashop/renewed_ui/shop_viewer/bg.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/business/slot_box.png")

		if app.EXTEND_IKASHOP_ULTIMATE:
			self.slotBox.SetMouseWheelScrollEvent(self._OnScrollMouseWheel)

		if app.USE_IKASHOP_UI_RENEWED:
			self.durationBox = self.CreateWidget(ui.ExpandedImageBox, pos = ((10 + 8), 353))
		else:
		...
		if app.USE_IKASHOP_UI_RENEWED:
			self.durationBox.SetScale(1.2, 1.15)

		self.durationText = self.CreateWidget(ui.TextLine, parent = self.durationBox, pos = (self.durationBox.GetWidth() / 2 if app.EXTEND_IKASHOP_ULTIMATE else 30, 13 if app.EXTEND_IKASHOP_ULTIMATE else 4))
		if app.EXTEND_IKASHOP_ULTIMATE:
			self.durationText.SetHorizontalAlignCenter()
			self.durationText.SetVerticalAlignCenter()
		else:
		...
		if app.EXTEND_IKASHOP_ULTIMATE:
			self.selectPageButtons = []
			for i in xrange(self.PAGE_COUNT):
				selectPageButton = self.CreateWidget(ui.Button, pos = ((self.durationBox.GetLocalPosition()[0] + self.durationBox.GetWidth() + 15) + i * (52) if app.USE_IKASHOP_UI_RENEWED else (269 - 48 + i * 45), 353 if app.USE_IKASHOP_UI_RENEWED else 35))
				selectPageButton.SetUpVisual("ikashop/renewed_ui/common/page_button_normal.png" if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/default.png")
				selectPageButton.SetDownVisual("ikashop/renewed_ui/common/page_button_hover.png"  if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/disabled.png")
				selectPageButton.SetOverVisual("ikashop/renewed_ui/common/page_button_down.png"  if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/hover.png")
				selectPageButton.SetDisableVisual("ikashop/renewed_ui/common/page_button_disable.png"  if app.USE_IKASHOP_UI_RENEWED else "ikashop/lite/safebox/buttons/select_page/disabled.png")
		...
		self.slotWindow = self.CreateWidget(ui.GridSlotWindow, parent = self.slotBox, pos = (9 if app.USE_IKASHOP_UI_RENEWED else 6, 8 if app.USE_IKASHOP_UI_RENEWED else 6))
		self.slotWindow.ArrangeSlot(0, self.GRID_COLUMNS, self.GRID_ROWS, SLOT_SIZE, SLOT_SIZE, 0, 0)
		self.slotWindow.SetSlotBaseImage("d:/ymir work/ui/public/Slot_Base.sub", 1.0, 1.0, 1.0, 1.0)
		#self.slotWindow.SetSlotBaseImage("ikashop/lite/common/slot/default.png", 1.0, 1.0, 1.0, 1.0)
		self.slotWindow.SetSelectItemSlotEvent(self._OnClickItemSlot)
...
class IkarusShopComboBox(IkarusShopWindow):
	...
	def _LoadIkarusShopComboBox(self):
		self.SetMouseWheelScrollEvent(self._MouseWheelScrollView)
...
class IkarusSearchShopItem(IkarusShopWindow):
	...
	def _OverInItem(self, slot):
		tooltip = self.GetToolTip()
		tooltip.ClearToolTip()
		tooltip.AddItemData(self.data['vnum'], self.data['sockets'], self.data['attrs'], newname = self.data.get('varname', ''))
		tooltip.ShowToolTip()
	...
	def Setup(self, data):
		...
		# setting duration
		self.duration.SetText(locale.ShopSecondsToDHM(data['duration']*60))
...
class IkarusSearchShopBoard(IkashopBoardWithTitleBar):
	...
	def _LoadSearchShopWindow(self):
		...
		self.resultBox.SetMouseWheelScrollEvent(self._OnScrollMouseWheelResultItems)
	...
	def _ExtractFilterAttributes(self):
		def extractOne(combo, edit):
			if combo.GetSelectedItem() == 0:
				return 0,0
			bonus = self.attributeSettings[combo.GetSelectedItem()-1][0]
			value = edit.GetText()
	...
	def FastSearchByItem(self, vnum):
		# showing not using Open avoiding get back fullfill view
		if not self.IsShow():
			self.Show()
			self.SetTop()
		# resetting filters
		self._OnClickResetFilters()
		# setting up input name
		item.SelectItem(vnum)
		self.inputNameEdit.SetText(item.GetItemName())
...
if app.EXTEND_IKASHOP_ULTIMATE:
	...
	class IkarusShopSkinBoard(IkarusShopListBoard):
		...
		def _LoadIkarusShopSkinBoard(self):
			...
			self.acceptButton.SetUpVisual("ikashop/buttons/small/yes_button_0.png")
			self.acceptButton.SetDownVisual("ikashop/buttons/small/yes_button_2.png")
			self.acceptButton.SetOverVisual("ikashop/buttons/small/yes_button_1.png")
			...
			self.cancelButton.SetUpVisual("ikashop/buttons/small/no_button_0.png")
			self.cancelButton.SetDownVisual("ikashop/buttons/small/no_button_2.png")
			self.cancelButton.SetOverVisual("ikashop/buttons/small/no_button_1.png")
