...
####################################################
# Distribution of module loading for fast execution
####################################################
import uiCommon
...
class MyCharacters :
	class MyUnit :
		...
		if app.ENABLE_YOHARA_SYSTEM:
			def __init__(self, const_id, name, level, race, playtime, guildname, form, hair, acce, stat_str, stat_dex, stat_hth, stat_int, change_name, sungma_level, sungma_str, sungma_hp, sungma_move, sungma_immune):
				self.UnitDataDic = {
					"ID" : const_id,
					"NAME" : name,
					"LEVEL" : level,
					...
					"SUNGMA_LEVEL" : sungma_level,
					"SUNGMA_STR" : sungma_str,
					"SUNGMA_HP" : sungma_hp,
					"SUNGMA_MOVE" : sungma_move,
					"SUNGMA_IMMUNE" : sungma_immune,
				}
		else:
	...
	def __init__(self, stream) :
		...
		if app.ENABLE_YOHARA_SYSTEM:
			self.Sungma_Point = { 0 : None, 1 : None, 2 : None, 3 : None, 4 : None }
	...
	def LoadCharacterData(self) :
		...
		for i in xrange(net.CHARACTER_SLOT_COUNT_MAX) :
			...
			if app.ENABLE_SASH_SYSTEM:
				...
				acce = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_HAIR+1)

			if app.ENABLE_YOHARA_SYSTEM:
				sungma_level = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_CONQUEROR_LEVEL)
				sungma_str = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_SUNGMA_ST)
				sungma_hp = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_SUNGMA_HP)
				sungma_move = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_SUNGMA_MOVE)
				sungma_immune = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_SUNGMA_IMMUNE)

			self.SetPriorityData(last_playtime, i)

			if app.ENABLE_YOHARA_SYSTEM:
				self.myUnitDic[i] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, acce, stat_str, stat_dex, stat_hth, stat_int, change_name, sungma_level, sungma_str, sungma_hp, sungma_move, sungma_immune)
			else:
		...
		for i in xrange(len(self.PriorityData)):
			...
			if app.ENABLE_YOHARA_SYSTEM:
				self.SetSortingData(i, DestDataDic["RACE"], DestDataDic["GUILDNAME"], DestDataDic["PLAYTIME"], DestDataDic["STR"], DestDataDic["DEX"], DestDataDic["HTH"], DestDataDic["INT"], DestDataDic["CHANGENAME"], DestDataDic["SUNGMA_STR"], DestDataDic["SUNGMA_HP"], DestDataDic["SUNGMA_MOVE"], DestDataDic["SUNGMA_IMMUNE"])
			else:
			...
			if app.ENABLE_YOHARA_SYSTEM:
				self.MainStream.InitDataSet(i, DestDataDic["NAME"], DestDataDic["LEVEL"], DestDataDic["ID"], DestDataDic["SUNGMA_LEVEL"])
			else:
	...
	if app.ENABLE_YOHARA_SYSTEM:
		def SetSortingData(self, slot, race, guildname, playtime, pStr, pDex, pHth, pInt, changename, sStr, sHp, sMove, sImmune) :
			self.HowManyChar += 1
			self.Race[slot] = race
			...
			self.Sungma_Point[slot] = [sStr, sHp, sMove, sImmune]
	else:
	...
	if app.ENABLE_YOHARA_SYSTEM:
		def GetSungmaPoint(self, slot) :
			return self.Sungma_Point[slot]
...
class SelectCharacterWindow(ui.Window) :
	...
	class CharacterRenderer(ui.Window):
		def OnRender(self):
			...
			app.SetCenterPosition(0.0, 0.0, 0.0) #X, Z, Y ( X+ RIGHT, Z+ ??, Y+ DOWN ) Let's analyze more
	...
	def __init__(self, stream):
		...
		if app.ENABLE_YOHARA_SYSTEM:
			self.CLevel_FontColor = grp.GenerateColor(59.0/255.0, 131.0/255.0, 189.0/255.0, 1.0)
			self.ShowButtonToolTip = False
			self.sungmapoint = [0, 0, 0, 0]
			self.SungmaAttr = False
	...
	def Open(self):
		...
		##Select_Page##
		self.select_thinboard_page = getChild("select_thinboard_page")
		self.select_thinboard_page.Hide()

		if app.ENABLE_YOHARA_SYSTEM:
			# Original
			self.origin_stat_button = getChild("origin_stat_button")
			self.origin_stat_button.SetShowToolTipEvent(ui.__mem_func__(self.OverInToolTipButton), uiScriptLocale.SELECT_ORIGIN_STAT)
			self.origin_stat_button.SetHideToolTipEvent(ui.__mem_func__(self.OverOutToolTipButton))
			self.origin_stat_button.SetEvent(ui.__mem_func__(self.SetSungmaInfo), False)

			self.hth_img = getChild("hth_img")
			self.hth_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", uiScriptLocale.CREATE_HP)
			self.hth_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")

			self.int_img = getChild("int_img")
			self.int_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", uiScriptLocale.CREATE_SP)
			self.int_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")

			self.str_img = getChild("str_img")
			self.str_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", uiScriptLocale.CREATE_ATT_GRADE)
			self.str_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")

			self.dex_img = getChild("dex_img")
			self.dex_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", uiScriptLocale.CREATE_DEX_GRADE)
			self.dex_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")

			# Sungma
			self.sungma_stat_button = getChild("sungma_stat_button")
			self.sungma_stat_button.SetShowToolTipEvent(ui.__mem_func__(self.OverInToolTipButton), uiScriptLocale.SELECT_SUNGMA_STAT)
			self.sungma_stat_button.SetHideToolTipEvent(ui.__mem_func__(self.OverOutToolTipButton))
			self.sungma_stat_button.SetEvent(ui.__mem_func__(self.SetSungmaInfo), True)

			self.sungma_str_img = getChild("sungma_str_img")
			self.sungma_str_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_STR)
			self.sungma_str_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")
			self.sungma_str_img.Hide()

			self.sungma_hp_img = getChild("sungma_hp_img")
			self.sungma_hp_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_HP)
			self.sungma_hp_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")
			self.sungma_hp_img.Hide()

			self.sungma_move_img = getChild("sungma_move_img")
			self.sungma_move_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_MOVE)
			self.sungma_move_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")
			self.sungma_move_img.Hide()

			self.sungma_immune_img = getChild("sungma_immune_img")
			self.sungma_immune_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_IMMUNE)
			self.sungma_immune_img.SetEvent(ui.__mem_func__(self.ConquerorEventProgress), "mouse_over_out", "")
			self.sungma_immune_img.Hide()
	...
	if app.ENABLE_YOHARA_SYSTEM:
		def OverInToolTipButton(self, arg):
			...
			self.buttontooltip.AppendTextLine(arg, 0xffffff00)
		...
		def ConquerorEventProgress(self, event_type, arg) :
			#print "ConquerorEventProgress %s, %s" %(event_type, slot)
			if "mouse_click" == event_type :
				...
				print " New_introSelect.py ::ConquerorEventProgress : FALSE"

	if app.ENABLE_YOHARA_SYSTEM:
		def SetSungmaInfo(self, sungma = False):
			imgList = [
				self.hth_img,
				self.int_img,
				self.str_img,
				...
				self.sungma_str_img,
				self.sungma_hp_img,
				self.sungma_move_img,
				self.sungma_immune_img
			]

			for i in xrange(0, len(imgList)):
				imageElement = imgList[i]
				if i < 4:
					imageElement.Hide() if sungma else imageElement.Show()

				if i >= 4:
					imageElement.Show() if sungma else imageElement.Hide()

			self.ResetSungma() if sungma else self.ResetStat()
			self.SungmaAttr = sungma
	...
	def SelectButton(self, slot):
		...
		if app.ENABLE_YOHARA_SYSTEM and self.SungmaAttr:
			self.ResetSungma()
		else:
			self.ResetStat()
	...
	def Close(self):
		...
		if app.ENABLE_YOHARA_SYSTEM:
			self.CLevel_FontColor = None
			self.ShowButtonToolTip = False
			self.sungmapoint = None#[]
			self.buttontooltip = None
			self.origin_stat_button = None
			self.sungma_stat_button = None
	...
	def CreateCharacterButton(self):
		#Create a character by taking the number of an empty slot.
	...
	def OnUpdate(self):
		...
		motionDelay = 0.0
		if self.MotionStart and app.GetTime() - self.MotionTime >= motionDelay:
			self.MotionStart = False
	...
	if app.ENABLE_YOHARA_SYSTEM:
		def InitDataSet(self, slot, name, level, real_slot, sungma_level):
			...
			if localeInfo.IsARABIC():
				...
				if sungma_level > 0:
					self.CharacterButtonList[slot].AppendTextLine("Lv." + str(sungma_level), localeInfo.UI_DEF_FONT, self.CLevel_FontColor, "right", 7, height*3/4)
				else:
				...
				if sungma_level > 0:
					self.CharacterButtonList[slot].AppendTextLine("Lv." + str(sungma_level), localeInfo.UI_DEF_FONT, self.CLevel_FontColor, "left", width - 42, height*3/4)
				else:
			...
			self.RealSlot.append(real_slot) ## Real slot ID ##
	...
	if app.ENABLE_YOHARA_SYSTEM:
		def RefreshSungma(self):
			sungmaSummary = 90.0 
			self.curGauge =	[
				float(self.sungmapoint[0])/sungmaSummary,
				float(self.sungmapoint[1])/sungmaSummary,
				float(self.sungmapoint[2])/sungmaSummary,
				float(self.sungmapoint[3])/sungmaSummary,
			]

			for i in xrange(self.LEN_STATPOINT) :
				self.statValue[i].SetText(str(self.sungmapoint[i]))

		def ResetSungma(self):
			mySungmaPoint = self.mycharacters.GetSungmaPoint(self.SelectSlot)

			if not mySungmaPoint :
				return

			for i in xrange(self.LEN_STATPOINT) :
				self.sungmapoint[i] = mySungmaPoint[i]
	...
	def KeyInputUpDown(self, key) :
		...
		if 200 == key : #UP
			...
			if idx >= maxValue :
				idx = 0
		else: # There will be no such case, but in case you don't know, exception handling
			self.SelectButton(0)
	...
	def DisableWindow(self):
		...
		self.ShowToolTip = False
		if app.ENABLE_YOHARA_SYSTEM:
			self.origin_stat_button.Disable()
			self.sungma_stat_button.Disable()
			self.buttontooltip.Hide()
			self.ShowButtonToolTip = False
		self.Disable = True
		for button in self.CharacterButtonList :
	...
	def EnableWindow(self):
		...
		self.btnNext.Enable()
		if app.ENABLE_YOHARA_SYSTEM:
			self.origin_stat_button.Enable()
			self.sungma_stat_button.Enable()
		self.Disable = False
		for button in self.CharacterButtonList :
			button.Enable()
