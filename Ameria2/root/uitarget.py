...
#							itemScrollBar.SetScrollStep(float(self.MAX_ITEM_COUNT) / float(realCount))
	...
	def __init__(self):
		...
		totalhitsboard = ui.ExpandedImageBox()
		totalhitsboard.SetParent(self)
		totalhitsboard.LoadImage("d:/ymir work/ui/game/target/target_hp_bar_background.png")
		totalhitsboard.SetPosition(11, -20)
		totalhitsboard.Show()
		self.totalhitsboard = totalhitsboard
		...
		hpGauge.MakeGauge( 100 , "red" , "yellow" )
		hpGauge.Hide()

		if app.ENABLE_ACCUMULATE_DAMAGE_DISPLAY:
			damage_display = ui.TextLine()
			damage_display.SetParent( self.totalhitsboard )
			damage_display.SetPosition(15, 3)
	...
	def ResetTargetBoard(self):
		...
		self.hpboard.SetPosition(19, 39)
	...
	if app.ENABLE_VIEW_TARGET_DECIMAL_HP:
		def SetHP(self, hpPercentage, iMinHP, iMaxHP):
			...
			if app.ENABLE_VIEW_TARGET_PLAYER_HP:
				...
				if showingButtonCount > 0:
					if chr.GetInstanceType(self.vid) == chr.INSTANCE_TYPE_PLAYER:
						...
						self.SetSizeNew(150 + 7*self.nameLength, 40)
				else:
					self.SetSizeNew(150 + 7*self.nameLength, 40)
			else:
				self.SetSizeNew(150 + 7*self.nameLength, self.GetHeight())
			
			if localeInfo.IsARABIC():
				self.name.SetPosition( self.GetWidth()-23, 13)
			else:
				self.name.SetPosition(43, 13)
			...
			if race == 0 or race == 1 or race == 2 or race == 3 or race == 4 or race == 5 or race == 6 or race == 7 or race == 8:
				...
				self.totalhitsboard.Hide()
				self.hpboard.LoadImage("d:/ymir work/ui/game/target/target_hp_bar_background.png")
				self.hpboard.SetPosition(19, 0)
				self.name.SetPosition(220, 13)
