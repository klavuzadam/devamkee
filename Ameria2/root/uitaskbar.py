...
class ExpandedTaskBar(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		self.toggleButtonDict[ExpandedTaskBar.BUTTON_SHOPSEARCH] = self.GetChild("ShopSearchButton")
		self.toggleButtonDict[ExpandedTaskBar.BUTTON_SHOPSEARCH].SetParent(self)
		...
		self.toggleButtonDict[ExpandedTaskBar.BUTTON_PET] = self.GetChild("PetButton")
		self.toggleButtonDict[ExpandedTaskBar.BUTTON_PET].SetParent(self)
...
class TaskBar(ui.ScriptWindow):
	...
	def SelectMouseButtonEvent(self, dir, event):
		...
		elif app.__AUTO_QUQUE_ATTACK__ and event == self.EVENT_QUEUE_ON:
...
class ExpandedMoneyTaskBar(ui.ScriptWindow):
	def OnOverIn(self, num):
		...
		if num == 0:
			if app.USE_CURRENCY_EXCHANGE:
				self.toolTipDesc.AutoAppendTextLineSpecial(localeInfo.TOOLTIP_EXCHANGE_YANG_TO_WON)
				self.toolTipDesc.AppendSpace(5)
				self.toolTipDesc.AutoAppendTextLineSpecial(localeInfo.TOOLTIP_EXCHANGE_MULTIPLE_BUY)
				self.toolTipDesc.AppendSpace(2)
			else:
			...
			if app.USE_CURRENCY_EXCHANGE:
				self.toolTipDesc.AutoAppendTextLineSpecial(localeInfo.TOOLTIP_EXCHANGE_WON_TO_YANG)
				self.toolTipDesc.AppendSpace(5)
				self.toolTipDesc.AutoAppendTextLineSpecial(localeInfo.TOOLTIP_EXCHANGE_MULTIPLE_SELL)
				self.toolTipDesc.AppendSpace(2)
			else:
	...
	if app.USE_CURRENCY_EXCHANGE:
		def OnCloseDlg(self):
			if self.multiTradeDlg:
				self.multiTradeDlg.Close()

		def OnAcceptDlg(self):
			if self.multiTradeDlg:
				exchangeAmount = self.multiTradeDlg.GetExchangeAmount()
				if exchangeAmount > 0:
					net.fdsak432ukgfdsjk25(exchangeAmount, self.multiTradeDlg.exchangeType)

				self.multiTradeDlg.Close()

		def ClickExchange(self, eventName, exchangeAmount, exchangeType):
			try:
				exchangeAmount = long(exchangeAmount)
			except:
				return

			if app.IsPressed(app.DIK_RCONTROL) or app.IsPressed(app.DIK_LCONTROL):
				if not self.multiTradeDlg:
					import uicommon
					self.multiTradeDlg = uicommon.CurrencyExchangeDialog()
					self.multiTradeDlg.SetAcceptEvent(ui.__mem_func__(self.OnAcceptDlg))
					self.multiTradeDlg.SetCancelEvent(ui.__mem_func__(self.OnCloseDlg))

				if not self.multiTradeDlg:
					import exception
					exception.Abort("uicommon.CurrencyExchangeDialog")
					return

				(x, y) = self.GetGlobalPosition()
				if self.multiTradeDlg.Open(exchangeType, x, y):
					self.OnOverOut()
			else:
				net.fdsak432ukgfdsjk25(exchangeAmount, exchangeType)
	...
	def RefreshStatus(self):
		if self.wndMoneyText:
			self.wndMoneyText.SetText(localeInfo.NumberToMoneyString(player.GetElk()))

		if app.USE_CHEQUE_CURRENCY and self.wndChequeText:
			self.wndChequeText.SetText(localeInfo.NumberToMoneyStringNEW(player.GetCheque()))

		if app.ENABLE_GEM_SYSTEM and self.wndGemText:
			self.wndGemText.SetText(localeInfo.NumberToMoneyStringNEW(player.GetGem()))
	...
	def LoadWindow(self):
		try:
			...
			self.wndMoneyIcon = self.GetChild("money_icon_ui")
			self.wndMoneySlot = self.GetChild("money_slot_ui")
			self.wndMoneyText = self.GetChild("money_text_ui")

			if app.USE_CHEQUE_CURRENCY:
				self.wndChequeIcon = self.GetChild("cheque_icon_ui")
				self.wndChequeSlot = self.GetChild("cheque_slot_ui")
				self.wndChequeText = self.GetChild("cheque_text_ui")

				(x, y) = self.wndMoneyIcon.GetLocalPosition()
				self.wndChequeSlot.SetPosition(x - 47, y - 1)

				(x, y) = self.wndChequeSlot.GetLocalPosition()
				self.wndChequeIcon.SetPosition(x - 19, y + 1)

			if app.ENABLE_GEM_SYSTEM:
				self.wndGemIcon = self.GetChild("gem_icon_ui")
				self.wndGemSlot = self.GetChild("gem_slot_ui")
				self.wndGemText = self.GetChild("gem_text_ui")

				if app.USE_CHEQUE_CURRENCY:
					(x, y) = self.wndChequeIcon.GetLocalPosition()
					self.wndGemSlot.SetPosition(x - 57, y - 1)

					(x, y) = self.wndGemSlot.GetLocalPosition()
					self.wndGemIcon.SetPosition(x - 17, y + 3)
				else:
					(x, y) = self.wndMoneyIcon.GetLocalPosition()
					self.wndGemSlot.SetPosition(x - 57, y - 1)
			...
			import exception
			exception.Abort("ExpandedMoneyTaskbar_Window.LoadWindow.LoadObject")

		try:
			self.wndMoneyIcon.SetMouseOverInEvent(ui.__mem_func__(self.OnOverIn), 0)
			self.wndMoneyIcon.SetMouseOverOutEvent(ui.__mem_func__(self.OnOverOut))

			if app.USE_CURRENCY_EXCHANGE:
				self.wndMoneyIcon.SetEvent(ui.__mem_func__(self.ClickExchange), "mouse_click", 1, 0)

			if app.USE_CHEQUE_CURRENCY:
				self.wndChequeIcon.SetMouseOverInEvent(ui.__mem_func__(self.OnOverIn), 1)
				self.wndChequeIcon.SetMouseOverOutEvent(ui.__mem_func__(self.OnOverOut))

				if app.USE_CURRENCY_EXCHANGE:
					self.wndChequeIcon.SetEvent(ui.__mem_func__(self.ClickExchange), "mouse_click", 1, 1)
		except:
			import exception
			exception.Abort("ExpandedMoneyTaskbar_Window.LoadWindow.BindObject")

	def __init__(self):
		self.toolTipDesc = None

		if app.USE_CURRENCY_EXCHANGE:
			self.multiTradeDlg = None

		ui.Window.__init__(self)

		self.wndMoneyIcon = None
		self.wndMoneySlot = None
		self.wndMoneyText = None

		if app.USE_CHEQUE_CURRENCY:
			self.wndChequeIcon = None
			self.wndChequeSlot = None
			self.wndChequeText = None

		if app.ENABLE_GEM_SYSTEM:
			self.wndGemIcon = None
			self.wndGemSlot = None
			self.wndGemText = None

	def Destroy(self):
		if app.USE_CURRENCY_EXCHANGE and self.multiTradeDlg:
			self.multiTradeDlg.Destroy()
			del self.multiTradeDlg
			self.multiTradeDlg = None

		if self.wndMoneyIcon:
			self.wndMoneyIcon.SetMouseOverInEvent(None)
			self.wndMoneyIcon.SetMouseOverOutEvent(None)

			if app.USE_CURRENCY_EXCHANGE:
				self.wndMoneyIcon.SetEvent(None, "mouse_click")
		...
		if app.USE_CHEQUE_CURRENCY:
			if self.wndChequeIcon:
				self.wndChequeIcon.SetMouseOverInEvent(None)
				self.wndChequeIcon.SetMouseOverOutEvent(None)

				if app.USE_CURRENCY_EXCHANGE:
					self.wndChequeIcon.SetEvent(None, "mouse_click")
