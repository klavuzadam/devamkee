...
class FishGame(ui.ScriptWindow):
	...
	def __Load(self):
		...
		if self.backGroundWater:
			# self.backGroundWater.SAFE_SetStringEvent("mouse_click", self.OnMouseLeftButtonDownEvent)
	...
	def OnMouseLeftButtonDownEvent(self):
		...
		if xFishGap > -fishGap and xFishGap < fishGap and yFishGap > -fishGap and yFishGap < fishGap and self.fishInGoal:
			...
			player.FishingGameGoal(self.goalCount, self.fishing_key)
		else:
	...
	def Open(self, level, fishing_key):
	...
	def Show(self):
		...
		self.SetPosition(x + random.randint(60, 120), y + random.randint(10, 30))
