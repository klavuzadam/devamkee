...
class QuestDialog(ui.ScriptWindow):
	...
	if app.ENABLE_QUEST_PAGE_SCROLLING:
		def GetQuestButtonMaxNum(self, maxVal = 1):
			yStart = self.sy+event.GetLineCount(self.descIndex)*16+20+5
			yEnd = self.board.GetHeight()
	
			maxButtonNum = max(maxVal, min((yEnd - yStart) / 32, self.QUEST_BUTTON_MAX_NUM))
			return maxButtonNum
	...
	if app.ENABLE_QUEST_PAGE_SCROLLING:	
		def MakeQuestion(self, n):
			...
			if self.GetQuestButtonMaxNum(0) < n and n < self.QUEST_BUTTON_MAX_NUM:
				self.board.SetSize(self.board.GetWidth(), self.board.GetHeight() + (n - self.GetQuestButtonMaxNum(0)) * 32)
	
			entire_questpage_number = max(0, (n-1)/self.GetQuestButtonMaxNum())+1
			...
			self.firstButton = self.MakeNextPrevPageButton()
			self.firstButton.SetPosition(self.prevbutton.GetLocalPosition()[0], self.prevbutton.GetLocalPosition()[1] + self.prevbutton.GetHeight() + 5)
			self.firstButton.SetText("First Page")
			self.firstButton.SetEvent(self.FirstQuestPageEvent, n)
	
			self.lastButton = self.MakeNextPrevPageButton()
			self.lastButton.SetPosition(self.nextbutton.GetLocalPosition()[0], self.nextbutton.GetLocalPosition()[1] + self.nextbutton.GetHeight() + 5)
			self.lastButton.SetText("Last Page")
			self.lastButton.SetEvent(self.LastQuestPageEvent, n)
	
			self.roamPagesText = ui.MakeTextLineNew(self.board, 0, 133, localeInfo.QUEST_ROAM_AROUND_PAGES)
			self.roamPagesText.SetHorizontalAlignCenter()
			self.roamPagesText.SetVerticalAlignCenter()
			self.roamPagesText.SetWindowHorizontalAlignCenter()
			self.roamPagesText.SetWindowVerticalAlignCenter()
			self.roamPagesText.Show() if entire_questpage_number > 1 else self.roamPagesText.Hide()
		...
		def RefreshQuestPage(self, n):
			num = 0
			Showing_button_inx = (cur_questpage_number-1)* self.GetQuestButtonMaxNum()
				
			while num < n:
				if num >= Showing_button_inx and num < Showing_button_inx + self.GetQuestButtonMaxNum():
					self.btnAnswer[num].Show()
				else:
					self.btnAnswer[num].Hide()
				num = num + 1
	
			if entire_questpage_number > 2:
				if cur_questpage_number == 1:
		...
		def FirstQuestPageEvent(self, n):
			global cur_questpage_number
			cur_questpage_number = 1
			self.RefreshQuestPage(n)
	
		def LastQuestPageEvent(self, n):
			global cur_questpage_number
			cur_questpage_number = entire_questpage_number
			self.RefreshQuestPage(n)
	...
	if app.ENABLE_QUEST_PAGE_SCROLLING:	
		def OnMouseWheel(self, nLen):
			global cur_questpage_number, entire_questpage_number
	
			if nLen > 0:
				if  1 < cur_questpage_number <= entire_questpage_number:
					self.PrevQuestPageEvent(1, self.n)
			else:
				if cur_questpage_number < entire_questpage_number:
					self.NextQuestPageEvent(1, self.n)
			return True