...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	snprintf(szQuery, sizeof(szQuery),
		"SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name"
#ifdef ENABLE_YOHARA_SYSTEM
		", conquerorlevel, sungma_str, sungma_hp, sungma_move, sungma_inmune"
#endif
		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				if (pt)
				{
					...
					pkTab->players[j].bySungmaInmune			= pt->sungma_inmune;
#endif
				...
				else
				{
					...
					pkTab->players[j].bySungmaInmune			= 0;
#endif
					...
					str_to_number(pkTab->players[j].bySungmaInmune, row[col++]);
#endif
