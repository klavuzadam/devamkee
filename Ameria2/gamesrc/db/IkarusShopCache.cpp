...
{
	template<class T>
	bool IsItemExpired(const time_t now, const T& item){
	...
	*/
	...
	bool CShopCache::PutItem(DWORD dwOwnerID, DWORD dwItemID, const TShopCacheItemInfo& item)
	{
		...
		auto& itemsmap = cache->itemsmap;
		if(itemsmap.find(dwItemID) != itemsmap.end())
	...
	)
	{
		...
		if(m_shopsMap.find(dwOwnerID) != m_shopsMap.end())
			return false;
	...
	void CShopCache::ShopDurationProcess()
	{
		...
		for (auto& [owner, shop] : m_shopsMap)
		{
			...
			if(expiredShops.find(owner) != expiredShops.end())
				continue;
	...
	bool CSafeboxCache::PutSafebox(DWORD dwOwnerID, const TSafeboxCacheInfo& safebox)
	{
		...
		if(m_safeboxMap.find(dwOwnerID) != m_safeboxMap.end())
			return false;
		// inserting into map
	...
	bool CSafeboxCache::PutItem(DWORD dwOwnerID, DWORD dwItem, const TShopPlayerItem& item)
	{
		...
		auto& itemsmap = cache->itemsmap;
		if(itemsmap.find(dwItem) != itemsmap.end())
			return false;
	...
	bool COfferCache::Puts(const TOfferInfo& offer)
	{
		...
		if(m_mapOffersByBuyer.find(offer.offerid) != m_mapOffersByBuyer.end())
			return false;
