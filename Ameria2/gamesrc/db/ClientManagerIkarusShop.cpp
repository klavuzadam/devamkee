...
{
	// moving all shop items with no ikashop_data to safebox with no expiration
	CDBManager::Instance().DirectQuery("UPDATE `item` SET `ikashop_data` = '{\"expiration\": 0}', `window` = 'IKASHOP_SAFEBOX' WHERE `window` = 'IKASHOP_OFFLINESHOP' AND (`ikashop_data`='' OR `ikashop_data` is NULL)");
	...
	for(uint32_t i=0; i < msg->Get()->uiNumRows; i++)
	{
		...
		str_to_cstring(item.name, row[col++]);
#endif
		// fetching json
		auto jstr = row[col++];
		if(!jstr)
		{
			sys_err("INVALID JSON ON SHOP ITEM %d (owner %d)  json value (nullptr)", item.id, item.owner);
			continue;
		}
		const auto jlen = strnlen(jstr, 200);
		std::string jsonStr = {jstr, jlen};
...
#endif
#ifdef ENABLE_NEW_NAME_ITEM
		str_to_cstring(info.name, row[col++]);
#endif
...
void CClientManager::RecvIkarusShopPacket(CPeer* peer,const char* data)
{
    ...
    case ikashop::SUBHEADER_GD_BUY_LOCK_ITEM_MULTI:
    {
        success = RecvIkarusShopLockBuyItemMulti(peer, data);
        break;
    }
#endif
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
bool CClientManager::RecvIkarusShopLockBuyItemMulti(CPeer* pPeer, const char* data)
{
    const size_t count = Decode<size_t>(data);
    if (count == 0)
    {
        sys_err("something wrong with the count == 0");
        return false;
    }

    std::vector<ikashop::TSubPacketGDLockBuyItem> vecItems;
    vecItems.reserve(count);

    for (size_t i = 0; i < count; i++)
    {
        vecItems.emplace_back(Decode<ikashop::TSubPacketGDLockBuyItem>(data));
    }
    ...
    for (const auto& item : vecItems)
    {
        ...
        // searching shop
        auto shop = m_offlineshopShopCache.Get(item.ownerid);
        if (!shop)
        {
            sys_err("cannot find buy target item %u (owner %u , buyer %u).", item.itemid, item.ownerid, item.guestid);
            continue;
        }

        // searching item
        auto itemIter = shop->itemsmap.find(item.itemid);
        if(itemIter == shop->itemsmap.end())
        {
            sys_err("cannot find buy target item %u (owner %u , buyer %u) ", item.itemid, item.ownerid, item.guestid);
            continue;
        ...
        auto pItem = itemIter->second;

        // finally selling it
        if (m_offlineshopShopCache.LockSellItem(item.ownerid, item.itemid, item.seenprice))
        {
#ifdef ENABLE_IKASHOP_LOGS
            IkarusShopLog(item.guestid, item.itemid, "LOCK_ITEM_TO_BUY", item.ownerid, "", pItem->vnum, pItem->count, pItem->price.yang
#ifdef ENABLE_CHEQUE_SYSTEM
                , pItem->price.cheque
#endif
            );
            ...
            SendIkarusShopBuyLockedItemPacket(pPeer, item.ownerid, item.guestid, item.itemid);
#ifdef EXTEND_IKASHOP_PRO
            std::string itemPrice = fmt::format("{}", pItem->price.yang);
#ifdef ENABLE_CHEQUE_SYSTEM
            itemPrice += fmt::format("|{}", pItem->price.cheque);
#endif
            AppendIkarusShopNotification(
                ikashop::ENotificationType::SELLER_SOLD_ITEM, item.ownerid, pItem->vnum, "", itemPrice);
#endif

#ifdef EXTEND_IKASHOP_ULTIMATE
            ikashop::TSaleHistory data{};
            data.account = item.accountid;
            data.count = pItem->count;
            data.vnum = pItem->vnum;
            data.datetime = std::time(nullptr);
            data.price = item.normalizedPrice;
    ...
    if (pPeer)
    {
        TPacketDGNewIkarusShop p;
        p.bSubHeader = ikashop::SUBHEADER_DG_REFRESH_SHOPSEARCH;

        pPeer->EncodeHeader(HEADER_DG_NEW_OFFLINESHOP, 0, sizeof(TPacketDGNewIkarusShop) + sizeof(uint32_t));
        pPeer->Encode(&p, sizeof(TPacketDGNewIkarusShop));
        pPeer->Encode(&guestid, sizeof(uint32_t));
    }
...
bool CClientManager::RecvIkarusShopSafeboxAddItem(const char* data) {
	...
	if (!m_offlineshopSafeboxCache.AddItem(subpack.ownerid, subpack.itemid, subpack.item))
		return false;
#ifdef ENABLE_IKASHOP_LOGS
	IkarusShopLog(subpack.ownerid, subpack.itemid, "SHOPPING_SAFEBOX_ADD_ITEM", 0, "", subpack.item.vnum, subpack.item.vnum);
#endif
	return true;
}
...
bool CClientManager::RecvIkarusShopRestoreDuration(const char* data)
{
	...
	for(auto& peer : m_peerList)
	{
		if(peer->GetChannel() != 0)
		{
			...
			peer->Encode(&pack, sizeof(pack));
			peer->Encode(&owner, sizeof(owner));
		}
	}
	return true;
}
...
bool CClientManager::RecvIkarusShopDecorationUse(const char* data)
{
	...
	const auto query = fmt::format("UPDATE `ikashop_offlineshop` SET `decoration` = {}, `decoration_time` = {} WHERE `owner` = {}", info.index, (uint32_t)OFFSHOP_DECORATION_TIME, info.owner);
...
bool CClientManager::SendIkarusShopChangeName(DWORD dwOwnerID, const char* szName)
{
	...
	subpack.ownerid	= dwOwnerID;
	str_to_cstring(subpack.name, szName);
...
void CClientManager::IkarusShopExpiredAuction(DWORD ownerid)
{
	...
	if (winoffer)
	{
		...
		{
			if(notifiedIds.find(offer->buyerid) == notifiedIds.end())
			{
