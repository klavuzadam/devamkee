...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	bool		InitializeItemCostumeHairTable();
	bool		InitializeItemCostumeTable();
...
#endif
	bool 		InitializeItemRingsAttrTable();
	bool 		InitializeItemRingsRareTable();
	...
	void		__QUERY_PLAYER_HAIR_CHANGE(CPeer * peer, DWORD dwHandle, TPlayerChangeHairPacket *);
	...
	std::vector<TItemAttrTable>		m_vec_itemCostumeTable;
	std::vector<TItemAttrTable>		m_vec_itemCostumeHairTable;

	std::vector<TItemAttrTable>		m_vec_itemRingsAttrTable;
	std::vector<TItemAttrTable>		m_vec_itemRingsRareTable;
...
public:
    ...
    bool RecvIkarusShopLockBuyItemMulti(CPeer* pPeer, const char* data);
#endif
...
private:
	std::vector<TPacketDungeonCooldownMgrSet> vecDungeonCooldown;
	boost::unordered_map<std::string, TPacketDungeonCooldownMgr> m_mapDungeonCooldown;

public:
	bool InitializeDungeonCooldownMgrTable();
	void DungeonCooldownMgrUpdate();
	void DungeonCooldownMgrSet(CPeer* pPeer, const TPacketDungeonCooldownMgrSet* p);
	void DungeonCooldownMgrExpire(CPeer* pPeer, const TPacketDungeonCooldownMgrExpire* p, uint32_t dwLength);
#endif
