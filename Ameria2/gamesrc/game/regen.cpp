	...
	while (true)
	{
			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			if (regen->type == REGEN_TYPE_MOB)
			{
				...
				else if (p->m_table.bType == CHAR_TYPE_NPC || p->m_table.bType == CHAR_TYPE_WARP || p->m_table.bType == CHAR_TYPE_GOTO || (p->m_table.bType == CHAR_TYPE_MONSTER && p->m_table.bRank >= MOB_RANK_BOSS))
				{
					SECTREE_MANAGER::instance().InsertNPCPosition(lMapIndex,
							p->m_table.bType,
							regen->vnum,
							(regen->sx+regen->ex) / 2 - base_x,
							(regen->sy+regen->ey) / 2 - base_y, regen->time);
				}
#endif
			}
