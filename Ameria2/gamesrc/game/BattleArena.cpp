	...
	if (pInfo != NULL)
	{
		switch (pInfo->state)
		{
			...
			case 3 :
				{
					...
					{
						pInfo->state = 6;
						SendNoticeMap("21", pInfo->nMapIndex, false);
					}
					else
					{
						pInfo->wait_count++;
						...
						{
							pInfo->state++;
							SendNoticeMap("22", pInfo->nMapIndex, false);
						}
						else
						{
			...
			case 4 :
				{
					pInfo->state++;
					SendNoticeMap("23", pInfo->nMapIndex, false);
					SendNoticeMap("24", pInfo->nMapIndex, false);
			...
			case 6 :
				{
					...
					SendNoticeMap("25", pInfo->nMapIndex, false);
					SendNoticeMap("26", pInfo->nMapIndex, false);
			...
			case 7 :
				{
					if ( SECTREE_MANAGER::instance().GetMonsterCountInMap(pInfo->nMapIndex) <= 0 )
					{
						SendNoticeMap("27", pInfo->nMapIndex, false);
						SendNoticeMap("24", pInfo->nMapIndex, false);
					...
					{
						SendNoticeMap("28", pInfo->nMapIndex, false);
						SendNoticeMap("24", pInfo->nMapIndex, false);
...
bool CBattleArena::Start(int nEmpire)
{
	...
	char szBuf[1024];
	snprintf(szBuf, sizeof(szBuf), "29 %s", EMPIRE_NAME(m_nEmpire));
