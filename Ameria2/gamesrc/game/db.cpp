		...
		case QID_AUTH_LOGIN:
			{
				...
				{
					char szRoutherHwid[(18 + 1) * 2 + 1];
					DBManager::instance().EscapeString(szRoutherHwid, sizeof(szRoutherHwid), pinfo->hwid_router, strlen(pinfo->hwid_router));
					char szQueryUpdate[124];
					snprintf(szQueryUpdate, sizeof(szQueryUpdate), "UPDATE account.account SET hwid_routher = '%s' WHERE id=%u", szRoutherHwid, dwID);
					std::unique_ptr<SQLMsg> pMsgUpdate(DBManager::instance().DirectQuery(szQueryUpdate));
				}
#endif
				...
				if (ch)
				{
					...
					else
					{
						...
						if (pkItem)
						{
							...
							//ch->ChatPacket(CHAT_TYPE_INFO, "304 %s", pkItem->GetName());
...
void VCardUse(LPCHARACTER CardOwner, LPCHARACTER CardTaker, LPITEM item)
{
	...
	CardTaker->ChatPacket(CHAT_TYPE_INFO, "632 %d %d", item->GetSocket(1) / 60, item->GetSocket(0));
