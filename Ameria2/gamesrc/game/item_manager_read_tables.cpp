...
bool ITEM_MANAGER::ReadCommonDropNewItemFile(const char* c_pszFileName)
{
	if (m_map_pkCommonNewItemGroup.size())
	{
		for (auto it = m_map_pkCommonNewItemGroup.begin(); it != m_map_pkCommonNewItemGroup.end(); ++it)
			M2_DELETE(*it);
		m_map_pkCommonNewItemGroup.clear();
	}
	...
	{
		CCommonDropNew* dropPtr = M2_NEW CCommonDropNew;

		TTokenVector* pTok;
		loader.SetChildNode(i);
		loader.GetCurrentNodeName(&stName);
		if (loader.GetTokenVector("map_index", &pTok))
		{
			if (pTok->size() == 0)
			{
				//sys_err("ReadCommonDropNewItemFile : Syntax error %s : no map index, node %s", c_pszFileName, stName.c_str());
				loader.SetParentNode();
				M2_DELETE(dropPtr);
				return false;
			}
			for (DWORD j = 0; j < pTok->size(); ++j)
			{
				DWORD mapIndex;
				str_to_number(mapIndex, pTok->at(j).c_str());
				//sys_err("map_index %d", mapIndex);
				if (mapIndex == 0)
					continue;
				dropPtr->m_vecMapIndex.emplace_back(mapIndex);
			}
		}
		pTok->clear();
		if (loader.GetTokenVector("level_range", &pTok))
		{
			if (pTok->size() != 2)
			{
				//sys_err("ReadCommonDropNewItemFile : Syntax error %s : no level range, node %s", c_pszFileName, stName.c_str());
				loader.SetParentNode();
				M2_DELETE(dropPtr);
				return false;
			}
			str_to_number(dropPtr->minLevel, pTok->at(0).c_str());
			str_to_number(dropPtr->maxLevel, pTok->at(1).c_str());
			//sys_err("level_range %d %d", dropPtr->minLevel, dropPtr->maxLevel);
		}
		pTok->clear();
		for (int k = 1; k < 256; ++k)
		{
			...
			if (loader.GetTokenVector(buf, &pTok))
			{
				if (pTok->size() != 3)
				{
					//sys_err("ReadCommonDropNewItemFile : Syntax error %s : no item data, node %s", c_pszFileName, stName.c_str());
					loader.SetParentNode();
					M2_DELETE(dropPtr);
					return false;
				}
				...
				float itemPct;
				str_to_number(itemVnum, pTok->at(0).c_str());
				str_to_number(itemCount, pTok->at(1).c_str());
				str_to_number(itemPct, pTok->at(2).c_str());

				//sys_err("item %d %d %.2f", itemVnum, itemCount, itemPct);

				DWORD newPct = (DWORD)(40000.0f * itemPct);
				dropPtr->AddItem(itemVnum, itemCount, newPct);
			}
		}
		loader.SetParentNode();
		if (dropPtr->m_vecItems.size() == 0)
		{
			M2_DELETE(dropPtr);
			continue;
		}
		m_map_pkCommonNewItemGroup.emplace_back(dropPtr);
	}
	/*
	...
	while (fgets(buf, 1024, fp))
	{
		...
		split_argument(buf, vecArgs);

		if (vecArgs.size() == 0)
			continue;
		else if (vecArgs[0] == "DROP_BEGIN")
		{
			if (dropPtr)
				return false;
			dropPtr = M2_NEW CCommonDropNew;
			continue;
		}
		else if (vecArgs[0] == "DROP_END")
		{
			if (!dropPtr)
				continue;
			m_map_pkCommonNewItemGroup.emplace_back(dropPtr);
		...
		else if (vecArgs[0] == "LV_MIN")
		{
			if (!dropPtr)
				continue;
			if (vecArgs.size() < 2)
				continue;
			str_to_number(dropPtr->minLevel, vecArgs[1].c_str());
			continue;
		}
		else if (vecArgs[0] == "LV_MAX")
		{
			if (!dropPtr)
				continue;
			if (vecArgs.size() < 2)
				continue;
			str_to_number(dropPtr->maxLevel, vecArgs[1].c_str());
			continue;
		}
		else if (vecArgs[0] == "MAP_INDEX")
		{
			if (!dropPtr)
				continue;
			if (vecArgs.size() < 2)
			...
			if (vecArgs.size())
			{
				for (DWORD j = 1; j < vecArgs.size(); ++j)
				{
					DWORD mapIndex;
					str_to_number(mapIndex, vecArgs[j].c_str());
					if (mapIndex == 0)
						continue;
					dropPtr->m_vecMapIndex.emplace_back(mapIndex);
				}
		...
		else if (vecArgs[0] == "ITEM_ID")
		{
			...
			DWORD itemVnum, itemCount;
			float dropPct;
			str_to_number(itemVnum, vecArgs[1].c_str());
			str_to_number(itemCount, vecArgs[2].c_str());
			str_to_number(dropPct, vecArgs[3].c_str());
			DWORD newPct = (DWORD)(40000.0f * dropPct);
			dropPtr->AddItem(itemVnum, itemCount, newPct);
			continue;
		}
	}
...
bool ITEM_MANAGER::ReadCommonDropItemFile(const char * c_pszFileName)
{
	...
	int lines = 0;
#ifdef SHOW_DROPS_IN_DB
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("DELETE from common.common_drop_item;"));
#endif
	while (fgets(buf, 1024, fp))
	{
		++lines;
		...
		for (int i = 0; i <= MOB_RANK_S_KNIGHT; ++i)
		{
			...
			if (d[i].iLvStart == 0)
				continue;
#ifdef SHOW_DROPS_IN_DB
			std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("INSERT INTO common.common_drop_item (vnum, count, lv_start, lv_end, percent) VALUES(%d,%d,%d,%d,%d);", dwItemVnum, 1, d[i].iLvStart, d[i].iLvEnd, dwPct/100));
#endif
...
bool ITEM_MANAGER::ReadMonsterDropItemGroup(const char * c_pszFileName)
{
	...
	if (!loader.Load(c_pszFileName))
		return false;
#ifdef SHOW_DROPS_IN_DB
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("DELETE from common.mob_drop_item;"));
#endif
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		std::string stName("");
...
#ifdef SHOW_DROPS_IN_DB
					std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("INSERT INTO common.mob_drop_item (group_name, type, mob, kill_drop, vnum, count, iPartPct, iRarePct) VALUES('%s','%s',%d,%d,%d,%d,%d,%d);", stName.c_str(), strType.c_str(), iMobVnum, iKillDrop, dwVnum, iCount, iPartPct, iRarePct));
#endif
...
#ifdef SHOW_DROPS_IN_DB
					std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("INSERT INTO common.mob_drop_item (group_name, type, mob, kill_drop, vnum, count, iPartPct) VALUES('%s','%s',%d,%d,%d,%d,%d);", stName.c_str(), strType.c_str(), iMobVnum, iKillDrop, dwVnum, iCount, dwPct));
#endif
		...
		else if ( strType == "limit" )
		{
			...
			for ( int k=1; k < 256; k++ )
			{
				...
				if ( loader.GetTokenVector(buf, &pTok) )
				{
					...
					sys_log(0,"        name %s pct %d count %d", name.c_str(), dwPct, iCount);
#ifdef SHOW_DROPS_IN_DB
					std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("INSERT INTO common.mob_drop_item (group_name, type, mob, kill_drop, level_limit, vnum, count, iPartPct) VALUES('%s','%s',%d,%d,%d,%d,%d,%d);", stName.c_str(), strType.c_str(), iMobVnum, iKillDrop, iLevelLimit, dwItemVnum, iCount, dwPct));
#endif
		...
		m_map_pkItemMobVnumGroup.push_back(iMobVnum);
#endif		
		loader.SetParentNode();
	}
