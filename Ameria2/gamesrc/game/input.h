...
class CInputLogin : public CInputProcessor
{
	...
	protected:
		...
		void		CharacterHairChange(LPDESC d, const char * data);
...
class CInputMain : public CInputProcessor
{
	...
	protected:
		...
		void		BattleRoyale(LPCHARACTER ch, const char* c_pData);
#endif
        ...
        void Captcha(LPCHARACTER ch, const char* c_pData);
#endif
#ifdef USE_CURRENCY_EXCHANGE
		void CurrencyExchange(LPCHARACTER ch, const char* data);
#endif
#ifdef USE_CRYSTAL_SYSTEM
		int RecvShardCraft(LPCHARACTER ch, const char* data, size_t uiBytes);
#endif
};
...
protected:
	...
	protected:
		void DungeonCooldownAdd(const TPacketDungeonCooldownMgrSet* p);
		void DungeonCooldownExpire(const char* c_pData);
#endif

#ifdef USE_DAY_CHANGE_MGR
	protected:
		void DayChangeProcess(const int* p);
#endif
};
...
#if defined(__BL_BATTLE_ROYALE__)
		void		BattleRoyale(const char* c_pData);
#endif
#ifdef ENABLE_SWITCHBOT
...
#endif
		void		DisconnectP2P(const char * c_pData);

		void PlayerRank(LPDESC d, const char* c_pData);
		void PlayerRankReload(LPDESC d, const char* c_pData);
