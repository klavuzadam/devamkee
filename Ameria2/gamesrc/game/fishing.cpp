...
void FishingSuccess(LPCHARACTER ch)
{
	...
	p.bFishingGame = quest::CQuestManager::instance().GetEventFlag("fishing_game");
#endif

	TPacketGCFishing animation;
	animation.header = HEADER_GC_FISHING;
	animation.subheader = FISHING_SUBHEADER_GC_SUCCESS_ANIM;
	animation.info = ch->GetVID();
	animation.dir = (BYTE)(ch->GetRotation() / 5);

	ch->PacketAround(&animation, sizeof(TPacketGCFishing));
...
void FishingFail(LPCHARACTER ch)
{
	...
	TPacketGCFishing animation;
	animation.header = HEADER_GC_FISHING;
	animation.subheader = FISHING_SUBHEADER_GC_FAIL_ANIM;
	animation.info = ch->GetVID();
	animation.dir = (BYTE)(ch->GetRotation() / 5);
...
void FishingPractice(LPCHARACTER ch)
{
	...
	if (rod && rod->GetType() == ITEM_ROD)
	{
		if ( rod->GetRefinedVnum()>0 && rod->GetSocket(0) < rod->GetValue(2) && number(1,rod->GetValue(1))==1 )
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "665 %d %d",rod->GetSocket(0), rod->GetValue(2));
			if (rod->GetSocket(0) == rod->GetValue(2))
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "666");
				ch->ChatPacket(CHAT_TYPE_INFO, "667");
			}
		}
	}
	rod->SetSocket(2, 0);
...
#if defined(ENABLE_FISHING_GAME)
	BOOL bFishingGame = quest::CQuestManager::instance().GetEventFlag("fishing_game");
#endif
	switch (info->step)
	{
		case 0:
...
#if defined(ENABLE_FISHING_GAME)
			return bFishingGame ? 1 : PASSES_PER_SEC(6/*6*/);
#else
...
#if defined(ENABLE_FISHING_GAME)
	BOOL bFishingGame = quest::CQuestManager::instance().GetEventFlag("fishing_game");
	p.bFishingGame = bFishingGame;
	time = bFishingGame ? 15 : time;
#endif

	p.fishing_key = rand();

	ch->SetFishingKey(p.fishing_key);

	TPacketGCFishing animation;
	animation.header = HEADER_GC_FISHING;
	animation.subheader = FISHING_SUBHEADER_GC_START_ANIM;
	animation.info = ch->GetVID();
	animation.dir = (BYTE)(ch->GetRotation() / 5);
...
int Compute(LPCHARACTER ch, DWORD fish_id, DWORD ms, DWORD* item, int level, DWORD rod_vnum)
{
	if (fish_id == 0)
		return -2;
...
#if defined(ENABLE_FISHING_GAME)
		BOOL bFishingGame = quest::CQuestManager::instance().GetEventFlag("fishing_game");
		if (bFishingGame != FALSE)
		{
			int fish_difficulty = fish_info[fish_id].difficulty;

			// More chance to catch fish while using carbon rod
			if (ch)
			{
				bool carbon_rod = (rod_vnum == 27591);

				if (carbon_rod)
					fish_difficulty -= std::abs(fish_difficulty / 2);
			...
			if (number(0, fish_difficulty) <= level)
			{
...
void Take(fishing_event_info* info, LPCHARACTER ch, uint32_t fishing_key)
{
	ch->SetLastFishingTime();
	...
	if (info->step == 1)
	{
		...
		DWORD item_vnum = 0;
		int ret = Compute(ch, info->fish_id, ms, &item_vnum, GetFishingLevel(ch), ch->GetWear(WEAR_WEAPON)->GetVnum());
		...
		switch (ret)
		{
			...
			case 0:
				if (item_vnum)
				{
					...
					if (fishing_key != ch->GetFishingKey())
					{
						if (ch->IsGM())
							ch->ChatPacket(CHAT_TYPE_INFO, "Wrong key (%d != %d)", fishing_key, ch->GetFishingKey());

						ch->ChatPacket(CHAT_TYPE_INFO, "You lost the fish.");
						return;
					}

#ifdef ENABLE_BATTLE_PASS
					CHARACTER_MANAGER::Instance().DoMission(ch, MISSION_CATCH_FISH, 1, item_vnum);
#endif
					...
					if (item)
					{
                        ...
                        ch->UpdateMissionProgress(MISSION_CATCH_FISH, 1, item_vnum);
#endif
						item->SetSocket(0,GetFishLength(info->fish_id));
						if (test_server)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "670", item->GetSocket(0)/100.f);
						}
						...
						if (quest::CQuestManager::instance().GetEventFlag("fishevent") > 0 && (info->fish_id == 5 || info->fish_id == 6))
						{
							...
							if (info->fish_id == 5)
							{
								strlcpy(p.szBoard, "671", sizeof(p.szBoard));
							}
							else if (info->fish_id == 6)
							{
								strlcpy(p.szBoard, "672", sizeof(p.szBoard));
							}
...
#ifdef BATTLEPASS_WORLDARD
						CBattlePass::instance().UpdateMisionBattlePass(ch,CATCH_FISH_BP,1);					
#endif
	...
	else
	{
		...
		TPacketGCFishing animation;
		animation.header = HEADER_GC_FISHING;
		animation.subheader = FISHING_SUBHEADER_GC_START_ANIM_END;
		animation.info = ch->GetVID();
		animation.dir = (BYTE)(ch->GetRotation() / 5);
	...
	if (info->step)
	{
		if (fishing_key != 0 && fishing_key != ch->GetFishingKey() && ch->GetFishingGameGoals() < 3)
		{
			if (ch->IsGM())
				ch->ChatPacket(CHAT_TYPE_INFO, "[practice] Wrong key (%d != %d)", fishing_key, ch->GetFishingKey());
			return;
		}
...
void Simulation(int level, int count, int prob_idx, LPCHARACTER ch)
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		DWORD item = 0;
		Compute(ch, fish_id, (number(2000, 4000) + number(2000,4000)) / 2, &item, level, ch->GetWear(WEAR_WEAPON)->GetVnum());
	...
	for (std::map<std::string,int>::iterator it = fished.begin(); it != fished.end(); ++it)
		ch->ChatPacket(CHAT_TYPE_INFO, "673 %s %d", it->first.c_str(), it->second);

	ch->ChatPacket(CHAT_TYPE_INFO, "674 %d %d", fished.size(), total_count);
}
...
void Grill(LPCHARACTER ch, LPITEM item)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "678 %s", item->GetName());
	item->SetCount(0);
	ch->AutoGiveItem(fish_info[idx].grill_vnum, count);
#ifdef BATTLEPASS_WORLDARD
	CBattlePass::instance().UpdateMisionBattlePass(ch,FRY_FISH_BP,1);					
#endif
}
...
int RealRefineRod(LPCHARACTER ch, LPITEM item)
{
	...
	else
	{
		...
		{
			// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, "679 %u", rod->GetSocket(0));
			int cur = rod->GetSocket(0);
			rod->SetSocket(0, (cur > 0) ? (cur - (cur * 10 / 100)) : 0);
			// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, "680 %u", rod->GetSocket(0));
			// ch->ChatPacket(CHAT_TYPE_INFO, "681 %%");
