...
namespace ikashop
{
	...
	enum EShopActionWeight 
	{
		...
		SHOP_ACTION_WEIGHT_TELEPORT_TO_SHOP_ENTITY = 5,
#endif
	};
...
#endif
#ifdef USE_IKASHOP_UI_RENEWED
/*cli*/void RecvTeleportToShopEntityClientPacket(LPCHARACTER ch, DWORD dwPID);
#endif
#endif
...
#ifdef USE_IKASHOP_UI_RENEWED
/*cli.*/void SendShopSafeboxValutesRefreshClientPacket(LPCHARACTER ch, const TValutesInfo& valute);
#endif
...
/*cli.*/void RecvShopMoveItemClientPacket(LPCHARACTER ch, int srcPos, int destPos);
        ...
        void ShopSearchUpdateResults(LPCHARACTER ch, bool refreshpage, bool buysearch);

        void RecvShopSearchOpen(LPCHARACTER ch);
        void RecvShopSearchChangePage(LPCHARACTER ch, int32_t pageIdx);

        bool RecvShopSearchBuyItem(LPCHARACTER ch, const std::vector<ikashop::TSubPacketCGShopBuyItem>& vecItems);
#endif
