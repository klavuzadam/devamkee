...
int (*check_name) (const char* cpszName) = nullptr;
...
int check_name_gb2312(const char* cpszName)
{
	...
	if (!cpszName || !*cpszName)
	{
		return 0;
	}

	const auto nameLength = strlen(cpszName);
	...
	while (cpszName[i])
	{
		...
		{
			if (!isdigit(cpszName[i]) && !isalpha(cpszName[i]))
			{
				return 0;
			}
	...
	return check_name_independent(cpszName);
}

int check_name_big5(const char* cpszName)
{
	int i;
	BYTE b1, b2;
	...
	while (cpszName[i])
	{
		if (cpszName[i] & 0x80)
		{
			...
			if ((b12 < 0xa440 || b12 > 0xc67e) && (b12 < 0xc940 || b12 > 0xf9d5))
			{
				if (test_server)
				{
					sys_log(0, "check_name_big5[%d][%s] %x %x %x", i - 2, cpszName, b1, b2, b12);
				}
...
int check_name_euckr(const char* cpszName)
{
	int code;
	const char* tmp;
...
int check_name_latin1(const char* cpszName)
{
	int code;
	const char* tmp;
...
int check_name_alphabet(const char* cpszName)
{
	if (!cpszName || !*cpszName)
	{
		return 0;
...
int check_name_sjis(const char *cpszName)
{
	const char* p = cpszName;
	const char* e = cpszName + strlen(cpszName);
...
void LocaleService_TransferDefaultSetting()
{
	...
	if (!conqueror_exp_table)
		conqueror_exp_table = exp_table_conqueror_common;
#endif
