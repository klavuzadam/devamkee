...
EVENTFUNC(ready_to_start_event)
{
	...
	switch (info->state)
	{
		case 0:
			{
				...
				if (count > 10000)
				{
					chA->ChatPacket(CHAT_TYPE_INFO, "2");
					chB->ChatPacket(CHAT_TYPE_INFO, "2");
				}
				else
				{
					...
					chA->ChatPacket(CHAT_TYPE_INFO, "3 %d", chA->GetPotionLimit());
					chB->ChatPacket(CHAT_TYPE_INFO, "3 %d", chB->GetPotionLimit());
				}
				chA->ChatPacket(CHAT_TYPE_INFO, "4");
				chB->ChatPacket(CHAT_TYPE_INFO, "4");
		...
		case 1:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, "5");
				chB->ChatPacket(CHAT_TYPE_INFO, "5");
		...
		default:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, "6");
				chB->ChatPacket(CHAT_TYPE_INFO, "6");
...
EVENTFUNC(duel_time_out)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, "1");
	...
	else
	{
		switch (info->state)
		{
			case 0:
				...
				chA->ChatPacket(CHAT_TYPE_NOTICE, "7");
				chA->ChatPacket(CHAT_TYPE_NOTICE, "8");

				chB->ChatPacket(CHAT_TYPE_NOTICE, "7");
				chB->ChatPacket(CHAT_TYPE_NOTICE, "8");
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	{
		pCharB->ChatPacket(CHAT_TYPE_NOTICE, "10");
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, "11");
		restart = false;
	}
	else if (pCharA != NULL && pCharB == NULL)
	{
		pCharA->ChatPacket(CHAT_TYPE_NOTICE, "10");
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, "11");
		restart = false;
	}
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "12 %s", pCharA->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "12 %s", pCharA->GetName());
			...
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, "13 %s", pCharA->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, "13 %s", pCharA->GetName());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "12 %s", pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "12 %s", pCharB->GetName());
			...
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, "13 %s", pCharB->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, "13 %s", pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacket(CHAT_TYPE_INFO, "14");

		if (pCharB != NULL)
			pCharB->ChatPacket(CHAT_TYPE_INFO, "14");
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacket(CHAT_TYPE_INFO, "15");

		if (pCharB != NULL)
			pCharB->ChatPacket(CHAT_TYPE_INFO, "15");

		SendChatPacketToObserver(CHAT_TYPE_INFO, "15");
...
void CArena::OnDisconnect(DWORD pid)
{
	...
	{
		if (GetPlayerB() != NULL)
			GetPlayerB()->ChatPacket(CHAT_TYPE_INFO, "16");
	...
	{
		if (GetPlayerA() != NULL)
			GetPlayerA()->ChatPacket(CHAT_TYPE_INFO, "16");
...
bool IsAllowedPotionOnPVP(DWORD dwVnum)
{
	switch (dwVnum)
	{
		...
		case 72730:
#ifdef USE_BOOST_POTIONS
		case BOOST_POTION_ATTACK_SPEED:
		case BOOST_POTION_MOVE_SPEED:
#endif
		{
			return true;
		}
...
bool IsLimitedPotion(DWORD dwVnum)
{
	...
	switch (dwVnum)
	{
		...
		{
#ifdef USE_BOOST_POTIONS
			const auto* proto = ITEM_MANAGER::instance().GetTable(dwVnum);
			if (proto && proto->bType == ITEM_USE && proto->bSubType == USE_SPECIAL_BOOST)
			{
				return true;
			}
#endif
