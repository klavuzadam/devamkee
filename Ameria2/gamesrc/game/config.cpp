...
int		g_aiItemDestroyTime[ITEM_DESTROY_TIME_MAX] = {300, 150, 60}; // autoitem, dropgold, dropitem
...
/*
 * NOTE : ÇÙ Ã¼Å© On/Off. CheckInÇÒ¶§ false·Î ¼öÁ¤ÇßÀ¸¸é ¹Ýµå½Ã È®ÀÎÇÏ°í °íÃÄ³õÀ»°Í!
 * ÀÌ°É·Î »ý±æ¼öÀÖ´Â ¶ËÀº Ã¥ÀÓ¾ÈÁü ~ ity ~
 */
...
static bool __LoadConnectConfigFile(const char* configName)
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fpOnlyForDB))
	{
		...
		TOKEN("VERSION")
		{
			str_to_number(versionKey, value_string);
			continue;
		}
#endif
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("auto_use")
		{
			str_to_number(g_bAutoUse, value_string);
		}
#endif
		...
		{
			str_to_number(gConquerorMaxLevel, value_string);

			gConquerorMaxLevel = MINMAX(1, gConquerorMaxLevel, PLAYER_CONQUEROR_LEVEL);
...
void config_init(const string& st_localeServiceName)
{
	...
	// public ip°¡ ¾ø¾îµµ BIND_IPÇÏ¸é °ÔÀÓ µ¹¾Æ°¡´Âµ¥¿¡´Â ¾Æ¹«·± ÁöÀåÀÌ ¾ø±â ¶§¹®¿¡
	// ÁÖ¼®Ã³¸® ÇÔ.
	if (!GetIPInfo())
	{
...
void CheckClientVersion()
{
	...
	while (it != set.end())
	{
		...
		{
			d->GetCharacter()->ChatPacket(CHAT_TYPE_NOTICE, "625");
