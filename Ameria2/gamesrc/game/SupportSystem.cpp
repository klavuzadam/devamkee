...
void CSupportActor::SetName()
{
	...
	supportName+="'s Buff";
	std::string supportnames = "'s Buff";
...
void CSupportActor::SetFlag()
{
	if (true == IsSummoned())
		m_pkChar->SetCharacterLanguage(m_pkChar->GetOwner()->GetCharacterLanguage());
}
...
void CSupportActor::SetCostume()
{
	...
	if(pSummonItem->GetSocket(1) == 0)
		m_pkChar->SetSupportArmor(11899);
	else
...
void CSupportActor::SetWeapon()
{
	...
	if(pSummonItem != NULL)
	{
		if(pSummonItem->GetSocket(0) != 0)
		{
			m_pkChar->SetSupportWeapon(pSummonItem->GetSocket(0));
		}
		else{
			m_pkChar->SetSupportWeapon(7159);
		}
	}
}
...
DWORD CSupportActor::Summon(const char* supportName, LPITEM pSummonItem, bool bSpawnFar)
{
	...
	this->SetHair();
	this->SetWeapon();
	this->SetLevel(m_pkOwner->GetLevel());
	this->SetFlag();
	...
	DWORD m_dwHairVnum = 0;
	DWORD m_dwArmorVnum = 11899;
	DWORD m_dwWeapon = 7159;
	
	if (pSummonItem->GetSocket(1) != 0)
		m_dwArmorVnum = pSummonItem->GetSocket(1);
	if (pSummonItem->GetSocket(2) != 0)
		m_dwHairVnum = pSummonItem->GetSocket(2);
	if (pSummonItem->GetSocket(0) != 0)
		m_dwWeapon = pSummonItem->GetSocket(0);

	m_pkOwner->ComputePoints();
	m_pkChar->Show(m_pkOwner->GetMapIndex(), x, y, z);
	m_pkChar->UpdatePacket();
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportLv %d", m_dwlevel);
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportArmor %d %d %d", m_dwArmorVnum, m_pkOwner->GetQuestFlag("costume_support.costume_body")-get_global_time(), m_pkOwner->GetQuestFlag("costume_support.costume_body_permanente"));
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportWeapon %u %d %d", m_dwWeapon, m_pkOwner->GetQuestFlag("costume_support.costume_weapon")-get_global_time(),m_pkOwner->GetQuestFlag("costume_support.costume_weapon_permanente"));
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportHair %u %d %d", m_pkOwner->GetQuestFlag("costume_support.costume_hair_vnum"), m_pkOwner->GetQuestFlag("costume_support.costume_hair")-get_global_time(),m_pkOwner->GetQuestFlag("costume_support.costume_hair_permanente"));
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SetSupportSkill 4 %u %d", pSummonItem->GetAttributeValue(1)+55500, m_pkOwner->GetQuestFlag("items_support.item_support_extra_1")-get_global_time());	
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SetSupportSkill 5 %u %d", pSummonItem->GetAttributeValue(2)+55500, m_pkOwner->GetQuestFlag("items_support.item_support_extra_2")-get_global_time());
	m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SetSupportSkill 6 %u %d", pSummonItem->GetAttributeValue(3)+55500, m_pkOwner->GetQuestFlag("items_support.item_support_extra_3")-get_global_time());
...
bool CSupportActor::_UpdateFollowAI()
{
	...
	if (fDist >= RESPAWN_DISTANCE)
	{
		...
		if (m_pkChar->Show(m_pkOwner->GetMapIndex(), ownerX + fx, ownerY + fy))
		{
			...
			m_pkChar->SetOwner(m_pkOwner);
			m_pkChar->SetEmpire(m_pkOwner->GetEmpire());
			this->RefreshCostume(); // I have to test this
			return true;
		}
	}
...
void CSupportActor::CheckItemCostume()
{
	LPITEM pSummonItem = ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID());
	if (pSummonItem != NULL){
		const time_t current = get_global_time();
		if (current > m_pkOwner->GetQuestFlag("costume_support.costume_body") && m_pkOwner->GetQuestFlag("costume_support.costume_body_permanente") == 0)
		{
			pSummonItem->SetSocket(1,0);
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportClear %d",1);
			m_pkOwner->SetQuestFlag("costume_support.costume_body",time(0));
		}

		if (current > m_pkOwner->GetQuestFlag("costume_support.costume_hair") && m_pkOwner->GetQuestFlag("costume_support.costume_hair_permanente") == 0)
		{
			pSummonItem->SetSocket(2,0);
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportClear %d",2);
			m_pkOwner->SetQuestFlag("costume_support.costume_hair",time(0));
			m_pkOwner->SetQuestFlag("costume_support.costume_hair_vnum",0);
		}

		if (current > m_pkOwner->GetQuestFlag("costume_support.costume_weapon") && m_pkOwner->GetQuestFlag("costume_support.costume_weapon_permanente") == 0)
		{
			pSummonItem->SetSocket(0,0);
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportClear %d",3);
			m_pkOwner->SetQuestFlag("costume_support.costume_weapon",time(0));
		}

		if (current > m_pkOwner->GetQuestFlag("items_support.item_support_extra_1"))
		{
			pSummonItem->SetForceAttribute(1, 0, 0);
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportClear %d",4);
			m_pkOwner->SetQuestFlag("items_support.item_support_extra_1",time(0));
		}

		if (current > m_pkOwner->GetQuestFlag("items_support.item_support_extra_2"))
		{
			pSummonItem->SetForceAttribute(2, 0, 0);
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportClear %d",5);
			m_pkOwner->SetQuestFlag("items_support.item_support_extra_2",time(0));
		}

		if (current > m_pkOwner->GetQuestFlag("items_support.item_support_extra_3"))
		{
			pSummonItem->SetForceAttribute(3, 0, 0);
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "SupportClear %d",6);
			m_pkOwner->SetQuestFlag("items_support.item_support_extra_3",time(0));
		}
	}
}
...
void CSupportActor::UseSkill()
{
	...
	if (pSummonItem != NULL){
		...
		if (pSummonItem->GetAttributeValue(1)+55500 != 55500){
			if(m_pkChar->GetOwner()->IsAffectFlag(AFF_HOSIN) == false && get_dword_time() - this->GetLastSkillTime() >= 3000)
			{
				DWORD type = 1;
				if (pSummonItem->GetAttributeValue(1)+55500 == 55502)
					type = 20;
				else if (pSummonItem->GetAttributeValue(1)+55500 == 55503)
					type = 30;
				else if (pSummonItem->GetAttributeValue(1)+55500 == 55504)
					type = 40;
				m_pkChar->ComputeSkill(94,m_pkChar->GetOwner(),type);
				this->SetLastSkillTime(get_dword_time());
				m_pkChar->SendSupportSkillPacket(SKILL_HOSIN, type);
			}
		}
		
		if (pSummonItem->GetAttributeValue(2)+55500 != 55500){
			if(m_pkChar->GetOwner()->IsAffectFlag(AFF_GICHEON) == false && get_dword_time() - this->GetLastSkillTime() >= 3000)
			{
				DWORD type = 1;
				if (pSummonItem->GetAttributeValue(2)+55500 == 55505)
					type = 20;
				else if (pSummonItem->GetAttributeValue(2)+55500 == 55506)
					type = 30;
				else if (pSummonItem->GetAttributeValue(2)+55500 == 55507)
					type = 40;
				m_pkChar->ComputeSkill(96,m_pkChar->GetOwner(),type);		
				this->SetLastSkillTime(get_dword_time());
				m_pkChar->SendSupportSkillPacket(SKILL_REFLECT, type);
			}
		}
		
		if (pSummonItem->GetAttributeValue(3)+55500 != 55500){
			if(m_pkChar->GetOwner()->IsAffectFlag(AFF_BOHO) == false && get_dword_time() - this->GetLastSkillTime() >= 3000)
			{
				DWORD type = 1;
				if (pSummonItem->GetAttributeValue(3)+55500 == 55508)
					type = 20;
				else if (pSummonItem->GetAttributeValue(3)+55500 == 55509)
					type = 30;
				else if (pSummonItem->GetAttributeValue(3)+55500 == 55510)
					type = 40;
				this->SetLastSkillTime(get_dword_time());
				m_pkChar->ComputeSkill(95,m_pkChar->GetOwner(),type);		
				m_pkChar->SendSupportSkillPacket(SKILL_GICHEON, type);
			}
		}
	}
}
