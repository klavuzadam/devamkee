...
void CHARACTER::OpenAuraRefineWindow(LPENTITY pOpener, EAuraWindowType type)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Only one <Aura> Aura Refine window can be open at a time.."));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Unable to open the Aura window."));
		return;
	}
	...
	if (m_playerData && !m_playerData->m_pAuraRefineWindowOpener)
	{
		m_playerData->m_pAuraRefineWindowOpener = pOpener;
	}
...
void CHARACTER::AuraRefineWindowClose()
{
	...
	if (m_playerData)
	{
		m_playerData->m_pAuraRefineWindowOpener = NULL;
	}
...
void CHARACTER::AuraRefineWindowCheckIn(BYTE bAuraRefineWindowType, TItemPos AuraCell, TItemPos ItemCell)
{
	...
	if (!IsAuraRefineWindowOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> You must first open a specific aura refining window."));
		return;
	}
	...
	if (!IsAuraRefineWindowCanRefine())
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Close all windows to continue the <aura> process."));
			return;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> The aura refining process encountered an error and could not continue."));
		return;
	}
	...
	switch (bAuraRefineWindowType)
	{
		case AURA_WINDOW_TYPE_ABSORB:
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<aura> This item could not be absorbed."));
				return;
			}

			if (ItemCell.IsEquipPosition())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Cannot attach equipped items for absorption."));
				return;
			}
			...
			if (!pItem)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> An undefined error occurred."));
				return;
			}
			...
			*/
			...
			//{
			//	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<¾Æ¿ì¶ó> ÀÌ ¾ÆÀÌÅÛÀº Èí¼ö¸¦ À§ÇØ ºÎÂø ÇÒ ¼ö ¾ø½À´Ï´Ù."));
			//	return;
			//}
			...
			if (pItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This item is used and cannot be attached for absorption."));
				return;
			}
			...
			if (pItem->IsBound())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This item is bound to the soul and cannot be attached for absorption."));
				return;
			}
#endif
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Only Aura costumes can be attached in this slot."));
				return;
			}

			if (AuraCell.cell == AURA_SLOT_MAIN && pItem->GetSocket(ITEM_SOCKET_AURA_DRAIN_ITEM_VNUM))
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This aura costume already has an absorption value."));
				return;
			}
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Only shields, bracelets, necklaces, and earrings can be attached in this slot."));
				return;
			}
		...
		case AURA_WINDOW_TYPE_GROWTH:
		{
			if (!ItemCell.IsValidItemPosition())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This aura cannot be upgraded."));
				return;
			}

			if (ItemCell.IsEquipPosition())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> cannot attach equipped items for upgrades."));
				return;
			}
			...
			*/
			//if (IS_SET(pItem->GetAntiFlag(), ITEM_ANTIFLAG_ABSORB))
			//{
			//	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<¾Æ¿ì¶ó> ¾÷±×·¹ÀÌµå¸¦ À§ÇØÀÌ Ç×¸ñÀ» Ã·ºÎ ÇÒ ¼ö ¾ø½À´Ï´Ù."));
			//	return;
			//}
			...
			if (pItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This item is in use and cannot be attached for upgrading."));
				return;
			}
			...
			if (pItem->IsBound())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This item is bound to the soul and cannot be attached for upgrades."));
				return;
			}
#endif
			...
			if (AuraCell.cell == AURA_SLOT_MAIN)
			{
				...
				else
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This aura costume cannot be upgraded."));
						if (aiAuraRefineTable[AURA_REFINE_INFO_STEP] == AURA_GRADE_RADIANT)
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This Aura costume has reached its maximum level and rank."));
						else
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Use the Refine pane for additional steps."));
						return;
					}
				}
			}
			if (AuraCell.cell == AURA_SLOT_SUB)
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> First you need to attach the aura costume."));
					return;
				}
				else if (!(pItem->GetType() == ITEM_RESOURCE && pItem->GetSubType() == RESOURCE_AURA))
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Only aura materials can be attached in this slot."));
					return;
				}
			}
		...
		case AURA_WINDOW_TYPE_EVOLVE:
		{
			if (!ItemCell.IsValidItemPosition())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> I cannot refine this aura."));
				return;
			}

			if (ItemCell.IsEquipPosition())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> cannot attach equipped items for evolution."));
				return;
			}
			...
			if (pItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This item is used and cannot be attached for evolution."));
				return;
			}
			...
			if (pItem->IsBound())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This item is bound to your soul and cannot be attached for evolution."));
				return;
			}
#endif
			...
			if (AuraCell.cell == AURA_SLOT_MAIN)
			{
				...
				else
				{
					...
					if (!(c_bCurrentLevel == aiAuraRefineTable[AURA_REFINE_INFO_LEVEL_MAX] && c_wCurrentExp == aiAuraRefineTable[AURA_REFINE_INFO_NEED_EXP]))
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This aura costume cannot be improved yet."));
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> You must be at level %d with %d experience."), aiAuraRefineTable[AURA_REFINE_INFO_LEVEL_MAX], aiAuraRefineTable[AURA_REFINE_INFO_NEED_EXP]);
						return;
					}
				}
			}
			...
			if (AuraCell.cell == AURA_SLOT_SUB)
			{
				...
				else
				{
					...
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Only certain refining materials can be attached in this slot."));
						TItemTable* pTable = ITEM_MANAGER::instance().GetTable(aiAuraRefineTable[AURA_REFINE_INFO_MATERIAL_VNUM]);
						if (pTable)
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<aura> i.e., %s - %u"), pTable->szLocaleName, aiAuraRefineTable[AURA_REFINE_INFO_MATERIAL_COUNT]);
						return;
					}
				}
			}
...
void CHARACTER::AuraRefineWindowCheckOut(BYTE bAuraRefineWindowType, TItemPos AuraCell)
{
	...
	if (!IsAuraRefineWindowOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> You can't do this without an aura window open."));
		return;
	}
	...
	if (m_bAuraRefineWindowType != bAuraRefineWindowType)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> You cannot do this while another aura window is open."));
		return;
	}
...
void CHARACTER::AuraRefineWindowAccept(BYTE bAuraRefineWindowType)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Attach the item to the <Aura> window."));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Items equipped with <Aura> cannot be used."));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Items bound to the <Aura> spirit cannot be used."));
		return;
	}
#endif
	switch (bAuraRefineWindowType)
	{
		case AURA_WINDOW_TYPE_ABSORB:
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> absorption is complete."));
		...
		case AURA_WINDOW_TYPE_GROWTH:
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your <Aura> upgrade is complete."));
		...
		case AURA_WINDOW_TYPE_EVOLVE:
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This Aura costume cannot be refined."));
				if (aiAuraRefineTable[AURA_REFINE_INFO_STEP] == AURA_GRADE_RADIANT)
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> This Aura costume has reached its maximum level and rank."));
				else
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> Use the Upgrade window for additional steps."));
				return;
			}
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> You don't have enough gold to refine the Aura Outfit."));
				return;
			}

			if (CountSpecifyItem(aiAuraRefineTable[AURA_REFINE_INFO_MATERIAL_VNUM]) < aiAuraRefineTable[AURA_REFINE_INFO_MATERIAL_COUNT])
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> You don't have the materials needed to refine the Aura Costume."));
				return;
			}
			...
			if (number(1, 100) <= c_bRefinePct)
			{
				...
				if (pkNewItem)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The <Aura> upgrade was successful."));
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Aura> failed to upgrade."));
