...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? "873" : "874");
		}
	}

	pkChar->ChatPacket(CHAT_TYPE_INFO, "875 %d", c);
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	{
		case 0:
			SendNoticeMap("876", OXEVENT_MAP_INDEX, true);
			flag++;
			return PASSES_PER_SEC(10);

		case 1:
			SendNoticeMap("877", OXEVENT_MAP_INDEX, true);

			if (info->answer == true)
			{
				COXEventManager::instance().CheckAnswer(true);
				SendNoticeMap("878", OXEVENT_MAP_INDEX, true);
			}
			else
			{
				COXEventManager::instance().CheckAnswer(false);
				SendNoticeMap("879", OXEVENT_MAP_INDEX, true);
			}

			SendNoticeMap("880", OXEVENT_MAP_INDEX, true);
		...
		case 2:
			...
			SendNoticeMap("881", OXEVENT_MAP_INDEX, true);
			flag = 0;
			break;
	}
	return 0;
...
bool COXEventManager::Quiz(unsigned char level, int timelimit)
{
	...
	SendNoticeMap("882", OXEVENT_MAP_INDEX, true);
	SendNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX, true);
	SendNoticeMap("883", OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::CheckAnswer(bool answer)
{
...
#ifdef ENABLE_NEW_OX_FEATURES
	std::vector<SEventItem> m_vect_Items;
	std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT item_vnum, item_count FROM player.event_items"));
	...
	if (pRes->uiNumRows)
	{
		...
		{
			SEventItem item = SEventItem();
			str_to_number(item.dwVnum, row[0]);
			str_to_number(item.dwCount, row[1]);
			m_vect_Items.push_back(item);
		}
	}

	if (m_vect_Items.size() > 0)
	{
		for (std::map<DWORD, DWORD>::iterator it = m_map_attender.begin(); it != m_map_attender.end(); ++it)
		{
			LPCHARACTER pkChar = CHARACTER_MANAGER::instance().FindByPID(it->second);
			if (pkChar)
			{
				SEventItem sItem = m_vect_Items[number(0, m_vect_Items.size() - 1)];

				TItemTable* pItemTable = ITEM_MANAGER::instance().GetTable(sItem.dwVnum);
				if (!pItemTable)
				{
					sys_err("[CheckAnswer] Item vnum: %u doesn't exists -> playerID %u.", sItem.dwVnum, pkChar->GetPlayerID());
					continue;
				}

				pkChar->AutoGiveItem(sItem.dwVnum, sItem.dwCount);
			}
		}
	}
#endif
