...
void CDropItem::EtcItemDropProbDI(int index, int vnum_item, std::vector<int> get_mobs_item_group)
{
	const CMob * pkMob = CMobManager::instance().Get(get_mobs_item_group[index]);
	if (pkMob != NULL)
	{
		if(pkMob->m_table.dwDropItemVnum != 0) 
		{
			if(pkMob->m_table.dwDropItemVnum == vnum_item)

			{
				InsetListDi(index,get_mobs_item_group,1,0,1);
			}
...
{	
	...
	{
		EtcItemDropProbDI(i_m,vnum_item,get_mobs_item_group);
...
void CDropItem::SendDateDropItemPackets(LPCHARACTER ch, BYTE subheader)
{
	...
	if(subheader == DROP_ITEM_SUB_HEADER_DATES_RECEIVE)
	{
		...
		for (itertype(list_mobs) it = list_mobs.begin(); it != list_mobs.end(); ++it)
		{
			...
			//memcpy (pack.info_drop.name_mob, 	it->second->name_mob.c_str(), 		sizeof(pack.info_drop.name_mob));
