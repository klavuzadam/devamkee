...
static const std::set<int> ALLOWED_SPAWN_MAPS = { 358 };
...
std::string StringToLower(const char* name, size_t len)
{
	...
	const char test = static_cast<char>(std::tolower('F'));
	return res;
}
...
bool IsGoodSalePrice(const ikashop::TPriceInfo price)
{
    ...
    if (price.yang < 0 || price.cheque < 0)
    {
        return false;
    }

    if (price.yang < 1 && price.cheque < 1)
    {
        return false;
    }

    if (price.yang > GOLD_MAX || price.cheque > CHEQUE_MAX)
    {
        return false;
    }
#else
    if (price.yang < 1 || price.yang > GOLD_MAX)
    {
        return false;
    }
#endif
...
bool MatchFilterType(const ikashop::TShopItem& info, const ikashop::TFilterInfo& filter, TItemTable* table)
{
    ...
    if (filter.subtype == -1)
    {
        switch (filter.type)
        {
            ...
            case 4:
            {
                ...
                {
#ifdef ENABLE_SASH_SYSTEM
                    if (table->bSubType == COSTUME_ACCE)
                    {
                        return false;
                    }
#endif
                    ...
                    if (table->bSubType == COSTUME_MOUNT)
                    {
                        return false;
                    }
#endif
                    ...
                    if (table->bSubType == COSTUME_MOUNT_SKIN)
                    {
                        return true;
                    }
#endif
            ...
            {
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
                if (table->bType == ITEM_COSTUME && table->bSubType == COSTUME_MOUNT)
                {
                    return true;
                }
#endif
            ...
            {
#ifdef ENABLE_NEW_PET_SYSTEM
                if (table->bType == ITEM_COSTUME && table->bSubType == COSTUME_PET)
                {
                    return true;
                }
#endif
            ...
            {
#ifdef ENABLE_NEW_PET_SYSTEM
                if (table->bType == ITEM_PET && (table->bSubType == PET_LEVELABLE || table->bSubType == PET_EGG))
                {
                    return true;
                }
#endif
    ...
    return (filter.type == table->bType && filter.subtype == table->bSubType) ? true : false;
#else
    if (filter.type == 0)
    {
        return true;
	...
	static auto MatchPet = [](const ikashop::TShopItem& info, const ikashop::TFilterInfo& filter, TItemTable* table) {
		if(table->bType != ITEM_PET || (table->bSubType != PET_EGG && table->bSubType != PET_LEVELABLE))
			return false;
		if(filter.subtype == 0)
			return true;
		switch(filter.subtype)
		...
		case ikashop::SEARCH_SHOP_SUBTYPE_PET_SEALS:
			return table->bSubType == PET_LEVELABLE;
		}
		return false;
	};
...
bool MatchFilter(const ikashop::TShopItem& info, const ikashop::TFilterInfo& filter, const std::string& filtername, LPCHARACTER ch)
{
	...
	if (!filtername.empty())
	{
		auto name = multilang::find_item_name(info.vnum, ch->GetCharacterLanguage());
		if (!MatchItemName(filtername, name.c_str(), name.size()))
			return false;
	}
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
bool MatchFilterV2(const uint32_t ownerpid, const char* ownername, const std::string& filtername, const ikashop::TShopItem& info, const ikashop::TFilterInfo& filter, LPCHARACTER ch)
{
    auto table = ITEM_MANAGER::instance().GetTable(info.vnum);
    if (!table)
    {
    ...
    if (filter.player)
    {
        ...
        {
            if (!filtername.empty() && !MatchItemName(filtername, ownername, strlen(ownername)))
            {
                return false;
            }
        }
    ...
    else
    {
        if (!filtername.empty())
        {
            if (filter.exact && filter.vnum != info.vnum)
            {
                return false;
            }
    ...
    if (filter.antiflags != 0)
    {
        static const uint32_t s_antiflags[] = {
            ITEM_ANTIFLAG_MALE,
        ...
        const size_t t = sizeof(s_antiflags) / sizeof(uint32_t);
        for (size_t i = 0; i < t; i++)
        {
            if (IS_SET(filter.antiflags, s_antiflags[i]) && !IS_SET(table->dwAntiFlags, s_antiflags[i]))
            {
                return false;
            }
        }
    ...
    if (filter.levelstart > 0 || filter.levelend > 0)
    {
        ...
        if (filter.levelstart > 0 && level < filter.levelstart)
        {
            return false;
        }

        if (filter.levelend > 0 && level > filter.levelend)
        {
            return false;
        }
    }
...
namespace ikashop
{
	...
	long long CShopManager::GetNormalizedPrice(DWORD vnum, DWORD count, long long price)
	{
		...
		return MINMAX(minprice, price, maxprice);
	}
#endif
	...
	void CShopManager::_RemoveShopOffers(SHOP_HANDLE shop)
	{
		...
		for (auto& iter : shop->GetOffers())
		{
			...
			if (it != m_mapOffersByBuyer.end())
			{
				...
				EraseIfContainer(offers, [=](auto&& boffer) { return boffer->offerid == offer->offerid; });
			}
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
        if (ch->GetShopSearchIds(itemid))
        {
            if (ch->SetShopSearchEraseItem(itemid))
            {
	...
	bool CShopManager::RecvShopCreateNewDBPacket(const TShopInfo& info)
	{
		...
		if(m_mapShops.find(info.ownerid) != m_mapShops.end())
			return false;
	...
	bool CShopManager::RecvShopOfferCancelDBPacket(DWORD offerid, DWORD ownerid, bool removing) //ikashop-updated 05/08/19
	{
		...
		if (buyiter != m_mapOffersByBuyer.end())
		{
			...
			EraseIfContainer(vec, [&](auto&& offer){ return offer->offerid == offerid; });
	...
	bool CShopManager::RecvShopSafeboxAddValutesDBPacket(DWORD ownerid, const TValutesInfo& valute)
	{
		...
		if (ch)
		{
			if (ch->GetIkarusShopSafebox())
			{
				safebox->RefreshToOwner(ch);
			}
			else
			{
				safebox->RefreshToOwnerValutes(ch);
			}
		}
#else
	...
	void CShopManager::RecvShopRegisterSaleHistoryDBPacket(const TSaleHistory& sale)
	{
		...
		EraseIfContainer(vector, [&currentTime](auto&& sale){ return sale.datetime + OFFLINESHOP_EXPIRING_SALE_HISTORY < currentTime; });
...
#endif
#ifdef ENABLE_IKASHOP_ENTITIES
		if(!ch->GetIkarusShop() && ALLOWED_SPAWN_MAPS.find(ch->GetMapIndex()) == ALLOWED_SPAWN_MAPS.end())
		{
...
// upper is doing same
    ...
    bool CShopManager::RecvShopBuyItemClientPacket(LPCHARACTER ch, DWORD ownerid, DWORD itemid, bool searching, long long seenprice)  //patch seen price check
    {
        ...
        {
            SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_CANNOT_BUY_FROM_OWNSHOP");
            return false;
        }
        ...
        if (shop->GetDuration() == 0)
        {
            SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_EXPIRED_SHOP_TIME_CANNOTBUY");
            return false;
        }
#endif
	...
	bool CShopManager::RecvMoveShopEntityClientPacket(LPCHARACTER ch)
	{
		...
		if (ALLOWED_SPAWN_MAPS.find(ch->GetMapIndex()) == ALLOWED_SPAWN_MAPS.end())
		{
			SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_CANNOT_CREATE_SHOP_ON_WRONG_PLACE");
			return false;
		}
...
#ifdef USE_IKASHOP_UI_RENEWED
	void CShopManager::RecvTeleportToShopEntityClientPacket(LPCHARACTER ch, DWORD dwPID)
	{
		LPDESC d = ch ? ch->GetDesc() : nullptr;
		if (!d)
		{
		...
		auto shop = (dwPID == 0 && dwPID != ch->GetPlayerID()) ? ch->GetIkarusShop() : GetShopByOwnerID(dwPID);
		if (!shop)
		{
			SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_CANNOT_TELEPORT_TO_CLOSED_SHOP");
			return;
		}

		if (!ch->IkarusShopFloodCheck(SHOP_ACTION_WEIGHT_TELEPORT_TO_SHOP_ENTITY))
		{
			return;
		}
		...
		{
			SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_CANNOT_TELEPORT_NOW");
			return;
		}

		if (ch->GetX() == spawn.x && ch->GetY() == spawn.y)
		{
			SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_CANNOT_TELEPORT_ALREADY");
			return;
		}
		...
		ch->PointChange(POINT_GOLD, -50000);

		if (ch->GetMapIndex() == spawn.map)
		{
			ch->Show(spawn.map, spawn.x, spawn.y, 0);
			ch->Stop();
		}
		else
		{
			ch->WarpSet(spawn.x, spawn.y, spawn.map);
		}
	}
#endif
#endif
	...
	void CShopManager::SendShopOpenMyShopNoShopClientPacket(LPCHARACTER ch)
	{
		...
		TValutesInfo subpack{};
		if (safebox)
		{
			subpack = safebox->GetValutes();
		}
#endif
...
#ifdef USE_IKASHOP_UI_RENEWED
		pack.size = sizeof(pack) + sizeof(TValutesInfo);
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
		ch->GetDesc()->BufferedPacket(&pack, sizeof(pack));
		ch->GetDesc()->Packet(&subpack, sizeof(TValutesInfo));
#else
	...
	void CShopManager::SendShopOpenMyShopClientPacket(LPCHARACTER ch)
	{
		...
		if (!ch || !ch->GetDesc() || !ch->GetIkarusShop())
		{
			return;
		}
	...
	bool CShopManager::RecvShopAddItemClientPacket(LPCHARACTER ch, const TItemPos& pos, const TPriceInfo& price, int destPos)
	{
		...
		sitem.dwTransmutation = item->GetTransmutation();
#endif
#ifdef ENABLE_NEW_NAME_ITEM
		str_to_cstring(sitem.name, item->GetNewName());
#endif
	...
	bool CShopManager::RecvShopFilterRequestClientPacket(LPCHARACTER ch, const TFilterInfo& filter)
	{
		...
		for (auto& [id, auction] : m_mapAuctions)
		{
			...
			if (!MatchFilter(info, filter, filtername, ch))
				continue;
	...
	bool CShopManager::RecvShopSearchFillRequestClientPacket(LPCHARACTER ch)
	{
		...
		for (int i = 0; i < 20; i++)
		{
			...
			if(matchedIds.find(item->GetInfo().id) != matchedIds.end())
				continue;
		...
		for(int i = 0; i < 5; i++)
		{
			...
			if (matchedIds.find(auction->GetInfo().id) != matchedIds.end())
				continue;
	...
	bool CShopManager::RecvShopSafeboxGetValutesClientPacket(LPCHARACTER ch)
	{
		...
		const long long nTotalMoney = static_cast<long long>(ch->GetGold()) + static_cast<long long>(claimingValute.yang);
		if (nTotalMoney > GOLD_MAX)
		{
#ifdef ENABLE_CHEQUE_SYSTEM
			// checking about yang & cheque overflow
			const int64_t yangSpace = GOLD_MAX - ch->GetGold();
			const int64_t chequeSpace = CHEQUE_MAX - ch->GetCheque();
...
#else
			claimingValute.yang = GOLD_MAX - ch->GetGold();
	...
	{
		if (!ch || !ch->GetDesc() || !ch->GetIkarusShopSafebox())
		{
			return;
		}
...
#ifdef USE_IKASHOP_UI_RENEWED
	void CShopManager::SendShopSafeboxValutesRefreshClientPacket(LPCHARACTER ch, const TValutesInfo& valute)
	{
		if (!ch || !ch->GetDesc())
		{
			return;
		...
		TPacketGCNewIkarusShop pack{};
		pack.header = HEADER_GC_NEW_OFFLINESHOP;
		pack.size = sizeof(pack) + sizeof(TSubPacketGCShopSafeboxRefreshValutes);
		pack.subheader = SUBHEADER_GC_SHOP_SAFEBOX_REFRESH_VALUTES;

		TSubPacketGCShopSafeboxRefreshValutes subpack{};
		subpack.valute = valute;
...
#endif
#ifdef ENABLE_NEW_NAME_ITEM
		str_to_cstring(auction.name, item->GetNewName());
#endif
#ifdef ENABLE_IKASHOP_ENTITIES
	#ifdef EXTEND_IKASHOP_ULTIMATE
	...
	void CShopManager::SendBoardCountersClientPacket(LPCHARACTER ch)
	{
...
#ifdef USE_IKASHOP_UI_RENEWED
		const auto safebox = ch->GetIkarusShopSafebox() ? ch->GetIkarusShopSafebox() : GetShopSafeboxByOwnerID(pid);
		if (safebox)
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
		const auto auction = ch->GetIkarusAuction() ? ch->GetIkarusAuction() : GetAuctionByOwnerID(pid);
		if (auction)
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
		const auto shop = ch->GetIkarusShop() ? ch->GetIkarusShop() : GetShopByOwnerID(pid);
		if (shop)
#else
		...
		d->Packet(&subpack, sizeof(subpack));
	}
	...
	void CShopManager::RecvPriceAverageRequestClientPacket(LPCHARACTER ch, DWORD vnum, DWORD count)
	{
		...
		ch->GetDesc()->BufferedPacket(&pack, sizeof(pack));
		ch->GetDesc()->Packet(&average, sizeof(average));
	}
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
    void CShopManager::ShopSearchUpdateResults(LPCHARACTER ch, bool refreshpage, bool buysearch)
    {
        LPDESC d = ch ? ch->GetDesc() : nullptr;
        if (!d)
        {
        ...
        if (ch->IsGM() || !ch->IsObserverMode())
        {
            ...
            if (refreshpage)
            {
                auto currPage = ch->GetShopSearchPage();
                auto start = (currPage - 1) * OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE;
                auto end = ((currPage - 1) * OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE) + OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE;

                const auto items = iter::slice(ch->m_vecShopSearchItem, start, end);
                std::vector<ikashop::TSearchResultInfo> tmpShopSearchItem;
                for (const auto& item : items)
                {
                    tmpShopSearchItem.emplace_back(item);
                }

                size_t itemcount = tmpShopSearchItem.size();

                if (buysearch && itemcount == 0)
                {
                    if (currPage > 1)
                    {
                        ch->SetShopSearchPage(currPage - 1);

                        currPage = ch->GetShopSearchPage();
                        start = (currPage - 1) * OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE;
                        end = ((currPage - 1) * OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE) + OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE;

                        const auto retryitems = iter::slice(ch->m_vecShopSearchItem, start, end);
                        for (const auto& item : retryitems)
                        {
                            tmpShopSearchItem.emplace_back(item);
                        }
                    }

                    itemcount = tmpShopSearchItem.size();
                }

                if (buysearch)
                {
                    ch->RefreshShopSearchPageCount();
                }

                TPacketGCNewIkarusShop p = {};
                p.header = HEADER_GC_NEW_OFFLINESHOP;
                p.subheader = SUBHEADER_GC_SHOP_SEARCH_RESULT;
                p.size = sizeof(TPacketGCNewIkarusShop) + sizeof(TGCShopSearchPacket) + (itemcount * sizeof(TSearchResultInfo));

                TGCShopSearchPacket p2 = {};
                p2.page = currPage;
                p2.itemcount = itemcount;
                p2.pagecount = ch->GetShopSearchMaxPageCount();

                TEMP_BUFFER tmpBuff;
                tmpBuff.write(&p, sizeof(TPacketGCNewIkarusShop));
                tmpBuff.write(&p2, sizeof(TGCShopSearchPacket));

                for (size_t i = 0; i < itemcount; i++)
                {
                    tmpBuff.write(&tmpShopSearchItem[i], sizeof(TSearchResultInfo));
                }

                d->Packet(tmpBuff.read_peek(), tmpBuff.size());

                tmpShopSearchItem.clear();
            }
            else
            {
                auto const chfilter = ch->GetShopSearchFilterInfo();
                const auto filtername = StringToLower(chfilter.name, strnlen(chfilter.name, sizeof(chfilter.name)));

/*
                for (auto& [id, auction] : m_mapAuctions)
                {
#ifdef EXTEND_IKASHOP_PRO
                    const auto duration = auction->GetInfo().duration;
                    if (duration == 0)
                    {
                        continue;
                    }
                    ...
                    const auto ownerpid = auction->GetInfo().owner;
                    if (ownerpid == pid)
                    {
                        continue;
                    }

                    const auto ownername = auction->GetInfo().ownername;

                    if (auction->GetPrice().GetTotalYangAmount() < 1)
                    {
                        continue;
                    }

                    const auto& info = auction->GetInfo();
                    if (!MatchFilterV2(ownerpid, ownername, filtername, info, chfilter, ch))
                    {
                        continue;
                    }

                    auto& result = ch->m_vecShopSearchItem.emplace_back(DerivedFromBase<TSearchResultInfo, TShopItem>(info));
                    result.auction = true;
                    result.duration = duration;
#ifdef EXTEND_IKASHOP_ULTIMATE
                    result.priceavg = GetAveragePrice(result.vnum, result.count);
#endif
                    str_to_cstring(result.seller_name, ownername.c_str());
                }
*/
                ...
                {
#ifdef EXTEND_IKASHOP_PRO
                    const auto duration = shop->GetDuration();
                    if (duration == 0)
                    {
                        continue;
                    }
                    ...
                    const auto ownerpid = shop->GetOwnerPID();
                    if (ownerpid == pid)
                    {
                        continue;
                    }

                    const auto ownername = shop->GetName();

                    for (auto& [id, sitem] : shop->GetItems())
                    {
                        if (sitem->GetPrice().GetTotalYangAmount() < 1)
                        {
                            continue;
                        }
                        ...
                        auto& result = ch->m_vecShopSearchItem.emplace_back(DerivedFromBase<TSearchResultInfo>(info));
                        result.auction = false;
                        result.duration = duration;
#ifdef EXTEND_IKASHOP_ULTIMATE
                        result.priceavg = GetAveragePrice(result.vnum, result.count);
#endif
                        str_to_cstring(result.seller_name, ownername);
                    }
                }

#ifndef USE_PREMIUM_AFFECT
                if (!ch->m_vecShopSearchItem.empty())
                {
					// Obtain a time-based seed:
					unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();

					// Shuffle the vector randomly using the seed
					std::shuffle(ch->m_vecShopSearchItem.begin(), ch->m_vecShopSearchItem.end(), std::default_random_engine(seed));
                }
#endif

                if (!ch->RefreshShopSearchPageCount())
                {
                    return;
                }

                size_t itemcount = ch->GetShopSearchMaxPageCount() > 1 ? OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE : ch->m_vecShopSearchItem.size();
                ...
                for (size_t i = 0; i < itemcount; i++)
                {
                    tmpBuff.write(&ch->m_vecShopSearchItem[i], sizeof(TSearchResultInfo));
                }
    ...
    void CShopManager::RecvShopSearchOpen(LPCHARACTER ch)
    {
        LPDESC d = ch ? ch->GetDesc() : nullptr;
        if (!d)
        {
        ...
        if (ch->IsGM() || !ch->IsObserverMode())
        {
            ...
            {
                ch->ChatPacket(CHAT_TYPE_INFO, "1130");
                ch->ChatPacket(CHAT_TYPE_INFO, "1131");
                return;
            }

            ch->SetIkarusShopUseTime();

            ch->SetShopSearchPage(1);
            ShopSearchUpdateResults(ch, false, false);
        }
    }

    void CShopManager::RecvShopSearchChangePage(LPCHARACTER ch, int32_t pageIdx)
    {
        LPDESC d = ch ? ch->GetDesc() : nullptr;
        if (!d)
        {
        ...
        if (ch->IsGM() || !ch->IsObserverMode())
        {
            ...
            if (pageIdx == ch->GetShopSearchPage())
            {
                return;
            }
            ...
            {
                const auto maxpage = ch->GetShopSearchMaxPageCount();
                if (pageIdx > maxpage)
                {
                    pageIdx = maxpage;
                }
            ...
            ch->SetShopSearchPage(pageIdx);
            ShopSearchUpdateResults(ch, true, false);
        }
    }

    bool CShopManager::RecvShopSearchBuyItem(LPCHARACTER ch, const std::vector<ikashop::TSubPacketCGShopBuyItem>& vecItems)
    {
        LPDESC d = ch ? ch->GetDesc() : nullptr;
        if (!d || !CheckGMLevel(ch))
        {
        ...
        if (ch->IsGM() || !ch->IsObserverMode())
        {
...
#ifdef EXTEND_IKASHOP_ULTIMATE
            const auto accid = d->GetAccountTable().id;
            if (accid == 0)
            {
                SendPopupMessage(ch, "IKASHOP_SERVER_POPUP_MESSAGE_GENERIC_ERROR");
                return false;
            ...
            long long total = 0;
            std::vector<ikashop::TSubPacketCGShopBuyItem> vecItemsWhiteList;
            std::vector<uint32_t> vecItemsIdsCannot;
            std::unordered_map<uint32_t, long long> mapItemsIdsCannot;

            ikashop::TSubPacketCGShopBuyItem tmp;

            bool cannot = false;

            for (size_t i = 0; i < vecItems.size(); ++i)
            {
                const ikashop::TSubPacketCGShopBuyItem& item = vecItems[i];
                if (item.ownerid == pid)
                {
                    continue;
                }

                SHOP_HANDLE shop = GetShopByOwnerID(item.ownerid);
                if (!shop)
                {
                    vecItemsIdsCannot.push_back(item.itemid);
                    continue;
                }
                ...
                auto pitem = shop->GetItem(item.itemid);
                if (!pitem)
                {
                    vecItemsIdsCannot.push_back(item.itemid);
                    continue;
                ...
                const auto price = pitem->GetPrice().GetTotalYangAmount();
                if (price != item.seenprice)
                {
                    if (price < 1)
                    {
                        vecItemsIdsCannot.push_back(item.itemid);
                    ...
                    {
                        mapItemsIdsCannot[item.itemid] = price;
                    }
                ...
                vecItemsWhiteList.push_back(tmp);
            }

            bool update = false;

            for (const auto& k : vecItemsIdsCannot)
            {
                if (ch->SetShopSearchEraseItem(k))
                {
                    if (!update)
                    {
                        update = true;
            ...
            for (const auto& [k, v] : mapItemsIdsCannot)
            {
                if (ch->UpdateShopSearchItemPrice(k, v))
                {
                    if (!update)
                    {
                        update = true;
            ...
            if (cannot || total < 1 || total > static_cast<long long>(ch->GetGold()))
            {
                vecItemsWhiteList.clear();
            ...
            if (vecItemsWhiteList.empty())
            {
                ...
                if (update)
                {
                    ShopSearchUpdateResults(ch, true, true);
                }
            ...
            else
            {
                ...
                TPacketGDNewIkarusShop p = {};
                p.bSubHeader = SUBHEADER_GD_BUY_LOCK_ITEM_MULTI;

                const size_t count = vecItemsWhiteList.size();

                TEMP_BUFFER tmpBuff;
                tmpBuff.write(&p, sizeof(TPacketGDNewIkarusShop));
                tmpBuff.write(&count, sizeof(size_t));

                TSubPacketGDLockBuyItem tmp;
                for (const auto& item : vecItemsWhiteList)
                {
                    tmp = {};
                    ...
                    if (!shop)
                    {
                        tmpBuff.write(&tmp, sizeof(TSubPacketGDLockBuyItem));
                        continue;
                    }
...
#ifdef EXTEND_IKASHOP_ULTIMATE
                    tmp.accountid = accid;
                    tmp.normalizedPrice = GetNormalizedPrice(pitem->GetInfo().vnum, pitem->GetInfo().count, item.seenprice);
#endif
                    tmp.guestid = pid;
                    tmp.ownerid = item.ownerid;
                    tmp.itemid = pitem->GetID();
                    tmp.seenprice = item.seenprice;

                    ch->SetShopSearchIds(tmp.itemid);

                    tmpBuff.write(&tmp, sizeof(TSubPacketGDLockBuyItem));
                }

                ch->SetIkarusShopUseTime();
                db_clientdesc->DBPacket(HEADER_GD_NEW_OFFLINESHOP, 0, tmpBuff.read_peek(), tmpBuff.size());
                return true;
            }
        }
