...
//Some Config
constexpr bool SAVE_WITH_ITEM_ID = true; // IF FALSE WILL SAVE WITH OWNER PID!
...
void BUFFI::LoadSpecialName()
{
	...
	char filename[124];
	snprintf(filename, sizeof(filename), "share/buffi_name/%u", SAVE_WITH_ITEM_ID ? m_dwItemPID : m_dwOwnerPID);
	FILE* fp;
...
void BUFFI::CheckSupportWear(bool bIsCreating)
{
	...
	// buffi->SetPart(PART_MAIN, 21385);
	// buffi->SetPart(PART_HAIR, 1);
	// buffi->SetPart(PART_WEAPON, 7345);
	buffi->SetPart(PART_MAIN, owner->GetWear(WEAR_BUFFI_BODY) ? owner->GetWear(WEAR_BUFFI_BODY)->GetVnum() : 11899);
	buffi->SetPart(PART_HAIR, owner->GetWear(WEAR_BUFFI_HEAD) ? owner->GetWear(WEAR_BUFFI_HEAD)->GetValue(3) : 0);
	buffi->SetPart(PART_WEAPON, owner->GetWear(WEAR_BUFFI_WEAPON) ? owner->GetWear(WEAR_BUFFI_WEAPON)->GetVnum() : 7159);
...
void BUFFI::CheckOwnerSupport()
{
	...
	constexpr DWORD skilList[][5] = { 
		{SKILL_BUFFI_1, SKILL_HOSIN, AFF_BUFFI_HOSIN, AFF_HOSIN, AFF_HOSIN_PERFECT },
		{SKILL_BUFFI_2, SKILL_GICHEON, AFF_BUFFI_GICHEON, AFF_GICHEON, AFF_GICHEON_PERFECT},
		{SKILL_BUFFI_3, SKILL_KWAESOK, AFF_BUFFI_KWAESOK, AFF_KWAESOK, AFF_KWAESOK_PERFECT},
		{SKILL_BUFFI_4, SKILL_JEUNGRYEOK, AFF_BUFFI_JEUNGRYEOK,AFF_JEUNGRYEOK,AFF_JEUNGRYEOK_PERFECT},
	};
	...
	for (DWORD j = 0; j < 4; ++j)
	{
		...
		if (owner->IsAffectFlag(skilList[j][2]) || owner->IsAffectFlag(skilList[j][3]) || owner->IsAffectFlag(skilList[j][4]) || !owner->GetSkillLevel(skillIdx))
			continue;
