...
void CSwitchbot::SetActive(BYTE slot, bool active)
{
	...
	if (!active)
	{
		LPCHARACTER pkOwner = CHARACTER_MANAGER::Instance().FindByPID(m_table.player_id);
		if (pkOwner)
			pkOwner->SetSwitchbotItemSwitcherUsed(slot, 0);
	}
#endif
...
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
	LPCHARACTER pkOwnerW = CHARACTER_MANAGER::Instance().FindByPID(m_table.player_id);

	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		if (pkOwnerW)
			pkOwnerW->SetSwitchbotItemSwitcherUsed(bSlot, 0);
	}
#endif
	...
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
...
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
				int len = snprintf(buf, sizeof(buf), LC_TEXT("Attribute switcher finshed %s (Slot: %d, Switcher Used %d)"), pkItem->GetName(), bSlot + 1, pkOwner->GetSwitchbotItemSwitcherUsed(bSlot));
#else
		...
		else
		{
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				...
				if (bZodiacItem) {
					if (pkOwner->CountSpecifyItem(94912) >= SWITCHBOT_PRICE_AMOUNT) {
						stop = false;
					}

					LPITEM tpkItem = pkOwner->FindSpecifyItem(94912);
					if (tpkItem && !stop)
					{
						...
						if (itemVnum == 71151 || itemVnum == 76023)
						{
							if ((pkItem->GetType() == ITEM_WEAPON) || (pkItem->GetType() == ITEM_ARMOR && pkItem->GetSubType() == ARMOR_BODY))
							{
								...
								for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
								{
									...
									if (pkItem->GetLimitType(i) == LIMIT_CONQUEROR_LEVEL && pkItem->GetLimitValue(i) > 0)
									{
										bCanUse = false;
										break;
									}
...
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
			pkItem->SetSkipUpdatePacket(true);
#endif

			pkItem->ChangeAttribute();
			
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
			pkItem->SetSkipUpdatePacket(false);
#endif
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
			pkOwner->SetSwitchbotItemSwitcherUsed(bSlot, pkOwner->GetSwitchbotItemSwitcherUsed(bSlot) + 1);
#endif
...
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
	update.CellInven = item->GetCell();
#endif

	std::memcpy(update.alSockets, item->GetSockets(), sizeof(update.alSockets));
	std::memcpy(update.aAttr, item->GetAttributes(), sizeof(update.aAttr));
...
#if defined(ENABLE_SWITCHBOT) && defined(ENABLE_SWITCHBOT_RENEWAL)
bool CSwitchbotManager::IsItemIDInSwitchbot(DWORD dwPID, DWORD dwItemID)
{
	CSwitchbot* pkSwitchbot = FindSwitchbot(dwPID);
	if (!pkSwitchbot)
		return false;
	
	auto pTable = pkSwitchbot->GetTable();
	
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		if (pTable.items[bSlot] == dwItemID)
			return true;
	}
