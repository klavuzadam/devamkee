...
EVENTFUNC(castle_siege_event)
{
	...
	// °ø¼º ½ÃÀÛÈÄ 30ºÐ ÀÌ³»¶ó¸é ¾È³»¸¸ ÇÏÀÚ
	if (info->pulse < PASSES_PER_SEC(30*60))
	{
		snprintf(buf, sizeof(buf), "31 %s",EMPIRE_NAME(GET_SIEGE_EMPIRE()));
		BroadcastNotice(buf);
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_STRUGGLE:
			{
				snprintf(buf, sizeof(buf), "32 %s", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);

				snprintf(buf, sizeof(buf), "33 %s", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);

				GET_SIEGE_STATE() = CASTLE_SIEGE_END;

				return PASSES_PER_SEC(60*30);	// 30ºÐ
			}
			break;
...
bool castle_spawn_tower(int empire, int tower_count)
{
	...
	{
		char buf[1024];
		snprintf(buf, sizeof(buf), "35 %s", EMPIRE_NAME(empire));
		BroadcastNotice(buf);
	}
	return true;
}
...
void castle_tower_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_END:
			{
				int	siege_end = true;
				snprintf(buf, sizeof(buf), "36 %s", EMPIRE_NAME(killer_empire));
				BroadcastNotice(buf);
				...
				if (siege_end)
				{
					...
					{
						snprintf(buf, sizeof(buf), "37 %s", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					else
					{
						snprintf(buf, sizeof(buf), "38 %s", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					castle_end_siege();
				}
