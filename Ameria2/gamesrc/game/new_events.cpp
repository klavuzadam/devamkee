...
#include "new_events.h"
			...
			void operator() (LPENTITY ent)
			{
				if (ent->IsType(ENTITY_CHARACTER))
				{
					...
					if (pChar->IsPC())
					{
						...
						std::memcpy(sPacket.iKills, m_iKills, sizeof(sPacket.iKills));
		...
		bool CMgrKingdomsWar::Start()
		{
			...
			{
				char buf[201] = {0};
				snprintf(buf, sizeof(buf), "%s", KingdomsWar::MSG[i]);
				BroadcastNotice(buf);
			}
		...
		void CMgrKingdomsWar::Register(LPCHARACTER pChar)
		{
			...
			if (pDesc)
			{
				...
				std::memcpy(sPacket.iKills, kills, sizeof(sPacket.iKills));
				sPacket.iLimitKills = KingdomsWar::LIMIT_WIN_KILLS;
				...
				case 1:
					{
						...
						if (iterFind != mPartecipants1.end())
						{
							...
							if (kills_by > 0)
							{
								...
								{
									FRefreshKills f;
									std::memcpy(f.m_iKills, kills, sizeof(f.m_iKills));
			...
			if (bEmpire != 0)
			{
				...
				for (int i = 5; i < 8; ++i)
				{
					...
					if (i == 6)
						...
						snprintf(buf, sizeof(buf), KingdomsWar::MSG[i], g_nation_name[bEmpire]);
					BroadcastNotice(buf);
				}
		...
		void CMgrKingdomsWar::OnKill(DWORD dwPlayerID1, BYTE bEmpire1, DWORD dwPlayerID2, BYTE bEmpire2)
		{
			...
			char buf_notice[1024] = {0};
			snprintf(buf_notice, sizeof(buf_notice), "Guerra de Reinos: |cFFFF0000|hShinsoo: %d muertes - |cFFFFFF00|hChunjo: %d muertes - |cFF0080FF|hJinnos: %d muertes", GetKillsScore(1), GetKillsScore(2), GetKillsScore(3));
			BroadcastNotice(buf_notice);
