...
bool CSafebox::Add(DWORD dwPos, LPITEM pkItem)
{
	...
	std::memcpy(pack.alSockets, pkItem->GetSockets(), sizeof(pack.alSockets));
	std::memcpy(pack.aAttr, pkItem->GetAttributes(), sizeof(pack.aAttr));
#ifdef ENABLE_YOHARA_SYSTEM
	std::memcpy(pack.aApplyRandom, pkItem->GetApplysRandom(), sizeof(pack.aApplyRandom));
#endif
#ifdef __SOULBINDING_SYSTEM__
	pack.bind = pkItem->GetBind();
#endif

	//m_pkChrOwner->ChatPacket(1,"packetsize: %d", sizeof(TPacketGCItemSet));
...
bool CSafebox::MoveItem(BYTE bCell, BYTE bDestCell, DWORD count)
{
...

	{
		...
		if(item2 != NULL)
		{
			if (item != item2 && item2->IsStackable() && !IS_SET(item2->GetAntiFlag(), ITEM_ANTIFLAG_STACK) && item2->GetVnum() == item->GetVnum()) // ÇÕÄ¥ ¼ö ÀÖ´Â ¾ÆÀÌÅÛÀÇ °æ¿ì
			{
