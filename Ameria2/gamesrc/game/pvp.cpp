...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if ((pkPVP = Find(kPVP.m_dwCRC)))
	{
		...
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, "896 %s", pkChr->GetName());
			pkChr->ChatPacket(CHAT_TYPE_INFO, "896 %s", pkVictim->GetName());
		}
		return;
	}
	...
	snprintf(msg, sizeof(msg), "897 %s", pkChr->GetName());

	pkVictim->ChatPacket(CHAT_TYPE_INFO, msg);
	pkChr->ChatPacket(CHAT_TYPE_INFO, "898 %s", pkVictim->GetName());
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if ((pkChr->GetPKMode() == PK_MODE_FEAR && pkChr->IsPC()) || (pkVictim->GetPKMode() == PK_MODE_FEAR && pkVictim->IsPC()))
		return false;
