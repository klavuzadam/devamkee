...
namespace quest
{
	...
	ALUA(_item_name)
	{
		if (lua_isnumber(L,1))
		{
			...
			if (pTable)
			{
				...
				char name[124];
				sprintf(name,"^QUEST_NAME I%d*^",dwVnum);
...
#ifdef ENABLE_MULTILANGUAGE_SYSTEM
				char name[64];
				sprintf(name,"^QUEST_NAME M%d*^",dwVnum);
				lua_pushstring(L, name);
#else
	...
	ALUA(_warp_all_to_village)
	{
		...
		SendNoticeMap("899", iMapIndex, false);
...
#ifdef ENABLE_MOUNT_SYSTEM
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsMountSystem() && !ch->IsNewPet()
...
#ifdef ENABLE_MOUNT_SYSTEM
				if (!pChar->IsPet() && !pChar->IsMountSystem() && !pChar->IsBuffNPC() && (true == pChar->IsMonster() || true == pChar->IsStone()))
#else				
