...
void ShutdownOnFatalError()
{
	if (!g_bShutdown)
	{
		sys_err("ShutdownOnFatalError!!!!!!!!!!");
		{
			...
			strlcpy(buf, "824", sizeof(buf));
			SendNotice(buf);
			strlcpy(buf, "825", sizeof(buf));
			SendNotice(buf);
			strlcpy(buf, "826", sizeof(buf));
			SendNotice(buf);
		}
...
//#ifdef ENABLE_MINI_GAME_CATCH_KING
//	if (!g_bAuthServer && g_bChannel == 99)
//	{
...
int main(int argc, char **argv)
{
...
#endif
#ifdef __FAKE_PLAYER__
	CFakePlayerManager			FakePlayerManager;	
#endif
...
#endif
#ifdef USE_DUNGEON_COOLDOWN_MANAGER
	CDungeonCooldownMgr dungeon_cooldown_mgr;
#endif

#ifdef USE_DAY_CHANGE_MGR
	CDayChangeMgr day_change_mgr;
#endif
...
#endif
#ifdef __FAKE_PLAYER__
	CFakePlayerManager::Instance().Initialize();
#endif
	Cube_init();
	Blend_Item_init();
	ani_init();
	...
	char szPetExpTable[256];
	snprintf(szPetExpTable, sizeof(szPetExpTable), "%s/exppettable.txt", LocaleService_GetBasePath().c_str());
	std::ifstream exppet_table_open(szPetExpTable);
	...
	{
#ifdef ENABLE_ULTIMATE_REGEN
        sys_log(0, "<shutdown> Destroying mobTimer.");
        mobTimer.Destroy();
#endif
...
#ifdef ENABLE_IKASHOP_RENEWAL
	sys_log(0, "<shutdown> Destroying ikashop::CShopManager...");
	offshopManager.Destroy();
#endif
...
#if defined(__BL_BATTLE_ROYALE__)
	sys_log(0, "<shutdown> Destroying CBattleRoyale...");
	battle_royale.Destroy();
#endif
#if defined(WJ_COMBAT_ZONE)
...
#endif
#ifdef __FAKE_PLAYER__
	sys_log(0, "<shutdown> Destroying CFakePlayerManager...");
	CFakePlayerManager::Instance().Destroy();
#endif
...
#ifdef ENABLE_NEWSTUFF
	while ((ch = getopt(argc, argv, (char*)"npverltIC")) != -1)
#else
...
#if defined(__BL_BATTLE_ROYALE__)
	BattleRoyale::instance().BattleCore_Update(thecore_heart->pulse);
#endif
