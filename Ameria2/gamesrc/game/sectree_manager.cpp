...
bool SECTREE_MANAGER::LoadAttribute(LPSECTREE_MAP pkMapSectree, const char * c_pszFileName, TMapSetting & r_setting)
{
	...
	for (int y = 0; y < iHeight; ++y)
		for (int x = 0; x < iWidth; ++x)
		{
			...
			tree->BindAttribute(M2_NEW CAttribute((uint32_t*)attr, SECTREE_SIZE / CELL_SIZE, SECTREE_SIZE / CELL_SIZE));
		}
...
void SECTREE_MANAGER::SendEntity(const LPCHARACTER c_lpChar)
{
	...
	const LPDESC c_lpDesc = c_lpChar->GetDesc(); /*IsPC()*/
	if (c_lpDesc == nullptr)
		return;
	...
	for (y = 0; y < 4 * iMapHeight; ++y)
	{
		for (y2 = 0; y2 < SECTREE_SIZE / CELL_SIZE; ++y2)
		{
			for (x = 0; x < 4 * iMapWidth; ++x)
			{
				...
				pSec->m_pkAttribute->CopyRow(y2, (uint32_t*)pdwLine);
...
struct FPurgeMonsters
{
	void operator() (LPENTITY ent)
	{
		if ( ent->IsType(ENTITY_CHARACTER) == true )
		{
			...
			if (lpChar->IsMonster() == true && !lpChar->IsPet() && !lpChar->IsMountSystem() && !lpChar->IsNewPet() && !lpChar->IsBuffNPC())
#else
...
#ifdef ENABLE_MOUNT_SYSTEM
			if (lpChar->IsNPC() == true && !lpChar->IsPet() && !lpChar->IsMountSystem() && !lpChar->IsNewPet() && !lpChar->IsBuffNPC())
#else
