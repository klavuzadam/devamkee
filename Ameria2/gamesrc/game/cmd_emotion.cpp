...
ACMD(do_emotion)
{
	int i;
	{
		if (ch->IsRiding())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "384");
			return;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "385");
		return;
	}

	if (IS_SET(emotion_types[i].flag, WOMAN_ONLY) && SEX_MALE==GET_SEX(ch))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "386");
		return;
	}
	...
	if (IS_SET(emotion_types[i].flag, NEED_TARGET | NEED_PC))
	{
		if (!victim)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "387");
			return;
		}
	}
	...
	if (victim)
	{
		...
		if (victim->IsRiding())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "388");
			return;
		}
		...
		if (distance < 10)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "389");
			return;
		}

		if (distance > 500)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "390");
			return;
		}
	...
	CHARACTER_MANAGER::Instance().DoMission(ch, MISSION_USE_EMOTICON, 1, 0);
#endif
}
