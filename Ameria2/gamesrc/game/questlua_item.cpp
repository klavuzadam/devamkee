...
namespace quest
{
	...
	ALUA(item_set_count)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentItem();
		if (!item || false == lua_isnumber(L, 1))
		{
			lua_pushnumber(L, 0);
			return 1;
		}
		const int count = (int)lua_tonumber(L, 1);
		item->SetCount(count);
		if(count == 0)
			q.ClearCurrentItem();
		return 0;
...
#define NS_ITEM_GETMODE0(x)	\
	...
	ALUA(item_set_count0)
	{
		...
		if (item)
			item->SetCount(MIN(g_bItemCountLimit - item->GetCount(), lua_tonumber(L, 1)));
...
#ifdef ENABLE_SPECIAL_STORAGE
			{ "select_stone_cell",	item_select_stone_cell	},
#endif
			{ "remove",		item_remove		},
