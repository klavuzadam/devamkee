...
namespace ikashop
{
    ...
    bool CShopItem::CanBuy(LPCHARACTER ch)
    {
        ...
        if (m_info.price.cheque < 0 || m_info.price.yang < 0)
        {
            return false;
        }

        if (m_info.price.cheque < 1 && m_info.price.yang < 1)
        {
            return false;
        }
        ...
        else if (m_info.price.yang > static_cast<long long>(ch->GetGold()))
        {
            return false;
        }
#else
        if (m_info.price.yang < 1)
        {
            return false;
        }
	...
	{
		// checking item exist
		if(m_items.find(offer->itemid) == m_items.end())
			return false;
	...
	bool CShop::AcceptOffer(OFFER_HANDLE offer)
	{
		...
		if(auto item = GetItem(offer->itemid))
		{
			...
			// removing offer
			EraseIfContainer(m_offers, [&](auto& iter) { return iter.second->offerid == offer->offerid; });
			return true;
		}
	...
	{
		EraseIfContainer(m_offers, [&](auto&& iter){ return offerid == iter.second->offerid; });
	}
