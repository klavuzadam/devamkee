...
// ½Ì±ÛÅÏ »ý¼ºÀÚ È£ÃâÇÏ´Âµ¥°¡ ¾îµòÁö ¸ô¶ó¼­ ¿©±â´Ù ÇÏ³ª ¸¸µé¾î³õÀ½
...
void CShutdownManager::Update()
{
	...
	if(!m_bShutdownAlarm)	// 10ºÐÀü¿¡ ¾Ë¸² Ãâ·Â.
	{
		if(m_pTime->tm_hour >= 23 && m_pTime->tm_min >= 50)
		{
			for(std::list<LPDESC>::iterator i = m_lstDesc.begin(); i != m_lstDesc.end(); i++)	// ´ë»ó ¸ðµÎ PHASE_CLOSE
			{
				if((*i) != NULL)
				{
					(*i)->ChatPacket(CHAT_TYPE_INFO, "933");
				}
			}
...
bool CShutdownManager::CheckShutdownAge(const char* szSocialID)
{
	...
	if(CharToInt(szSocialID[6]) < 3)	// ÁÖ¹Î¹øÈ£ 7¹øÂ° ÀÚ¸®°¡ 3~4ÀÎ °æ¿ì´Â 2000³â ÀÌÈÄ Ãâ»ý
	{
