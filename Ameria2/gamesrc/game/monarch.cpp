...
struct FHealMyEmpire
{
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "841");
...
int CMonarch::HealMyEmpire(LPCHARACTER ch ,DWORD price)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "843 %d", ch->GetMCLTime(CHARACTER::MI_HEAL));
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "444 %u %u", GetMoney(Empire), price);
		return 0;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, "844 %d", GetMoney(Empire) - price);
	return 1;
}
...
bool CMonarch::SendtoDBDecMoney(int Money, BYTE bEmpire, LPCHARACTER ch)
{
	...
	// ½ÇÁ¦ ÁÙÀÌ´Â ºÎºÐÀº ¼­¹ö¿¡ °¬´Ù ¿Â ´ÙÀ½¿¡ Ã³¸®µÈ´Ù
	int nEmpire = bEmpire;
