...
static void __proc_teen_notice(char *login, int hour)
{
	...
	switch (hour)
	{
		...
		case 2:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, "806 %d", hour);
				SET_OVER_TIME(ch, OT_NONE);
			}
			break;
		case 3:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, "807 %d", hour);
				ch->ChatPacket(CHAT_TYPE_NOTICE, "808");
				SET_OVER_TIME(ch, OT_3HOUR);
			}
			break;
		case 4:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, "809");
				ch->ChatPacket(CHAT_TYPE_NOTICE, "810");
				ch->ChatPacket(CHAT_TYPE_NOTICE, "811");
				ch->ChatPacket(CHAT_TYPE_NOTICE, "812");
				SET_OVER_TIME(ch, OT_3HOUR);
			}
			break;
		default:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, "813");
				ch->ChatPacket(CHAT_TYPE_NOTICE, "814");
				ch->ChatPacket(CHAT_TYPE_NOTICE, "815");
				ch->ChatPacket(CHAT_TYPE_NOTICE, "816");
...
static inline void __sec_to_timestring(int sec, char *buf, size_t buflen)
{
	...
	if (hour>0)
		snprintf(buf, buflen, "817 %d", hour);
	else
		snprintf(buf, buflen, "818 %d", min);
}
...
static void __proc_login_notice(char *login, int on_time, int off_time)
{
	...
	if (0==on_time)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, "819 %s", off_time_string);
		ch->ChatPacket(CHAT_TYPE_NOTICE, "820");
		return;
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, "821 %s %s",
