	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->ChatPacket(CHAT_TYPE_INFO, "885");
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, "781");
		return;
	}
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, "886");
		return;
	}
	...
	if (n == 0)
		l->ChatPacket(CHAT_TYPE_INFO, "887");
	else
	{
		int i = number(0, n - 1);
...
void CParty::Update()
{
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		if (bLongTimeExpBonusChanged && ch->GetDesc())
			ch->ChatPacket(CHAT_TYPE_INFO, "888");
...
{
	// ÆÄÆ¼¿øÀÌ mapIndexÀÎ ´øÁ¯¾È¿¡ ÀÖ´ÂÁö ¼ø¼­´ë·Î °Ë»ç
	for(TMemberMap::iterator it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		LPCHARACTER ch = it->second.pCharacter;
