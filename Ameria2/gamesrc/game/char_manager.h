...
public:
	void			LoadBanword();
	bool			IsBanword(LPCHARACTER ch, const char* szChat);
protected:
	std::map<std::string, std::pair<int, bool>> m_mapBanwords;
#endif
#ifdef ENABLE_EVENT_MANAGER
public:
	void			ClearEventData();
...
#ifdef ENABLE_BATTLEPASS
		public:
			int32_t GetBattlePassMissionIndex(const std::string& mission_text);

			void LoadBattlePass();

			BattlePassInfo* GetBattlePassInfo() { return &m_battlepass_info; }
            std::unordered_map<uint32_t /*mission id*/, BattlePassMission>* GetBattlePassMissionMap()
            {
                return &m_battle_pass_missions;
            }
            std::unordered_map<uint32_t /*mission id*/, TierInfo>* GetBattlePassTierMap()
            {
                return &m_battle_pass_tier_map;
            }
            int32_t GetMaxBattlePassLevel() const;
