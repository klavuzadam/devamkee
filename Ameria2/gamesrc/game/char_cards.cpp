...
void CHARACTER::Cards_open(DWORD safemode)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "1105");
		return;
	}
	...
	if (character_cards.cards_left <= 0)
	{
		if (GetGold() < 100000)
		{
			ChatPacket(CHAT_TYPE_INFO, "1106");
			return;
		}
		if (CountSpecifyItem(79506) < 1)
		{
			ChatPacket(CHAT_TYPE_INFO, "1107");
			return;
		}

		if (CHARACTER_MANAGER::Instance().CheckEventIsActive(CARD_EVENT, GetEmpire()) == NULL)
		{
			ChatPacket(CHAT_TYPE_INFO, "event deactive!");
			return;
		}

		PointChange(POINT_GOLD, -100000, true);
...
void CHARACTER::Cards_pullout()
{
	...
	if (empty_space == i)
	{
		ChatPacket(CHAT_TYPE_INFO, "1108");
		return;
	}
	if (character_cards.cards_left < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, "1109");
		return;
	}
	if (GetAllCardsCount() >= 5)
	{
		ChatPacket(CHAT_TYPE_INFO, "1110");
		return;
	}
	RandomizeCards();
	SendUpdatedInformations();
...
{
	if (character_cards.points >= 290)
		AutoGiveItem(50275); //Golden box
	else if (character_cards.points < 290 && character_cards.points >= 230)
		AutoGiveItem(50276); //Silver box
	else if (character_cards.points > 0)
		AutoGiveItem(50277); //Bronze box
...
void CHARACTER::GetGlobalRank(char* buffer, size_t buflen)
{
	...
	*buffer = '\0';
	while (nullptr != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		...
		len2 = snprintf(buffer + len, buflen - len, "%d. - %s - %d Points.", pos, row[0], points);
		++pos;
		if (len2 < 0 || len2 >= (int)buflen - len)
			len += (buflen - len) - 1;
		else
...
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, points FROM log.log_okey_event ORDER BY points DESC LIMIT 10", get_table_postfix()));
	if (pMsg->Get()->uiNumRows == 0)
	{
		return;
	}
