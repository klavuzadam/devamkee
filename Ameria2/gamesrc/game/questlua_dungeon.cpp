...
namespace quest
{
	...
	ALUA(dungeon_set_dungeon_difficulty)
	{
		...
		if (pDungeon)
			pDungeon->SetDungeonDifficulty((BYTE) lua_tonumber(L, 1));
...
#ifdef ENABLE_DUNGEON_INFO
		if (CDungeonInfoExtern::instance().CheckTimeDungeonRoom(ch,pDungeon->GetMapIndex()) == true){
			CDungeonInfoExtern::instance().SetTimeDungeonRoom(ch,pDungeon->GetMapIndex());
		}
#endif
	...
	struct FPurgeArea
	{
		...
		void operator () (LPENTITY ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!pChar->IsPet() && !pChar->GetRider() && (true == pChar->IsMonster() || true == pChar->IsStone()))
				{
	...
	ALUA(dungeon_exit) // ´øÀü¿¡ µé¾î¿À±â Àü À§Ä¡·Î º¸³¿
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	ALUA(dungeon_exit_all) // ´øÀü¿¡ ÀÖ´Â ¸ðµç »ç¶÷À» ´øÀü¿¡ µé¾î¿À±â Àü À§Ä¡·Î º¸³¿
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
	...
	ALUA(dungeon_exit_all_by_item_group) // Æ¯Á¤ ¾ÆÀÌÅÛ ±×·ì¿¡ ¼ÓÇÑ ¾ÆÀÌÅÛÀÌ ¾ø´Â»ç¶÷Àº °­Åð
	{
		if (!lua_isstring(L, 1))
		{
			sys_log(0, "QUEST wrong set flag");
	...
	ALUA(dungeon_delete_item_in_item_group_from_all) // Æ¯Á¤ ¾ÆÀÌÅÛÀ» ´øÀü ³» pc¿¡°Ô¼­ »èÁ¦.
	{
		if (!lua_isstring(L, 1))
		{
			sys_log(0, "QUEST wrong set flag");
	...
	ALUA(dungeon_count_monster_new)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
		...
		{
			sys_err("dungeon_count_monster_new not in a dungeon");
...
, bHwidCooldown(false)
...
#ifdef USE_DUNGEON_COOLDOWN_MANAGER
	struct FDungeonManagerSetCooldown
	{
		int m_iCooldown;
		long m_lMapIndex;

		FDungeonManagerSetCooldown(int iCooldown, long lMapIndex) : m_iCooldown(iCooldown), m_lMapIndex(lMapIndex)
		{
		...
		void operator()(LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
				{
					...
					const auto bRes = dungeonCooldownMgr.Replace(ch, m_lMapIndex, m_iCooldown + get_global_time());
	...
	EVENTFUNC(global_warp_all_to_base_event)
	{
		...
		{
#ifdef USE_DUNGEON_COOLDOWN_MANAGER
			if (info->bHwidCooldown)
			{
				LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(info->MapIndex);
				if (pDungeon)
				{
					FDungeonManagerSetCooldown f(pDungeon->GetFlag("time_to_wait"), static_cast<long>(pDungeon->GetFlag("map_index")));
					pSecMap->for_each(f);
				}
			}
#endif
...
#ifdef USE_DUNGEON_COOLDOWN_MANAGER
		bool m_bHwidCooldown = true;

		if (lua_isboolean(L, 5))
		{
			m_bHwidCooldown = static_cast<bool>(lua_toboolean(L, 5));
		}
#endif
...
#ifdef USE_DUNGEON_COOLDOWN_MANAGER
		info->bHwidCooldown = m_bHwidCooldown;
#endif
	...
	void RegisterDungeonFunctionTable()
	{
		luaL_reg dungeon_functions[] =
		{
			...
			{ "count_monster_new",	dungeon_count_monster_new },
