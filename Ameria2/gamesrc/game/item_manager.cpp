...
bool ITEM_MANAGER::Initialize(TItemTable * table, int size)
{
	...
	std::memcpy(&m_vec_prototype[0], table, sizeof(TItemTable) * size);
	...
	for (i = 0; i < size; ++i)
	{
		...
		if (m_vec_prototype[i].bType == ITEM_QUEST || IS_SET(m_vec_prototype[i].dwFlags, ITEM_FLAG_QUEST_USE | ITEM_FLAG_QUEST_USE_MULTIPLE)
			...
			|| m_vec_prototype[i].bType == ITEM_MATERIAL
			#endif
		)
			quest::CQuestManager::instance().RegisterNPCVnum(m_vec_prototype[i].dwVnum);

#ifdef __BUFFI_SUPPORT__
		if (m_vec_prototype[i].bType == ITEM_BUFFI && m_vec_prototype[i].bSubType == BUFFI_NAME_SCROLL)
			quest::CQuestManager::instance().RegisterNPCVnum(m_vec_prototype[i].dwVnum);
#endif
	...
	quest::CQuestManager::instance().RegisterNPCVnum(50348);
	quest::CQuestManager::instance().RegisterNPCVnum(50349);
#endif
	int len = 0, len2;
	char buf[512];
...
#endif
{
	...
	const auto* table = GetTable(vnum);
	if (!table)
	{
		sys_err(">> WRONG item doesn't exists: vnum (%u).", vnum);
		return NULL;
	}

	if (table->bSize == 0)
	{
		sys_err(">> WRONG item size: vnum (%u) -> size (%u).", vnum, table->bSize);
		return NULL;
	}
	...
	if (m_map_pkItemByID.find(id) != m_map_pkItemByID.end())
	{
		...
		sys_err("ITEM_ID_DUP (1): item vnum (%u - %u).", id, item->GetVnum());

		LPCHARACTER owner = item->GetOwner();
		if (owner)
		{
			sys_err("ITEM_ID_DUP (2): %u %s owner %p", id, item->GetName(), get_pointer(owner));
		}
...
//	{
//		const auto eventData = CHARACTER_MANAGER::Instance().CheckEventIsActive(CATCH_KING_EVENT);
//		if (eventData)
//			item->SetSocket(0, eventData->endTime);
//	}
//#endif
...
void ITEM_MANAGER::SaveSingleItem(LPITEM item)
{
	...
	std::memcpy(t.alSockets, item->GetSockets(), sizeof(t.alSockets));
	std::memcpy(t.aAttr, item->GetAttributes(), sizeof(t.aAttr));
#ifdef ENABLE_YOHARA_SYSTEM
	std::memcpy(t.aApplyRandom, item->GetApplysRandom(), sizeof(t.aApplyRandom));
#endif
...
void ITEM_MANAGER::RemoveItem(LPITEM item, const char * c_pszReason)
{
	...
	if (item->GetType() == ITEM_BUFFI && item->GetSubType() == BUFFI_SCROLL && item->GetSocket(1) == 1)
		BUFFI_MANAGER::Instance().Summon(false, item);
#endif
...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	if (pkKiller->GetPremiumRemainSeconds(PREMIUM_ITEM) > 0 || //mall
		...
		iDeltaPercent += iDeltaPercent/2;//if not half add lucky.
	// END_OF_ADD_PREMIUM
	...
	iRandRange = iRandRange * 100 / 
		(100 + CPrivManager::instance().GetPriv(pkKiller, PRIV_ITEM_DROP) + bonus);
...
bool ITEM_MANAGER::CreateDropItemVector(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item)
{
	...
	{
#ifdef __BL_BATTLE_ROYALE__
		if (pkKiller->IsBattleRoyale()) // Only mob_drop_item is allowed in battle royale
		{
	...
	if (pkChr->IsStone())
	{
		...
		if (pkChr->GetDropChestBookVnum())
		{
				item = CreateItem(pkChr->GetDropChestBookVnum(), 1, 0, true);
				if (item)
					vec_item.push_back(item);
		}
	}
...
bool ITEM_MANAGER::CreateDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item)
{
	...
	if (LEVEL_DELTA(pkChr->GetLevel(), pkKiller->GetLevel(), 20))
	{
		if (m_map_pkCommonNewItemGroup.size())
		{
			...
			{
				auto commonNew = *it;
				int playerLevel = pkKiller->GetLevel();
				if (playerLevel >= commonNew->minLevel && playerLevel <= commonNew->maxLevel)
				{
					if (commonNew->CanDropThisMap(pkKiller->GetMapIndex()))
					{
						for (DWORD i = 0; i < commonNew->m_vecProbs.size(); i++)
						{
							int iPercent = (commonNew->m_vecProbs[i] * iDeltaPercent) / 100;
							if (iPercent >= number(1, iRandRange))
							{
								auto itemData = commonNew->m_vecItems[i];
								item = CreateItem(itemData.first, itemData.second, 0, true);
								if (item) vec_item.push_back(item);
							}
						}
					}
	...
	if (pkChr->IsStone())
	{
		...
		if (pkChr->GetDropChestBookVnum())
		{
			int iPercent = (pkChr->GetDropChestBookPct() * iDeltaPercent) * 400;
			if (iPercent >= number(1, iRandRange))
			{
				item = CreateItem(pkChr->GetDropChestBookVnum(), 1, 0, true);
				if (item)
...
#endif
	for (const auto& itemNew : vec_item)
	{
		DBManager::instance().SendMoneyLog(MONEY_LOG_DROP, itemNew->GetVnum(), itemNew->GetCount());
	}
...
//#ifdef ENABLE_MINI_GAME_CATCH_KING
//	if (CHARACTER_MANAGER::Instance().CheckEventIsActive(CATCH_KING_EVENT))
//	{
//		if (iDeltaPercent < number(1, iRandRange))
//			return;
//
//		const static DWORD dwVnum = CATCH_KING_DROP_ITEM;	// K?igskarte
//
//		if ((item = CreateItem(dwVnum, 1, 0, true)))
//			vec_item.push_back(item);
//	}
...
{
	pkNewItem->SetSockets(pkOldItem->GetSockets());
