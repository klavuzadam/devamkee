...
void Command_ApplyAffect(LPCHARACTER ch, const char* argument, const char* affectName, int cmdAffect)
{
	...
	switch (cmdAffect)
	{
		case COMMANDAFFECT_STUN:
			SkillAttackAffect(tch, 1000, IMMUNE_STUN, AFFECT_STUN, POINT_NONE, 0, AFF_STUN, 30000000, "GM_STUN");
			break;
...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		{
			int iEmptyPos = ch->GetEmptyChangeInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(ch, TItemPos(CHANGE_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, item, "GM", item->GetName());
			}
			else
			{
		...
		{
			int iEmptyPos = ch->GetEmptyCostumeInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(ch, TItemPos(COSTUME_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, item, "GM", item->GetName());
			}
			else
			{
...
ACMD(do_item_mod)
{
	...
	if (vecArgs.size() < 12)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: item_mod <vnum> <b1> <v1> <b2> <v2> <b3> <v3> <b4> <v4> <b5> <v5>");
		return;
	}
	...
	DWORD dwVnum;
	str_to_number(dwVnum, vecArgs[j].c_str());
	j++;

	TItemTable* pTable = ITEM_MANAGER::Instance().GetTable(dwVnum);
	if (!pTable)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "There's no item with vnum: %u.", dwVnum);
		return;
	}

	int iEmptyPos = ch->GetEmptyInventory(pTable->bSize);

	if (iEmptyPos == -1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You don't have enough empty space in you're inventory.");
		return;
	}
	...
	if (pTable->bType == ITEM_WEAPON)
	{
		switch (pTable->bSubType)
		{
			...
			default:
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "The item with vnum: %u cannot be created.", dwVnum);
				return;
			}
		}
	}
	else if (pTable->bType == ITEM_ARMOR)
	{
		switch (pTable->bSubType)
		{
			case ARMOR_BODY:
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can create just weapons, armors & accessories.");
		return;
	}

	std::vector<TPlayerItemAttribute> vecAttrs;
	...
	for (auto i = 0; i < 5; i++)
	{
		str_to_number(t.bType, vecArgs[j].c_str());
		j++;

		str_to_number(t.sValue, vecArgs[j].c_str());
		j++;
		...
		vecAttrs.push_back(t);
	}

	if (ret)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "The value of a bonus cannot exceed 32.000.");
		return;
	}

	LPITEM pItem = ITEM_MANAGER::Instance().CreateItem(dwVnum);
	if (!pItem)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "The item with vnum: %u cannot be created.", dwVnum);
		return;
	...
	for (const auto& attr : vecAttrs)
	{
		pItem->SetForceAttribute(j, attr.bType, attr.sValue);
		j++;
	}

	pItem->AddToCharacter(ch, TItemPos(INVENTORY, iEmptyPos));
	LogManager::instance().ItemLog(ch, pItem, "GM", pItem->GetName());
}
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && !pkChr->IsBuffNPC() && !pkChr->IsMountSystem() && pkChr->GetRider() == NULL)
#else
...
ACMD(do_purge)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Yard©¥mc©¥n©¥ gondermeden bu i?lemi yapamazs©¥n. ");
		return;
	}
#endif
	char arg1[256];
...
ACMD(do_item_purge)
{
	...
	else if (!strArg.compare(0, 5, "equip"))
	{
		for (i = 0; i < WEAR_MAX_NUM; ++i)
		{
			...
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
				ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, INVENTORY_MAX_NUM + i, WORD_MAX);
			}
		}
	}
	...
	else if (!strArg.compare(0, 4, "belt"))
	{
		for (i = 0; i < BELT_INVENTORY_SLOT_COUNT; ++i)
		{
			...
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
				ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, BELT_INVENTORY_SLOT_START + i, WORD_MAX);
			}
		}
	}
#ifdef ENABLE_SPECIAL_STORAGE
	...
	for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
	{
		if ((item = ch->GetItem(TItemPos(CHANGE_INVENTORY, i ))))
		{
			ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
		}
	}
	for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
	{
		if ((item = ch->GetItem(TItemPos(COSTUME_INVENTORY, i ))))
		{
			ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
		}
	}
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "NEW ELEMENTAL BONUSES   ELEC:%3d%% FIRE:%3d%% ICE:%3d%% WIND:%3d%% EARTH:%3d%% DARK:%3d%%",
			tch->GetPoint(POINT_ENCHANT_ELEC),
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "DOUBLE DROP BONUSES   METIN:%3d%% BOSS:%3d%%",
			tch->GetPoint(POINT_DOUBLE_DROP_METIN),
			tch->GetPoint(POINT_DOUBLE_DROP_BOSS)
	);
#endif
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Channel %u", g_bChannel);
}
...
struct happy_packet_func
{
	const char * m_str;
	happy_packet_func(const char * str) : m_str(str)
	{
	}
	...
	void operator () (LPDESC d)
	{
		if (!d->GetCharacter())
			return;
		d->GetCharacter()->ChatPacket(CHAT_TYPE_COMMAND, "HappyHourEvent %s", m_str);
	}
};

void SendHappyHour(const char * c_pszBuf)
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), happy_packet_func(c_pszBuf));
}
#endif
	...
	switch (i)
	{
		...
		case DoSetTypes::CONQUEROR_EXP:	//conqueror_exp
		{
			...
			if (tch && amount)
			{
				...
				//tch->ChatPacket(CHAT_TYPE_INFO, "ConquerorExp: ADD[%d] TOTAL[%d]", amount, tch->GetConquerorExp());
				tch->PointsPacket();
			}
		}
		break;
		...
		case DoSetTypes::CHEQUE:
		{
			...
			const auto cheque = tch->GetCheque();
			if (cheque > CHEQUE_MAX)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "%s's %s set to [%u]", tch->GetName(), set_fields[i].cmd, CHEQUE_MAX);
				return;
			}

			long long total = static_cast<long long>(cheque) + amount;
			if (total < 0)
			{
				tch->PointChange(POINT_CHEQUE, -static_cast<long long>(cheque), true);
				ch->ChatPacket(CHAT_TYPE_INFO, "%s's %s set to 0", tch->GetName(), set_fields[i].cmd);
				return;
			}
			else if (total > CHEQUE_MAX)
			{
				const long long dif = CHEQUE_MAX - cheque;
				tch->PointChange(POINT_CHEQUE, dif, true);
				ch->ChatPacket(CHAT_TYPE_INFO, "%s's %s set to %u", tch->GetName(), set_fields[i].cmd, CHEQUE_MAX);
				return;
			}
...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, "572 %s", cp.name);
}
...
ACMD(do_reload)
{
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'l':
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloading new banwords,");
				quest::CQuestManager::instance().RequestSetEventFlag("reload_banword", 0);
				break;
#endif
			...
			{
				char szFileName[250];
				snprintf(szFileName, sizeof(szFileName), "%s/common_drop_new.txt", LocaleService_GetBasePath().c_str());
				if (ITEM_MANAGER::instance().ReadCommonDropNewItemFile(szFileName)) {
...
ACMD(do_gwlist)
{
	ch->ChatPacket(CHAT_TYPE_NOTICE, "573");
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "574 %s", arg1);
	}
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, "575");
		else
		{
			char buf[1024+1];
...
ACMD(do_ban)
{
	...
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account.account SET availDt = FROM_UNIXTIME(UNIX_TIMESTAMP(CURRENT_TIMESTAMP()) + %i), reason = '%s' WHERE id = %d", iDuration * 3600, szReason, tch->GetDesc()->GetAccountTable().id));
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "576");
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "577");
						return;
					}
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "578");
							return;
						}
					}
				}
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "579");
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "580");
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "581");
							return;
						}
					}
				}
				...
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, "582");
					return;
				}
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "583");
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "584");
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "585");
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, "423");
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, "423");
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "586");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "587");
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "588");
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "589");
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "590");
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "591");
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, "592");
			return;
			break;
	}
#endif
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "593 %d %d", pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, "594");
	ch->ChatPacket(CHAT_TYPE_INFO, "595 %d %d %d %d", GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3));
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, "596");
...
ACMD(do_set_stat)
{
	...
	else
	{
		...
		case POINT_HT:
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "597");
				return;
			}
			n = 0;
			break;
...
ACMD(do_send_player)
{
	...
	if (!*arg1 || !*arg2 || !*arg3)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: set <name> <vnum_item> <count_item>");
		return;
	}
	...
	int count_item = atoi(arg3);
	int count_items_mall = 0;

	char InsertSql[1024];
	char szQuery3[1024];

	LPITEM item = ITEM_MANAGER::instance().CreateItem(vnum_item,count_item);
	if (!item){
		ch->ChatPacket(CHAT_TYPE_INFO,"not exist item.");
		return;
	}

	if (item->IsDragonSoul())
	{
		int iEmptyPos = tch->GetEmptyDragonSoulInventory(item);

		if (iEmptyPos != -1)
		{
			item->AddToCharacter(tch, TItemPos(DRAGON_SOUL_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
		}
		else
		{
			M2_DESTROY_ITEM(item);
			if (!tch->DragonSoul_IsQualified())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "ÀÎº¥ÀÌ È°¼ºÈ­ µÇÁö ¾ÊÀ½.");
			}
			else{
		...
		{
			int iEmptyPos = tch->GetEmptyUpgradeInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(tch, TItemPos(UPGRADE_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
		...
		{
			int iEmptyPos = tch->GetEmptyBookInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(tch, TItemPos(BOOK_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
		...
		{
			int iEmptyPos = tch->GetEmptyStoneInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(tch, TItemPos(STONE_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
			}
			else
			{
		...
		{
			int iEmptyPos = tch->GetEmptyChangeInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(tch, TItemPos(CHANGE_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
			}
			else
			{
		...
		{
			int iEmptyPos = tch->GetEmptyCostumeInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(tch, TItemPos(COSTUME_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
			}
			else
			{
		...
		{
			int iEmptyPos = tch->GetEmptyChestInventory(item);

			if (iEmptyPos != -1)
			{
				item->AddToCharacter(tch, TItemPos(CHEST_INVENTORY, iEmptyPos));
				LogManager::instance().ItemLog(ch, tch->GetPlayerID(), vnum_item, "SEND_PLAYER", item->GetName());
			}
			else
			{
		...
		{
			int iEmptyPos = tch->GetEmptyInventory(item->GetSize());
	...
	ch->ChatPacket(CHAT_TYPE_INFO,"Item entragado con exito.");
}
...
bool checkSqlInject(const char* data)
{
	const std::string text(data);
	const std::string words[] = { "SELECT", "TRUNCATE", "INSERT", "REPLACE", "DELETE", "ALTER", "DROP", ";", ":", "*", "'", "=" };
...
ACMD(do_hwid_ban)
{
	...
	split_argument(argument, vecArgs);
	if (vecArgs.size() < 3) {
		ch->ChatPacket(CHAT_TYPE_INFO, "/hwid_ban player_name reason");
		return;
	}
	else if (checkSqlInject(vecArgs[1].c_str()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "In name has sql inject words.");
		return;
	}
	else if (checkSqlInject(vecArgs[2].c_str()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "In reason has sql inject words.");
		return;
	}

	char buf[254];
	snprintf(buf, sizeof(buf), "SELECT account_id, id FROM player.player WHERE name = '%s'", vecArgs[1].c_str());
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery(buf));
	if (pMsg->Get()->uiNumRows != 0)
	{
		MYSQL_ROW mRow = mysql_fetch_row(pMsg->Get()->pSQLResult);
		if (mRow != NULL)
		{
			DWORD dwPlayerID = 0, dwAccID;
			if (!str_to_number(dwPlayerID, mRow[1]) || !str_to_number(dwAccID, mRow[0]))
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "This player can't find");
				return;
			}

			snprintf(buf, sizeof(buf), "SELECT hwid_routher, login FROM account.account WHERE id = %u", dwAccID);
			std::unique_ptr<SQLMsg> pMsg2(DBManager::Instance().DirectQuery(buf));
			if (pMsg2->Get()->uiNumRows != 0)
			{
				mRow = mysql_fetch_row(pMsg2->Get()->pSQLResult);
				if (mRow != NULL)
				{

					char routher_hwid[18 + 1], login[31];
					strlcpy(routher_hwid, mRow[0], sizeof(routher_hwid));
					strlcpy(login, mRow[1], sizeof(login));


					snprintf(buf, sizeof(buf), "SELECT id FROM account.hwid_routher WHERE hwid = '%s'", routher_hwid);
					std::unique_ptr<SQLMsg> pMsg3(DBManager::Instance().DirectQuery(buf));
					if (pMsg3->Get()->uiNumRows != 0)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "This player routher hwid already banned!");
						return;
					}

					snprintf(buf, sizeof(buf), "INSERT INTO account.hwid_routher(hwid, reason, date) VALUES('%s', '%s', NOW())", routher_hwid, vecArgs[2].c_str());
					std::unique_ptr<SQLMsg> pMsg4(DBManager::Instance().DirectQuery(buf));


					LPDESC d2 = DESC_MANAGER::instance().FindByLoginName(login);
					if (d2)
					...
					else
					{
						...
						strlcpy(pgg.szLogin, login, sizeof(pgg.szLogin));
						P2P_MANAGER::instance().Send(&pgg, sizeof(TPacketGGDisconnect));
					}

					ch->ChatPacket(CHAT_TYPE_INFO, "%s successfully banned.", vecArgs[1].c_str());

					snprintf(buf, sizeof(buf), "[Ban-Info] %s Banned. Reason: %s", vecArgs[1].c_str(), vecArgs[2].c_str());
					BroadcastNotice(buf);
				}
			}
			return;
...
ACMD(do_affect_add)
{
	...
	ch->AddAffect(dwType, wApplyOn, lApplyValue, dwFlag, lDuration, 0, bOverride, true/*cube*/);
...
ACMD(do_set_happy_hour)
{
	...
	if (!*arg1 || !*arg2)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "usage: happy_hour <first> <second>");
		return;
	}
	
	int firstVal = 0;
	str_to_number(firstVal, arg1);
	
	int secondVal = 0;
	str_to_number(secondVal, arg2);

	if (firstVal > 0)
	{
		quest::CQuestManager::instance().RequestSetEventFlag("happyFirstNumber", firstVal);
		quest::CQuestManager::instance().RequestSetEventFlag("happySecondNumber", secondVal);
		
		SendNotice("The Happy Hour is active! Check the window next to the minimap.", true);

		char buf[256];
		snprintf(buf, sizeof(buf), "%d#%d", firstVal, secondVal);
		SendHappyHour(buf);
	}
	else
	{
		SendNotice("The Happy Hour was disabled, see you next time!", true);
		quest::CQuestManager::instance().RequestSetEventFlag("happyFirstNumber", 69);
		quest::CQuestManager::instance().RequestSetEventFlag("happySecondNumber", 0);
		
		char buf[256];
		snprintf(buf, sizeof(buf), "%d#%d", 69, 0);
		SendHappyHour(buf);
...
ACMD(do_battleroyale)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[BATTLE_ROYALE] Usage: /battleroyale [action(open, register, enter, close)]");
		return;
	}

	if (strcasecmp(arg1, "open") == 0)
	{
		BattleRoyale::Instance().RequestStateChange(ch, BattleRoyale::BR_STATE_REGISTER);
	}
	else if (strcasecmp(arg1, "close") == 0)
	{
