...
void CAnticheatManager::DeleteClientHandle(DWORD dwPlayerID)
{
}
...
bool CAnticheatManager::__ParseCheatBlacklist(DWORD uiScanID, const std::string & szTableName)
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT * FROM anticheat.%s%s", szTableName.c_str(), get_table_postfix()));
