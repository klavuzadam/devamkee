...
void CHARACTER::SkillLevelPacket()
{
	...
	std::memcpy(&pack.skills, m_pSkillLevels, sizeof(TPlayerSkill) * SKILL_MAX_NUM);
	GetDesc()->Packet(&pack, sizeof(TPacketGCSkillLevel));
	
#ifdef __LEADERSHIP__BONUS__
	float kLeaderShip = (float) GetSkillPowerByLevel( MIN(SKILL_MAX_LEVEL, GetLeadershipSkillLevel() ) )/ 100.0f;
	ChatPacket(CHAT_TYPE_COMMAND, "SkillPowerLeadership %f", kLeaderShip);
#endif
}
...
void CHARACTER::SetSkillLevel(DWORD dwVnum, BYTE bLev)
{
	...
	if (GetSkillGroup())
	{
		if (dwVnum <= 111 && bLev >= 40)
		{
			CHARACTER_MANAGER::Instance().SetRewardData(REWARD_PSKILL, GetName(), true);
		}
	}
...
bool CHARACTER::IsLearnableSkill(DWORD dwSkillVnum) const
{
	...
	if ((dwSkillVnum >= 164 && dwSkillVnum <= 167) || pkSkill->dwType - 1 == GetJob())
#else
	...
	if(dwSkillVnum >= SKILL_P_PALBANG && dwSkillVnum <= SKILL_PP_SALPOONG)
#else
	if(dwSkillVnum >= SKILL_P_PALBANG && dwSkillVnum <= SKILL_PP_BYEORAK)
#endif
	{
...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	{
		if (GetSkillMasterType(dwSkillVnum) > SKILL_GRAND_MASTER)
			ChatPacket(CHAT_TYPE_INFO,  "338");
		else
			ChatPacket(CHAT_TYPE_INFO,  "339");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_TALKING,  "340");
		ChatPacket(CHAT_TYPE_INFO,  "341");
		LogManager::instance().CharLog(this, dwSkillVnum, "GM_READ_FAIL", "");
		return false;
	}

	ChatPacket(CHAT_TYPE_TALKING,  "342");
	ChatPacket(CHAT_TYPE_TALKING,  "343");
	ChatPacket(CHAT_TYPE_INFO,  "344");
...
)
{
	...
	// bTypeÀÌ 0ÀÌ¸é Ã³À½ºÎÅÍ Ã¥À¸·Î ¼ö·Ã °¡´É
#ifdef ENABLE_78_SKILLS
	if (pkSk->dwType != 0 && pkSk->dwType != 6) 
#else
	if (pkSk->dwType != 0)
#endif
	{
	...
	if (get_global_time() < GetSkillNextReadTime(dwSkillVnum))
	{
		...
		{
#ifdef USE_PASSIVE_ABILITY_TAMER
			DWORD bookDelayAffect = bIsTamerSkill ? AFFECT_SKILL_TAMER_NO_BOOK_DELAY : AFFECT_SKILL_NO_BOOK_DELAY;

			if (FindAffect(bookDelayAffect))
			{
				RemoveAffect(bookDelayAffect);
#else
			if (FindAffect(AFFECT_SKILL_NO_BOOK_DELAY))
			{
				RemoveAffect(AFFECT_SKILL_NO_BOOK_DELAY);
	...
	if (bProb != 0)
	{
		...
		if (bProb < 100)
		{
			...
			if (bIsTamerSkill)
			{
				if (FindAffect(AFFECT_SKILL_TAMER_BOOK_BONUS))
				{
					bProb = 100;
					RemoveAffect(AFFECT_SKILL_TAMER_BOOK_BONUS);
				}
			}
			else
			{
		...
		if (number(1, 100) <= bProb)
		{
			...
			if (bIsTamerSkill)
			{
				...
				{
					char szFlag[128 + 1];
					snprintf(szFlag, sizeof(szFlag), "traning_master_skill.%u.read_count", dwSkillVnum);

					const auto needCount = m_bPassiveAbilityTamerBookCount[bLastLevel];
					if (needCount == 0)
					{
					...
					uint8_t readCount = static_cast<uint8_t>(pPC->GetFlag(szFlag));
					readCount += 1;

					if (readCount >= needCount)
					{
						...
						ChatPacket(CHAT_TYPE_INFO,  "348");
						return true;
					}
					else
					{
						pPC->SetFlag(szFlag, readCount);

						switch (number(1, 3))
						{
							case 1:
							{
								ChatPacket(CHAT_TYPE_TALKING,  "349");
								break;
							}
							case 2:
							{
								ChatPacket(CHAT_TYPE_TALKING,  "350");
								break;
							}
							case 3:
							default:
							{
								ChatPacket(CHAT_TYPE_TALKING,  "351");
								break;
							}
						}

						// ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have to succesfully learn other %d book(s) the next level of this ability"), needCount - readCount);
						ChatPacket(CHAT_TYPE_INFO,  "1563 %d", needCount - readCount);
						return true;
					}
				}
			}
			...
			if (pPC)
			{
				...
				if (number(1, 100) > percent)
				{
					...
					else
					{
						...
						ChatPacket(CHAT_TYPE_INFO,  "352 %d", need_bookcount - read_count);
						return true;
					}
				}
			}
...
#ifdef ENABLE_78_SKILLS
#ifdef ENABLE_WOLFMAN_CHARACTER
	if(dwVnum >= SKILL_P_PALBANG && dwVnum <= SKILL_PP_SALPOONG)
#else
	if(dwVnum >= SKILL_P_PALBANG && dwVnum <= SKILL_PP_BYEORAK)
#endif
	{
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
...
#ifdef ENABLE_78_SKILLS
		BYTE ANTI_SKILL_ID = 0, HELP_SKILL_ID = 0;
		switch (m_pkSk->dwVnum)
		{
			case SKILL_PALBANG:			ANTI_SKILL_ID = SKILL_P_PALBANG;		break;
			case SKILL_AMSEOP:			ANTI_SKILL_ID = SKILL_P_AMSEUP;			break;
			case SKILL_SWAERYUNG:			ANTI_SKILL_ID = SKILL_P_SWAERYEONG;		break;
			case SKILL_YONGBI:			ANTI_SKILL_ID = SKILL_P_YONGPA;			break;
			case SKILL_GIGONGCHAM:			ANTI_SKILL_ID = SKILL_P_GINGONGCHAM;		break;
			case SKILL_HWAJO:			ANTI_SKILL_ID = SKILL_P_HWAJO;			break;
			case SKILL_MARYUNG:			ANTI_SKILL_ID = SKILL_P_MARYEONG;		break;
			case SKILL_BYEURAK:			ANTI_SKILL_ID = SKILL_P_BYEORAK;		break;
#ifdef ENABLE_WOLFMAN_CHARACTER
			case SKILL_SALPOONG:			ANTI_SKILL_ID = SKILL_P_SALPOONG;		break;
#endif
		}
		...
		switch (m_pkSk->dwVnum)
		{
			case SKILL_PALBANG:			HELP_SKILL_ID = SKILL_PP_PALBANG;		break;
			case SKILL_AMSEOP:			HELP_SKILL_ID = SKILL_PP_AMSEUP;		break;
			case SKILL_SWAERYUNG:			HELP_SKILL_ID = SKILL_PP_SWAERYONG;		break;
			case SKILL_YONGBI:			HELP_SKILL_ID = SKILL_PP_YONGPA;		break;
			case SKILL_GIGONGCHAM:			HELP_SKILL_ID = SKILL_PP_GIGONGCHAM;		break;
			case SKILL_HWAJO:			HELP_SKILL_ID = SKILL_PP_HWAJO;			break;
			case SKILL_MARYUNG:			HELP_SKILL_ID = SKILL_PP_MARYEONG;		break;
			case SKILL_BYEURAK:			HELP_SKILL_ID = SKILL_PP_BYEORAK;		break;
#ifdef ENABLE_WOLFMAN_CHARACTER
			case SKILL_SALPOONG:			HELP_SKILL_ID = SKILL_PP_SALPOONG;		break;
#endif
		}
...
EVENTFUNC(skill_gwihwan_event)
{
	...
	{
		//½ÇÆÐ
		ch->ChatPacket(CHAT_TYPE_INFO,  "354");
	}
	return 0;
}
...
, bFromAutoHunt
	...
	if (pParty && pParty->GetNearMemberCount())
	{
...
)
{
	...
	if (false == bCanUseHorseSkill && true == IsRiding())
	{
		...
		if (!(bFromAutoHunt
			 ...
			 && GetAutoUseType(AUTO_ONOFF_SKILL)
			 && ItIsAllowedSkillFromHorse(dwVnum)))
		{
#endif
		return BATTLE_NONE;
#if defined(__AUTOMATIC_HUNTING_SYSTEM__) && defined(USE_AUTOMATIC_HUNTING_USE_SKILLS_RENEWED)
...
#ifdef __BUFFI_SUPPORT__
	float k = 1.0 * GetSkillPower(pkSk->dwVnum, bSkillLevel) * pkSk->bMaxLevel / 100;

	const std::map<DWORD, std::pair<DWORD, DWORD>> m_duplicateList = {
	{94, {AFF_HOSIN, AFF_BUFFI_HOSIN}},
	{164, {AFF_HOSIN, AFF_BUFFI_HOSIN}},

	{96, {AFF_GICHEON, AFF_BUFFI_GICHEON}},
	{165, {AFF_GICHEON, AFF_BUFFI_GICHEON}},

	{110, {AFF_KWAESOK, AFF_BUFFI_KWAESOK}},
	{166, {AFF_KWAESOK, AFF_BUFFI_KWAESOK}},

	{111, {AFF_JEUNGRYEOK, AFF_BUFFI_JEUNGRYEOK}},
	{167, {AFF_JEUNGRYEOK, AFF_BUFFI_JEUNGRYEOK}},
	};
	auto it = m_duplicateList.find(dwVnum);
	if (it != m_duplicateList.end())
	{
		if (pkVictim->IsAffectFlag(it->second.first) || pkVictim->IsAffectFlag(it->second.second))
			return BATTLE_NONE;
	}
	if(dwVnum >= 164 && dwVnum <= 167)
		k = 1.0 * GetSkillPower(pkSk->dwVnum, bSkillLevel) * 1 / 100;
#else
	const float k = 1.0 * GetSkillPower(pkSk->dwVnum, bSkillLevel) * pkSk->bMaxLevel / 100;
#endif
...
, bFromAutoHunt
		...
		case 111:
			{
				if (pkVictim)
				{
					...
					{
						if (pkVictim->GetMapIndex() == DOJANG_MAPINDEX)
						{
							ChatPacket(CHAT_TYPE_INFO, "Nu poti folosi pe jucatori aceast skill in mapa PVP"); 
							return false;
						}
					}
				}
	...
	if (dwVnum == SKILL_HORSE_SUMMON)
	{
		...
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO,  "355");
		else
			ChatPacket(CHAT_TYPE_INFO,  "356");
...
, 0, bFromAutoHunt
	...
	else
	{
		...
		if (test_server)
			//ChatPacket(CHAT_TYPE_INFO,  "357 %s %d", pkSk->szName, iNeededSP);
			ChatPacket(CHAT_TYPE_INFO,  "357 S%d %d", pkSk->dwVnum, iNeededSP);
...
void CHARACTER::SkillLearnWaitMoreTimeMessage(DWORD ms)
{
	...
	//
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "358");
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "359");
	else if (ms < 10 * 60) // 10ºÐ
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "360");
	else if (ms < 30 * 60) // 30ºÐ
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "361");
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "362");
	}
	else if (ms < 1 * 3600) // 1½Ã°£
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "363");
	else if (ms < 2 * 3600) // 2½Ã°£
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "364");
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "365");
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "366");
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "367");
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "368");
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "369");
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "370");
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "371");
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "372");
		ChatPacket(CHAT_TYPE_TALKING, "%s",  "373");
	}
	/*
	   str = "30%";
	   else if (ms < 3 * 86400)
...
bool CHARACTER::UseMobSkill(unsigned int idx)
{
	...
	if(GetRaceNum() >= 9836 && GetRaceNum() <= 9859)
	{
		BYTE LuckyBitch = number(1,15);
		if(LuckyBitch == 1)
		{
			EffectPacket(EFFECT_SKILL_DAMAGE_ZONE);
		}
		else if(LuckyBitch == 2)
		{
			EffectPacket(EFFECT_SKILL_DAMAGE_ZONE_BUYUK);
		}
		else if(LuckyBitch == 3)
		{
			EffectPacket(EFFECT_SKILL_DAMAGE_ZONE_ORTA);
		}
		else if(LuckyBitch == 4)
		{
			EffectPacket(EFFECT_SKILL_DAMAGE_ZONE_KUCUK);
		}
		else if(LuckyBitch == 5)
		{
			EffectPacket(EFFECT_SKILL_SAFE_ZONE);
		}
		else if(LuckyBitch == 6)
		{
			EffectPacket(EFFECT_SKILL_SAFE_ZONE_BUYUK);
		}
		else if(LuckyBitch == 7)
		{
			EffectPacket(EFFECT_SKILL_SAFE_ZONE_ORTA);
		}
		else if(LuckyBitch == 8)
		{
			EffectPacket(EFFECT_SKILL_SAFE_ZONE_KUCUK);
		}
		else if(LuckyBitch == 9)
		{
			EffectPacket(EFFECT_METEOR);
		}
		else if(LuckyBitch == 10)
		{
			EffectPacket(EFFECT_BEAD_RAIN);
		}
		else if(LuckyBitch == 11)
		{
			EffectPacket(EFFECT_FALL_ROCK);
		}
		else if(LuckyBitch == 12)
		{
			EffectPacket(EFFECT_ARROW_RAIN);
		}
		else if(LuckyBitch == 13)
		{
			EffectPacket(EFFECT_HORSE_DROP);
		}
		else if(LuckyBitch == 14)
		{
			EffectPacket(EFFECT_EGG_DROP);
		}
		else if(LuckyBitch == 15)
		{
			EffectPacket(EFFECT_DEAPO_BOOM);
		}
	}
#endif
...
eMountType GetMountLevelByVnum(DWORD dwMountVnum, bool IsNew) // updated to 2014/12/10
{
	...
	switch (dwMountVnum)
	{
		...
		case 20228: // Unicorn (no skill, yes atk)
...
#ifdef ENABLE_YOHARA_SYSTEM
		DWORD tmpSkillCount = number(0, SKILL_COUNT-2);
#else
...
#if defined(__AUTOMATIC_HUNTING_SYSTEM__) && defined(USE_AUTOMATIC_HUNTING_USE_SKILLS_RENEWED)
		if (bFromAutoHunt
			 && GetAutoUseTypeCst(AUTO_ONOFF_START)
			 && GetAutoUseTypeCst(AUTO_ONOFF_SKILL)
			 && ItIsAllowedSkillFromHorse(dwSkillVnum))
		{
			return true;
		}
#endif
