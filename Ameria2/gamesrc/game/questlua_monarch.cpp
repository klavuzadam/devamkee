...
namespace quest
{
	...
	ALUA(monarch_bless)
	{
		...
		if (false==ch->IsMonarch())
		{
			if (!ch->IsGM())
			{
				ch->ChatPacket(CHAT_TYPE_INFO ,"842");
		...
		if (CMonarch::instance().HealMyEmpire(ch, HealPrice))
		{
			...
			snprintf(szNotice, sizeof(szNotice),
					"900 %s", EMPIRE_NAME(ch->GetEmpire()));
			SendNoticeMap(szNotice, ch->GetMapIndex(), false);

			ch->ChatPacket(CHAT_TYPE_INFO ,"901");
		}
	...
	ALUA(monarch_powerup)
	{
		...
		if (!CMonarch::instance().IsMoneyOk(money_need, ch->GetEmpire()))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "444 %u %u", NationMoney, money_need);
			return 0;
		}
		...
		if (!CMonarch::instance().CheckPowerUpCT(ch->GetEmpire()))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "902 %d", next_sec);
			return 0;
		}
		...
		char szNotice[256];
		snprintf(szNotice, sizeof(szNotice), "903 %s %%", EMPIRE_NAME(ch->GetEmpire()));

		SendNoticeMap(szNotice, ch->GetMapIndex(), false);

		return 1;
	}
	// ±ºÁÖÀÇ ±Ý°­±Ç Äù½ºÆ® ÇÔ¼ö
	ALUA(monarch_defenseup)
	{
		...
		char szNotice[256];
		snprintf(szNotice, sizeof(szNotice), "904 %s %%", EMPIRE_NAME(ch->GetEmpire()));
	...
	ALUA(spawn_mob)
	{
...

		{
			...
			long y = ch->GetY();
#if 0
			if (11505 == mob_vnum)	// È²±ÝµÎ²¨ºñ
			{
				//±ºÁÖ ±¹°í °Ë»ç
				if (!CMonarch::instance().IsMoneyOk(CASTLE_FROG_PRICE, ch->GetEmpire()))
				{
					int NationMoney = CMonarch::instance().GetMoney(ch->GetEmpire());
					ch->ChatPacket(CHAT_TYPE_INFO, "444 %u %u", NationMoney, CASTLE_FROG_PRICE);
					return 0;
				}
	...
	ALUA(spawn_guard)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "905");
			return 0;
		}
	...
	ALUA(frog_to_empire_money)
	{
		...
		if (castle_frog_to_empire_money(ch))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "906");
			ch->ChatPacket(CHAT_TYPE_INFO, "907 %d", empire_money);
			castle_save();
			return 1;
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "908");
			return 0;
		}
	}
		...
		if (!tch)
		{
			...
			if (pkCCI)
			{
				...
				else
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, "449 %s", name.c_str());
	...
	ALUA(monarch_transfer)
	{
		...
		if (!tch)
		{
			...
			if (pkCCI)
			{
				...
				if (pkCCI->bChannel != g_bChannel)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "452 %s %s %d", name.c_str(), pkCCI->bChannel, g_bChannel);
					return 0;
				}
				...
				ch->ChatPacket(CHAT_TYPE_INFO, "454 %s", name.c_str());
	...
	ALUA(monarch_transfer2)
	{
		...
		{
			int NationMoney = CMonarch::instance().GetMoney(ch->GetEmpire());
			ch->ChatPacket(CHAT_TYPE_INFO, "444 %u %u", NationMoney, ciTransferCost);
			return 0;
		}
		...
		if (pTargetChar == NULL)
		{
			...
			if (pCCI != NULL)
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "909 %s %d %d",
				...
				ch->ChatPacket(CHAT_TYPE_INFO, "910");
		...
		else
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "911 %s", pTargetChar->GetName());
				return 0;
			}
