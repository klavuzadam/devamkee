...
public:
	...
	bool	GetRefineGradeValues(BYTE ds_type, BYTE grade_idx, OUT int& need_count, OUT int64_t& fee, OUT std::vector<float>& vec_probs);
	bool	GetRefineStepValues(BYTE ds_type, BYTE step_idx, OUT int& need_count, OUT int64_t& fee, OUT std::vector<float>& vec_probs);
	bool	GetRefineStrengthValues(BYTE ds_type, BYTE material_type, BYTE strength_idx, OUT int64_t& fee, OUT float& prob);
