...
namespace quest
{
	...
	EVENTFUNC(SungMahiMonthRewardTimer)
	{
		...
		if (quest::CQuestManager::instance().GetEventFlag("sungMahiLastMonth") != currentMonth)
		{
			...
			// {
				// std::unique_ptr<SQLMsg> pMsg((DBManager::instance().DirectQuery("SELECT player_login FROM `sung_mahi_ranking` WHERE tower_level = %d ORDER BY tower_time ASC LIMIT 1", index));
			...
			// dofile to clear the set
			{
				...
				snprintf(questDungeonInformation, sizeof(questDungeonInformation), "%s/dungeonLibrary/dungeonInfoLibrary.lua", LocaleService_GetQuestPath().c_str());
...
#ifdef ENABLE_SUNG_MAHI_TOWER
		if (strcmp(g_stHostname.c_str(), "srv1-ch1-core1") == 0)
		{
...
#define ENABLE_PARTYKILL
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, "920");

				return;
			}

			//Äù½ºÆ® Ã¢¿¡¼­ Äù½ºÆ® Å¬¸¯°ú NPC Å¬¸¯½ÃÀÇ ±¸ºÐÀ» À§ÇÑ ÇÃ·¡±×
			m_bQuestInfoFlag = 1;
	...
	{
		itertype(m_mapQuestNameByIndex) it = m_mapQuestNameByIndex.find(idx);
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
		...
		else if (name == "auto_use")
		{
			const DESC_MANAGER::DESC_SET& c_rkDescSet = DESC_MANAGER::instance().GetClientSet();
			for (itertype(c_rkDescSet) it = c_rkDescSet.begin(); it != c_rkDescSet.end(); ++it)
			{
				LPCHARACTER lpCh = (*it)->GetCharacter();
				if (lpCh != nullptr)
				{
					if (value > 0)
						lpCh->ChatPacket(CHAT_TYPE_COMMAND, "auto_on");
					else
						lpCh->ChatPacket(CHAT_TYPE_COMMAND, "auto_off");
				}
			}
		}
#endif
		...
		{
			if (!value)
				SendNotice("Spin Wheel closed.", false);
			else
				SendNotice("Spin Wheel opened.", false);
...
//					continue;
//
//				ch->ChatPacket(CHAT_TYPE_COMMAND, "MiniGameCatchKing %d", value);
//			}
//
//		}
...
//#endif
		...
		else if (name == "enable_battle_royale")
		{
			const DESC_MANAGER::DESC_SET& c_ref_set = DESC_MANAGER::instance().GetClientSet();
			for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
			{
				LPCHARACTER ch = (*it)->GetCharacter();
				if (ch)
					ch->ChatPacket(CHAT_TYPE_COMMAND, "battle_royale %d", (value == 1));
			}
		}
#endif
...
#if defined(__BL_BATTLE_ROYALE__)
		if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("enable_battle_royale")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "battle_royale %d", (iEventFlagValue == 1));
		}
#endif

#if defined(__AUTOMATIC_HUNTING_SYSTEM__)
		if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("auto_use")))
		{
