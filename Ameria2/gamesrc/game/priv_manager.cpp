...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		...
		{
			char buf[100];
			snprintf(buf, sizeof(buf), "892 %s %s %d%%", g->GetName(), GetPrivName(type), value);
			SendNotice(buf);
		}
		else
		{
			char buf[100];
			snprintf(buf, sizeof(buf), "893 %s %s", g->GetName(), GetPrivName(type));
			SendNotice(buf);
		}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), "894 %s %s %d %%", GetEmpireName(empire), GetPrivName(type), value);
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), "895 %s %s", GetEmpireName(empire), GetPrivName(type));
