...
long int global_time_maintenance = 0;
...
#define MAINTENANCE_ADMIN_NAME	"[A]ivory" // Name for acces
#define MAINTENANCE_ADMIN_IP	"86.123.121.223" // Ip for acces

/*********
* Table with translate for all informations, have careful with %s or %u when you try to translate in other language.
*/
static const char* maintenance_translate[] = {"-----------------------------------------------------------------------------------",
...
void MaintenanceManager::Send_UpdateBinary(LPCHARACTER ch)
{
	...
	if (ch)
	{
		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT time,duration,reason from player.maintenance"));
