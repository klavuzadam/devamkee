...
namespace multilang
{
	std::map<DWORD, std::map<BYTE, std::string>> g_itemNames;
	
	void load_item_names()
	{
		const std::map<BYTE, std::string> languages = {
			{LOCALE_EN, "en"},
			{LOCALE_TR, "tr"},
			{LOCALE_ES, "es"},
			{LOCALE_PL, "pl"},
		...
		for (auto it = languages.begin(); it != languages.end(); ++it)
		{
			...
			if (fp != NULL)
			{
				...
				{
					auto tokens = msl::string_split(one_line, "\t");
					if (tokens.size() < 2)
						continue;
					if(!str_to_number(vnum, tokens[0].c_str()))
						continue;
					g_itemNames[vnum][it->first] = tokens[1];
				}
				fclose(fp);
			}
		}
	...
	{
		static std::string s_UNKNOWN = "unknown";
		// searching by vnum
		auto vnumIter = g_itemNames.find(vnum);
		if(vnumIter == g_itemNames.end())
			return s_UNKNOWN;
		// searching by lang
		auto& langmap = vnumIter->second;
		auto langIter = langmap.find(lang);
