...
namespace quest
{
	...
	ALUA(pc_sung_mahi_curse_hp)
	{
		if (lua_isnumber(L, 1))
		{
			...
			if (ch)
			{
				BYTE bCurseAmount = (BYTE) lua_tonumber(L, 1);
	...
	ALUA(pc_ranking_dungeon_complete)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		RankPlayer::instance().SendInfoPlayer(ch, RANK_BY_DUNGEON_COMPLETE, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), RANK_BY_DUNGEON_COMPLETE) + 1);
		return 0;
	}
	...
	ALUA(pc_give_item_from_special_item_group)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "914 %d", dwCounts[i]);
				}
				else if (dwVnums[i] == 2)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "915");
					ch->ChatPacket(CHAT_TYPE_INFO, "916 %d", dwCounts[i]);
				}
			}
		}
		return 0;
	...
	ALUA(pc_get_playtime_new)
	{
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetPoint(POINT_PLAYTIME));
		return 1;
	}
	...
	ALUA(pc_get_wear)
	{
		...
		WORD wCell = (WORD)lua_tonumber(L, 1);
	...
	ALUA(pc_get_empire)
	{
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr() ? CQuestManager::instance().GetCurrentCharacterPtr()->GetEmpire() : 0);
		return 1;
	}
	...
	ALUA(pc_mount)
	{
		...
		if (CCombatZoneManager::Instance().IsCombatZoneMap(ch->GetMapIndex()))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "108");
			return 0;
		}
#endif
#ifdef TOURNAMENT_PVP_SYSTEM
	...
	if (g_NoMountAtGuildWar && ch->GetWarMap())
	{
		...
		if (mount_vnum)
		{
			...
			case 32002:
			case 32003:
			ch->AddAffect(AFFECT_MOUNT, POINT_MOV_SPEED, 10, AFF_NONE, length, 0, true, true);
			break;
...
teleport_area:
	...
	ALUA(pc_change_name)
	{
		...
		RankPlayer::Instance().LoadInfoPlayer(ch->GetPlayerID(), true);
	...
	ALUA(pc_is_friend)
	{
		...
		lua_pushboolean(L, MessengerManager::instance().IsInFriendList(ch->GetName(), tch->GetName()));
		return 1;
	}
#endif
	...
	ALUA(pc_charge_cash)
	{
		...
		{
			sys_err("pc_charge_cash: not enough arguments.");
			return 0;
		}
		...
		if (!d)
		{
			sys_err("pc_charge_cash: no desc pointer.");
			return 0;
		}

		long long amount = static_cast<long long>(lua_tonumber(L, 1));
		if (amount < 1 || amount > 1000)
		{
			sys_err("pccharge_cash: invalid charge amount: %d.", amount);
			return 0;
		}

#ifdef USE_ITEMSHOP_RENEWED
		ch->SetAccountMoney(amount, 0, true);
#else
		ch->SetAccountMoney(amount, true);
#endif
		return 0;
	}
	...
	ALUA(pc_open_shop0)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "917");
			return 0;
		}
	...
	enum MKGLD : int
	{
		MKGLD_INVALID_NAME_LENGTH = -2,
		MKGLD_INVALID_NAME_INPUT = -1,
		MKGLD_GUILD_NOT_CREATED = 0,
		MKGLD_GUILD_CREATED = 1,
		MKGLD_ALREADY_GUILDED = 2,
		MKGLD_ALREADY_MASTER_GUILD = 3
	};
	...
	ALUA(pc_make_guild0)
	{
		...
		const auto checkNameRes = check_name(cp.name);
	...
	int pc_get_language(lua_State* L){
		...
		lua_pushnumber(L, ch->GetCharacterLanguage());
		return 1;
	}
	...
	int pc_set_language(lua_State* L){
		...
		ch->SetCharacterLanguage(lua_tonumber(L, 1));
		return 1;
	}
#ifdef HORSE_FIX
	...
	ALUA(_getexp_blocked)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch->block_exp);
		return 1;
	}  
	...
	int pc_set_observer_true(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch || ch == NULL)
			return 0;
	...
	int pc_set_observer_false(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch || ch == NULL)
			return 0;
	...
	int pc_is_observer(lua_State* L)
	{
		...
		if (!ch || ch == NULL)
			return 0;
		lua_pushboolean(L, ch->IsObserverMode());
		return 1;
	}
#endif
#ifdef __GEM_SYSTEM__
	int pc_calculate_gaya_piece(lua_State* L)
	{
		int makeCount = 0;
		if (!lua_isnumber(L, 1) || !lua_isboolean(L, 2))
		{
			lua_pushnumber(L, makeCount);
			return 1;
		}
		const int pieceCount = (int)lua_tonumber(L, 1);
		const bool onlyPieceCount = (bool)lua_toboolean(L, 2);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch)
		{
			int findCount = 0;
			for (WORD j = 0; j < INVENTORY_AND_EQUIP_SLOT_MAX; ++j)
			{
				LPITEM item = ch->GetInventoryItem(j);
				if (!item)
					continue;
				if (item->IsStone())
				{
					if (item->GetRefineLevel() >= 0 && item->GetRefineLevel() <= 3)
						findCount += item->GetCount();
				}
			}
			for (WORD j = 0; j < SPECIAL_INVENTORY_MAX_NUM; ++j)
			{
				LPITEM item = ch->GetStoneInventoryItem(j);
				if (!item)
					continue;
				if (item->IsStone())
				{
			...
			if (findCount)
			{
				if (pieceCount < findCount)
					findCount = pieceCount;
				if ((findCount % 2) != 0)
					findCount -= 1;
				if (findCount <= 0)
				{
				...
				for (WORD j = 0; j < INVENTORY_AND_EQUIP_SLOT_MAX; ++j)
				{
					...
					if (item->IsStone())
					{
						if (item->GetRefineLevel() >= 0 && item->GetRefineLevel() <= 3)
						{
							const int removeCount = item->GetCount() > findCount ? findCount : item->GetCount();
							findCount = removeCount == findCount ? 0 : findCount-removeCount;
	...
	int pc_get_vote_coin(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, ch ? ch->GetVoteCoin() : 0);
		return 1;
	}
	int pc_set_vote_coin(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch && lua_isnumber(L, 1))
	...
	ALUA(pc_set_buffi_name)
	{
		...
		if (ch->FindAffect(AFFECT_NAME_BUFFI))
			return 1;
		
		buffi->SetBuffiName(szBuffiName);
		ch->AddAffect(AFFECT_NAME_BUFFI, POINT_ATTBONUS_BOSS, 10, 0, 604800, 0, false);
	...
	ALUA(pc_set_conqueror_level)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "You didnt reached the maximum Level!");
			return 0;
		}

		if (/*ch->GetLevel() == gPlayerMaxLevel && */ch->GetExp() < (exp_table_common[120] / 4))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "You dont have the matching experience!");
			return 0;
		}
	...
	ALUA(pc_give_auto_hunt)
	{
		...
		lua_pushnumber(L, RETURN_ERROR_ARG);
#else
		if (!lua_isnumber(L, 1))
		{
			lua_pushnumber(L, RETURN_ERROR_ARG);
		...
		const LPCHARACTER pChar = rQMgr.GetCurrentCharacterPtr();
		if (!pChar)
		{
			lua_pushnumber(L, RETURN_ERROR_PTR);
			return 1;
		}
		...
		int iSec = (int)lua_tonumber(L, 1);
		pChar->AddAffect(AFFECT_AUTO_HUNT, 0, 1, AFF_NONE, iSec, 0, false/*bOverride*/);

		// char szQuery[1024];
		// snprintf(szQuery, sizeof(szQuery), "UPDATE account.account SET auto_use_expire = DATE_ADD(CURRENT_TIME, INTERVAL %d SECOND) WHERE id = %u;",
			// iSec, pDesc->GetAccountTable().id);
		// std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));

		lua_pushnumber(L, RETURN_SUCCESS);
#endif
		return 1;
	}
#endif
	ALUA(pc_set_weapon_status)
	{
		if (!lua_isnumber(L, 1))
		{
		...
		BYTE questWeaponStatus = (int) lua_tonumber(L, 1);
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		
		if (ch)
		{
			ch->bWeaponStatus = questWeaponStatus;
		}
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "get_playtime_new" , pc_get_playtime_new},
			...
			{ "rank_dungeon_complete",	pc_ranking_dungeon_complete		},
			...
			//pc.pointchange(uint32_t type, int amount, bool bAmount, bool bBroadcast)
			...
			{ "getexp_blocked",	_getexp_blocked	}, 
#endif
#ifdef dracaryS_DUNGEON_LIB
			{"jump",				pc_jump},
#endif
#ifdef ENABLE_DRAGON_LAIR
			{ "observer_true",		pc_set_observer_true},
			{ "observer_false",		pc_set_observer_false},
			{ "is_observer",		pc_is_observer},
#endif
#ifdef __GEM_SYSTEM__
			{ "calculate_gaya_piece", pc_calculate_gaya_piece },
			{ "get_gem", pc_get_gem },
...
#if defined(__AUTOMATIC_HUNTING_SYSTEM__)
			{ "give_auto_hunt", pc_give_auto_hunt },
#endif
			{ "setWeaponStatus",	pc_set_weapon_status	},
			{ "reset_hp",			pc_reset_hp				},
#ifdef USE_CRYSTAL_SYSTEM
			{ "shard_craft_open", pc_shard_craft_open },
#endif
			{ NULL,			NULL			}
		};
