...
void CInputP2P::LoginEx(LPDESC d, const char * c_pData)
{
	P2P_MANAGER::instance().LoginEx(d, (TPacketGGLoginEx *) c_pData);
}
#endif
...
void CInputP2P::DisconnectP2P(const char * c_pData)
{
	auto * p = (SPacketGGDisconnectName*) c_pData;

	const LPDESC d = DESC_MANAGER::instance().FindByCharacterName(p->szName);

	if (!d)
	{
		sys_err("[DC P2P] Couldn't find by character name %s", p->szName);
		return;
	}
...
void CInputP2P::WarpCharacter(const char* c_pData)
{
	...
	{
#ifdef __RENEWAL_WARP__
		ch->WarpSet(p->x, p->y, p->mapIndex, p->port);
#else
		ch->WarpSet(p->x, p->y);
#endif
	}
...
void CInputP2P::Transfer(const char * c_pData)
{
	...
	if (ch)
#ifdef __RENEWAL_WARP__
		ch->WarpSet(p->lX, p->lY, p->mapIndex, p->port);
#else
...
{
	const auto* p = reinterpret_cast<const TPacketGGPlayerRank*>(c_pData);
	if (!p)
	{
		return;
	}

	RankPlayer::Instance().ReceiveP2P(p->iMode, p->dwPid, p->szName, p->bEmpire, p->lValue, p->szGuildName);
}

void CInputP2P::PlayerRankReload(LPDESC d, const char* c_pData)
{
	const auto* p = reinterpret_cast<const TPacketGGPlayerRankReload*>(c_pData);
	if (!p)
	{
		return;
	}

	RankPlayer::Instance().LoadInfoPlayer(p->dwPID, true);
}
...
int CInputP2P::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
		...
		case HEADER_GG_DISCONNECT_NAME:
			DisconnectP2P(c_pData);
			break;
		...
		case HEADER_GG_PLAYER_RANK:
			...
			break;
		case HEADER_GG_PLAYER_RANK_RELOAD:
			PlayerRankReload(d, c_pData);
			break;
#if defined(__BL_BATTLE_ROYALE__)
		case HEADER_GG_BATTLE_ROYALE:
			BattleRoyale(c_pData);
			break;
#endif
...
void CInputP2P::BattleRoyale(const char* c_pData)
{
	TPacketGGBattleRoyale* p = (TPacketGGBattleRoyale*)c_pData;

	switch (p->subheader)
	{
	// Received by battle royale
	case BattleRoyale::BR_SUB_CHANGE_STATE:
		BattleRoyale::Instance().BattleCore_ReceiveRequestStateChange(p->state, p->dwPID);
		break;
	case BattleRoyale::BR_SUB_REGISTER:
	case BattleRoyale::BR_SUB_ENTER:
	case BattleRoyale::BR_SUB_CANCEL_REGISTRATION:
		BattleRoyale::Instance().BattleCore_ReceiveActionRequest(p->dwPID, p->subheader);
		break;

	// Received on other cores
	case BattleRoyale::BR_SUB_REGISTER_REPLY_FAIL:
	case BattleRoyale::BR_SUB_REGISTER_REPLY_SUCCESS:
	case BattleRoyale::BR_SUB_REGISTER_REPLY_ALREADY:
	case BattleRoyale::BR_SUB_REGISTER_REPLY_FULL:
	case BattleRoyale::BR_SUB_ENTER_REPLY_SUCCESS:
	case BattleRoyale::BR_SUB_CANCEL_REGISTRATION_REPLY_SUCCESS:
	case BattleRoyale::BR_SUB_CANCEL_REGISTRATION_REPLY_NOT_REGISTERED:
		BattleRoyale::Instance().ReceiveActionRequestReply(p->dwPID, p->subheader);
		break;
	case BattleRoyale::BR_SUB_SYNC:
		BattleRoyale::Instance().NormalCore_ReceiveSync(p->state);
		break;

	default:
		sys_err("[BATTLE_ROYALE] Unhandled packet subheader %u", p->subheader);
		return;
	}
}
#endif
