...
namespace mining
{
	...
	void OreDrop(LPCHARACTER ch, DWORD dwLoadVnum)
	{
        ...
        ch->UpdateMissionProgress(MISSION_MINE, 1, dwRawOreVnum);
#endif
...
#ifdef ENABLE_PICKAXE_RENEWAL
			{
				// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, "830 %u", Pick_GetCurExp(*item));
				Pick_SetPenaltyExp(*item);
				// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, "831 %u", Pick_GetCurExp(*item));
				// ch->ChatPacket(CHAT_TYPE_INFO, "832 %%");
	...
	void CHEAT_MAX_PICK(LPCHARACTER ch, LPITEM item)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "833 %d", Pick_GetCurExp(pick));
	}
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "834");
				ch->ChatPacket(CHAT_TYPE_INFO, "835");
			}
			else
			{
				Pick_IncCurExp(pick);

				ch->ChatPacket(CHAT_TYPE_INFO, "836 %d %d",
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "837");
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "838");
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "839");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "840");
		}
