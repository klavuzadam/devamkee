...
CGuild::CGuild(TGuildCreateParameter & cp)
{
	...
	strlcpy(m_data.grade_array[0].grade_name, "682", sizeof(m_data.grade_array[0].grade_name));
	...
	{
		strlcpy(m_data.grade_array[i].grade_name, "683", sizeof(m_data.grade_array[i].grade_name));
...
bool CGuild::RemoveMember(DWORD pid)
{
	...
	RankPlayer::Instance().LoadInfoPlayer(pid, true);
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
    ...
    if (iPulse - ch->GetLastOfferExpTime() < PASSES_PER_SEC(2)) // 2 sec
    {
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "684");
		return false;
	}
...
void CGuild::Disband()
{
	...
	for (const auto& [k, v] : m_member)
	{
		RankPlayer::Instance().LoadInfoPlayer(k, true);
	}
}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->ChatPacket(CHAT_TYPE_INFO, "685");
	else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (GetSP() < iNeededSP)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "686 %d %d", GetSP(), iNeededSP);
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "687");
		return;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, "688 %d %d %d %u", dwVnum, GetSP(), iNeededSP, pid);
		...
		case GUILD_SKILL_TELEPORT:
			...
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "689 %d %d", pcci->bChannel, g_bChannel);
					}
					else
					{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, "690");
			}
			break;
		...
		default:
			{
				/*if (ch->GetPlayerID() != GetMasterPID())
				  {
				  ch->ChatPacket(CHAT_TYPE_INFO, "691");
				  return;
				  }*/

				if (!UnderAnyWar())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "692");
					return;
				}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "693 %u", iSP);
	}
	return true;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "694");
		return;
	}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	if (ch->GetPlayerID() != GetMasterPID())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "695");
		return;
	}
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, "697");
	    return;
	}
	...
	switch ( errcode )
	{
		...
		case GERR_COMMISSIONPENALTY:
						...
						return;
		case GERR_ALREADYJOIN:	pchInviter->ChatPacket(CHAT_TYPE_INFO, "703"); return;
		case GERR_GUILDISFULL:	pchInviter->ChatPacket(CHAT_TYPE_INFO, "704"); return;
		case GERR_GUILD_IS_IN_WAR : pchInviter->ChatPacket( CHAT_TYPE_INFO, "705" ); return;
		case GERR_INVITE_LIMIT : pchInviter->ChatPacket( CHAT_TYPE_INFO, "706" ); return;
	...
	switch ( errcode )
	{
		...
		case GERR_COMMISSIONPENALTY:
						pchInvitee->ChatPacket( CHAT_TYPE_INFO,
								...
								quest::CQuestManager::instance().GetEventFlag( "guild_disband_delay") );
						return;
		case GERR_ALREADYJOIN:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, "703"); return;
		case GERR_GUILDISFULL:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, "704"); return;
		case GERR_GUILD_IS_IN_WAR : pchInvitee->ChatPacket( CHAT_TYPE_INFO, "705" ); return;
		case GERR_INVITE_LIMIT : pchInvitee->ChatPacket( CHAT_TYPE_INFO, "706" ); return;
