...
CShopManager::CShopManager(){Destroy();}
CShopManager::~CShopManager(){Destroy();}
...
bool CShopManager::Initialize(TShopTable * table, int size)
{
	...
	snprintf(szShopTableExFileName, sizeof(szShopTableExFileName),
		"%s/shop_table_ex.txtshop_table_ex.txt", LocaleService_GetBasePath().c_str());
#endif
...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	if (pkChr->IsOpenSafebox()
		...
		|| pkChr->IsAuraRefineWindowOpen()
		 ...
		 || pkChr->isSashOpened(true)
		 || pkChr->isSashOpened(false)
#endif
#ifdef USE_CRYSTAL_SYSTEM
		 || pkChr->IsShardCraftOpened()
#endif
		)
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, "917");
		return false;
	}
...
bool CShopManager::StartShoppingNew(LPCHARACTER pkChr, int iShopVnum)
{
	// fix time revive
	if(pkChr->IsDead())
		return false;
	...
	pkShop->AddGuest(pkChr, iShopVnum, false);
	sys_log(0, "SHOP: START: %s", pkChr->GetName());
	return true;
}
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
    ...
    if (get_dword_time() < ch->GetLastBuyTime() + 300) // 3 seconds
    {
        ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[1554]Please try again later."));
        return;
    }

    ch->SetLastBuyTime(get_dword_time());
#endif	
	...
	if (0 != g_BuySellTimeLimitValue)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "926");
			return;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "927");
		return;
	}
	*/
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bInv, WORD wCell, DWORD bCount)
#else
void CShopManager::Sell(LPCHARACTER ch, WORD wCell, DWORD bCount)
#endif
{
#ifdef ENABLE_NEWSTUFF
	if (0 != g_BuySellTimeLimitValue)
	...
	if (DISTANCE_APPROX(ch->GetX()-ch->GetShopOwner()->GetX(), ch->GetY()-ch->GetShopOwner()->GetY())>2000)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "928");
		return;
	}
	*/

#ifdef ENABLE_SPECIAL_STORAGE
	LPITEM item = ch->GetItem(TItemPos(bInv, wCell));
#else
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "929");
		return;
	}
	...
	if(item->IsSealed()){
		ch->ChatPacket(CHAT_TYPE_INFO, "930");
		return;
	}
#endif
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't sell this item because is binded!");
		return;
	}
#endif
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "1312");
		ch->ChatPacket(CHAT_TYPE_INFO, "1311 %lld", GOLD_MAX);
		return;
	}
	...
	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, "932 %d%%", iVal);
