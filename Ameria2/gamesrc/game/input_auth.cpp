...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	if(pinfo->versionKey != versionKey)
	{
		TPacketGCLoginFailure failurePacket;
		failurePacket.header = HEADER_GC_LOGIN_FAILURE;
...
#ifdef RENEWAL_HWID_BLOCK
	{
		...
		snprintf(szQueryCheck, sizeof(szQueryCheck), "SELECT hwid FROM account.hwid_routher WHERE hwid = '%s'", szRoutherHwid);
		std::unique_ptr<SQLMsg> pMsgCheck(DBManager::instance().DirectQuery(szQueryCheck));
		if (pMsgCheck->Get()->uiNumRows != 0)
		{
			LoginFailure(d, "BLOCK_ROUTHER");
			return;
		}
	}
#endif
