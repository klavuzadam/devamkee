...
void CSwitchBonus::ChangeSB(LPCHARACTER ch,const TPlayerItemAttribute* aAttr)
{
	...
	if (item->GetLevelLimit() <= 40 && ch->CountSpecifyItem(71151) > 0)
		changerVnum = 71151;
	else
		changerVnum = VNUM_CHANGE_ITEM;

	count_change = item->IsArmorGlove() ? ch->CountSpecifyItem(VNUM_TALISMAN_CHANGE_ITEM) : ch->CountSpecifyItem(changerVnum);
	int remove_total = 0;
	...
	if (count_change > 0)
	{
		...
		if(result > 0)
		{
			...
			ch->RemoveSpecifyItem(item->IsArmorGlove() ? VNUM_TALISMAN_CHANGE_ITEM : changerVnum, result);
			RankPlayer::instance().SendInfoPlayer(ch, RANK_BY_BONUS_CHANGED, RankPlayer::instance().GetProgressByPID(ch->GetPlayerID(), RANK_BY_BONUS_CHANGED) + result, false);
