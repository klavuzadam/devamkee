...
void CEffectMeshInstance::OnRender()
{
	...
	for (DWORD i = 0; i < pEffectMesh->GetMeshCount(); ++i)
	{
		...
		D3DXMATRIX v3Out;
		D3DXMatrixTransformation(&v3Out, 0, 0, &m_v3Scale, 0, 0, 0);
		m_matWorld._41 += Position.x;
		m_matWorld._42 += Position.y;
		m_matWorld._43 += Position.z;
		D3DXMatrixMultiply(&m_matWorld, &m_matWorld, &v3Out);
#else
		m_matWorld._41 = Position.x;
		m_matWorld._42 = Position.y;
		m_matWorld._43 = Position.z;
...
#if defined(__BL_BATTLE_ROYALE__)
void CEffectMeshInstance::SetScale(const D3DXVECTOR3& c_rv3Scale)
{
	m_v3Scale = c_rv3Scale;
}
#endif