...
void CParticleSystemInstance::OnSetDataPointer(CEffectElementBase * pElement)
{
	...
	for (DWORD i = 0; i < m_pParticleProperty->m_ImageVector.size(); ++i)
	{
		...
		if(i < m_pParticleProperty->m_ImageVectorScale.size())
		{
			const auto scale = m_pParticleProperty->m_ImageVectorScale[i];
			pkImgInstNew->SetScale(scale.first, scale.second);
		}
