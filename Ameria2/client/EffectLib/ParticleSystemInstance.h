	...
	public:
		...
		inline void ForEachParticleRendering(T & FunObj)
		{
			...
			for(dwFrameIndex=0; dwFrameIndex<m_kVct_pkImgInst.size(); dwFrameIndex++)
			{
				...
				if (dwFrameIndex < m_pParticleProperty->m_ImageVectorScale.size())
				{
					const auto scale = m_pParticleProperty->m_ImageVectorScale[dwFrameIndex];
					CParticleInstance* particle = *itor;
					particle->SetScale(scale.first, scale.second);
				}
