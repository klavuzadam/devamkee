...
void CSlotWindow::DeactivateSlot(DWORD dwIndex)
{
	...
	pSlot->bActive = FALSE;
	SetNewSlotFlashEffect(dwIndex, false);
}
...
void CSlotWindow::RefreshSlot()
{
	...
	// NOTE : Refresh μE¶§ ToolTip μμ °≫˝? C?´?´? - [levites]
	if (IsRendering())
	{
		TSlot* pSlot;
...
{
	// FIXME : ?ⓒμμ??¸| μ?·ˇ±e C?´A μμAß?ˇ SetTop?? μC?i?o¸®¸e Capture°ˇ C®?iA® ?o¸°´?. ±×°I?C ??Ao AUμ?.
	//         A≫ ´? ±??≫???I C?°a???≫ ?Ł???ß C? μi - [levites]
...
void CSlotWindow::SetUnusableSlot(DWORD dwIndex)
{
	...
	SET_BIT(pSlot->dwState, SLOT_STATE_REDSLOT);
}
...
#endif
		if (IS_SET(rSlot.dwState, SLOT_STATE_REDSLOT))
		{
			CPythonGraphic::Instance().SetDiffuseColor(1.0f, 0.0f, 0.0f, 0.3f);
			CPythonGraphic::Instance().RenderBar2d(m_rect.left + rSlot.ixPosition,
				m_rect.top + rSlot.iyPosition,
...
void CSlotWindow::__CreateSlotEnableEffect()
{
	...
	{
		if (i < 10)
			_snprintf(szCount, sizeof(szCount), "d:/ymir work/ui/public/newitemeffect/newitemeffect_00000%d.sub", i);
		else
			_snprintf(szCount, sizeof(szCount), "d:/ymir work/ui/public/newitemeffect/newitemeffect_0000%d.sub", i);

		m_pSlotActiveEffect->AppendImage(szCount);
	}
	...
	for (int i = 24; i < 57; ++i)
	{
		...
		m_pSlotActiveEffectSlot2->AppendImage(szCount);
	}
	...
	for (int i = 57; i < 94; ++i)
	{
		...
		m_pSlotActiveEffectSlot3->AppendImage(szCount);
	}
