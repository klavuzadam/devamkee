...
void CPythonCharacterManager::__RenderSortedAliveActorList()
{
    ...
    const auto& rkMap_pkInstAlive = m_kAliveInstMap;
    for (const auto& [k, v] : rkMap_pkInstAlive)
    {
        s_kVct_pkInstAliveSort.emplace_back(v);
    }

    std::sort(std::execution::par,
                s_kVct_pkInstAliveSort.begin(),
                s_kVct_pkInstAliveSort.end(),
                [](CInstanceBase* pkLeft, CInstanceBase* pkRight)
                {
                    return pkLeft->LessRenderOrder(pkRight);
                }
    );

    std::for_each(s_kVct_pkInstAliveSort.begin(),
                    s_kVct_pkInstAliveSort.end(),
...
void CPythonCharacterManager::__RenderSortedDeadActorList()
{
    ...
    std::for_each(s_kVct_pkInstDeadSort.begin(),
                    s_kVct_pkInstDeadSort.end(),
                    [](CInstanceBase* pInstance)
                    {
                        pInstance->Render();
...
void CPythonCharacterManager::__NEW_Pick()
{
	...
	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	if (CPythonPlayer::Instance().GetAutoUseOnOff())
	{
		if ((rkChrMgr.GetAutoSystemByType(CInstanceBase::AUTO_ONOFF_ATTACK) || rkChrMgr.GetAutoSystemByType(CInstanceBase::AUTO_ONOFF_ATTACK_STONE)))
			return;
	}
#endif
...
{
	#if defined(__BL_BATTLE_ROYALE__)
	if (CPythonBackground::Instance().IsBattleRoyaleMap() && PythonBattleRoyaleManager::Instance().IsPCTextTailHided())
		return;
#endif
	CPythonCharacterManager::CharacterIterator itor = CharacterInstanceBegin();
	CPythonCharacterManager::CharacterIterator itorEnd = CharacterInstanceEnd();
...
#if defined(ENABLE_AUTOMATIC_HUNTING_SYSTEM)
void CPythonCharacterManager::SetAutoOnOff(const bool c_bOnOff
#ifndef USE_PREMIUM_AFFECT
, const bool c_bCommand
#endif
)
{
	m_bAutoHuntOnOff = c_bOnOff;
#ifndef USE_PREMIUM_AFFECT
	m_bAutoHuntCommand = c_bCommand;
#endif
}

void CPythonCharacterManager::SetAutoSystemByType(const uint8_t c_iType, const bool c_bOnOff)
{
	if (c_iType < CInstanceBase::AUTO_ONOFF_MAX)
		m_abAutoHuntType[c_iType] = c_bOnOff;

	if (c_iType == CInstanceBase::AUTO_ONOFF_RANGE)
	{
		CInstanceBase* pMainInst = CPythonPlayer::Instance().NEW_GetMainActorPtr();
		if (pMainInst == nullptr)
			return;

		TPixelPosition kInstPos;
		pMainInst->NEW_GetPixelPosition(&kInstPos);
		SetLastPixelPosition(kInstPos);
	}
}
#endif

#ifdef ENABLE_ENTITY_LOADING
void CPythonCharacterManager::CreateEntity(const DWORD c_dwVID, CInstanceBase::SCreateData& c_rkCreateData)
{
	EntityMap::const_iterator it = m_kEntityMap.find(c_dwVID);
	if (it != m_kEntityMap.end())
		return;

	m_kEntityMap.emplace(c_dwVID, c_rkCreateData);

	CInstanceBase* pCharacterInstance = CInstanceBase::New();
	if (pCharacterInstance != nullptr)
	{
		pCharacterInstance->Create(c_rkCreateData);
		pCharacterInstance->Destroy();
	}
}
#endif
...
#if defined(ENABLE_AUTOMATIC_HUNTING_SYSTEM)
	m_bAutoHuntOnOff = false;
#ifndef USE_PREMIUM_AFFECT
	m_bAutoHuntCommand = false;
#endif
	std::memset(m_abAutoHuntType, false, sizeof(m_abAutoHuntType));
#endif
}
