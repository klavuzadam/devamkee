...
void CPythonItem::Update(const POINT& c_rkPtMouse)
{
    ...
    {
        if (itor->second->ThingInstance.GetBaseThingPtr()->GetMotionCount() > 0)
        {
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, bool bDrop, long alSockets[ITEM_SOCKET_SLOT_MAX_NUM], TPlayerItemAttribute aAttrs[ITEM_ATTRIBUTE_SLOT_MAX_NUM], unsigned short itemCount, int ownerTime)
#else
...
#endif
{
	...
	if (pGroundItemInstance->ThingInstance.GetBaseThingPtr()->GetMotionCount() > 0) {
		pGroundItemInstance->ThingInstance.RegisterMotionThing(pGroundItemInstance->ThingInstance.GetMotionIndex(), 0, pGroundItemInstance->ThingInstance.GetBaseThingPtr());
	...
	switch (pItemData->GetType())
	{
		case CItemData::ITEM_TYPE_POLYMORPH:
		{
			...
			if(itemCount > 1)
				len += snprintf(szItemName, sizeof(szItemName), "%s (x%u)", c_szTmp, itemCount);
			else
		...
		case CItemData::ITEM_TYPE_SKILLFORGET:
		{
			...
			{
				if (itemCount > 1)
					len += snprintf(szItemName, sizeof(szItemName), "%s (x%u)", c_pSkillData->GradeData[0].strName.c_str(), itemCount);
				else
	...
	if (itemCount > 1)
		len += snprintf(szItemName + len, sizeof(szItemName) - len, (len>0)?" %s (x%u)":"%s (x%u)", pItemData->GetName(), itemCount);
	else
	...
	{
		if (bHasAttr)
			rkTextTail.RegisterItemTextTail(dwVirtualID,szItemName, ownerTime, &pGroundItemInstance->ThingInstance,D3DXCOLOR(1.0f, 0.67f, 0.0f, 1.0f));
		else
			rkTextTail.RegisterItemTextTail(dwVirtualID,szItemName, ownerTime, &pGroundItemInstance->ThingInstance);
	}
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
	rkTextTail.SetItemTextTailOwner(dwVID, c_pszName, true);
#else
...
bool CPythonItem::GetCloseMoney(const TPixelPosition & c_rPixelPosition, DWORD * pdwItemID, DWORD dwDistance)
{
	...
	DWORD dwCloseItemDistance = 3000 * 2000;
...
bool CPythonItem::GetCloseItem(const std::string& myName, const TPixelPosition & c_rPixelPosition, DWORD * pdwItemID, DWORD dwDistance)
{
	...
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
		...
		if (dwxDistance*dwxDistance + dwyDistance*dwyDistance < dwCloseItemDistance  && (pInstance->stOwnership == "" || pInstance->stOwnership == myName || CPythonPlayer::Instance().IsPartyMemberByName(pInstance->stOwnership.c_str())))
		{
			dwCloseItemID = i->first;
			dwCloseItemDistance = dwDistance;
		}
...
bool CPythonItem::GetCloseItemVector(const std::string& myName, const TPixelPosition& c_rPixelPosition, std::vector<DWORD>& itemVIDVector)
{
	DWORD dwCloseItemDistance = 2000 * 3000;
	TGroundItemInstanceMap::iterator i;
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
		TGroundItemInstance * pInstance = i->second;
		...
		DWORD dwyDistance = DWORD(c_rPixelPosition.y - (-pInstance->v3EndPosition.y));
		DWORD dwDistance = DWORD(dwxDistance * dwxDistance + dwyDistance * dwyDistance);
		if (dwDistance < dwCloseItemDistance && (pInstance->stOwnership == "" || pInstance->stOwnership == myName || CPythonPlayer::Instance().IsPartyMemberByName(pInstance->stOwnership.c_str())))
		{
			itemVIDVector.push_back(i->first);
		}
	}
	return true;
}
