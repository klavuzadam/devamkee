...
void CPythonNetworkStream::SetSelectPhase()
{
	...
	{
		PyCallClassMemberFunc(m_poHandler, "SetLoadingPhase", Py_BuildValue("(i)", __DirectEnterMode_IsWarp()));
	}
	else
	{
		if (IsSelectedEmpire())
...
bool CPythonNetworkStream::SendChangeCharacterHairPacket(BYTE index, int hairIndex)
{
	TPacketCGChangeCharacterHair packet{};
	packet.header = HEADER_CG_PLAYER_HAIR_CHANGE;
	packet.index = index;
	packet.hairIndex = hairIndex;

	if (!Send(sizeof(TPacketCGChangeCharacterHair), &packet))
	{
		Tracen("SendChangeCharacterHairPacket");
		return false;
	}
