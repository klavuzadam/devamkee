...
BOOL CInstanceBase::CheckAdvancing()
{
	...
	if (!__IsMainInstance() && !IsAttacking())
	{
		if (IsPC() && IsWalking())
		{
			...
			for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
			{
				...
				if (pkInstEach->IsBuffi() || IsBuffi())
				{
					if (m_GraphicThingInstance.TestActorCollision(pkInstEach->GetGraphicThingInstanceRef(), pkInstEach->__EffectContainer_IsEffect(EFFECT_TARGET), true))
					{
						BlockMovement();
						return true;
					}
				...
				{ 
					if (m_GraphicThingInstance.TestActorCollision(pkInstEach->GetGraphicThingInstanceRef(), pkInstEach->__EffectContainer_IsEffect(EFFECT_TARGET)))
					{
						BlockMovement();
						return true;
					}
	...
	for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
	{
		...
		if (pkInstEach->IsBuffi() || IsBuffi())
		{
			if (rkActorSelf.TestActorCollision(rkActorEach, pkInstEach->__EffectContainer_IsEffect(EFFECT_TARGET), true))
			{
				uCollisionCount++;
				...
				{
					if (rkActorSelf.TestActorCollision(rkActorEach, pkInstEach->__EffectContainer_IsEffect(EFFECT_TARGET)))
					{
						rkActorSelf.BlockMovement();
						return TRUE;
					}
