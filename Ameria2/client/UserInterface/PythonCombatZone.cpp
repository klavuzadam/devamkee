...
void CPythonCombatZone::SendDataDays(DWORD m_pSendedData[7][8], bool m_vec_info_days_get[7] ,bool bIsOnline)
{
	bIsRunning = bIsOnline;
	memcpy(&m_pDataDays, m_pSendedData, sizeof(m_pDataDays));
	memcpy(&m_pDataDaysCheck, m_vec_info_days_get, sizeof(m_pDataDaysCheck));
}
...
PyObject * pyObjectGetDayActivate(PyObject* poSelf, PyObject* poArgs)
{
	int iDay;
	...
	return Py_BuildValue("b",m_pDataDaysCheck[iDay]);
}
...
void initCombatZoneSystem()
{
	static PyMethodDef s_methods[] = {
		...
		{ "GetDayActivate",							pyObjectGetDayActivate,							METH_VARARGS },
		{ NULL,						NULL,							NULL }
	};
