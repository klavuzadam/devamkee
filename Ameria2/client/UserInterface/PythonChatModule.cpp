...
PyObject* chatSetDelayProcess(PyObject* poSelf, PyObject* poArgs) {
    int iEnable;
	if (!PyTuple_GetInteger(poArgs, 0, &iEnable))
		return Py_BuildException();

    CPythonChat::Instance().SetDelayProcess(iEnable);
	return Py_BuildNone();
}
...
void initChat()
{
	static PyMethodDef s_methods[] =
	{
		...
		// Chat
		{ "SetDelayProcess",				chatSetDelayProcess,				METH_VARARGS },
