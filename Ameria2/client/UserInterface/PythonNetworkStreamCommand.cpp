...
#if defined(ENABLE_AUTOMATIC_HUNTING_SYSTEM)
	else if (!strcmpi(szCmd, "auto_onoff"))
	{
		...
		const bool c_bOnOff = static_cast<bool>(atoi(TokenVector[1].c_str()));
		CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
		rkPlayer.SetAutoUseOnOff(c_bOnOff);

#ifdef USE_PREMIUM_AFFECT
		CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
		rkChrMgr.SetAutoOnOff(c_bOnOff);
#endif
	}
#endif
	// GIFT NOTIFY
