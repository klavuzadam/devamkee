...
namespace ikashop
{
	...
	void ShopInstance::LoadShopSkinSettings()
	{
		...
		if(!CEterPackManager::instance().Get(file, skinSettings, &data, __FUNCTION__))
		{
	...
	{
		if(s_SkinMap.find(index) != s_SkinMap.end())
		{
	...
	void ShopInstance::Show(float x, float y, float z) {
		...
		if (!animation.empty()) {
			m_GraphicThing.RegisterMotionThing(
				0, 0, (CGraphicThing*)CResourceManager::Instance().GetResourcePointer(animation.c_str()));
...
CPythonIkarusShop::CPythonIkarusShop()
{
	...
	m_bAscendingPrices = false;
#endif
    m_iShopSearchCurrPage = 1;
    m_iShopSearchMaxxPage = 1;
...
PyObject* CPythonIkarusShop::GetSearchShopBoard()
{
	return m_searchShopBoard;
}
...
, const ikashop::TValutesInfo& valute
#endif
)
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
	CallOnBusinessBoard("OpenShopOwner", Py_BuildValue("(OK)", shopDict.weakRef(), valute.yang));
#else
...
{
#ifdef USE_IKASHOP_UI_RENEWED
	CallOnBusinessBoard("OpenShopOwnerEmpty", Py_BuildValue("(K)", valute.yang));
#else
...
{
#ifdef USE_OFFLINESHOP_SEARCH_V2
    m_vecShopSearchItem.assign(vec.begin(), vec.end());

#ifdef USE_PREMIUM_AFFECT
	if (!m_vecShopSearchItem.empty())
	{
		if (m_bAscendingPrices)
		{
			std::sort(m_vecShopSearchItem.begin(), m_vecShopSearchItem.end(),
						[](ikashop::TSearchResultInfo a, ikashop::TSearchResultInfo b)
						{
							return (a.price.GetTotalYangAmount() / b.count) < (b.price.GetTotalYangAmount() / b.count);
						}
			);
		}
		else
		{
			...
			std::shuffle(m_vecShopSearchItem.begin(), m_vecShopSearchItem.end(), std::default_random_engine(seed));
		}
	}
#endif
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
void CPythonIkarusShop::SafeboxRefreshValutes(const ikashop::TValutesInfo& valute)
{
	CallOnBusinessBoard("RefreshSafeboxValutes", Py_BuildValue("(K)", valute.yang));
}
#endif
...
#ifdef USE_IKASHOP_UI_RENEWED
#ifdef EXTEND_IKASHOP_PRO
	CallOnBusinessBoard("SetNotificationCounterFull", Py_BuildValue("(i)", safebox + auction + offers + notifications));
#else
	CallOnBusinessBoard("SetNotificationCounterFull", Py_BuildValue("(i)", safebox + auction + offers));
#endif
#endif
}
...
{
#ifdef EXTEND_IKASHOP_PRO
    const auto settingDistance = CPythonSystem::instance().GetShopRangeView();
	...
	{
#ifdef EXTEND_IKASHOP_PRO
		if (!mainInstance || mainInstance->GetDistance(iter->GetThingInstancePtr()->GetPosition()) > settingDistance) {
			iter->HideAllEffects();
			continue;
		}
#endif
...
#ifdef EXTEND_IKASHOP_PRO
			if (!mainInstance || mainInstance->GetDistance(instance->GetThingInstancePtr()->GetPosition()) > settingDistance)
				continue;
#endif
			instance->RenderText();
		}
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
#ifdef USE_PREMIUM_AFFECT
bool CPythonIkarusShop::ChangeAscendingOrder(const bool bFlag)
{
	if (m_vecShopSearchItem.empty())
	{
		return false;
	...
	if (bFlag && CPythonPlayer::Instance().GetStatus(POINT_PREMIUM_USER) < 1)
	{
		return false;
	}

	m_bAscendingPrices = bFlag;
	...
	CPythonNetworkStream::Instance().RefreshShopSearchWindow();
	return true;
}
#endif

void CPythonIkarusShop::ClearShopSearchItem()
{
    m_vecShopSearchItem.clear();
}

void CPythonIkarusShop::SetShopSearchItemPageData(const int32_t curr, const int32_t maxx)
{
    m_iShopSearchCurrPage = curr;
    m_iShopSearchMaxxPage = maxx;
}

size_t CPythonIkarusShop::GetShopSearchItemCount() const
{
    return m_vecShopSearchItem.size();
}

int32_t CPythonIkarusShop::GetShopSearchPageCurr() const
{
    return m_iShopSearchCurrPage;
}

int32_t CPythonIkarusShop::GetShopSearchPageMaxx() const
{
    return m_iShopSearchMaxxPage;
}

bool CPythonIkarusShop::GetShopSearchItemData(uint32_t index, const ikashop::TSearchResultInfo** c_pItemData)
{
    try
    {
        *c_pItemData = &m_vecShopSearchItem.at(index);
        return true;
    }
...
PyObject* ikashopGetSearchShopBoard(PyObject* poSelf , PyObject* poArgs)
{
	PyObject* poInterface = CPythonIkarusShop::instance().GetSearchShopBoard();
...
#ifdef USE_IKASHOP_UI_RENEWED
PyObject* ikashopSendTeleportToShopEntity(PyObject*/* poSelf*/, PyObject* poArgs)
{
	unsigned int dwPID;
	if (!PyTuple_GetUnsignedInteger(poArgs, 0, &dwPID))
	{
		return Py_BuildException();
	}

	CPythonNetworkStream::instance().SendTeleportToShopEntity(static_cast<DWORD>(dwPID));
	return Py_BuildNone();
}
#endif
#endif
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
PyObject* ikashopSendShopSearchOpen(PyObject*/* poSelf*/, PyObject* poArgs)
{
    ikashop::TFilterInfo tmp = {};
    if (!PyTuple_GetInteger(poArgs, 0, &tmp.type))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetInteger(poArgs, 1, &tmp.subtype))
    {
        return Py_BuildException();
    }

    char* name = nullptr;
    if (!PyTuple_GetString(poArgs, 2, &name))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetBoolean(poArgs, 3, &tmp.exact))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetUnsignedInteger(poArgs, 4, &tmp.vnum))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetBoolean(poArgs, 5, &tmp.player))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetUnsignedInteger(poArgs, 6, &tmp.pid))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetUnsignedInteger(poArgs, 7, &tmp.antiflags))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetLongLong(poArgs, 8, &tmp.pricestart.yang))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetLongLong(poArgs, 9, &tmp.pricend.yang))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetInteger(poArgs, 10, &tmp.levelstart))
    {
        return Py_BuildException();
    }

    if (!PyTuple_GetInteger(poArgs, 11, &tmp.levelend))
    {
        return Py_BuildException();
    }
    ...
    netMgr.SendShopSearchOpen(tmp);
    return Py_BuildNone();
}

PyObject* ikashopSendShopSearchChangePage(PyObject*/* poSelf*/, PyObject* poArgs)
{
    int32_t pageIdx = 0;
    if (!PyTuple_GetInteger(poArgs, 0, &pageIdx))
    {
        return Py_BuildException();
    }

    auto& netMgr = CPythonNetworkStream::instance();
    netMgr.SendShopSearchChangePage(pageIdx);
    return Py_BuildNone();
}

PyObject* ikashopSendShopSearchBuyItem(PyObject*/* poSelf*/, PyObject* poArgs)
{
    PyObject* poTupleItems;
    if (!PyTuple_GetObject(poArgs, 0, &poTupleItems))
    {
        return Py_BuildException();
    }

    const size_t count = PyTuple_Size(poTupleItems);
    if (count == 0)
    {
        return Py_BuildNone();
    }
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (!PyTuple_GetObject(poTupleItems, i, &poItemInfo))
        {
            return Py_BuildException();
        }
        ...
        uint32_t ownerid = 0;
        if (!PyTuple_GetUnsignedInteger(poItemInfo, 0, &ownerid))
        {
            return Py_BuildException();
        }

        uint32_t itemid = 0;
        if (!PyTuple_GetUnsignedInteger(poItemInfo, 1, &itemid))
        {
            return Py_BuildException();
        }

        long long seenprice = 0;
        if (!PyTuple_GetLongLong(poItemInfo, 2, &seenprice))
        {
            return Py_BuildException();
        }
        ...
        tmp.ownerid = ownerid;
        tmp.itemid = itemid;
        tmp.seenprice = seenprice;

        vecItems.push_back(tmp);
    }

    CPythonNetworkStream::Instance().SendShopSearchBuyItem(vecItems);
    return Py_BuildNone();
}

PyObject* ikashopGetShopSearchItemCount(PyObject*/* poSelf*/, PyObject* poArgs)
{
    auto& offshopMgr = CPythonIkarusShop::Instance();
    return Py_BuildValue("i", offshopMgr.GetShopSearchItemCount());
}

PyObject* ikashopGetShopSearchPageData(PyObject*/* poSelf*/, PyObject* poArgs)
{
    auto& offshopMgr = CPythonIkarusShop::Instance();
    return Py_BuildValue("ii", offshopMgr.GetShopSearchPageCurr(), offshopMgr.GetShopSearchPageMaxx());
}

PyObject* ikashopGetShopSearchItemData(PyObject*/* poSelf*/, PyObject* poArgs)
{
    uint32_t index = 0;
    if (!PyTuple_GetUnsignedInteger(poArgs, 0, &index))
    {
        return Py_BuildException();
    }
    ...
    const ikashop::TSearchResultInfo* c_pItemData;
    if (!offshopMgr.GetShopSearchItemData(index, &c_pItemData))
    {
        return Py_BuildValue("iiii", 0, 0, 0, 0);
    }

    return Py_BuildValue("iiii", c_pItemData->owner, c_pItemData->id, c_pItemData->vnum, c_pItemData->count);
}

PyObject* ikashopGetShopSearchItemPrice(PyObject*/* poSelf*/, PyObject* poArgs)
{
    uint32_t index = 0;
    if (!PyTuple_GetUnsignedInteger(poArgs, 0, &index))
    {
...
#ifdef ENABLE_CHEQUE_SYSTEM
    return Py_BuildValue("KiK", c_pItemData->price.yang, c_pItemData->price.cheque, c_pItemData->priceavg);
#else
    return Py_BuildValue("KK", c_pItemData->price.yang, c_pItemData->priceavg);
#endif
}

PyObject* ikashopGetShopSearchItemSocket(PyObject*/* poSelf*/, PyObject* poArgs)
{
    uint32_t index = 0;
    if (!PyTuple_GetUnsignedInteger(poArgs, 0, &index))
    {
    ...
    uint8_t i = 0;
    if (!PyTuple_GetByte(poArgs, 1, &i) || i >= ITEM_SOCKET_SLOT_MAX_NUM)
    {
        return Py_BuildException();
    }
    ...
    return Py_BuildValue("i", c_pItemData->alSockets[i]);
}

PyObject* ikashopGetShopSearchItemAttr(PyObject*/* poSelf*/, PyObject* poArgs)
{
    uint32_t index = 0;
    if (!PyTuple_GetUnsignedInteger(poArgs, 0, &index))
    {
    ...
    uint8_t i = 0;
    if (!PyTuple_GetByte(poArgs, 1, &i) || i >= ITEM_ATTRIBUTE_SLOT_MAX_NUM)
    {
        return Py_BuildException();
    }
    ...
    return Py_BuildValue("ii", c_pItemData->aAttr[i].bType, c_pItemData->aAttr[i].sValue);
}

PyObject* ikashopGetShopSearchSellerDetails(PyObject*/* poSelf*/, PyObject* poArgs)
{
    uint32_t index = 0;
    if (!PyTuple_GetUnsignedInteger(poArgs, 0, &index))
    {
    ...
    if (!offshopMgr.GetShopSearchItemData(index, &c_pItemData))
    {
        return Py_BuildValue("sib", "", 0, false);
    }

    return Py_BuildValue("sib", c_pItemData->seller_name, c_pItemData->owner, c_pItemData->auction);
}

#ifdef USE_PREMIUM_AFFECT
PyObject* ikashopChangeAscendingOrder(PyObject*/* poSelf*/, PyObject* poArgs)
{
	bool bisAscendingOrder = false;
	if (!PyTuple_GetBoolean(poArgs, 0, &bisAscendingOrder))
	{
		return Py_BuildException();
	}

	auto& offshopMgr = CPythonIkarusShop::Instance();
	return Py_BuildValue("b", offshopMgr.ChangeAscendingOrder(bisAscendingOrder));
}
#endif
#endif
...
void initikashop()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetSearchShopBoard", ikashopGetSearchShopBoard, METH_VARARGS },
...
#endif
#ifdef USE_IKASHOP_UI_RENEWED
		{ "SendTeleportToShopEntity", ikashopSendTeleportToShopEntity, METH_VARARGS },
#endif
#endif
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
        { "SendShopSearchOpen", ikashopSendShopSearchOpen, METH_VARARGS },
        { "SendShopSearchChangePage", ikashopSendShopSearchChangePage, METH_VARARGS },
        { "SendShopSearchBuyItem", ikashopSendShopSearchBuyItem, METH_VARARGS },
        { "GetShopSearchItemCount", ikashopGetShopSearchItemCount, METH_VARARGS },
        { "GetShopSearchPageData", ikashopGetShopSearchPageData, METH_VARARGS },
        { "GetShopSearchItemData", ikashopGetShopSearchItemData, METH_VARARGS },
        { "GetShopSearchItemPrice", ikashopGetShopSearchItemPrice, METH_VARARGS },
        { "GetShopSearchItemSocket", ikashopGetShopSearchItemSocket, METH_VARARGS },
        { "GetShopSearchItemAttr", ikashopGetShopSearchItemAttr, METH_VARARGS },
        { "GetShopSearchSellerDetails", ikashopGetShopSearchSellerDetails, METH_VARARGS },
#ifdef USE_PREMIUM_AFFECT
        { "ChangeAscendingOrder", ikashopChangeAscendingOrder, METH_VARARGS },
#endif
#endif
		{ NULL, NULL, NULL },
	};
...
#endif
#ifdef USE_OFFLINESHOP_SEARCH_V2
    PyModule_AddIntConstant(poModule, "OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE", OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE);
#endif
#endif
}
#endif
