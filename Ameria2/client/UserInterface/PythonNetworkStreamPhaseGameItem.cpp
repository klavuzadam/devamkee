...
bool CPythonNetworkStream::RecvSafeBoxSetPacket()
{
    ...
    if (!Recv(sizeof(kItemSet), &kItemSet))
		return false;
	TraceError("CheckNetworkProblem: Step 0 Size: %d", sizeof(kItemSet));
	...
	strcpy(kItemData.name, kItemSet.name);
#endif
...
bool CPythonNetworkStream::RecvItemUpdatePacket(bool bJustCount/* = false*/)
#else
...
#endif
{
    ...
    if (bJustCount)
    {
        ...
        if (!Recv(sizeof(TPacketGCItemUpdateCount), &p2))
        {
            return false;
        }

        auto& playerMgr2 = IAbstractPlayer::GetSingleton();
        playerMgr2.SetItemCount(p2.pos, p2.count);
    ...
    if (!Recv(sizeof(TPacketGCItemUpdate), &p))
    {
        return false;
    }

    auto& playerMgr = IAbstractPlayer::GetSingleton();
    playerMgr.SetItemCount(p.Cell, p.count);

#ifdef ENABLE_NEW_NAME_ITEM
    playerMgr.SetItemNewName(p.Cell, p.name);
#endif

    for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
    {
        playerMgr.SetItemMetinSocket(p.Cell, i, p.alSockets[i]);
    }

    for (int j = 0; j < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++j)
    {
        playerMgr.SetItemAttribute(p.Cell, j, p.aAttr[j].bType, p.aAttr[j].sValue);
    }
#ifdef ENABLE_YOHARA_SYSTEM
    for (int k = 0; k < APPLY_RANDOM_SLOT_MAX_NUM; ++k)
    {
        playerMgr.SetSungmaAttribute(p.Cell, k, p.aApplyRandom[k].bType, p.aApplyRandom[k].sValue);
    }
#endif

#ifdef ENABLE_SOULBIND_SYSTEM
    playerMgr.SetItemBind(p.Cell, p.bind);
#endif
...
bool CPythonNetworkStream::RecvItemGroundAddPacket()
{
	...
	CPythonItem::Instance().CreateItem(packet_item_ground_add.dwVID,
									   ...
									   true,
									   packet_item_ground_add.alSockets,
									   packet_item_ground_add.aAttrs
									,packet_item_ground_add.itemCount
									,packet_item_ground_add.ownerTime+time(0));
#else
	CPythonItem::Instance().CreateItem(packet_item_ground_add.dwVID, 
									   packet_item_ground_add.dwVnum,
									   packet_item_ground_add.lX,
...
bool CPythonNetworkStream::SendShopSellPacketNew(BYTE byInv, WORD wSlot, DWORD byCount)
#else
bool CPythonNetworkStream::SendShopSellPacketNew(WORD wSlot, DWORD byCount)
#endif
...
bool CPythonNetworkStream::SendQuickSlotAddPacket(BYTE bPos, BYTE type, WORD wPos)
{
	if (!__CanActMainInstance())
		return true;
	...
	quickSlotAddPacket.pos			= bPos;
	quickSlotAddPacket.slot.Type	= type;
	quickSlotAddPacket.slot.Position = wPos;
...
bool CPythonNetworkStream::RecvSpecialEffect()
{
	...
	switch (kSpecialEffect.type)
	{
		...
		case SE_VOICE_CHAT:
			effect = CInstanceBase::EFFECT_VOICE_CHAT;
			break;
#endif
...
bool CPythonNetworkStream::RecvCalendarSystem()
{
	TPacketGCCalendarReceive KCalendar;

	if (!Recv(sizeof(KCalendar), &KCalendar))
		return false;

	switch (KCalendar.bSubType)
	{
		case CALENDAR_RECEIVE_OPEN:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_OPEN", Py_BuildValue("(i)", KCalendar.dwCurrentTimeStamp));
			break;

		case CALENDAR_RECEIVE_LOAD_EVENTS:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_LOAD_EVENTS", Py_BuildValue("(issssi)", KCalendar.bDay, KCalendar.szName, KCalendar.szImage, KCalendar.szStartAt, KCalendar.szEndAt, KCalendar.dwDuration));
			//PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_LOAD_EVENTS", Py_BuildValue("(isssssss)", KCalendar.day,KCalendar.event,KCalendar.event_descript,KCalendar.banner,KCalendar.hora_inicio,KCalendar.hora_final,KCalendar.hora_inicio_2,KCalendar.hora_final_2));
			break;

		case CALENDAR_RECEIVE_CLEAR_EVENTS:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_CLEAR_EVENTS", Py_BuildValue("()"));
			break;

		case CALENDAR_RECEIVE_LOADING_EVENTS:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_LOADING_EVENTS", Py_BuildValue("()"));
			break;
	}
...
bool CPythonNetworkStream::RecvCalendarSystemADM()
{
	TPacketGCCalendarReceiveADM KCalendar;
	...
	switch (KCalendar.bSubType)
	{
		case CALENDAR_RECEIVE_OPEN_ADM:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_OPEN_ADM", Py_BuildValue("()"));
			break;

		case CALENDAR_RECEIVE_LOAD_EVENTS_ADM:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_LOAD_EVENTS_ADM", Py_BuildValue("(iss)", KCalendar.index_event,KCalendar.event,KCalendar.event_descript));
			break;

		case CALENDAR_RECEIVE_CLEAR_EVENTS_ADM:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_CLEAR_EVENTS_ADM", Py_BuildValue("()"));
			break;

		case CALENDAR_RECEIVE_LOADING_EVENTS_ADM:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_CALENDAR_LOADING_EVENTS_ADM", Py_BuildValue("()"));
			break;
	}
