...
#define SWM_WEBSITE	WM_APP + 2
#define SWM_DISCORD	WM_APP + 3
#define SWM_FORUM	WM_APP + 4
...
void ShowContextMenu(HWND hWnd)
{
	...
	if (hMenu)
	{
		InsertMenu(hMenu, -1, MF_BYPOSITION, SWM_WEBSITE, "Website");
		InsertMenu(hMenu, -1, MF_BYPOSITION, SWM_DISCORD, "Discord");
		InsertMenu(hMenu, -1, MF_BYPOSITION, SWM_FORUM, "Forum");
		InsertMenu(hMenu, -1, MF_BYPOSITION, SWM_ABOUT, "Minimize");
...
*/
	...
	case WM_APP + 1:
		...
		case 0x0405: // 1029
		{
			CPythonNetworkStream& networkMgr = CPythonNetworkStream::Instance();
			if (networkMgr.GetPhaseName() == "Game")
			{
				if (!IsVisible())
				{
					m_isMinimizedWnd = false;
				...
				char cmd[15];
				strcpy(cmd, "message_");
				networkMgr.ServerCommand(cmd);
			}
			else
				ShellExecute(NULL, "open", "https://discord.gg/S5dknXqSv6", NULL, NULL, SW_SHOWNORMAL);
		}
		break;
		}
		break;
	case WM_COMMAND:
		...
		case SWM_EXIT:
			...
			break;
		case SWM_WEBSITE:
			ShellExecute(NULL, "open", "https://www.solana2.global/", NULL, NULL, SW_SHOWNORMAL);
			break;
		case SWM_DISCORD:
			ShellExecute(NULL, "open", "https://discord.gg/S5dknXqSv6", NULL, NULL, SW_SHOWNORMAL);
			break;
