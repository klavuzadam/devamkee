...
PyObject* chrmgrSetPathName (PyObject* poSelf, PyObject* poArgs)
{
	...
	//if (!strstr(szPathName, XOR("d:/ymir"))) 
	//{
...
{
	bool bOnOff = false, bCommand = false;
	if (!PyTuple_GetBoolean(poArgs, 0, &bOnOff))
		return Py_BadArgument();

	if (PyTuple_Size(poArgs) == 2)
	{
		if (!PyTuple_GetBoolean(poArgs, 1, &bCommand))
			return Py_BadArgument();
	}

	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	rkChrMgr.SetAutoOnOff(bOnOff, bCommand);
	return Py_BuildNone();
}
#endif
...
{
	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	return Py_BuildValue("i", rkChrMgr.GetAutoOnOff());
}

PyObject* chrmgrSetAutoSystemByType(PyObject* poSelf, PyObject* poArgs)
{
	uint8_t iType;
	...
	bool bOnOff;
	if (!PyTuple_GetBoolean(poArgs, 1, &bOnOff))
		return Py_BadArgument();

	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	rkChrMgr.SetAutoSystemByType(iType, bOnOff);
	return Py_BuildNone();
}

PyObject* chrmgrGetAutoSystemByType(PyObject* poSelf, PyObject* poArgs)
{
	uint8_t iType;
	if (!PyTuple_GetInteger(poArgs, 0, &iType))
		return Py_BadArgument();
	...
	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	return Py_BuildValue("b", rkChrMgr.GetAutoSystemByType(iType));
}
#endif
...
void initchrmgr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SetAutoSystemByType",		chrmgrSetAutoSystemByType,				METH_VARARGS },
		{ "GetAutoSystemByType",		chrmgrGetAutoSystemByType,				METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "AFFECT_BATTLE_ROYALE_SLOW_1", CInstanceBase::AFFECT_BATTLE_ROYALE_SLOW_1);
	PyModule_AddIntConstant(poModule, "AFFECT_BATTLE_ROYALE_SLOW_2", CInstanceBase::AFFECT_BATTLE_ROYALE_SLOW_2);
	PyModule_AddIntConstant(poModule, "AFFECT_BATTLE_ROYALE_SLOW_3", CInstanceBase::AFFECT_BATTLE_ROYALE_SLOW_3);
#endif
...
#if defined(ENABLE_AUTOMATIC_HUNTING_SYSTEM)
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_START", CInstanceBase::AUTO_ONOFF_START);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_ATTACK", CInstanceBase::AUTO_ONOFF_ATTACK);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_RANGE", CInstanceBase::AUTO_ONOFF_RANGE);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_POTION", CInstanceBase::AUTO_ONOFF_POTION);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_SKILL", CInstanceBase::AUTO_ONOFF_SKILL);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_RESTART", CInstanceBase::AUTO_ONOFF_RESTART);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_ATTACK_STONE", CInstanceBase::AUTO_ONOFF_ATTACK_STONE);
	PyModule_AddIntConstant(poModule, "AUTO_ONOFF_MAX", CInstanceBase::AUTO_ONOFF_MAX);
#endif
