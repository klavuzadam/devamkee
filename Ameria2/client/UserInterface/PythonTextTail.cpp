		...
		if (pLang)
		{
			...
			if (pLevel)
			{
				...
				if (pTitle)
				{
					int iTitleWidth, iTitleHeight;
					pTitle->GetTextSize (&iTitleWidth, &iTitleHeight);
					pLang->SetPosition (pTextTail->x - (iNameWidth / 2) - 8.0f - iTitleWidth - iLevelWidth - pLang->GetWidth() - 1.0f, pTextTail->y - 10);
				}
				if (!pTitle)
				{
					pLang->SetPosition (pTextTail->x - (iNameWidth / 2) - 4.0f - iLevelWidth - pLang->GetWidth() - 1.0f, pTextTail->y - 10);
				}
			}
		}
#endif
...
void CPythonTextTail::Render()
{
	...
	bool hideFlag = CPythonSystem::Instance().GetCountryFlag();
#endif
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
		int iLeftTime = MINMAX(0, pTextTail->ownerTime-time(0), INT_MAX);
		char newItemName[96];
		snprintf(newItemName, sizeof(newItemName), "|cffFFD5A1(%dm %ds)|r |cff%s%s|r", ((iLeftTime / 60) % 60), (iLeftTime % 60), pTextTail->isMineItem ? "FFC800" : "FE5148", pTextTail->ownerName.c_str());
		SetItemTextTailOwner(pTextTail->dwVirtualID, newItemName);
#endif
...
void CPythonTextTail::ShowAllTextTail()
{
	...
	for (itor = m_ChatTailMap.begin(); itor != m_ChatTailMap.end(); ++itor)
	{
...
void CPythonTextTail::ShowCharacterTextTail(DWORD VirtualID)
{
	...
	if (!pInstance->GetRenderStatus())
		return;
	if (pInstance->IsNPC()/* && !pInstance->IsShop()*/ && !CPythonSystem::Instance().GetShopNameStatus())
		return;
#endif
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	if (pCharacterInstance->IsCombatZoneMap() && pCharacterInstance != pkInstMain && !pkInstMain->IsGameMaster())
		return;
#endif
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
void CPythonTextTail::RegisterItemTextTail(DWORD VirtualID, const char * c_szText, int ownerTime, CGraphicObjectInstance * pOwner, const D3DXCOLOR itemColor)
#else
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
	TTextTail * pTextTail = RegisterTextTail(VirtualID, c_szText, pOwner, c_TextTail_Name_Position, itemColor);
	pTextTail->ownerTime = ownerTime;
	pTextTail->ownerName = "";
	pTextTail->isMineItem = false;
#else
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
void CPythonTextTail::SetItemTextTailOwner(DWORD dwVID, const char * c_szName, bool isRealName)
#else
...
#endif
{
...
#ifndef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
...
#endif
	{
		...
		if (isRealName)
		{
			pTextTail->ownerName = c_szName;
			CInstanceBase* pInstance = CPythonCharacterManager::Instance().GetMainInstancePtr();
			if (pInstance)
			{
				if (!strcmp(pInstance->GetNameString(), c_szName))
					pTextTail->isMineItem = true;
				else
					pTextTail->isMineItem = false;
			}
		}
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
		pTextTail->pOwnerTextInstance->SetColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
#else
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
		pTextTail->yEnd = float(yOwnerSize + 4)*2;
#else
