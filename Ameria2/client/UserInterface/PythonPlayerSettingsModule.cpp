        ...
        if (info.bJobIndex == bJobIndex)
        {
            for (const auto & motion : info.vecMotions)
            {
                ...
                if (bJobIndex == NRaceData::JOB_WARRIOR
                    || bJobIndex == NRaceData::JOB_ASSASSIN
                )
                {
...
auto CPythonPlayerSettingsModule::__LoadGameNPC() -> void
{
    ...
    if (!pack_manager.Get(file, NPC_LIST_FILE_NAME, &pvData, __FUNCTION__))
    {
...
auto CPythonPlayerSettingsModule::IsVisibleInstance(uint32_t dwVID, const uint8_t bType, const uint32_t dwVnum, const uint32_t dwDistance) -> bool
{
    static constexpr auto SHOP_VIEW_DISTANCE = 1200;
    static constexpr auto PET_VIEW_DISTANCE = 2500;
    static constexpr auto MOUNT_VIEW_DISTANCE = 2500;

    if ((dwVnum >= 30000 && dwVnum <= 30009) && dwDistance > SHOP_VIEW_DISTANCE)
    {
        return true;
    }

    // if ((bType == CActorInstance::TYPE_PET_PAY || bType == CActorInstance::TYPE_PET) && dwDistance > PET_VIEW_DISTANCE)
    //     return true;
    //
    // if ((bType == CActorInstance::TYPE_HORSE || (dwVnum >= 20029 && dwVnum <= 21220)) && dwDistance > MOUNT_VIEW_DISTANCE)
    //     return true;
...
auto appRegisterSkill(PyObject* poSelf, PyObject* poArgs)
{
    ...
    if (!PyTuple_GetByte(poArgs, 0, &bRaceIndex)
        || !PyTuple_GetByte(poArgs, 1, &bSkillGroupIndex)
        || !PyTuple_GetByte(poArgs, 2, &bEmpireIndex))
    {
	...
	for(BYTE j=0;j<4;++j)
		CPythonPlayer::Instance().SetSkill(16 + j, 164 + j);
#endif
