...
bool CPythonNetworkStream::RecvOfflineshopPacket()
{
	...
	switch (pack.subheader)
	{
		...
		case ikashop::SUBHEADER_GC_SHOP_SAFEBOX_REFRESH_VALUTES:
			return RecvOfflineshopShopSafeboxRefreshValutes();
#endif
        ...
        case ikashop::SUBHEADER_GC_SHOP_SEARCH_RESULT:
        {
            return RecvShopSearchResult();
        }
#endif
		default:
...
#ifdef USE_IKASHOP_UI_RENEWED
	CPythonIkarusShop::Instance().OpenShopOwner(subpack.shop, items, subpack.valute);
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
	CPythonIkarusShop::instance().OpenShopOwnerNoShop(subpack);
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
bool CPythonNetworkStream::RecvOfflineshopShopSafeboxRefreshValutes()
{
	ikashop::TSubPacketGCShopSafeboxRefreshValutes subpack;
	if (!Recv(sizeof(subpack), &subpack))
	{
		return false;
	}

	CPythonIkarusShop::instance().SafeboxRefreshValutes(subpack.valute);
	return true;
}
#endif
...
#ifdef USE_IKASHOP_UI_RENEWED
void CPythonNetworkStream::SendTeleportToShopEntity(DWORD dwPID)
{
	TPacketCGNewOfflineShop pack;
	pack.header = HEADER_CG_NEW_OFFLINESHOP;
	pack.subheader = ikashop::SUBHEADER_CG_TELEPORT_TO_SHOP_ENTITY;
	pack.size = sizeof(pack) + sizeof(ikashop::TSubPacketCGShopClickEntity);
	...
	if (!Send(sizeof(pack), &pack))
	{
		TraceError("CANNOT SEND OFFLINESHOP PACKET : SUBHEADER %u - pack", pack.subheader);
		return;
	}

	if (!Send(sizeof(subpack), &subpack))
	{
		TraceError("CANNOT SEND OFFLINESHOP PACKET : SUBHEADER %u - subpack ", pack.subheader);
	}
}
#endif
#endif
...
#ifdef USE_OFFLINESHOP_SEARCH_V2
void CPythonNetworkStream::SendShopSearchOpen(const ikashop::TFilterInfo tmp)
{
    if (m_strPhase != "Game")
    {
        return;
    ...
    TPacketCGNewOfflineShop p;
    p.header = HEADER_CG_NEW_OFFLINESHOP;
    p.subheader = ikashop::SUBHEADER_CG_SHOP_SEARCH_OPEN;
    p.size = sizeof(TPacketCGNewOfflineShop) + sizeof(ikashop::TSubPacketCGFilterRequest);

    ikashop::TSubPacketCGFilterRequest p2;
    p2.filter = tmp;

    if (!Send(sizeof(TPacketCGNewOfflineShop), &p))
    {
        TraceError("Cannot send offlineshop packet: subheader %u - p.", p.subheader);
        return;
    }

    if (!Send(sizeof(ikashop::TSubPacketCGFilterRequest), &p2))
    {
        TraceError("Cannot send offlineshop packet: subheader %u - idx.", p.subheader);
        return;
    }
}

void CPythonNetworkStream::SendShopSearchChangePage(const int32_t pageidx)
{
    if (m_strPhase != "Game")
    {
        return;
    ...
    TPacketCGNewOfflineShop p;
    p.header = HEADER_CG_NEW_OFFLINESHOP;
    p.subheader = ikashop::SUBHEADER_CG_SHOP_SEARCH_CHANGE_PAGE;
    p.size = sizeof(TPacketCGNewOfflineShop) + sizeof(int32_t);
    ...
    if (!Send(sizeof(int32_t), &pageidx))
    {
        TraceError("Cannot send offlineshop packet: subheader %u - idx.", p.subheader);
        return;
    }
...
void CPythonNetworkStream::SendShopSearchBuyItem(const std::vector<ikashop::TSubPacketCGShopBuyItem>& vecItems)
{
    if (m_strPhase != "Game")
    {
        return;
    ...
    const size_t count = vecItems.size();
    if (count > OFFLINESHOP_MAX_ITEM_PER_SEARCH_PAGE)
    {
        return;
    }
    ...
    TPacketCGNewOfflineShop p;
    p.header = HEADER_CG_NEW_OFFLINESHOP;
    p.subheader = ikashop::SUBHEADER_CG_SHOP_SEARCH_BUY_ITEM;
    p.size = sizeof(TPacketCGNewOfflineShop) + sizeof(size_t) + (sizeof(ikashop::TSubPacketCGShopBuyItem) * count);
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (!Send(sizeof(ikashop::TSubPacketCGShopBuyItem), &item))
        {
            TraceError("Cannot send offlineshop packet: subheader %u - p.", p.subheader);
            return;
        }
...
{
    ikashop::TGCShopSearchPacket p;
    if(!Recv(sizeof(ikashop::TGCShopSearchPacket), &p))
    {
        TraceError("Cannot recv TGCShopSearchPacket.");
        return false;
    }

    auto& offshopMgr = CPythonIkarusShop::Instance();
    offshopMgr.ClearShopSearchItem();

    if (p.itemcount > 0)
    {
        std::vector<ikashop::TSearchResultInfo> items(static_cast<size_t>(p.itemcount));
        if(!Recv(items.size() * sizeof(items[0]), items.data()))
        {
            TraceError("Cannot recv TSearchResultInfo.");
            return false;
        }
    ...
    offshopMgr.SetShopSearchItemPageData(p.page, p.pagecount);
