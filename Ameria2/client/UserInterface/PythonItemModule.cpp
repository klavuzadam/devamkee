...
PyObject* itemGetPointTypeByApplyType(PyObject*/* poSelf*/, PyObject* poArgs)
{
	BYTE bType = 0;
	...
	return Py_BuildValue("i", ApplyTypeToPointType(bType));
}

PyObject* itemGetApplyTypeByPointType(PyObject*/* poSelf*/, PyObject* poArgs)
{
	BYTE bType = 0;
	if (!PyTuple_GetByte(poArgs, 0, &bType))
	{
	...
	return Py_BuildValue("i", PointTypeToApplyType(bType));
}
...
PyObject* itemGetDropClear(PyObject* poSelf, PyObject* poArgs)
{
	int iItemIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iItemIndex))
		return Py_BadArgument();
	CItemManager::Instance().ClearChest(iItemIndex);
	return Py_BuildNone();
}
PyObject* itemGetDropInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		char cmd[54];
		snprintf(cmd, sizeof(cmd), "/chestdrop %d",iItemIndex);
		CPythonNetworkStream::Instance().SendChatPacket(cmd, CHAT_TYPE_COMMAND);
	}
...
{
    std::string stHint;
    if (!PyTuple_GetString(poArgs, 0, stHint))
    {
        return Py_BuildException();
    }
    ...
    std::string recvHint;
    uint32_t recvVnum;
    if (CItemManager::Instance().GetNameByHint(stHint, recvHint, recvVnum))
    {
        return Py_BuildValue("(si)", recvHint.c_str(), recvVnum);
    }
...
PyObject* itemIsDropAffectBonus(PyObject*/* poSelf*/, PyObject* poArgs)
{
	DWORD dwItemVnum;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &dwItemVnum))
	{
		return Py_BuildException();
	}

	return Py_BuildValue("b", ItIsDropAffectBonus(dwItemVnum));
}
#endif

#ifdef USE_PREMIUM_AFFECT
PyObject* itemIsPremiumUser(PyObject*/* poSelf*/, PyObject* poArgs)
{
	DWORD dwItemVnum;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &dwItemVnum))
	{
	...
	return Py_BuildValue("b", IsPremiumUserItem(dwItemVnum));
}
#endif
...
void initItem()
{
	static PyMethodDef s_methods[] =
	{
...
#endif
		{ "GetPointTypeByApplyType", itemGetPointTypeByApplyType, METH_VARARGS },
		{ "GetApplyTypeByPointType", itemGetApplyTypeByPointType, METH_VARARGS },
		...
		{ "GetDropClear",					itemGetDropClear,						METH_VARARGS },
#endif
#ifdef ENABLE_WIKI
...
#endif
#ifdef USE_DROP_AFFECT_BONUSES
		{ "IsDropAffectBonus", itemIsDropAffectBonus, METH_VARARGS },
#endif
#ifdef USE_PREMIUM_AFFECT
		{ "IsPremiumUser", itemIsPremiumUser, METH_VARARGS },
#endif
		{ NULL,								NULL,									NULL		 },
	};
	...
	PyModule_AddIntConstant(poModule, "USE_PUT_INTO_AURA_SOCKET",	USE_PUT_INTO_AURA_SOCKET);
	...
	PyModule_AddIntConstant(poModule, "EQUIPMENT_OLDPET", c_Equipment_OldPet);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_PET", c_Equipment_Pet);
#endif

	PyModule_AddIntConstant(poModule, "RINGS_SLOT_START",			c_Rings_Slot_Start);
	PyModule_AddIntConstant(poModule, "RINGS_SLOT_COUNT",			c_Rings_Slot_Count);
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_RINGS", 			CItemData::ITEM_TYPE_RINGS);
	
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_SPECIAL_RING", 			CItemData::ITEM_TYPE_SPECIAL_RING);

#ifdef ENABLE_SASH_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "SKIN_SASH_SLOT_START",			c_sash_skin_Slot_Start);
	PyModule_AddIntConstant(poModule, "SKIN_SASH_SLOT_COUNT",			c_sash_sking_Slot_Count);
	PyModule_AddIntConstant(poModule, "COSTUME_TYPE_SKIN_SASH", 		CItemData::COSTUME_NEW_SASH);

#endif

#ifdef ENABLE_MOUNT_SKIN
	PyModule_AddIntConstant(poModule, "COSTUME_TYPE_MOUNT_SKIN", CItemData::COSTUME_MOUNT_SKIN);
	PyModule_AddIntConstant(poModule, "SLOT_MOUNT_SKIN", c_Equipment_MountSkin);
#endif
...
#ifdef ELEMENT_NEW_BONUSES
	PyModule_AddIntConstant(poModule, "APPLY_ENCHANT_ELEC", CItemData::APPLY_ENCHANT_ELEC);
	...
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_BUFFI", CItemData::ITEM_TYPE_BUFFI);
	PyModule_AddIntConstant(poModule, "BUFFI_SCROLL", CItemData::BUFFI_SCROLL);
	PyModule_AddIntConstant(poModule, "BUFFI_NAME_SCROLL", CItemData::BUFFI_NAME_SCROLL);
	
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_BODY", c_Buffi_Slot_Body);
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_HEAD", c_Buffi_Slot_Head);
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_WEAPON", c_Buffi_Slot_Weapon);
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_SASH", c_Buffi_Slot_Sash);
#ifdef __BUFFI_SHINING__
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_SHININGWEAPON", c_Buffi_Slot_ShiningWeapon);
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_SHININGARMOR", c_Buffi_Slot_ShiningArmor);
#endif
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_RING_0", c_Buffi_Slot_Ring0);
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_RING_1", c_Buffi_Slot_Ring1);
	PyModule_AddIntConstant(poModule, "BUFFI_SLOT_RING_2", c_Buffi_Slot_Ring2);
#endif
...
#ifdef ENABLE_YOHARA_SYSTEM
	PyModule_AddIntConstant(poModule, "LIMIT_CONQUEROR_LEVEL", CItemData::LIMIT_CONQUEROR_LEVEL);
#endif
	...
	PyModule_AddIntConstant(poModule, "ANTIFLAG_BIND", ITEM_ANTIFLAG_BIND);
	PyModule_AddIntConstant(poModule, "ITEM_ANTIFLAG_BIND", ITEM_ANTIFLAG_BIND);
#endif
	...
	PyModule_AddIntConstant(poModule, "APPLY_RESIST_HUMAN",	CItemData::APPLY_RESIST_HUMAN);
#endif
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING1", c_Equipment_New_Ring1);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING2", c_Equipment_New_Ring2);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING3", c_Equipment_New_Ring3);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING4", c_Equipment_New_Ring4);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING5", c_Equipment_New_Ring5);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING6", c_Equipment_New_Ring6);
	PyModule_AddIntConstant(poModule, "EQUIPMENT_NEW_RING7", c_Equipment_New_Ring7);
	...
	PyModule_AddIntConstant(poModule, "SPECIAL_ATTR_COSTUME_MOUNT_SKIN_LIMIT", CItemData::SPECIAL_ATTR_COSTUME_MOUNT_SKIN_LIMIT);
	PyModule_AddIntConstant(poModule, "SPECIAL_ATTR_COSTUME_SHINING_LIMIT", CItemData::SPECIAL_ATTR_COSTUME_SHINING_LIMIT);
	PyModule_AddIntConstant(poModule, "SPECIAL_ATTR_COSTUME_SASH_SKIN_LIMIT", CItemData::SPECIAL_ATTR_COSTUME_SASH_SKIN_LIMIT);
#endif
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_NEWRING", CItemData::ITEM_TYPE_NEWRING);
#ifdef USE_OFFLINESHOP_SEARCH_V2
    PyModule_AddIntConstant(poModule, "DS_WHITE", CItemData::DS_SLOT1);
    PyModule_AddIntConstant(poModule, "DS_RED", CItemData::DS_SLOT2);
    PyModule_AddIntConstant(poModule, "DS_GREEN", CItemData::DS_SLOT3);
    PyModule_AddIntConstant(poModule, "DS_BLUE", CItemData::DS_SLOT4);
    PyModule_AddIntConstant(poModule, "DS_YELLOW", CItemData::DS_SLOT5);
    PyModule_AddIntConstant(poModule, "DS_BLACK", CItemData::DS_SLOT6);
#endif
#ifdef USE_BOOST_POTIONS
	PyModule_AddIntConstant(poModule, "USE_SPECIAL_BOOST", USE_SPECIAL_BOOST);
	PyModule_AddIntConstant(poModule, "BOOST_POTION_AFFECT_VALUE", ITEM_BOOST_POTION_AFFECT_VALUE);
	PyModule_AddIntConstant(poModule, "BOOST_POTION_STATUS", ITEM_BOOST_POTION_STATUS);
#endif
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_BOOSTER", CItemData::ITEM_TYPE_BOOSTER);

	PyModule_AddIntConstant(poModule, "ATTRIBUTE_NORM_NUM", ITEM_ATTRIBUTE_NORM_NUM);
	PyModule_AddIntConstant(poModule, "ATTRIBUTE_RARE_NUM", ITEM_ATTRIBUTE_RARE_NUM);

#ifdef USE_CRYSTAL_SYSTEM
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_CRYSTAL", CItemData::ITEM_TYPE_CRYSTAL);

	PyModule_AddIntConstant(poModule, "USE_ADD_ATTR_CRYSTAL", USE_ADD_ATTR_CRYSTAL);
	PyModule_AddIntConstant(poModule, "USE_CHANGE_ATTR_CRYSTAL", USE_CHANGE_ATTR_CRYSTAL);
	PyModule_AddIntConstant(poModule, "USE_RECHARGE_CRYSTAL", USE_RECHARGE_CRYSTAL);

	PyModule_AddIntConstant(poModule, "CRYSTAL_BONUS_TYPE_PVM", CRYSTAL_BONUS_TYPE_PVM);
	PyModule_AddIntConstant(poModule, "CRYSTAL_BONUS_TYPE_PVP", CRYSTAL_BONUS_TYPE_PVP);

	PyModule_AddIntConstant(poModule, "CRYSTAL_BONUS_TYPE_VALUE_SLOT", CRYSTAL_BONUS_TYPE_VALUE_SLOT);

	PyModule_AddIntConstant(poModule, "CRYSTAL_BONUS_TYPE_SOCKET_SLOT", CRYSTAL_BONUS_TYPE_SOCKET_SLOT);

	PyModule_AddIntConstant(poModule, "CRYSTAL_ADD_BONUS_UPGRADE_PRICE", CRYSTAL_ADD_BONUS_UPGRADE_PRICE);
	PyModule_AddIntConstant(poModule, "CRYSTAL_ADD_BONUS_UPGRADE_RESULT_VNUM", CRYSTAL_ADD_BONUS_UPGRADE_RESULT_VNUM);

	PyModule_AddIntConstant(poModule, "CRYSTAL_STATUS_SOCKET_SLOT", CRYSTAL_STATUS_SOCKET_SLOT);

	PyModule_AddIntConstant(poModule, "CRYSTAL_ADD_TIME_SLOT_VALUE_SLOT", CRYSTAL_ADD_TIME_SLOT_VALUE_SLOT);
	PyModule_AddIntConstant(poModule, "CRYSTAL_LEFT_TIME_SOCKET_SLOT", CRYSTAL_LEFT_TIME_SOCKET_SLOT);
#endif
#ifdef USE_DROP_AFFECT_BONUSES
	PyModule_AddIntConstant(poModule, "APPLY_DOUBLE_DROP_METIN", CItemData::APPLY_DOUBLE_DROP_METIN);
	PyModule_AddIntConstant(poModule, "APPLY_DOUBLE_DROP_BOSS", CItemData::APPLY_DOUBLE_DROP_BOSS);

	PyModule_AddIntConstant(poModule, "DOUBLE_DROP_BONUSES_AFFECT_VALUE_SLOT", DOUBLE_DROP_BONUSES_AFFECT_VALUE_SLOT);
	PyModule_AddIntConstant(poModule, "DOUBLE_DROP_BONUSES_TIME_VALUE_SLOT", DOUBLE_DROP_BONUSES_TIME_VALUE_SLOT);
#endif
}
