...
void CPythonPlayer::ClearAffects()
{
	...
	PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "ClearAffects", Py_BuildValue("()"));
}
...
void CPythonPlayer::SetAffect(UINT uAffect)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetAffect", Py_BuildValue("(i)", uAffect));
	/////
...
void CPythonPlayer::ResetAffect(UINT uAffect)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "ResetAffect", Py_BuildValue("(i)", uAffect));
	DWORD dwSkillIndex;
	if (!AffectIndexToSkillIndex(uAffect, &dwSkillIndex))
		return;
...
void CPythonPlayer::ChangeCurrentSkillNumberOnly(DWORD dwSlotIndex)
{
	...
	if (!__IsRightButtonSkillMode())
	{
		if (!__IsTarget())
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NEED_TARGET"));
			return;
		}
	...
	else
	{
		...
		PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "ChangeCurrentSkill", Py_BuildValue("(i)", dwSlotIndex));
	}
}
...
)
{
	...
	if (pkInstMain->IsMountingHorse())
	{
#if defined(ENABLE_AUTOMATIC_HUNTING_SYSTEM) && defined(USE_AUTOMATIC_HUNTING_USE_SKILLS_RENEWED)
		if (bFromAutoHunt
			 && ItIsAllowedSkillFromHorse(rkSkillInst.dwIndex))
		{
			//
		}
		else
		...
		if (!pSkillData->IsHorseSkill())
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_HORSE_SKILL"));
			return false;
		}
#if defined(ENABLE_AUTOMATIC_HUNTING_SYSTEM) && defined(USE_AUTOMATIC_HUNTING_USE_SKILLS_RENEWED)
		}
	...
	if (pSkillData->IsHorseSkill())
	{
		if (!pkInstMain->IsMountingHorse())
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "HAVE_TO_RIDE"));
			return false;
		}
	}
	...
	if (pSkillData->IsAttackSkill())
	{
		if (pkInstMain->IsInSafe())
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "IN_SAFE"));
			return false;
		}
	}
...
//		return false;
	...
	if (pSkillData->IsNeedEmptyBottle())
	{
		if (!__HasItem(27995))
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NEED_EMPTY_BOTTLE"));
			return false;
		}
	}
	...
	if (pSkillData->IsNeedPoisonBottle())
	{
		if (!__HasItem(27996))
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NEED_POISON_BOTTLE"));
			return false;
		}
	}
	...
	if (pkInstMain->IsFishingMode())
	{
		...
		PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "REMOVE_FISHING_ROD"));
		return false;
	}
	...
	if (m_sysIsLevelLimit)
	{
		if (rkSkillInst.iLevel <= 0)
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_YET_LEARN"));
			return false;
		}
	}
	...
	if (!pSkillData->CanUseWeaponType(pkInstMain->GetWeaponType()))
	{
		...
		PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_MATCHABLE_WEAPON"));
		return false;
	}
	...
	if (__CheckDashAffect(*pkInstMain))
	{
		if (!pSkillData->IsChargeSkill())
		{
			if (__CheckRestSkillCoolTime(dwSlotIndex))
			{
				...
				PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "WAIT_COOLTIME"));
				return false;
			}
		}
	}
	else
	{
		...
		if (__CheckShortLife(rkSkillInst, *pSkillData))
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_ENOUGH_HP"));
			return false;
		}
		...
		if (__CheckShortMana(rkSkillInst, *pSkillData))
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_ENOUGH_SP"));
			return false;
		}
	}
...
bool CPythonPlayer::__ProcessEnemySkillTargetRange(CInstanceBase& rkInstMain, CInstanceBase& rkInstTarget, CPythonSkill::TSkillData& rkSkillData, DWORD dwSkillSlotIndex)
{
	...
	if (rkBG.IsBlock(kPPosTarget.x, kPPosTarget.y))
	{
		...
		CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
		PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "CANNOT_APPROACH"));
		return false;
	}
		...
		pSkillData->IsAutoSearchTarget())
	{
		...
		if (pkInstTarget)
		{
			if (pSkillData->IsOnlyForAlliance())
			{
				if (pkInstMain == pkInstTarget)
				{
					if (!pSkillData->CanUseForMe())
					{
						...
						PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "CANNOT_USE_SELF"));
						return false;
					}
				}
				...
				else
				{
					...
					else
					{
						...
						PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "ONLY_FOR_ALLIANCE"));
						return false;
					}
				}
			}
			else
			{
				if (pkInstTarget->IsInSafe())
				{
					...
					PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "CANNOT_ATTACK_ENEMY_IN_SAFE_AREA"));
					return false;
				}
				...
				else
				{
					...
					PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "CANNOT_ATTACK"));
					return false;
				}
			}
		...
		else
		{
			...
			else if (pSkillData->IsNeedCorpse())
			{
				...
				PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "ONLY_FOR_CORPSE"));
				return false;
			}
			else
			{
...
void CPythonPlayer::__RunCoolTime(DWORD dwSkillSlotIndex)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "RunUseSkillEvent", Py_BuildValue("(if)", dwSkillSlotIndex, rkSkillInst.fCoolTime));
}
...
bool CPythonPlayer::__HasEnoughArrow()
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotShotError", Py_BuildValue("(is)", GetMainCharacterIndex(), "EMPTY_ARROW"));
	return false;
}
...
bool CPythonPlayer::__HasItem(DWORD dwItemID)
{
	...
	for (int j = 0; j < c_Special_ItemSlot_Count; ++j)
	{
		...
		if (dwItemID == GetItemIndex(TItemPos (STONE_INVENTORY, j)))
			return true;
		if (dwItemID == GetItemIndex(TItemPos (CHANGE_INVENTORY, j)))
			return true;
		if (dwItemID == GetItemIndex(TItemPos (COSTUME_INVENTORY, j)))
			return true;
...
void CPythonPlayer::UseGuildSkill(DWORD dwSkillSlotIndex)
{
	...
	if (pSkillData->IsOnlyForGuildWar())
	{
		if (!CPythonGuild::Instance().IsDoingGuildWar())
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "ONLY_FOR_GUILD_WAR"));
			return;
		}
	}
...
bool CPythonPlayer::__CheckSpecialSkill(DWORD dwSkillIndex)
{
	...
	if (c_iSkillIndex_Fishing == dwSkillIndex)
	{
		...
		else
		{
			...
			PyCallClassMemberFunc(rkNetStream.m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "EQUIP_FISHING_ROD"));
		}
		return true;
	}
