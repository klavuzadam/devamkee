...
void CInstanceBase::SetAutoFarmAffect(const bool bStatus)
{
	if (bStatus)
...
void CInstanceBase::ProcessDamage()
{
	...
	const float scaleImg = CPythonSystem::Instance().GetScaleDamage();
	const DWORD FONT_WIDTH = ceil(scaleImg * (30 / 0.5));
	...
	*/
	...
	while (damage > 0)
	{
		...
		rkEftMgr.SetEffectTextures(ms_adwCRCAffectEffect[rdwCRCEft], textures, scaleImg, scaleImg);
