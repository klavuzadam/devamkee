...
CPythonSkill::CPythonSkill()
{
	...
	m_SkillWeaponTypeIndexMap.insert(std::make_pair(std::string("ARROW"), CItemData::WEAPON_ARROW || CItemData::WEAPON_QUIVER));
...
const char * CPythonSkill::SSkillData::GetAffectDescriptionWithPower(DWORD dwIndex, int skillPower)
{
	if (dwIndex >= AffectDataVector.size())
		return NULL;

    // Assuming c_rstrAffectDescription is a string like "Physical Attack Resistance %.1f%%"
    const std::string & c_rstrAffectDescription = AffectDataVector[dwIndex].strAffectDescription;

    // Format the description to include the skillPower as a floating point number with one decimal place.
    static char szDescription[256];
    _snprintf(szDescription, sizeof(szDescription), c_rstrAffectDescription.c_str(), static_cast<float>(skillPower));
...
PyObject * skillGetAffectDescriptionWithPower(PyObject * poSelf, PyObject * poArgs)
{
	int iSkillIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
		return Py_BadArgument();
	...
	int skillPower;
	if (!PyTuple_GetInteger(poArgs, 2, &skillPower))
		return Py_BadArgument();
	...
	return Py_BuildValue("s", c_pSkillData->GetAffectDescriptionWithPower(iAffectIndex, skillPower));
}
...
void initskill()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetAffectDescriptionWithPower",				skillGetAffectDescriptionWithPower,				METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "PASSIVE_ABILITY_VNUM_TAMER_1", PASSIVE_ABILITY_VNUM_TAMER_1);
	PyModule_AddIntConstant(poModule, "PASSIVE_ABILITY_VNUM_TAMER_2", PASSIVE_ABILITY_VNUM_TAMER_2);
	PyModule_AddIntConstant(poModule, "PASSIVE_ABILITY_VNUM_TAMER_3", PASSIVE_ABILITY_VNUM_TAMER_3);
	PyModule_AddIntConstant(poModule, "PASSIVE_ABILITY_VNUM_TAMER_4", PASSIVE_ABILITY_VNUM_TAMER_4);
	PyModule_AddIntConstant(poModule, "PASSIVE_ABILITY_VNUM_TAMER_MAX", PASSIVE_ABILITY_VNUM_TAMER_MAX);
#endif
}
