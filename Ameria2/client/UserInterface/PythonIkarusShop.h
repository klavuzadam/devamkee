...
namespace ikashop
{
	...
	enum eConstOfflineshop {
		...
		OFFLINESHOP_GRID_WIDTH = 10,
		OFFLINESHOP_GRID_HEIGHT = 8,
#else
	...
	public:
		...
		{
			const auto [col, row] = _DecomposeCell(cell);
			for (int crow = row; crow < row + size; crow++)
				if (_GetCellState(_ComposeCell(col, crow)))
					return false;
			return true;
		}
	...
	namespace python 
	{
		...
		struct PythonGarbageCollector
		{
			...
			template<class ...Args>
			PyObject* makeValue(const std::string& str, const Args& ...arg) {
				return _Register(Py_BuildValue(str.c_str(), arg...));
			}

			PyObject* registerObject(PyObject* obj){ objects.emplace_back(obj); return obj; }

			~PythonGarbageCollector(){ std::for_each(objects.begin(), objects.end(), [](auto&& obj) { Py_XDECREF(obj); }); }
		...
		template<class Type>
		constexpr bool IsPythonSafeType = std::is_same_v<Type, PythonSafeDict> || std::is_same_v<Type, PythonSafeList> || std::is_same_v<Type, PythonReference>;
		...
		struct PythonSafeDict
		{
			...
			template<typename Type, std::enable_if_t<IsPythonSafeType<Type>, int> = 0>
			void set(const std::string& key, const Type& value) { PyDict_SetItemString(dict, key.c_str(), value.weakRef()); }
		...
		struct PythonSafeList
		{
			...
			template<typename Type, std::enable_if_t<IsPythonSafeType<Type>, int> = 0>
			void append(const Type& value) { PyList_Append(list, value.weakRef()); }
		...
		template<class Type>
		inline PythonSafeDict ExtractPythonDictPlayerItem(const Type& info) {
			...
			itemDict.set("varname", info.name);
#endif
		...
		template<class Type>
		inline PythonSafeDict ExtractPythonDictShopItem(const Type& info) {
...
#ifdef EXTEND_IKASHOP_ULTIMATE
		template<class Type>
		inline PythonSafeDict ExtractPythonDictShopItemView(const Type& info) {
		...
		template<class ContainerType, class FuncType>
		inline PythonSafeList ExtractPythonListFromContainer(const ContainerType& container, FuncType func) {
			PythonSafeList list;
		...
		inline PythonSafeList ExtractPythonPlayerItemList(const ContainerType& container) {
			static auto func = [](auto&& value) { return ExtractPythonDictPlayerItem(value); };
			return ExtractPythonListFromContainer(container, func);
		}

		template<class ContainerType>
		inline PythonSafeList ExtractPythonShopItemList(const ContainerType& container) {
...
#ifdef EXTEND_IKASHOP_ULTIMATE
		template<class ContainerType>
		inline PythonSafeList ExtractPythonShopItemViewList(const ContainerType& container) {
...
public:
	struct TFilterHistoryElement : public ikashop::TFilterInfo
	{
		...
		bool operator<(const TFilterHistoryElement& right)  const{
...
#ifdef EXTEND_IKASHOP_ULTIMATE
#ifdef USE_IKASHOP_UI_RENEWED
	static constexpr auto OFFSHOP_LOCK_INDEX_INIT = (10 * 8) * 2;
#else
...
#ifdef USE_IKASHOP_UI_RENEWED
	void SafeboxRefreshValutes(const ikashop::TValutesInfo& valute);
#endif
...
private:
	...
	bool m_bAscendingPrices;
#endif
    std::vector<ikashop::TSearchResultInfo> m_vecShopSearchItem;
    int32_t m_iShopSearchCurrPage, m_iShopSearchMaxxPage;

public:
#ifdef USE_PREMIUM_AFFECT
	bool ChangeAscendingOrder(const bool bFlag);
#endif

    void ClearShopSearchItem();

    void SetShopSearchItemPageData(const int32_t curr, const int32_t maxx);
    int32_t GetShopSearchPageCurr() const;
    int32_t GetShopSearchPageMaxx() const;

    size_t GetShopSearchItemCount() const;
    bool GetShopSearchItemData(uint32_t index, const ikashop::TSearchResultInfo** c_pItemData);
#endif
};
