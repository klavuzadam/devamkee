...
void CPythonPlayer::PickCloseItem()
{
	...
	if (!rkItem.GetCloseItem(pkInstMain->GetNameString(), kPPosMain, &dwItemID, __GetPickableDistance()))
	{
		return;
	}
...
void CPythonPlayer::PickCloseItemVector()
{
	...
	std::vector<DWORD> itemVIDVector;
	CPythonItem& rkItem = CPythonItem::Instance();
	if (!rkItem.GetCloseItemVector(pkInstMain->GetNameString(), kPPosMain, itemVIDVector))
		return;

	if (itemVIDVector.empty())
		return;

	SendPickUPItemPacket(itemVIDVector);
}
#endif
...
//		return;
	...
	if (CPythonBackground::Instance().IsBattleRoyaleMap() && PythonBattleRoyaleManager::Instance().IsDisableOpenCharacterMenu())
		return;
#endif
