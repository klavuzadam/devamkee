...
std::string g_strEffectNameShop = "d:/ymir work/effect/direction/direction_land2.mse";
...
bool CPythonBackground::SetShadowLevel(int eLevel)
{
	...
	auto& rkMap = GetMapOutdoorRef();

	const auto lastShadow = m_eShadowLevel;
	...
	switch (m_eShadowLevel)
	{
		...
		default:
		{
			TraceError("Unknown shadow level: %d.", m_eShadowLevel);
			m_eShadowLevel = lastShadow;
			return false;
		}
	}
...
{
	if (!m_PropertyManager.Initialize("property.xml"))
	{
		TraceError("Cannot initialize property.");
		abort();
	}
}
...
void CPythonBackground::Update(float fCenterX, float fCenterY, float fCenterZ)
{
	...
	// shearh offline
	{
		std::map<DWORD, DWORD>::iterator itor = m_kMap_dwTargetID_dwChrID2.begin();
		for (; itor != m_kMap_dwTargetID_dwChrID2.end(); ++itor)
		{
			DWORD dwTargetID = itor->first;
			DWORD dwChrID = itor->second;
...
void CPythonBackground::Warp(DWORD dwX, DWORD dwY)
{
...
#if defined(__BL_BATTLE_ROYALE__)
	if (IsBattleRoyaleMap() || pkMapInfo->m_strName == "metin2_map_battleroyale")
		PythonBattleRoyaleManager::Instance().BATTLE_ROYALE_Unknown_1();
#endif
	...
	if (!LoadMap(rMapInfo.m_strName, float(dwX - rMapInfo.m_dwBaseX), float(dwY - rMapInfo.m_dwBaseY), 0, CPythonSystem::Instance().IsSnowTerrainTexture()))
	{
	...
	{
		SetFogEnvironment(true);
	}
	else
	{
		SetFogEnvironment(CPythonSystem::Instance().IsFogEnvironment());
	}
...
{
	m_kMapCombatZone_dwTargetID_dwChrID.insert(std::make_pair(dwID, dwChrVID));
}
...
{
	m_kMap_dwTargetID_dwChrID2.insert(std::make_pair(dwID, dwChrVID));
}
...
void CPythonBackground::CreateTargetEffectShop(DWORD dwID, long lx, long ly)
{
	...
	CreateSpecialEffect(dwID, ilx, ily, fHeight, g_strEffectNameShop.c_str());
}
// search offline end

#if defined(__BL_BATTLE_ROYALE__)
bool CPythonBackground::IsBattleRoyaleMap() const
{
	return m_strMapName == "metin2_map_battleroyale";
}
#endif