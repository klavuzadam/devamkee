...
PyObject* netSendChangeCharacterHairPacket(PyObject* poSelf, PyObject* poArgs)
{
	int index;
	if (!PyTuple_GetInteger(poArgs, 0, &index))
		return Py_BuildException();
	...
	int hairIndex;
	if (!PyTuple_GetInteger(poArgs, 1, &hairIndex))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChangeCharacterHairPacket((BYTE)index, hairIndex);
	return Py_BuildNone();
}
...
{
	int iSlotInv;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotInv))
		return Py_BuildException();
	int iSlotNumber;
...
{
	long long lldValue;
	if (!PyTuple_GetLongLong(poArgs, 0, &lldValue))
	{
		return Py_BuildException();
	}
	...
	netMgr.SendExchangeElkAddPacket(lldValue, EXCHANGE_SUBHEADER_GOLD_ADD);
#else
	netMgr.SendExchangeElkAddPacket(lldValue);
#endif
	return Py_BuildNone();
}
...
PyObject* netSendExchangeChequeAddPacket(PyObject*/* poSelf*/, PyObject* poArgs)
{
	...
	auto& netMgr = CPythonNetworkStream::Instance();
#ifdef USE_CHEQUE_CURRENCY
	netMgr.SendExchangeElkAddPacket(lldValue, EXCHANGE_SUBHEADER_CHEQUE_ADD);
#else
	netMgr.SendExchangeElkAddPacket(lldValue);
#endif
	return Py_BuildNone();
...
PyObject* netSendRefinePacket(PyObject*/* poSelf*/, PyObject* poArgs)
{
	...
	BYTE bySpecialStorage;
	if (!PyTuple_GetByte(poArgs, 2, &bySpecialStorage))
	{
		return Py_BuildException();
	}
	...
	bool bUseSealOfGod;
	if (!PyTuple_GetBoolean(poArgs, 3, &bUseSealOfGod))
	{
		return Py_BuildException();
	}
#endif
	...
	auto& netMgr = CPythonNetworkStream::Instance();
	netMgr.SendRefinePacket(wPos, byType, bySpecialStorage
#ifdef ENABLE_PITTY_REFINE
, bUseSealOfGod
#endif
	);
...
PyObject* netSendCalendarSystemOpen(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendCalendarSystemOpen();
	return Py_BuildNone();
}

PyObject* netSendCalendarSystemClose(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendCalendarSystemClose();
	return Py_BuildNone();
}

PyObject* netSendCalendarSystemSend(PyObject* poSelf, PyObject* poArgs)
{
	int day;
	...
	int index_event;
	if (!PyTuple_GetInteger(poArgs, 1, &index_event))
		return Py_BuildException();

	char * hora_inicio;
	if (!PyTuple_GetString(poArgs, 2, &hora_inicio))
		return Py_BuildException();

	char * hora_final;
	if (!PyTuple_GetString(poArgs, 3, &hora_final))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendCalendarSystem(day, index_event, hora_inicio, hora_final);
...
PyObject* netOpenCatchKing(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendOpenCatchKing();
	return Py_BuildNone();
}
...
PyObject* netSendConfirmCaptcha(PyObject* poSelf, PyObject* poArgs)
{
    int32_t code;
    if (!PyTuple_GetInteger(poArgs, 0, &code))
    {
        return Py_BuildException();
    }
    ...
    CPythonNetworkStream::Instance().SendCaptcha(code);
    return Py_BuildNone();
}
#endif

#ifdef USE_CURRENCY_EXCHANGE
PyObject* netSendCurrencyExchange(PyObject*/* poSelf*/, PyObject* poArgs)
{
	long long exchangeAmount;
	if (!PyTuple_GetLongLong(poArgs, 0, &exchangeAmount))
	{
		return Py_BuildException();
	}

	uint8_t exchangeType;
	if (!PyTuple_GetInteger(poArgs, 1, &exchangeType))
	{
		return Py_BuildException();
	}

	auto& netMgr = CPythonNetworkStream::Instance();
	netMgr.SendCurrencyExchange(exchangeAmount, exchangeType);
	return Py_BuildNone();
}
#endif

#if defined(__BL_BATTLE_ROYALE__)
PyObject* netSendBattleRoyaleApplication(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendBattleRoyaleApplication();
	return Py_BuildNone();
}

PyObject* netSendBattleRoyaleApplicationCancel(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendBattleRoyaleApplicationCancel();
	return Py_BuildNone();
}

PyObject* netSendBattleRoyaleExit(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendBattleRoyaleExit();
	return Py_BuildNone();
}

PyObject* netSendBattleRoyaleRanking(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendBattleRoyaleRanking();
	return Py_BuildNone();
}

PyObject* netSendBattleRoyaleStart(PyObject* poSelf, PyObject* poArgs)
{
	bool bUseSpecialItem;
	if (!PyTuple_GetBoolean(poArgs, 0, &bUseSpecialItem))
		return Py_BuildException();
	
	CPythonNetworkStream::Instance().SendBattleRoyaleStart(bUseSpecialItem);
	return Py_BuildNone();
}

PyObject* netSendBattleRoyaleClaimReward(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendBattleRoyaleClaimReward();
	return Py_BuildNone();
}
#endif

#ifdef USE_CRYSTAL_SYSTEM
PyObject* netSendShardCraft(PyObject*/* poSelf*/, PyObject* poArgs)
{
	uint8_t bSubHeader;
	...
	switch (bSubHeader)
	{
		...
		{
			auto& netMgr = CPythonNetworkStream::Instance();
			netMgr.SendShardCraft(bSubHeader);
			return Py_BuildNone();
		}
		case SUBHEADER_SHARD_CRAFT_ADD:
		{
			...
			int32_t iSourceSlot;
			if (!PyTuple_GetInteger(poArgs, 1, &iSourceSlot))
			{
				return Py_BuildException();
			}

			int32_t iTargetSlot;
			if (!PyTuple_GetInteger(poArgs, 2, &iTargetSlot))
			{
				return Py_BuildException();
			}

			auto& netMgr = CPythonNetworkStream::Instance();
			netMgr.SendShardCraft(bSubHeader, iSourceSlot, iTargetSlot);
			return Py_BuildNone();
		}
		case SUBHEADER_SHARD_CRAFT_OUT:
		{
			...
			int32_t iTargetSlot;
			if (!PyTuple_GetInteger(poArgs, 1, &iTargetSlot))
			{
				return Py_BuildException();
			}

			auto& netMgr = CPythonNetworkStream::Instance();
			netMgr.SendShardCraft(bSubHeader, -1, iTargetSlot);
			return Py_BuildNone();
		}
		default:
		{
			TraceError("net.SendShardCraft >> invalid subHeader (%u).", bSubHeader);
			return Py_BuildException();
		}
	}
}
...
void initnet()
{
	...
	{
#if defined(__BL_BATTLE_ROYALE__)
		{ "SendBattleRoyaleApplication",		netSendBattleRoyaleApplication,			METH_VARARGS },
		{ "SendBattleRoyaleApplicationCancel",	netSendBattleRoyaleApplicationCancel,	METH_VARARGS },
		{ "SendBattleRoyaleExit",				netSendBattleRoyaleExit,				METH_VARARGS },
		{ "SendBattleRoyaleRanking",			netSendBattleRoyaleRanking,				METH_VARARGS },
		{ "SendBattleRoyaleStart",				netSendBattleRoyaleStart,				METH_VARARGS },
		{ "SendBattleRoyaleClaimReward",		netSendBattleRoyaleClaimReward,			METH_VARARGS },
#endif
		...
		{ "SendChangeCharacterHairPacket",		netSendChangeCharacterHairPacket,		METH_VARARGS },
...
#endif
#ifdef ENABLE_CALENDAR_SYSTEM
		{ "CalendarOpen",								netSendCalendarSystemOpen,						METH_VARARGS },
		{ "CalendarClose",								netSendCalendarSystemClose,						METH_VARARGS },
		{ "CalendarSend",								netSendCalendarSystemSend,						METH_VARARGS },
#endif
		...
		{ "OpenCatchKing",							netOpenCatchKing,							METH_VARARGS },
#endif
#ifdef ENABLE_SELL_ITEM
...
#endif
#ifdef USE_CAPTCHA_SYSTEM
        { "ajf35iuhfs55539141", netSendConfirmCaptcha, METH_VARARGS },
#endif
#ifdef USE_CURRENCY_EXCHANGE
		{ "fdsak432ukgfdsjk25", netSendCurrencyExchange, METH_VARARGS },
#endif
#ifdef USE_CRYSTAL_SYSTEM
		{ "SendShardCraft", netSendShardCraft, METH_VARARGS },
#endif
		{ NULL,										NULL,										NULL },
	};
...
#ifdef NEW_SELECT_CHARACTER
	PyModule_AddIntConstant(poModule, "CHARACTER_SLOT_COUNT_MAX", PLAYER_PER_ACCOUNT4);
...
#endif
#ifdef USE_CRYSTAL_SYSTEM
	PyModule_AddIntConstant(poModule, "SUBHEADER_SHARD_CRAFT_CLOSE", SUBHEADER_SHARD_CRAFT_CLOSE);
	PyModule_AddIntConstant(poModule, "SUBHEADER_SHARD_CRAFT_ADD", SUBHEADER_SHARD_CRAFT_ADD);
	PyModule_AddIntConstant(poModule, "SUBHEADER_SHARD_CRAFT_OUT", SUBHEADER_SHARD_CRAFT_OUT);
	PyModule_AddIntConstant(poModule, "SUBHEADER_SHARD_CRAFT_REFINE", SUBHEADER_SHARD_CRAFT_REFINE);
#endif
}
