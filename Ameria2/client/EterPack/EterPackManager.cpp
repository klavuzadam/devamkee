...
const char* white_file_list_ext[] = {"xml", "tga", "png", "bmp", "mp3", "jpg", "fnt"};
...
bool CEterPackManager::Get (CMappedFile& rMappedFile, const char* c_szFileName, LPCVOID* pData, const char* func, bool ignoreChecks)
{
...
void CEterPackManager::OutputError(int32_t errorCode, bool showAsError, const std::string & stFilename, const char* func)
{
	std::wstring ws;
	ws.assign(stFilename.begin(), stFilename.end());
	OutputError(errorCode, showAsError, ws, func);
}
void CEterPackManager::OutputError (int32_t errorCode, bool showAsError, const std::wstring & stFilename, const char* func)
{
	if (showAsError)
...
bool CEterPackManager::isExistInPack (const char* c_szFileName, const char* func) const
{
	assert (c_szFileName);
...
bool CEterPackManager::isExist (const char* c_szFileName, const char* func, bool ignoreChecks) const
{
...
bool CEterPackManager::LoadArchive (const std::wstring & stName) const
{
	if (m_pFoxFS)
	{
		int32_t errorCode;
...
bool CEterPackManager::UnloadArchive (const std::wstring & stName) const
{
	if (m_pFoxFS)
	{
		int32_t errorCode;
...
CEterPackManager::CEterPackManager() : m_pFoxFS (nullptr)
{
