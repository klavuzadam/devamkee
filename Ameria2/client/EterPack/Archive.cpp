	...
	int32_t Archive::get(const char* filename, void* buffer, uint32_t maxsize, uint32_t* outsize) const
	{
		...
		if (iter != files.end())
		{
			...
			else
			{
                ...
                uint32_t uiSize = LZ4_decompress_safe(reinterpret_cast<char*>(&tmp[0]), reinterpret_cast<char*>(buffer), iter->second.size, iter->second.decompressed);
                if (uiSize != iter->second.decompressed)
                {
