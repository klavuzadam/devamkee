...
inline CThreadPool::CThreadPool(std::size_t threads) : m_stop(false)
{
	for (std::size_t i = 0; i < threads; ++i)
	{
		m_vec_threads.emplace_back(
			[this] {
				for (;;)
				{
					std::function<void()> task;
					{
						...
						this->condition.wait(lock,
							[this] { return this->m_stop || !this->m_queue_tasks.empty(); });

						if (this->m_stop && this->m_queue_tasks.empty())
							return;

						task = std::move(this->m_queue_tasks.front());
						this->m_queue_tasks.pop();
					}
					task();
				}
			});
...
-> std::future<std::invoke_result_t<F, Args...>>
{
	...
	std::future<return_type> res = task->get_future();
	{
		...
		m_queue_tasks.emplace([task]() { (*task)(); });
	}
...
inline CThreadPool::~CThreadPool()
{
	...
	for (std::thread& thread : m_vec_threads)
		thread.join();
}
#endif
