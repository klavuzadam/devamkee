...
CMapManager::CMapManager() : mc_pcurEnvironmentData(NULL), m_bFogEnviroment(true), m_bSnowTerrainTexture(false)
{
	m_pkMap = NULL;
...
bool CMapManager::LoadMap(const std::string & c_rstrMapName, float x, float y, float z, bool bSnowTerrainTexture)
{
	...
	{
		if (!rkMap.Load(x, y, z, bSnowTerrainTexture))
		{
...
void CMapManager::BeginEnvironment()
{
	...
	// Fog
 	STATEMANAGER.SaveRenderState(D3DRS_FOGENABLE, m_bFogEnviroment ? mc_pcurEnvironmentData->bFogEnable : 0);
...
void CMapManager::__LoadMapInfoVector()
{
	CMappedFile kFile;
	LPCVOID pData;
	if (!CEterPackManager::Instance().Get(kFile, m_stAtlasInfoFileName.c_str(), &pData, __FUNCTION__))
		if (!CEterPackManager::Instance().Get(kFile, "AtlasInfo.txt", &pData, __FUNCTION__))
			return;
...
void CMapManager::SetFogEnvironment(const bool bFlag)
{ 
	m_bFogEnviroment = bFlag;
}

bool CMapManager::IsFogEnvironment() const
{ 
	return m_bFogEnviroment;
}

void CMapManager::SetSnowTerrainTexture(const bool bFlag)
{
	m_bSnowTerrainTexture = bFlag;

	if (m_pkMap)
	{
		m_pkMap->LoadSetting(nullptr, bFlag);
	}
}

bool CMapManager::IsSnowTerrainTexture() const
{ 
	return m_bSnowTerrainTexture;
}
