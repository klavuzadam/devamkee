...
#if defined(ENABLE_CSHIELD) && !defined(_DEBUG)
	if (IsPC() && !CheckAttackspeed(m_fAtkSpd, m_fReachScale, IsTwoHandMode(), CRaceMotionData::MODE_HORSE_TWOHAND_SWORD == GetMotionMode()))
	{
		clientCode = 43;
...
bool CActorInstance::__CanPushDestActor(CActorInstance& rkActorDst)
{
	...
	if (IS_HUGE_RACE(rkActorDst.GetRace()) || rkActorDst.IsBoss())
		return false;
