...
void CArea::__UpdateEffectList()
{
	...
	static auto& background = CPythonBackground::Instance();
	static auto& effect_manager = CEffectManager::Instance();
	
	background.GetDistanceSetInfo(&peNum, &pfStart, &pfEnd, &pfFarClip);
	auto* const player = CPythonCharacterManager::instance().GetMainInstancePtr();
	if (!player)
		return;
	...
	for (auto i = m_EffectInstanceMap.begin(); i != m_EffectInstanceMap.end();)
	{
		...
		const auto& gMatrix = effect->GetGlobalMatrix();
		if (pfStart < GetPixelPositionDistance(player->GetGraphicThingInstanceRef().GetPosition(), TPixelPosition(gMatrix._41, gMatrix._42, gMatrix._43)))
		{
			effect->Hide();
			++i;
			continue;
		...
		if (!effect->isAlive())
		{
			...
			effect_manager.DestroyUnsafeEffectInstance(effect);
		}
		else
			++i;
	}
...
void CArea::RenderEffect()
{
    ...
    else
    {
        ...
        s_kVct_pkEftInstSort.reserve(m_EffectInstanceMap.size());

        for (const auto& [k, effect] : m_EffectInstanceMap)
        {
            s_kVct_pkEftInstSort.emplace_back(effect);
        }

        std::sort(std::execution::par, s_kVct_pkEftInstSort.begin(), s_kVct_pkEftInstSort.end(), CArea_LessEffectInstancePtrRenderOrder());
...
{
#ifdef USE_D3D9
    D3DPERF_BeginEvent(D3DCOLOR_ARGB(255, 255, 0, 0), L"** CArea::RenderCollision **");
#endif
...
{
#ifdef USE_D3D9
    D3DPERF_BeginEvent(D3DCOLOR_ARGB(255, 255, 0, 0), L"** CArea::RenderAmbience **");
#endif
    ...
    for (const auto &clone : m_AmbienceCloneInstanceVector)
    {
        clone->Render();
    }
...
{
#ifdef USE_D3D9
    D3DPERF_BeginEvent(D3DCOLOR_ARGB(255, 255, 0, 0), L"** CArea::RenderDungeon **");
#endif
...
void CArea::__SetObjectInstance_SetBuilding(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
    ...
    if (iMotionCount)
    {
        pObjectInstance->pThingInstance->RegisterMotionThing(pObjectInstance->pThingInstance->GetMotionIndex(), 0, pThing);
    }
...
void CArea::__LoadAttribute(TObjectInstance * pObjectInstance, const char * c_szAttributeFileName)
{
	...
	if (false == bFileExist)
	{
		...
		// NOTE: dungeon ¿ÀºêÁ§Æ®´Â Dummy CollisionÀ» ÀÚµ¿À¸·Î »ý¼ºÇÏÁö ¾Êµµ·Ï ÇÔ (´øÀüÀÇ °æ¿ì ´õ¹Ì ÄÃ¸®Àü¶§¹®¿¡ ¹®Á¦°¡ µÈ °æ¿ì°¡ ¼öÂ÷·Ê ÀÖ¾úÀ½. ÀÌ·¸°Ô ÇÏ±â·Î ±×·¡ÇÈ ÆÀ°ú ÇùÀÇ ¿Ï·á)
	...
	if (false == bFileExist)
	{
		if (pAttributeData->IsEmpty())
		{
			if (NULL != pObjectInstance && NULL != pObjectInstance->pThingInstance)
			{
				...
				D3DXVECTOR3 vDelta = (v3Max - v3Min);
				collision.fDimensions[0] = vDelta.x * 0.5f; // v3Min, v3Max¸¦ ±¸ÇÏ±â À§ÇÑ °¡·Î, ¼¼·Î, ³ôÀÌÀÇ Àý¹Ý°ª ÀúÀå
				collision.fDimensions[1] = vDelta.y * 0.5f;
				collision.fDimensions[2] = vDelta.z * 0.5f;
