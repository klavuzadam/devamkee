...
#ifdef ENABLE_FAST_MOB_MOVE_SPEED
	if(!IsPC() && !(CRaceMotionData::NAME_WALK == dwMotion || dwMotion == CRaceMotionData::NAME_RUN) && fSpeedRatio > 1.5)
		fSpeedRatio = 1.5;
#endif
...
#ifdef ENABLE_FAST_MOB_MOVE_SPEED
	if (!IsPC() && !(CRaceMotionData::NAME_WALK == wMotion || wMotion == CRaceMotionData::NAME_RUN) && fSpeedRatio > 1.0)
		fSpeedRatio = 1.0;
#endif
	MOTION_KEY dwMotionKey;
