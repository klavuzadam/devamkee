...
void CRaceData::LoadAllMotions()
{
    for (const auto& itor : m_pMotionModeDataMap)
    {
        if (!itor.second)
        {
            continue;
        ...
        for (const auto& itorMotion : itor.second->MotionVectorMap)
        {
            auto i = 0;
            for (const auto& it : itorMotion.second)
            {
                CGraphicThingInstance::RegisterMotionThing(m_dwRaceIndex, static_cast<DWORD>(MAKE_RANDOM_MOTION_KEY(itor.first, itorMotion.first, i)), it.pMotion);
                ++i;
            }
        }
    }
