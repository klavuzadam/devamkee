...
bool CMapOutdoor::Load(float x, float y, float z, bool bSnowTerrainTexture)
{
	Destroy();
	...
	if (!LoadSetting(strFileName.c_str(), bSnowTerrainTexture))
	{
...
bool CMapOutdoor::LoadSetting(const char* c_szFileName, bool bSnowTerrainTexture)
{
	NANOBEGIN
	...
	{
		bChangeTexture = true;
		sprintf(szFilePath, "%s\\Setting.txt", GetMapDataDirectory().c_str());
	}
	else
	{
		sprintf(szFilePath, "%s", c_szFileName);
	}

	CTokenVectorMap stTokenVectorMap;

	if (!LoadMultipleTextData(szFilePath, stTokenVectorMap))
	{
		TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LoadMultipleTextData", szFilePath);
		return false;
	}
	...
	if (!bChangeTexture)
	{
		if (stTokenVectorMap.end() == stTokenVectorMap.find("scripttype"))
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - FIND 'scripttype' - FAILED", szFilePath);
			return false;
		}

		if (stTokenVectorMap.end() == stTokenVectorMap.find("viewradius"))
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - FIND 'viewradius' - FAILED", szFilePath);
			return false;
		}

		if (stTokenVectorMap.end() == stTokenVectorMap.find("cellscale"))
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - FIND 'cellscale' - FAILED", szFilePath);
			return false;
		}

		if (stTokenVectorMap.end() == stTokenVectorMap.find("heightscale"))
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - FIND 'heightscale' - FAILED", szFilePath);
			return false;
		}

		if (stTokenVectorMap.end() == stTokenVectorMap.find("mapsize"))
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - FIND 'mapsize' - FAILED", szFilePath);
			return false;
		}
	}
	...
	{
		TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - FIND 'textureset' - FAILED", szFilePath);
		return false;
	}
	...
	if (bChangeTexture)
	{
		std::string strTextureSetNow;
		TTokenVector & rkVec_strToken = stTokenVectorMap["textureset"];
		if (!rkVec_strToken.empty())
		{
			strTextureSetNow = rkVec_strToken[0];
		}

		std::string stTextureSetFileNameNow;

		if (bSnowTerrainTexture)
		{
			stTextureSetFileNameNow = "textureset\\snow\\";
			stTextureSetFileNameNow.append(CFileNameHelper::NoPath(strTextureSetNow));
		}
		else
		{
			stTextureSetFileNameNow = strTextureSetNow;

			if (0 != stTextureSetFileNameNow.find_first_of("textureset", 0))
			{
				stTextureSetFileNameNow = "textureset\\" + strTextureSetNow;
			}
		}

		if (!m_TextureSet.Load(stTextureSetFileNameNow.c_str(), m_fTerrainTexCoordBase))
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR (1a)", szFilePath, stTextureSetFileNameNow.c_str());
			...
			if (bSnowTerrainTexture)
			{
				...
				if (!m_TextureSet.Load(stTextureSetFileNameNow.c_str(), m_fTerrainTexCoordBase))
				{
					TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR (2a)", szFilePath, stTextureSetFileNameNow.c_str());
					return false;
				}
			}
			else
			{
				...
				if (!m_TextureSet.Load(stTextureSetFileNameNow.c_str(), m_fTerrainTexCoordBase))
				{
					TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR (3a)", szFilePath, stTextureSetFileNameNow.c_str());
					return false;
				}
			}
		}
	...
	else
	{
		...
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - Resourse Type ERROR", szFilePath);
			return false;
		}
		...
		if (0L >= m_lViewRadius)
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - VIEWRADIUS IS NOT GREATER THAN 0", szFilePath);
			return false;
		}
		...
		std::string stTextureSetFileName;

		if (bSnowTerrainTexture)
		{
			stTextureSetFileName = "textureset\\snow\\";
			stTextureSetFileName.append(CFileNameHelper::NoPath(strTextureSet));
		}
		else
		{
			stTextureSetFileName = strTextureSet;
		...
		{
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR (1b)", szFilePath, stTextureSetFileName.c_str());
			...
			if (bSnowTerrainTexture)
			{
				...
				if (!m_TextureSet.Load(stTextureSetFileName.c_str(), m_fTerrainTexCoordBase))
				{
					TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR (2b)", szFilePath, stTextureSetFileName.c_str());
					return false;
				}
			}
			else
			{
				...
				if (!m_TextureSet.Load(stTextureSetFileName.c_str(), m_fTerrainTexCoordBase))
				{
					TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR (3b)", szFilePath, stTextureSetFileName.c_str());
					return false;
				}
			}
		}
		...
		{
#ifdef WORLD_EDITOR
			LogBoxf("TextureSet not successfully applied to [%s].\nTexture not found [%s].", szFilePath, stTextureSetFileName.c_str());
#else
			TraceError("MapOutdoor::LoadSetting(szFilePath=%s) - LOAD TEXTURE SET(%s) ERROR", szFilePath, stTextureSetFileName.c_str());
			return false;
#endif
		}
		...
		if (stTokenVectorMap.end() != stTokenVectorMap.find("environment"))
		{
			...
			{
				TraceError("CMapOutdoor::LoadSetting(szFilePath=%s) - Failed to load environment data\n", szFilePath);
			}
		}
...
bool CMapOutdoor::LoadMonsterAreaInfo()
{
	...
	if (!CEterPackManager::Instance().Get(File, c_szFileName, &pModelData, __FUNCTION__))
	{
