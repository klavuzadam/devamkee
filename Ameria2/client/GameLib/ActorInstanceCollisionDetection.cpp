...
void CActorInstance::UpdatePointInstance(TCollisionPointInstance * pPointInstance)
{
	...
	if (pPointInstance->isAttached)
	{
		...
		CGrannyLODController* pGrnLODController=m_LODControllerVector[pPointInstance->dwModelIndex].get();
...
void CActorInstance::UpdateAdvancingPointInstance()
{
	...
	for (; itor != m_BodyPointInstanceList.end(); ++itor)
	{
		...
		if (rInstance.isAttached)
		{
			...
			CGrannyLODController* pGrnLODController=m_LODControllerVector[rInstance.dwModelIndex].get();
			if (!pGrnLODController)
			{
...
BOOL CActorInstance::__NormalAttackProcess(CActorInstance & rVictim)
{
	...
	if (IS_HUGE_RACE(rVictim.GetRace()) || rVictim.IsBoss())
	{
...
bool CActorInstance::RaceMustHaveCollision(DWORD race) {
	switch (race) {
	case 2493:
	case 20387:
	case 20388:
...
#define ENABLE_EXTENDED_COLLISIONS
...
#ifdef ENABLE_EXTENDED_COLLISIONS
	if (!rVictim.IsBoss() && !isAttacking() && ((rVictim.IsEnemy() || rVictim.IsNPC())))
		return FALSE;
#endif
...
//		//}
//
//		if (!isMyTarget && !isAttacking()) {
//			if (rVictim.IsNPC() || rVictim.IsEnemy() || rVictim.IsPoly()) {
//				return false;
//			}
//		}
//	}
...
//	if (rVictim.GetRace()>=34001,65000 && rVictim.GetRace()<=34099)
//		return FALSE;
//#endif
...
//		return FALSE;
//#endif
	if (rVictim.GetRace() == 65000)
		return false;

#ifdef ENABLE_MOUNTS_WITHOUT_COLLISIONS
	if (rVictim.GetRace()>=20110 && rVictim.GetRace()<=65005)
		return FALSE;
#endif
