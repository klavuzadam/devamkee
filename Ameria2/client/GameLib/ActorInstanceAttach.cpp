...
void CActorInstance::AttachSash(CItemData * pItemData, float fSpecular)
{
    ...
    CGrannyLODController* lodInst = m_LODControllerVector[CRaceData::PART_SASH].get();
    if (lodInst)
    {
        CGrannyModelInstance* modelInst = lodInst->GetModelInstance();
        if (modelInst)
        {
            CGraphicThing* pItemGraphicThing = pItemData->GetModelThing();
            if (std::shared_ptr<CGrannyMotion> pItemMotion = pItemGraphicThing->GetMotionPointer(0))
            {
                modelInst->SetMotionPointer(pItemMotion);
            }
        }
    }
}
...
void CActorInstance::ShowAllAttachingEffectEx()
{
	std::list<TAttachingEffect>::iterator it;
	for (it = m_AttachingEffectList.begin(); it != m_AttachingEffectList.end(); ++it)
...
void CActorInstance::HideAllAttachingEffectEx()
{
	std::list<TAttachingEffect>::iterator it;
	for (it = m_AttachingEffectList.begin(); it != m_AttachingEffectList.end(); ++it)
