...
#   include <cstdarg>
	...
	static inline int32_t TIXML_SNPRINTF( char* buffer, size_t size, const char* format, ... )
	{
		va_list va;
		va_start( va, format );
	...
	static inline int32_t TIXML_VSNPRINTF( char* buffer, size_t size, const char* format, va_list va )
	{
		int32_t result = vsnprintf_s( buffer, size, _TRUNCATE, format, va );
		return result;
	}
	...
	#else
		// Microsoft Visual Studio 2003 and earlier or WinCE.
		static inline int32_t TIXML_VSCPRINTF( const char* format, va_list va )
		{
			int32_t len = 512;
			for (;;) {
				len = len*2;
...
void XMLUtil::ConvertUTF32ToUTF8( uint32_t input, char* output, int32_t* length )
{
...
void XMLDocument::PrintError() const
{
    if ( Error() ) {
        ...
        printf( "XMLDocument error id=%d '%s' str1=%s str2=%s\n",
                static_cast<int32_t>( _errorID ), ErrorName(), buf1, buf2 );
    }
}
            ...
            if ( *q > 0 && *q < ENTITY_RANGE ) {
                ...
                if ( flag[(unsigned char)(*q)] ) {
                    while ( p < q ) {
                        ...
                        // %.*s accepts type int32_t as "precision"
                        const int32_t toPrint = ( INT_MAX < delta ) ? INT_MAX : (int32_t)delta;
                        Print( "%.*s", toPrint, p );
                        p += toPrint;
                    }
