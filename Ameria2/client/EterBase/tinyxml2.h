...
/* Versioning, past 1.0.14:
	http://semver.org/
*/
static const int32_t TIXML2_MAJOR_VERSION = 3;
static const int32_t TIXML2_MINOR_VERSION = 0;
static const int32_t TIXML2_PATCH_VERSION = 0;
...
public:
    ...
    void Set( char* start, char* end, int32_t flags ) {
        Reset();
        _start  = start;
        _end    = end;
    ...
    void SetStr( const char* str, int32_t flags=0 );

    char* ParseText( char* in, const char* endTag, int32_t strFlags );
...
public:
    ...
    inline static bool StringEqual( const char* p, const char* q, int32_t nChar=INT_MAX )  {
        if ( p == q ) {
            return true;
        }
        int32_t n = 0;
    ...
    static void ConvertUTF32ToUTF8( uint32_t input, char* output, int32_t* length );
