...
{
	auto hWndHandlerHook = SetWinEventHook(EVENT_MIN, EVENT_MAX, NULL, HandleWinEvent, 0, 0, WINEVENT_OUTOFCONTEXT);
	if (!hWndHandlerHook)
	{
		return 0;
	}
...
bool CAnticheatManager::InitializeWindowWatcherThread()
{
	...
	RegisterWaitForSingleObject(&m_hWndThreadWaitObj, m_hWndCheckThread, OnWindowWatcherThreadLost, NULL, INFINITE, WT_EXECUTEONLYONCE);
