...
bool CAnticheatManager::BlockAccess(HANDLE hTarget)
{
	...
	if (!ConvertStringSecurityDescriptorToSecurityDescriptorA(szSD.c_str(), SDDL_REVISION_1, &(sa.lpSecurityDescriptor), NULL))
	{
#ifdef _DEBUG
...
{
	typedef NTSTATUS(NTAPI* TRtlAddAccessDeniedAce)(PACL pAcl, DWORD dwAceRevision, ACCESS_MASK	AccessMask, PSID pSid);
