...
void CCheatQuarentineMgr::ProcessQuarentineList(int iIdx, bool bSingle)
{
...
//	Tracenf("Process md5 scan process has been started! Size: %u", m_vProcessMd5Blacklist.size());
	...
	for (const auto & pCurrScanCtx : m_vProcessMd5Blacklist)
	{
		...
		{
//			Tracenf("Current process md5 scan list object: '%s'-'%s' List size: %u", pCurrScanCtx->strProcessName, pCurrScanCtx->strMd5, m_vProcessMd5Blacklist.size());
			...
			for (const auto & pCurrProcessCtx : m_vProcessMd5List)
			{
				if (pCurrProcessCtx && pCurrProcessCtx.get())
				{
					...
					{
//						Tracenf("Blacklist'd object found! Data: '%s'-'%s'", pCurrScanCtx->strProcessName, pCurrScanCtx->strMd5);

						// Send violation msg
						CCheatDetectQueueMgr::Instance().AppendDetection(BAD_PROCESS_DETECT, std::stoul(pCurrScanCtx->szProcessName), pCurrScanCtx->szProcessMd5);
	...
	if (bSingle == false) // If current check type is not single check it's mean all processes compared with blacklisted, we don't need anymore
	{
		m_vProcessMd5List.clear();
	}
}
