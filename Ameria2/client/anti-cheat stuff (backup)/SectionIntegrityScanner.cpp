...
typedef DWORD(NTAPI* TRtlComputeCrc32)(DWORD dwInitial, const BYTE* pData, int iLen);
...
bool IsPackedExecutable(const std::string & szName)
{
	...
	auto hFile = CreateFileA(szName.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	if (!hFile || hFile == INVALID_HANDLE_VALUE)
	{
		return bIsPacked;
	}

	auto dwFileLen = GetFileSize(hFile, NULL);
	...
	auto pImage = reinterpret_cast<BYTE*>(malloc(dwFileLen));
	if (!pImage)
	{
		CloseHandle(hFile);
		return bIsPacked;
	...
	DWORD dwReadedBytes;
	BOOL readRet = ReadFile(hFile, pImage, dwFileLen, &dwReadedBytes, NULL);
...
std::string GetModuleOwnerName(LPVOID pModuleBase)
{
	...
	auto szRealName = DosDevicePath2LogicalPath(szFileName);
	if (szRealName.empty())
		return std::string("");
...
bool GetSectionInformation(const std::string & szSectionName, DWORD dwBaseAddress, LPDWORD dwOffset, LPDWORD dwLen)
{
...
DWORD CalculateChecksum(DWORD dwSectionStart, DWORD dwSectionLen)
{
	...
	__try
	{
		dwChecksumResult = RtlComputeCrc32(0, (const BYTE*)dwSectionStart, dwSectionLen);
	}
	__except (1) { }
...
inline bool GetSectionHash(DWORD dwModuleBase, const std::string & szSectionName, LPDWORD pdwSectionHash)
{
	...
	auto szModuleOwner = GetModuleOwnerName(reinterpret_cast<LPVOID>(dwModuleBase));
	...
	{
#ifdef _DEBUG
		TraceError("Section base address or size is null! Target Module base: %p Section: %s Module: %s IsPacked result: %d",
			dwModuleBase, szSectionName.c_str(), szModuleOwner.c_str(), IsPackedExecutable(szModuleOwner));
#endif
		return false;
	}
...
bool InitSectionHash(DWORD dwModuleBase, std::shared_ptr < SModuleSectionsHashCtx> & pModuleSectionHashContainer)
{
//	TraceError("Section checksum create init started! Target module: %p", dwModuleBase);

	auto szModuleOwner = GetModuleOwnerName(reinterpret_cast<LPVOID>(dwModuleBase));
	if (szModuleOwner.empty() || IsPackedExecutable(szModuleOwner))
	{
#ifdef _DEBUG
		TraceError("Section checksum fail! Executable: %s is probably packed!", szModuleOwner.c_str());
#endif
		return false;
	}

	if (!GetSectionHash(dwModuleBase, XOR("PEHEADER"), &pModuleSectionHashContainer->dwPeHeaderHash))
	{
...
VOID NTAPI LdrEnumCallBack(IN PLDR_DATA_TABLE_ENTRY_MY DataTableEntry, IN PVOID Context, IN OUT BOOLEAN *StopEnumeration)
{
	...
	if (pldrContext->bCheck == false)
	{
		...
		if (InitSectionHash((DWORD)DataTableEntry->DllBase, pModuleSectionHashContainer))
		{
//			TraceError("Module: %ls(%p) section hash list succesfully generated!", DataTableEntry->FullDllName.Buffer, DataTableEntry->DllBase);

			pModuleSectionHashContainer->dwModuleBase = (DWORD)DataTableEntry->DllBase;
	...
	else
	{
		...
		if (InitSectionHash((DWORD)DataTableEntry->DllBase, pCurrentHashList))
		{
			...
			{
				if (pCorrectHashList->dwModuleBase == (DWORD)DataTableEntry->DllBase)
				{
