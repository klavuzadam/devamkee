...
void CAnticheatManager::OnKeyPress(int iKeyIdx, bool bStatus)
{
	m_mapKeyStatusList[iKeyIdx] = bStatus;
}
bool CAnticheatManager::GetKeyStatus(int iKeyIdx)
{
...
LRESULT CALLBACK WindowMessageHandler(int code, WPARAM wParam, LPARAM lParam)
{
...
LRESULT CALLBACK RawWindowHookMsgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) 
{
	if (CAnticheatManager::Instance().GetMainWindow() == hWnd)
	{
		...
		case WM_INPUT: 
		{
			if (GET_RAWINPUT_CODE_WPARAM(wParam) == RIM_INPUT)
			{
				...
				UINT uiDeviceNameSize = sizeof(szDeviceName);
