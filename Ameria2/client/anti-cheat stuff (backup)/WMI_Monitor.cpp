...
bool DumpWMIClassObject(int iAnalyseType, IWbemClassObject * pClassObject, int iIndentationLevel)
{
...
void CWMI_Monitor::OnWMIMessage(int analyseType, std::map <std::string /* szType */, std::string /* szValue */> mDataMap)
{
#if 0
...
DWORD CWMI_Monitor::ThreadRoutine(void)
{
	...
	hError = pLoc->ConnectServer(wszRootCim2, NULL, NULL, NULL, WBEM_FLAG_CONNECT_USE_MAX_WAIT /* 0 */, NULL, NULL, &m_pWbemServices);
	if (FAILED(hError))
	{
#ifdef _DEBUG
	...
	hError = m_pWbemServices->ExecNotificationQueryAsync(wszWQL2, wszProcessCreateQuery2, WBEM_FLAG_SEND_STATUS, NULL, &eventProcCreated);
	if (FAILED(hError))
	{
#ifdef _DEBUG
	...
	hError = m_pWbemServices->ExecNotificationQueryAsync(wszWQL2, wszThreadLoadQuery2, 0, NULL, &eventThreadCreated);
	if (FAILED(hError))
	{
#ifdef _DEBUG
	...
	hError = m_pWbemServices->ExecNotificationQueryAsync(wszWQL2, wszModuleLoadQuery2, 0, NULL, &eventModuleLoaded);
	if (FAILED(hError))
	{
#ifdef _DEBUG
	...
	hError = m_pWbemServices->ExecNotificationQueryAsync(wszWQL2, wszDriverLoadQuery2, 0, NULL, &eventDriverLoaded);
	if (FAILED(hError))
	{
#ifdef _DEBUG
	...
	hError = m_pWbemServices->ExecNotificationQueryAsync(wszWQL2, wszServiceLoadQuery2, 0, NULL, &eventServiceLoaded);
	if (FAILED(hError))
	{
#ifdef _DEBUG
...
bool CWMI_Monitor::InitializeWMIWatcherThread()
{
	...
	RegisterWaitForSingleObject(&m_hWMIWaitObj, m_hWMIThread, OnWMIMonitorThreadLost, NULL, INFINITE, WT_EXECUTEONLYONCE);
