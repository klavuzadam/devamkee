...
class CGraphicThingInstance : public CGraphicObjectInstance
{
	...
	public:
        ...
        DWORD GetMotionIndex() const
        {
            return m_dwMotionIndex;
        }

        void SetMotionIndex(const DWORD dwMotionIndex)
        {
            m_dwMotionIndex = dwMotionIndex;
        }

        static void RegisterMotionThing(const DWORD& dwMotionIndex, const DWORD& dwMotionKey, CGraphicThing* pMotionThing);
	...
	protected:
        ...
        static std::unordered_map<DWORD, std::map<DWORD, std::unique_ptr<CGraphicThing::TRef>>> m_roMotionThingMap;
        DWORD m_dwMotionIndex;
