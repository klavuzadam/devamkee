...
SHOW_LIMIT_SUPPORT_SKILL_LIST = [121, 122, 123, 124, 126, 127, 129, 128, 131, 132, 137, 138, 139, 140]
...
class CharacterWindow(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.statusPlusCountCommandDict = {
			"HTH": "/con+ %d",
			"INT": "/int+ %d",
			"STR": "/str+ %d",
			"DEX": "/dex+ %d",
	...
	def __BindObject(self):
		...
		self.attributeListButton = self.GetChild("AttributeListButton")
		...
		self.sprintOnboardingTint = self.GetChild("SprintOnboardingTint")
		self.sprintOnboardingIcon = self.GetChild("SprintOnboardingIcon")
		...
		self.questScrollBar.SAFE_SetOnWheelEvent(self.pageDict["QUEST"])
		...
		for i in xrange(quest.QUEST_MAX_NUM):
			bar = ui.Bar()
			bar.SetParent(self.GetChild("Quest_BG"))
			bar.SetPosition(7, 12 + (9 + 32) * i)
			bar.SetSize(235, 41)
			bar.SetColor(0x10ffffff if i % 2 == 0 else 0x40000000)
			bar.Show()
			self.InsertChild("quest_bg" + str(i), bar)

			name = ui.TextLine()
			name.SetParent(self.pageDict["QUEST"])
			name.SetPosition(60, 13)
			name.SetBold()
			name.Show()
			self.questNameList.append(name)

			lastTime = ui.TextLine()
			lastTime.SetParent(self.pageDict["QUEST"])
			lastTime.SetPosition(60, 25)
			lastTime.Show()
			self.questLastTimeList.append(lastTime)

			lastCount = ui.TextLine()
			lastCount.SetParent(self.pageDict["QUEST"])
			lastCount.SetPosition(0, 38)
			lastCount.SetHorizontalAlignRight()
			lastCount.SetWindowHorizontalAlignRight()
			lastCount.Show()
			self.questLastCountList.append(lastCount)
	...
	# self.questNameList.append(self.GetChild("Quest_Name_0" + str(i)))
		...
		self.__HideSprintOnboarding()
		self.sprintOnboardingIcon.SAFE_SetStringEvent("MOUSE_OVER_IN", self.__HideSprintOnboarding)
	...
	def Destroy(self):
		...
		if self.statusInputDialog:
			self.statusInputDialog.Close()
	...
	def __CreateAddStatusInputDialog(self, statusKey):
		if self.statusInputDialog:
			self.statusInputDialog.Close()

		input = uiCommon.InputDialog()
		input.SetTitle(localeInfo.FAST_STAT_ADD_TITLE)
		input.SetNumberMode()
		input.SetMaxLength(2)
		input.SetAcceptEvent(lambda: self.__OnStatusInputAccept(statusKey))
		input.SetCancelEvent(input.Close)
		input.Open()
		self.statusInputDialog = input

	def __OnStatusInputAccept(self, statusKey):
		self.SendAddStatPacket(statusKey, int(self.statusInputDialog.GetText()))
		if self.statusInputDialog:
			self.statusInputDialog.Close()
			self.statusInputDialog = None

	def SendAddStatPacket(self, statusKey, count=1):
		if count == 1:
			statusPlusCommand = self.statusPlusCommandDict[statusKey]
			net.SendChatPacket(statusPlusCommand)
		else:
			statusPlusCommand = self.statusPlusCountCommandDict[statusKey]
			net.SendChatPacket(statusPlusCommand % count)

	def __OnClickStatusPlusButton(self, statusKey):
		try:
			if app.IsPressed(app.DIK_LCONTROL):
				self.__CreateAddStatusInputDialog(statusKey)
				return
			self.SendAddStatPacket(statusKey)
		except KeyError, msg:
	...
	def SetState(self, stateKey):
		...
		self.GetChild("CSPD_Slot").SAFE_SetStringEvent("MOUSE_OVER_IN", self.__ShowCastingSpeedToolTip)
		self.GetChild("CSPD_Slot").SAFE_SetStringEvent("MOUSE_OVER_OUT", self.__HideToolTip)

		self.GetChild("SP_Slot").SAFE_SetStringEvent("MOUSE_OVER_IN", self.__ShowBonusMagicDamageToolTip)
		self.GetChild("SP_Slot").SAFE_SetStringEvent("MOUSE_OVER_OUT", self.__HideToolTip)

		self.GetChild("DEF_Slot").SAFE_SetStringEvent("MOUSE_OVER_IN", self.__ShowDefGradeToolTip)
		self.GetChild("DEF_Slot").SAFE_SetStringEvent("MOUSE_OVER_OUT", self.__HideToolTip)
	...
	def __GetTotalAtkText(self):
		...
		atkBonus += player.GetStatus(player.POINT_DAGGER_ATT_GRADE_MONSTER)
		atkBonus += player.GetStatus(player.POINT_ATT_GRADE_MONSTER)
	...
	def __GetTotalDefText(self):
		...
		defBoost = player.GetStatus(player.POINT_DEF_BONUS)
		if defBoost > 0:
			defBoostInValue = defValue * defBoost / 100
			if defBoostInValue > 0:
				defValue += defBoostInValue
				return "%d |cffd5eb4b(+%d)|r" % (defValue, defBoostInValue)

		return "%d" % (defValue)

	def __GetTotalMagicDefText(self):
		defValue = player.GetStatus(player.DEF_GRADE)
		if constInfo.ADD_DEF_BONUS_ENABLE:
			defValue += player.GetStatus(player.DEF_BONUS)
	...
	def RefreshStatus(self):
		...
		try:
			...
			self.GetChild("MDEF_Value").SetText(self.__GetTotalMagicDefText())
	...
	def __RefreshStatusPlusButtonList(self):
		...
		if statusPlusPoint>0:
			self.statusPlusLabel.SetText(localeInfo.STATUS_PLUS_LABEL % statusPlusPoint)
	...
	def RefreshCastingSpeed(self):
		gradeColor = ui.GenerateColor(colorInfo.TITLE_RGB_NORMAL[0], colorInfo.TITLE_RGB_NORMAL[1],
									  colorInfo.TITLE_RGB_NORMAL[2])
		self.toolTip.ClearToolTip()
		self.toolTip.AppendTextLine(
			localeInfo.TOOLTIP_REAL_CAST_SPEED % ((1 - player.GetCoolTimeReduction()) * 100), gradeColor)

	def RefreshMagicBonusFromSP(self):
		gradeColor = ui.GenerateColor(colorInfo.TITLE_RGB_NORMAL[0], colorInfo.TITLE_RGB_NORMAL[1],
									  colorInfo.TITLE_RGB_NORMAL[2])
		self.toolTip.ClearToolTip()
		# self.toolTip.AppendTextLine(
		# 	localeInfo.TOOLTIP_MAGIC_DAMAGE_BONUS % (int(player.GetMagicDamageBonusFromManavsMob())), gradeColor)
		self.toolTip.AppendTextLine(
			localeInfo.TOOLTIP_MAGIC_DAMAGE_BONUS_PC % (int(player.GetMagicDamageBonusFromSP())), gradeColor)
		self.toolTip.AppendTextLine(
			localeInfo.TOOLTIP_MAGIC_BONUS_SP % (int(player.GetBonusSP())), gradeColor)
	...
	def __SelectQuest(self, slotIndex):
		if quest.IsExist(self.questShowingStartIndex + slotIndex):
	...
	def RefreshQuest(self):
		...
		if questCount > quest.QUEST_MAX_NUM:
			...
			progress = float(quest.QUEST_MAX_NUM) / questCount
			self.questScrollBar.SetMiddleBarSize(progress)
		else:
			self.questScrollBar.Hide()

		for i in questRange[:questCount]:
			if not quest.IsExist(self.questShowingStartIndex + i):
				continue
			...
			self.questNameList[i].SetPosition(60, 13 + step_per_quest * i)
			self.questNameList[i].Show()

			self.questLastTimeList[i].Show()
			self.questLastTimeList[i].SetPosition(60, 25 + step_per_quest * i)

			self.questLastCountList[i].Show()
			self.questLastCountList[i].SetPosition(60, 38 + step_per_quest * i)

			if len(questCounterName) > 0:
				new_text, value = utils.process_string(questCounterName, "m")
				self.questLastCountList[i].SetText("%s : %d" % (new_text, questCounterValue))
			else:
			...
			for i in xrange(min(quest.GetQuestCount(), quest.QUEST_MAX_NUM)):
				...
				if len(lastName) > 0:
					...
					if lastTime <= 0:
						...
						clockText = lastName + ": " + localeInfo.SecondToDHMS(lastTime, True)
	...
	def __ShowStatToolTip(self, statDesc):
		...
		self.toolTip.AppendTextLine(localeInfo.TOOLTIP_FAST_STAT_ADD)
		self.toolTip.Show()
	...
	def __ShowCastingSpeedToolTip(self):
		self.RefreshCastingSpeed()
		self.toolTip.ShowToolTip()

	def __ShowBonusMagicDamageToolTip(self):
		self.RefreshMagicBonusFromSP()
		self.toolTip.ShowToolTip()

	def __ShowDefGradeToolTip(self):
		self.toolTip.ClearToolTip()
		defValue = player.GetStatus(player.DEF_GRADE)
		if constInfo.ADD_DEF_BONUS_ENABLE:
			defValue += player.GetStatus(player.DEF_BONUS)
		...
		defBoost = player.GetStatus(player.POINT_DEF_BONUS)
		self.toolTip.AppendTextLine(localeInfo.TOOLTIP_DEF_GRADE_CHARACTER_WINDOW % (defValue, defBoost))
		self.toolTip.ShowToolTip()
	...
	def __HideSprintOnboarding(self):
		self.sprintOnboardingTint.Hide()

	def ShowSprintOnboarding(self):
		self.sprintOnboardingTint.Show()
