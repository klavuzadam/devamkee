...
class FancyBoard(ui.Bar):

	SCROLL_WAIT_TIME = 3.0
	MIN_TIP_DURATION = 4.0
	FONT_HEIGHT = 18
	STEP_HEIGHT = 34
	...
	def __init__(self):
		...
		self.tip_duration = self.MIN_TIP_DURATION

		self.SetPosition(0, 110)
		self.SetSize(wndMgr.GetScreenWidth(), 37)
		self.SetColor(grp.GenerateColor(0.0, 0.0, 0.0, 0.5))
		self.SetWindowHorizontalAlignCenter()
		self.__CreateTextBar()
	...
	def __CreateTextBar(self):
		...
		self.textBar.handle = grp.CreateBigTextBar(self.width, 300, self.FONT_HEIGHT)
		self.textBar.SetParent(self)
		self.textBar.SetPosition(3, 5)
		self.textBar.SetClipRect(0, y, wndMgr.GetScreenWidth(), y+18+20)
		self.textBar.SetTextColor(242, 231, 193)
		self.textBar.Show()
	...
	def __RefreshBoard(self):
		...
		for tip in self.tipList:
			text = tip[1]
			(text_width, text_height) = self.textBar.GetTextExtent(text)
			self.textBar.TextOut((self.width-text_width)/2, index*self.STEP_HEIGHT, text)
			index += 1

	def SetTip(self, text, is_gm):

		if not app.IsVisibleNotice():
			return

		if is_gm:
			self.textBar.SetTextColor(255, 153, 0)
		else:
			self.textBar.SetTextColor(242, 231, 193)
		...
		self.tip_duration = max(self.tip_duration, 0.093 * len(text))
