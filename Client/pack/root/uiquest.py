...
class BarButton(ui.Button):
	...
	def SetText(self, text):
		new_text, value = utils.process_string(text, "i")
		if value != None:
			self.itemVnum = value

		new_text, value = utils.process_string(new_text, "m")

		ui.Button.SetText(self, new_text)
	...
	def ShowToolTip(self):
		...
		if self.itemToolTip and self.itemVnum > 0:
			self.itemToolTip.SetRawItem(self.itemVnum)
...
class QuestDialog(ui.ScriptWindow):
	...
	def MakeEachButton(self, i):
		if self.skin == 3:
			...
			button.SetToolTip(self.itemTooltip)
