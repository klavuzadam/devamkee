...
class SelectCharacterWindow(ui.Window):
	...
	def OnCreateFailure(self, type):
		...
		if 0 == type:
			...
			self.PopupMessage(localeInfo.SELECT_CHANGE_FAILURE_SAME_AS_LOGIN)
		elif 100 == type:
	...
	def OnDeleteFailure(self, error_type):
		error_dict = {
			1: localeInfo.SELECT_CANNOT_DELETE_WRONG_SOCIAL_ID,
			2: localeInfo.SELECT_CANNOT_DELETE_LEVEL_LIMIT,
			3: localeInfo.SELECT_CANNOT_DELETE_USING_OFFLINESHOP,
		}
		if error_dict.has_key(error_type):
			self.PopupMessage(error_dict[error_type])
		else:
