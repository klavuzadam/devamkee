import flamewindPath

ROOT = "d:/ymir work/ui/minimap/"

window = {
	"name": "MiniMap",

	"x": SCREEN_WIDTH - 136,
	"y": 0,

	"width": 136,
	"height": 137,

	"children":
		(
			## OpenWindow
			{
				"name": "OpenWindow",
				"type": "window",

				"x": 0,
				"y": 0,

				"width": 136,
				"height": 137,

				"children":
					(
						{
							"name": "OpenWindowBGI",
							"type": "image",
							"x": 0,
							"y": 0,
							"image": ROOT + "minimap.sub",
						},
						## MiniMapWindow
						{
							"name": "MiniMapWindow",
							"type": "window",

							"x": 4,
							"y": 5,

							"width": 128,
							"height": 128,
						},
						## ScaleUpButton
						{
							"name": "ScaleUpButton",
							"type": "button",

							"x": 101,
							"y": 116,

							"default_image": ROOT + "minimap_scaleup_default.sub",
							"over_image": ROOT + "minimap_scaleup_over.sub",
							"down_image": ROOT + "minimap_scaleup_down.sub",
						},
						## ScaleDownButton
						{
							"name": "ScaleDownButton",
							"type": "button",

							"x": 115,
							"y": 103,

							"default_image": ROOT + "minimap_scaledown_default.sub",
							"over_image": ROOT + "minimap_scaledown_over.sub",
							"down_image": ROOT + "minimap_scaledown_down.sub",
						},
						## MiniMapHideButton
						{
							"name": "MiniMapHideButton",
							"type": "button",

							"x": 111,
							"y": 6,

							"default_image": ROOT + "minimap_close_default.sub",
							"over_image": ROOT + "minimap_close_over.sub",
							"down_image": ROOT + "minimap_close_down.sub",
						},
						## AtlasShowButton
						{
							"name": "AtlasShowButton",
							"type": "button",

							"x": 12,
							"y": 12,

							"default_image": ROOT + "atlas_open_default.sub",
							"over_image": ROOT + "atlas_open_over.sub",
							"down_image": ROOT + "atlas_open_down.sub",
						},

						## ItemShopButton
						{
							# "name" : "ItemShopButton",
							"type": "ani_image",

							"x": 8,
							"y": 104,

							"delay": 7,

							"images":
								(
									flamewindPath.GetItemShop("is_button1"),
									flamewindPath.GetItemShop("is_button2"),
									flamewindPath.GetItemShop("is_button3"),
									flamewindPath.GetItemShop("is_button4"),
									flamewindPath.GetItemShop("is_button5"),
									flamewindPath.GetItemShop("is_button6"),
									flamewindPath.GetItemShop("is_button7"),
									flamewindPath.GetItemShop("is_button8"),
									flamewindPath.GetItemShop("is_button9"),
									flamewindPath.GetItemShop("is_button10"),
									flamewindPath.GetItemShop("is_button11"),
									flamewindPath.GetItemShop("is_button12"),
									flamewindPath.GetItemShop("is_button13"),
									flamewindPath.GetItemShop("is_button14"),
									flamewindPath.GetItemShop("is_button15"),
									flamewindPath.GetItemShop("is_button16"),
									flamewindPath.GetItemShop("is_button17"),
								),
						},

						{
							"name": "ItemShopButton",
							"type": "window",

							"x": 8, "y": 104,
							"width": 25, "height": 25,
						},

						## ServerInfo
						{
							"name": "ServerInfo",
							"type": "text",

							"text_horizontal_align": "center",

							"outline": 1,

							"x": 70,
							"y": 140,

							"text": "",
						},
						## PositionInfo
						{
							"name": "PositionInfo",
							"type": "text",

							"text_horizontal_align": "center",

							"outline": 1,

							"x": 70,
							"y": 160,

							"text": "",
						},

						{
							"name": "FPSInfo",
							"type": "text",

							"text_horizontal_align": "center",

							"outline": 1,

							"x": 70,
							"y": 176,

							"fontname": "Tahoma:20",

							"text": "",
						},

						## ObserverCount
						{
							"name": "ObserverCount",
							"type": "text",

							"text_horizontal_align": "center",

							"outline": 1,

							"x": 70,
							"y": 180,

							"text": "",
						},

						{
							"name": "SpyInfo",
							"type": "text",

							"text_horizontal_align": "center",

							"outline": 1,

							"x": 70,
							"y": 190,

							"text": "(Spy Mode)",

							"color": 0xff836c94,
						},
					),
			},
			{
				"name": "CloseWindow",
				"type": "window",

				"x": 0,
				"y": 0,

				"width": 132,
				"height": 48,

				"children":
					(
						## ShowButton
						{
							"name": "MiniMapShowButton",
							"type": "button",

							"x": 100,
							"y": 4,

							"default_image": ROOT + "minimap_open_default.sub",
							"over_image": ROOT + "minimap_open_default.sub",
							"down_image": ROOT + "minimap_open_default.sub",
						},
					),
			},
		),
}
