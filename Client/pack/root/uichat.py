...
class ChatLine(ui.EditLine):
	...
	CHAT_MODE_NAME = {	chat.CHAT_TYPE_TALKING : localeInfo.CHAT_NORMAL,
						...
						chat.CHAT_TYPE_TRADE : localeInfo.CHAT_TRADE}
	...
	def __CanShowShoutTip(self):
		return not gameSettings.GetBoolKey("first_shout_tip")
	...
	def __SendShoutChatPacket(self, text):
		...
		if self.__CanShowShoutTip():
			eventManager.EventManager().send_event(uiCommon.POPUP_EVENT, localeInfo.FIRST_SHOUT_TIP)
			gameSettings.SetKey("first_shout_tip", True)

		self.lastShoutTime = app.GetTime()

	def __SendTradeChatPacket(self, text):

		if 1 == len(text):
			self.RunCloseEvent()
			return

		if self.lastTradeTime and app.GetTime() < self.lastTradeTime + 15:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.CHAT_TRADE_LIMIT)
			self.__ResetChat()
			return

		self.__SendChatPacket(text[1:], chat.CHAT_TYPE_TRADE)
		self.__ResetChat()

		self.lastTradeTime = app.GetTime()
	...
	def __PrevLastSentenceStack(self):
		sentence_stack = constInfo.CHAT_LAST_SENTENCE_STACK
	...
	def __PushLastSentenceStack(self, text):
		...
		if len(constInfo.CHAT_LAST_SENTENCE_STACK) > LAST_SENTENCE_STACK_SIZE:
			constInfo.CHAT_LAST_SENTENCE_STACK.pop(0)

		constInfo.CHAT_LAST_SENTENCE_STACK.append(text)
		# END_OF_LAST_SENTENCE_STACK
...
class ChatWindow(ui.Window):
	...
	def __RegisterChatColorDict(self):
		CHAT_COLOR_DICT = {
			...
			chat.CHAT_TYPE_TRADE: colorInfo.CHAT_RGB_TRADE,
		}
