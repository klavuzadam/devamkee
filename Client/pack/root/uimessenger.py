...
EVENT_UPDATE_BLOCK_STATE = "EVENT_UPDATE_BLOCK_STATE" # args: name: string, isBlock: boolean
...
class MessengerBlockedItem(MessengerMemberItem):
	...
	def OnRemove(self):
		net.SendMessengerUnblockPacket(self.key)
		return True
...
class MessengerBlockedGroup(MessengerGroupItem):

	def __init__(self, getParentEvent):
		MessengerGroupItem.__init__(self, getParentEvent)
		self.SetName(localeInfo.MESSENGER_BLOCKED)

	def AppendMember(self, key, name):
		item = MessengerBlockedItem(self.getParentEvent)
		return MessengerGroupItem.AppendMember(self, item, key, name)
...
class MessengerWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		self.blockButton.SetPosition(-30, 30)
	...
	def __AddGroup(self):
		...
		member = MessengerBlockedGroup(ui.__mem_func__(self.GetSelf))
		member.Open()
		member.Show()
		self.groupList.append(member)
	...
	def OnPressAddFriendButton(self):
		...
		friendNameBoard.SetMaxLength(chr.PLAYER_NAME_MAX_LEN)
	...
	def OnPressBlockButton(self):
		blockBoard = uiCommon.InputDialog()
		blockBoard.SetTitle(localeInfo.MESSENGER_BLOCK)
		blockBoard.SetAcceptEvent(ui.__mem_func__(self.OnBlockPlayer))
		blockBoard.SetCancelEvent(ui.__mem_func__(self.OnCloseInputDialog))
		blockBoard.SetMaxLength(chr.PLAYER_NAME_MAX_LEN)
		blockBoard.Open()
		self.inputDialog = blockBoard

	def OnBlockPlayer(self):
		text = self.inputDialog.GetText()
		if text:
			net.SendMessengerBlockPacket(text)
	...
	def OnRemoveList(self, groupIndex, key):
		if groupIndex == messenger.GROUP_INDEX_BLOCK:
			eventManager.EventManager().send_event(EVENT_UPDATE_BLOCK_STATE, key, False)

		print "OnRemoveList", groupIndex, key
		group = self.groupList[groupIndex]
	...
	def OnLogout(self, groupIndex, key, name=None):
		if groupIndex == messenger.GROUP_INDEX_BLOCK:
			eventManager.EventManager().send_event(EVENT_UPDATE_BLOCK_STATE, key, True)
