...
class GameButtonWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self, filename):
		...
		try:
			self.gameButtonDict={
				...
				"EXIT_WAR" : self.GetChild("GuildWarExit"),
				"EXIT_OBSERVER" : self.GetChild("ExitObserver"),
			}

			self.gameButtonDict["EXIT_WAR"].SetEvent(ui.__mem_func__(self.__OnClickExitWar))
	...
	def __OnClickExitWar(self):
		if self.questionDialog:
			self.questionDialog.Close()
			self.questionDialog = None

		dlg = uiCommon.QuestionDialog()
		dlg.SetText(localeInfo.EXIT_GUILD_WAR_ASK)
		dlg.SAFE_SetAcceptEvent(self.__OnAcceptExitWar)
		dlg.SetDefaultCancelEvent()
		dlg.Open()
		self.questionDialog = dlg

	def __OnAcceptExitWar(self):
		net.SendChatPacket("/guild_exit_war")
		self.questionDialog.Close()
		self.questionDialog = None
	...
	def SetWarMode(self, isEnable):
		if player.IsObserverMode():
			return

		if isEnable:
			self.gameButtonDict["EXIT_WAR"].Show()
		else:
			self.gameButtonDict["EXIT_WAR"].Hide()
