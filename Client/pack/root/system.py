...
def __hybrid_import(name,globals=None,locals=None,fromlist=None, level=-1):
	if __USE_CYTHON__:
		if rootlib.isExist(name):
			...
			return newmodule
		elif liblib.isExist(name):
			if name in sys.modules:
				dbg.Tracen('importing from sys.modules %s' % name)
				return sys.modules[name]

			dbg.Tracen('importing from liblib %s' % name)
			newmodule = liblib.moduleImport(name)
...
import gameSettings
if not gameSettings.GAME_SETTINGS.has_key("loaded"):
	gameSettings.LoadSettings()

import gamefont
gamefont.SetFontOnGameStart()
