...
void CItemData::SetLocaleName(const std::string& c_rstName)
{
	m_strLocaleName = c_rstName;
}
...
const char* CItemData::GetOriginalName() const
{
...
{
	if (m_strLocaleName.length() > 0)
		return m_strLocaleName.c_str();
...
DWORD CItemData::GetMaxStack() const
{
	return m_ItemTable.dwMaxStack;
}
...
YANG CItemData::GetIBuyItemPrice() const
{
	return m_ItemTable.dwIBuyItemPrice;
}

YANG CItemData::GetISellItemPrice() const
{
