...
class CActorInstance : public IActorInstance, public IFlyTargetableObject
{
	public:
		class IEventHandler
		{
			...
			public:
				...
				virtual void OnHit(UINT uSkill, CActorInstance& rkActorVictim, BOOL isSendPacket, int iHitType) = 0;
	...
	public:
		...
		void AttachWeapon(DWORD dwItemIndex,DWORD dwParentPartIndex = CRaceData::EQUIP_PART_MAIN, DWORD dwPartIndex = CRaceData::EQUIP_PART_WEAPON);
		void AttachWeapon(DWORD dwParentPartIndex, DWORD dwPartIndex, CItemData * pItemData);
		void AttachItem(CItemData* pItemData, BYTE bPart, const char* c_pBone, float fSpecular = 0.0f);
	...
	public:
		...
		void ClearFlyShootTargets();
		void AddFlyShootTarget(const CFlyTarget& cr_FlyTarget);
	...
	public:
		...
		DWORD GetLastGreatHitTime() { return m_dwLastGreatHitTime; }
		bool CanBePushedByGreatHit();
	...
	protected:
		...
		std::vector<CFlyTarget> m_vector_kFlyTarget;
	...
	protected:
		...
		void CreateFlyingInstance(const CRaceMotionData::TMotionFlyEventData* c_pFlyData, const CFlyTarget& cr_FlyTarget);
	...
	protected:
		...
		// Part
		DWORD						m_adwPartItemID[CRaceData::EQUIP_PART_MAX_NUM];
	...
	protected:
		void	__OnHit(UINT uSkill, CActorInstance& rkInstVictm, BOOL isSendPacket, int iHitType);
