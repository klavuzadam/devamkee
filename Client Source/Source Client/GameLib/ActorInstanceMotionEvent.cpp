...
void CActorInstance::ProcessMotionEventFly(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	if (m_kFlyTarget.IsValidTarget())
	{
		CreateFlyingInstance(c_pFlyData, m_kFlyTarget);
	...
	// ninja shoot to multiple targets
	if (!m_vector_kFlyTarget.empty()) {
		for (CFlyTarget& target : m_vector_kFlyTarget)
		{
			if (target.IsValidTarget())
			{
				CreateFlyingInstance(c_pFlyData, target);
			}
		}
		m_vector_kFlyTarget.clear();
	}
}

void CActorInstance::CreateFlyingInstance(const CRaceMotionData::TMotionFlyEventData* c_pFlyData, const CFlyTarget& cr_FlyTarget)
{
	...
	if (quiverEffect)
		pInstance = rfm.CreateIndexedFlyingInstanceFlyTarget(GetQuiverEffectID(), v3Start, cr_FlyTarget);
	else
		pInstance = rfm.CreateFlyingInstanceFlyTarget(c_pFlyData->dwFlyIndex, v3Start, cr_FlyTarget, true);
#else
	CFlyingInstance* pInstance = rfm.CreateFlyingInstanceFlyTarget(c_pFlyData->dwFlyIndex, v3Start, cr_FlyTarget, true);
#endif
