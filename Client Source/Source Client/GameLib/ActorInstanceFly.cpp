...
void CActorInstance::ClearFlyShootTargets()
{
	m_vector_kFlyTarget.clear();
}
...
void CActorInstance::AddFlyShootTarget(const CFlyTarget& cr_FlyTarget)
{
	if (m_kFlyTarget.IsValidTarget()) {
		m_vector_kFlyTarget.emplace_back(cr_FlyTarget);
	}
	else
