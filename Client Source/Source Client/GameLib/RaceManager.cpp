...
void CRaceManager::SetScaleData(DWORD dwRaceIndex, float fx, float fy, float fz)
{
	CRaceData* targetRace;
	if (!GetRaceDataPointer(dwRaceIndex, &targetRace))
	{
		Tracef("CRaceManager::SetScaleData Not RaceData by %d vnum!\n", dwRaceIndex);
		return;
	}

	targetRace->SetScale(fx, fy, fz);
}
