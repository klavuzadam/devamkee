...
bool CMapManager::LoadMap(const std::string & c_rstrMapName, const std::string& c_rstrPathName, float x, float y, float z)
{
	CMapOutdoor& rkMap = GetMapOutdoorRef();

	rkMap.Leave();
	rkMap.SetName(c_rstrMapName);
	rkMap.SetPath(c_rstrPathName);
	rkMap.LoadProperty();
...
bool CMapManager::UnloadMap(const std::string c_strMapPath)
{
	CMapOutdoor& rkMap=GetMapOutdoorRef();
	if (c_strMapPath != rkMap.GetPath() && "" != rkMap.GetPath())
	{
		LogBoxf("%s: Unload Map Failed", c_strMapPath.c_str());
		return false;
	}
...
void CMapManager::__LoadMapInfoVector()
{
	...
	std::string mapDirectioryPath = "maps/";
	char szMapName[256];
	int x, y;
	int width, height;
	for (UINT uLineIndex=0; uLineIndex<textFileLoader.GetLineCount(); ++uLineIndex)
	{
		...
		kMapInfo.m_strPath = mapDirectioryPath + szMapName;
