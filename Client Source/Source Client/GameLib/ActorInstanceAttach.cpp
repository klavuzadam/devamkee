...
void CActorInstance::AttachWeapon(DWORD dwItemIndex,DWORD dwParentPartIndex, DWORD dwPartIndex)
{
	...
	if (!CItemManager::Instance().GetItemDataPointer(dwItemIndex, &pItemData))
	{
		...
		RegisterModelThing(CRaceData::EQUIP_PART_WEAPON_LEFT, NULL);
		SetModelInstance(CRaceData::EQUIP_PART_WEAPON_LEFT, CRaceData::EQUIP_PART_WEAPON_LEFT, 0);
	...
	if (__IsRightHandWeapon(pItemData->GetWeaponType()))
		AttachWeapon(dwParentPartIndex, CRaceData::EQUIP_PART_WEAPON, pItemData);
	if (__IsLeftHandWeapon(pItemData->GetWeaponType()))
		AttachWeapon(dwParentPartIndex, CRaceData::EQUIP_PART_WEAPON_LEFT, pItemData);
}

void CActorInstance::AttachItem(CItemData* pItemData, BYTE bPart, const char* c_pBone, float fSpecular)
{
	if (!pItemData)
	{
		RegisterModelThing(bPart, NULL);
		SetModelInstance(bPart, bPart, 0);
		RefreshActorInstance();
		return;
	}

	RegisterModelThing(bPart, pItemData->GetModelThing());
	SetModelInstance(bPart, bPart, 0);
	AttachModelInstance(CRaceData::EQUIP_PART_MAIN, c_pBone, bPart);
	...
	if (fSpecular > 0.0f)
	{
		...
		SetMaterialData(bPart, NULL, kMaterialData);
	}
}
...
//	assert(m_pkCurRaceData);
	...
	if ((GetRace()==8) && (pItemData->GetType()==ITEM_ROD || pItemData->GetType()==ITEM_PICK))
	{
	...
	if (CRaceData::EQUIP_PART_WEAPON_LEFT == dwPartIndex)
	{
	...
	for (DWORD i = 0; i < m_pkCurRaceData->GetAttachingDataCount(); ++i)
	{
		...
		switch (c_pAttachingData->dwType)
		{
			case NRaceData::ATTACHING_DATA_TYPE_COLLISION_DATA:
			{
				...
				if (!CreateCollisionInstancePiece(CRaceData::EQUIP_PART_MAIN, c_pAttachingData, &PointInstance))
					continue;
...
//				}
	...
	for (DWORD j = 0; j < CRaceData::EQUIP_PART_MAX_NUM; ++j)
	{
...
void CActorInstance::AttachAcce(DWORD dwParentPartIndex, DWORD dwPartIndex, CItemData* pItemData)
{
	...
	if (CRaceData::EQUIP_PART_ACCE == dwPartIndex)
		RegisterModelThing(dwPartIndex, pItemData->GetSubModelThing());
	else
		RegisterModelThing(dwPartIndex, pItemData->GetModelThing());
	...
	AttachModelInstance(0, "Bip01 Spine2", CRaceData::EQUIP_PART_ACCE);
...
{
	if (dwPartIndex >= CRaceData::EQUIP_PART_MAX_NUM)
		return;
	...
	if (!CItemManager::Instance().GetItemDataPointer(dwItemIndex, &pItemData) || dwItemIndex == 0)
	{
		...
		RegisterModelThing(CRaceData::EQUIP_PART_ACCE, NULL);
		SetModelInstance(CRaceData::EQUIP_PART_ACCE, CRaceData::EQUIP_PART_ACCE, 0);

		RefreshActorInstance();
		return;
	}

	AttachAcce(dwParentPartIndex, CRaceData::EQUIP_PART_ACCE, pItemData);
}
#endif
