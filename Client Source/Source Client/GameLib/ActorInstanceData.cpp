...
bool CActorInstance::SetRace(DWORD eRace)
{
	...
	if( IsPC())
	{
		CGraphicThingInstance::ReserveModelThing(CRaceData::EQUIP_PART_MAX_NUM);
		CGraphicThingInstance::ReserveModelInstance(CRaceData::EQUIP_PART_MAX_NUM);
	}
	else
	{
...
)
{
	...
	if (pkHair)
	{
		if (!pkHair->m_stModelFileName.empty())
		{
			...
			RegisterModelThing(CRaceData::EQUIP_PART_HAIR, pkHairThing);
			SetModelInstance(CRaceData::EQUIP_PART_HAIR, CRaceData::EQUIP_PART_HAIR, 0, CRaceData::EQUIP_PART_MAIN);
		}
		...
		for (i = c_rkVct_kSkin.begin(); i != c_rkVct_kSkin.end(); ++i)
		{
			...
			if (pkRes)
			{
				...
				if (fSpecular > 0.0f)
				{
					...
					SetMaterialData(CRaceData::EQUIP_PART_HAIR, c_rkSkinItem.m_stSrcFileName.c_str(), kMaterialData);
				}
				else
				#endif
					SetMaterialImagePointer(CRaceData::EQUIP_PART_HAIR, c_rkSkinItem.m_stSrcFileName.c_str(), static_cast<CGraphicImage*>(pkRes));
			}
		}
	}
}
...
DWORD CActorInstance::GetPartItemID(DWORD dwPartIndex)
{
	...
	{
		TraceError("CActorInstance::GetPartIndex(dwPartIndex=%d/CRaceData::PART_MAX_NUM=%d)", dwPartIndex, CRaceData::EQUIP_PART_MAX_NUM);
		return 0;
	}
