...
float CActorInstance::__GetInvisibleTimeAdjust(const UINT uiSkill, const NRaceData::TAttackData& c_rAttackData) {
	...
	bool isAdjustableRace = GetRace() == MAIN_RACE_SHAMAN_W || GetRace() == MAIN_RACE_SHAMAN_M || GetRace() == MAIN_RACE_ASSASSIN_M || GetRace() == MAIN_RACE_ASSASSIN_W;
...
bool CActorInstance::__CanPushDestActor(CActorInstance& rkActorDst)
{
	...
	if (rkActorDst.IsResistFallen())
		return false;

	return true;
}

bool CActorInstance::CanBePushedByGreatHit()
{
	if (IsResistFallen())
		return false;

	const auto currentDwordTime = (DWORD)(CTimer::Instance().GetCurrentSecond() * 1000);
	return GetLastGreatHitTime() < currentDwordTime;
}
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	if (__CanPushDestActor(rVictim) && c_rAttackData.fExternalForce > 0.0f)
	{
		...
		if (c_rAttackData.iHittingType != HIT_TYPE_GOOD)
		{
	...
	__OnHit(uiSkill, rVictim, isSendPacket, c_rAttackData.iHittingType);
}
