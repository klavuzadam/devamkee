...
class CGraphicTextInstance
{
	...
	public:
		static void Hyperlink_UpdateMousePos(int x, int y, float scale);
	...
	public:
		...
		void SetFormattedValue(const char* c_szValue, size_t len = -1);
		...
		void SetUpdate(bool update) { m_isUpdate = update; };
		void SetLarge(bool bLarge) { m_isLarge = bLarge; }

		void GetTextSize(int* pRetWidth, int* pRetHeight);
		const std::string& GetValueStringReference();
		const std::string GetValueString() { return m_stText; }
		const std::string GetText();
		const bool IsUpdate() { return m_isUpdate; }
		const bool IsLarge() { return m_isLarge; }
