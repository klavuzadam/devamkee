...
void CGraphicTextInstance::Hyperlink_UpdateMousePos(int x, int y, float scale)
{
	gs_mx = x / scale;
	gs_my = y / scale;
	gs_hyperlinkText = L"";
...
void CGraphicTextInstance::Render(RECT * pClipRect)
{
	...
	if (m_isCursor)
	{
		...
		if (pClipRect)
		{
			sx = fMINMAX(pClipRect->left, sx, pClipRect->right);
			sy = fMINMAX(pClipRect->top, sy, pClipRect->bottom);
			ex = fMINMAX(pClipRect->left, ex, pClipRect->right);
			ey = fMINMAX(pClipRect->top, ey, pClipRect->bottom);
		}
...
void CGraphicTextInstance::SetFormattedValue(const char* c_szText, size_t len)
{
	if (0 == m_stFormattedText.compare(c_szText))
		return;

	m_stFormattedText = c_szText;
	m_isUpdate = false;
}
...
const std::string CGraphicTextInstance::GetText()
{
	if (m_stFormattedText.empty())
		return m_stText;
	return m_stFormattedText;
}
