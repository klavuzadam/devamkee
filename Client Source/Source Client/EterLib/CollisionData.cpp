...
/////////////////////////////////////////////
// Base
std::shared_ptr<CBaseCollisionInstance> CBaseCollisionInstance::BuildCollisionInstance(const CStaticCollisionData * c_pCollisionData, const D3DXMATRIX * pMat)
{
	switch(c_pCollisionData->dwType)
	{
		case COLLISION_TYPE_PLANE:
			{
				std::shared_ptr<CPlaneCollisionInstance> ppci(
					gs_pci.Alloc(),
					[](CPlaneCollisionInstance* ptr) {
						gs_pci.Free(ptr);
					});
		...
		case COLLISION_TYPE_BOX:
			{
				std::shared_ptr<CAABBCollisionInstance> paci(
					gs_aci.Alloc(),
					[](CAABBCollisionInstance* ptr) {
						gs_aci.Free(ptr);
					});
		...
		case COLLISION_TYPE_OBB:
			{
				std::shared_ptr<COBBCollisionInstance> poci(
					gs_oci.Alloc(),
					[](COBBCollisionInstance* ptr) {
						gs_oci.Free(ptr);
					});
		...
		case COLLISION_TYPE_SPHERE:
			{
				std::shared_ptr<CSphereCollisionInstance> psci(
					gs_sci.Alloc(),
					[](CSphereCollisionInstance* ptr) {
						gs_sci.Free(ptr);
					});
		...
		case COLLISION_TYPE_CYLINDER:
			{
				std::shared_ptr<CCylinderCollisionInstance> pcci(
					gs_cci.Alloc(),
					[](CCylinderCollisionInstance* ptr) {
						gs_cci.Free(ptr);
					});
