...
DWORD UI::CSlotWindow::GetBindedItemIndex(DWORD dwIndex)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
		return 0;
...
void UI::CSlotWindow::SetMaskColor(DWORD dwIndex, float r, float g, float b, float a)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
		return;

	DWORD val = a > 0 ? CPythonGraphic::Instance().GetColor(r, g, b, a) : 0;
	pSlot->m_dwMaskColor = val;
}
...
void CSlotWindow::OnRender()
{
	...
	for (itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (rSlot.m_dwMaskColor > 0)
		{
			CPythonGraphic::Instance().SetDiffuseColor(rSlot.m_dwMaskColor);
			CPythonGraphic::Instance().RenderBar2d(m_rect.left + rSlot.ixPosition,
				m_rect.top + rSlot.iyPosition,
