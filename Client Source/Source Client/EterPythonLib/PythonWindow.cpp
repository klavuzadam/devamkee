...
namespace UI
{
	...
	bool CWindow::IsGameWindow()
	{
		return strcmp(GetName(), "game") == 0;
	}
	...
	void CLine::OnRender()
	{
		...
		if (m_bEnableMask) {
			CPythonGraphic::Instance().RenderLine2d(MINMAX(m_rMaskRect.left, m_rect.left, m_rMaskRect.right),
				MINMAX(m_rMaskRect.top, m_rect.top, m_rMaskRect.bottom),
				MINMAX(m_rMaskRect.left, m_rect.right, m_rMaskRect.right),
				MINMAX(m_rMaskRect.top, m_rect.bottom, m_rMaskRect.bottom));
		}
	...
	void UI::CTextLine::SetOutlineColor(DWORD dwColor)
	{
	...
	BOOL CImageBox::LoadImage(const char * c_szFileName)
	{
		...
		return LoadImageFromGraphicImage(static_cast<CGraphicImage*>(pResource));
	}

	BOOL CImageBox::LoadImageFromGraphicImage(CGraphicImage* pGraphicImage)
	{
		OnCreateInstance();

		if (!pGraphicImage)
			return FALSE;
		if (!pGraphicImage->IsType(CGraphicImage::Type()))
			return FALSE;

		m_pImageInstance->SetImagePointer(static_cast<CGraphicImage*>(pGraphicImage));
