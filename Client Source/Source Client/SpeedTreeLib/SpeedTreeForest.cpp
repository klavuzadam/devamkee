...
CSpeedTreeForest::SpeedTreeWrapperPtr CSpeedTreeForest::CreateInstance(float x, float y, float z, DWORD dwTreeCRC, const char * c_szTreeName)
{
	...
	SpeedTreeWrapperPtr pTreeInst;

	TTreeMap::iterator itor = tree_map.find(dwTreeCRC);
	if (itor == tree_map.end()) {//create sub tre
		SpeedTreeWrapperPtr pTreeInst = pMainTree->MakeInstance();
		pTreeInst->SetPosition(0, 0, 0);
		//pTreeInst->RegisterBoundingSphere();
		tree_map.insert(std::map<DWORD, SpeedTreeWrapperPtr>::value_type(dwTreeCRC, pTreeInst));
	}

	TTreeMap::iterator itor_finish = tree_map.find(dwTreeCRC);
	if (itor_finish != tree_map.end()) {//is_it
		SpeedTreeWrapperPtr pToClone = itor_finish->second;
		pTreeInst = pMainTree->MakeClone(pToClone, x, y, z);
