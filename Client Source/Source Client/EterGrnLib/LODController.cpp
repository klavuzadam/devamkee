...
#ifdef MT2009_CINEMATIC
static const float LOD_APPLY_MAX = 50000.0f;
static const float LOD_APPLY_MIN = 50000.0f;
#else
...
void CGrannyLODController::SetScale(const D3DVECTOR& scale, const D3DVECTOR& scale_pos)
{
	std::deque<CGrannyModelInstance*>::iterator i;
	for (i = m_que_pkModelInst.begin(); i != m_que_pkModelInst.end(); ++i)
	{
		CGrannyModelInstance* pkModelInst = (*i);
		pkModelInst->SetScale(scale, scale_pos);
	}
}

void CGrannyLODController::SetRotation(const D3DVECTOR& rotation)
{
	std::deque<CGrannyModelInstance*>::iterator i;
	for (i = m_que_pkModelInst.begin(); i != m_que_pkModelInst.end(); ++i)
	{
		CGrannyModelInstance* pkModelInst = (*i);
		pkModelInst->SetRotation(rotation);
	}
}
