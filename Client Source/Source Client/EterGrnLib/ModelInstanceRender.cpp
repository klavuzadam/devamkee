...
void CGrannyModelInstance::RenderMeshNodeListWithOneTexture(CGrannyMesh::EType eMeshType, CGrannyMaterial::EType eMtrlType)
{
	...
	while (pMeshNode)
	{
		...
		while (pTriGroupNode)
		{
			...
			if (!material_data.pImage) {
				if (std::fabs(rkMtrl.GetSpecularPower() - material_data.fSpecularPower) >= std::numeric_limits<float>::epsilon())
					rkMtrl.SetSpecularInfo(material_data.isSpecularEnable, material_data.fSpecularPower, material_data.bSphereMapIndex);
			}
