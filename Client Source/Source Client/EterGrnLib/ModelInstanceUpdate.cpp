...
void CGrannyModelInstance::UpdateWorldMatrices(const D3DXMATRIX* c_pWorldMatrix)
{
	...
	//START mt2009 scale
	D3DXMATRIX properMatrix;
	D3DXMatrixIdentity(&properMatrix);

	// Add worldMatrix scale
	D3DXMATRIX scaleMat;
	...
	float yaw = D3DXToRadian(_eulerAngles.x);
	float pitch = D3DXToRadian(_eulerAngles.y);
	float roll = D3DXToRadian(_eulerAngles.z);
	D3DXMatrixRotationYawPitchRoll(&rotationMat, yaw, pitch, roll);
	D3DXMatrixMultiply(&properMatrix, &scaleMat, &rotationMat);

	D3DXMATRIX translationMatrix;
	D3DXMatrixTranslation(&translationMatrix, _scalePos.x, _scalePos.y, _scalePos.z);
	D3DXMatrixMultiply(&properMatrix, &properMatrix, &translationMatrix);

	D3DXMatrixMultiply(&properMatrix, &properMatrix, c_pWorldMatrix);
	//END mt2009 scale
...
#error "unknown granny version"
#endif
			D3DXMatrixMultiply(&rWorldMatrix, &boneMatrices[iBone], &properMatrix);
		}
	}
