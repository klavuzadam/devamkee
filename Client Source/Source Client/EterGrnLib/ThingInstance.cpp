...
D3DVECTOR RotateVector(D3DVECTOR position_offset, float rotation_angle) {
	D3DVECTOR new_position;
	float radians = rotation_angle * 3.141592f / 180.0;  // Convert degrees to radians
	new_position.x = position_offset.x * cos(radians) - position_offset.y * sin(radians);
	new_position.y = position_offset.x * sin(radians) + position_offset.y * cos(radians);
	new_position.z = position_offset.z;  // Z remains unchanged as the rotation is around Z-axis
	return new_position;
}

void CGraphicThingInstance::SetScaleData(UINT ePart, D3DVECTOR scale, D3DVECTOR scalePos)
{
	if (ePart >= m_LODControllerVector.size())
	{
		TraceError("CGraphicThingInstance::SetScaleData(ePart(%d)<uPartCount(%d)) - ePart OUT OF RANGE",
			ePart, m_LODControllerVector.size());
	...
	if (!m_LODControllerVector[ePart])
	{
		TraceError("CGraphicThingInstance::SetScaleData(ePart(%d)) - ePart Data is NULL",
			ePart, m_LODControllerVector.size());

		return;
	}
	m_LODControllerVector[ePart]->SetScale(scale, RotateVector(scalePos, 90));
}

void CGraphicThingInstance::SetRotationData(UINT ePart, D3DVECTOR rotation)
{
	if (ePart >= m_LODControllerVector.size())
	{
		TraceError("CGraphicThingInstance::SetRotation(ePart(%d)<uPartCount(%d)) - ePart OUT OF RANGE",
			ePart, m_LODControllerVector.size());
	...
	if (!m_LODControllerVector[ePart])
	{
		TraceError("CGraphicThingInstance::SetRotation(ePart(%d)) - ePart Data is NULL",
			ePart, m_LODControllerVector.size());

		return;
	}

	m_LODControllerVector[ePart]->SetRotation(rotation);
}
...
void CGraphicThingInstance::DeformAll()
{
	...
	for (auto i = 0; i != m_LODControllerVector.size(); i++)
	{
		...
		{
			#ifdef ENABLE_ACCE_COSTUME_SYSTEM
			if (i == CRaceData::EQUIP_PART_ACCE)
			{
