...
void CInstanceBase::__AttachHorseSaddle()
{
	if (!IsMountingHorse())
		return;
	m_kHorse.m_pkActor->AttachModelInstance(PART_MAIN, "saddle", m_GraphicThingInstance, CRaceData::EQUIP_PART_MAIN);
}
...
void CInstanceBase::__DetachHorseSaddle()
{
	if (!IsMountingHorse())
		return;
	m_kHorse.m_pkActor->DetachModelInstance(PART_MAIN, m_GraphicThingInstance, CRaceData::EQUIP_PART_MAIN);
}
...
bool CInstanceBase::LessRenderOrder(CInstanceBase* pkInst)
{
	...
	if (m_awPart[CRaceData::EQUIP_PART_WEAPON]<pkInst->m_awPart[CRaceData::EQUIP_PART_WEAPON])
		return true;
...
{
	bool isInvisible = IsAffect(AFF_INVISIBILITY) || IsAffect(AFF_SKILL_STEALTH);

#ifdef ENABLE_CANSEEHIDDENTHING_FOR_GM
	if (isInvisible && !__MainCanSeeHiddenThing())
		return true;
#else
	if (isInvisible)
		return true;
...
{
	if (m_kAffectFlagContainer.IsSet(AFF_YMIR))
		return true;
	return false;
}
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
	...
	{
		CRaceData* pRaceData;
		if (CRaceManager::Instance().GetRaceDataPointer(c_rkCreateData.m_dwRace, &pRaceData))
		{
			CRaceData::SScale scaleData = pRaceData->GetScale();
			m_GraphicThingInstance.SetScale(scaleData.scaleX, scaleData.scaleY, scaleData.scaleZ, true);
			m_GraphicThingInstance.UpdateBoundingSphere();
		}
	}
	...
	if (CPythonSystem::instance().IsShowEmpireFlag())
	...
	if (GetRace() == 20403) // quest npc
	{
		m_GraphicThingInstance.SetAlphaValue(0.5f);
...
BOOL CInstanceBase::IsHoldingNotWeapon()
{
	DWORD dwPartItemID = m_GraphicThingInstance.GetPartItemID(CRaceData::EQUIP_PART_WEAPON);
	...
	return IsHoldingPickAxe() || byItemType == ITEM_HERB_KNIFE || byItemType == ITEM_ROD;
}
...
BOOL CInstanceBase::IsHoldingPickAxe()
{
	...
	return m_awPart[CRaceData::EQUIP_PART_WEAPON] >= c_iPickAxeStart && m_awPart[CRaceData::EQUIP_PART_WEAPON] <= c_iPickAxeEnd;
}
...
DWORD CInstanceBase::GetPart(CRaceData::EEquipParts part)
{
	assert(part >= 0 && part < CRaceData::EQUIP_PART_MAX_NUM);
...
bool CInstanceBase::CanUseSkill(DWORD dwSkillIndex)
{
	...
	if (IsHoldingNotWeapon() && dwSkillIndex != SKILL_SPRINT)
		return false;

	if (!m_kHorse.CanUseSkill() && dwSkillIndex != SKILL_SPRINT)
		return false;
...
BOOL CInstanceBase::IsRoot()
{
	return m_GraphicThingInstance.IsRoot();
}
...
void CInstanceBase::RenderToShadowMap()
{
	...
	const float SHADOW_APPLY_DISTANCE = 12500.0f;
...
#ifdef ENABLE_CANSEEHIDDENTHING_FOR_GM
		if (IsAffect(AFF_REVIVE_INVISIBLE) && !__MainCanSeeHiddenThing())
			return false;
#else
		if (IsAffect(AFF_REVIVE_INVISIBLE))
			return false;
#endif
#ifdef ENABLE_CANSEEHIDDENTHING_FOR_GM
		if (IsAffect(AFF_INVISIBILITY) && !__MainCanSeeHiddenThing())
...
BOOL CInstanceBase::IsHerb()
{
	return m_GraphicThingInstance.IsHerb();
}
...
BOOL CInstanceBase::IsAOE()
{
	return m_GraphicThingInstance.IsAOE();
}

BOOL CInstanceBase::IsNPCSpeaker()
{
	return GetRace() == 20404;
}

BOOL CInstanceBase::IsChest()
{
	switch (m_dwVirtualNumber)
	{
	case 20701:
...
void CInstanceBase::SetHair(DWORD eHair)
{
	...
	if (IsPC()==false)
		return;
	m_awPart[CRaceData::EQUIP_PART_HAIR] = eHair;
...
void CInstanceBase::AttachBox(bool isVisible)
{
	if (isVisible)
	{
		SetBack(99900);
		...
		D3DVECTOR rotation = {};
		rotation.z = 90;
		m_GraphicThingInstance.SetScaleData(CRaceData::EQUIP_PART_BOX,
			scale,
			scalePos
		);

		m_GraphicThingInstance.SetRotationData(CRaceData::EQUIP_PART_BOX, rotation);
	}
	else
	{
		SetBack(0);
...
void CInstanceBase::SetBack(DWORD dwItem)
{
	if (!IsPC())
		return;
	...
	if (dwItem > 0)
		CItemManager::Instance().GetItemDataPointer(dwItem, &pItemData);
	m_awPart[CRaceData::EQUIP_PART_BOX] = dwItem;
	m_GraphicThingInstance.AttachItem(pItemData, CRaceData::EQUIP_PART_BOX, "Bip01");
}
...
void CInstanceBase::ChangeHair(DWORD eHair)
{
	...
	if (GetPart(CRaceData::EQUIP_PART_HAIR)==eHair)
		return;
...
bool CInstanceBase::SetAcce(DWORD dwAcce)
{
	...
	m_GraphicThingInstance.AttachAcce(dwAcce, 0, CRaceData::EQUIP_PART_ACCE);

	auto fClear = [this]() {
		ClearAcceEffect();
		m_GraphicThingInstance.SetAcceScale(1.0f, 1.0f, 1.0f);
		m_awPart[CRaceData::EQUIP_PART_ACCE] = 0;
	};
	...
	m_awPart[CRaceData::EQUIP_PART_ACCE] = dwAcce;
...
{
	DWORD dwWeapon = GetPart(CRaceData::EQUIP_PART_WEAPON);
	CItemData * pItemData;
...
#else
	DWORD refine = max(pItem->GetRefine() + pItem->GetSocketCount(),ITEM_SOCKET_MAX_NUM) - ITEM_SOCKET_MAX_NUM;
#endif
...
bool CInstanceBase::SetWeapon(DWORD eWeapon)
{
	...
	m_awPart[CRaceData::EQUIP_PART_WEAPON] = eWeapon;
...
{
	if (eWeapon == m_GraphicThingInstance.GetPartItemID(CRaceData::EQUIP_PART_WEAPON))
		return;
...
bool CInstanceBase::ChangeArmor(DWORD dwArmor)
{
	...
	DWORD eHair = GetPart(CRaceData::EQUIP_PART_HAIR);
	DWORD eWeapon = GetPart(CRaceData::EQUIP_PART_WEAPON);
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	DWORD dwAcce = GetPart(CRaceData::EQUIP_PART_ACCE);
#endif
...
void CInstanceBase::RefreshState(DWORD dwMotIndex, bool isLoop)
{
	...
	else if (IsHoldingPickAxe() || byItemType == ITEM_HERB_KNIFE)
	{
...
bool CInstanceBase::SetArrow(DWORD eArrow)
{
	...
	if (eArrow)
	{
		...
		if (pItemData->GetType() == ITEM_WEAPON)
		{
			...
			if (pItemData->GetSubType() == WEAPON_QUIVER)
			{
