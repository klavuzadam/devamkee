...
void CPythonExchange::SetElkToTarget(YANG	elk)
{
	m_victim.elk = elk;
}

void CPythonExchange::SetElkToSelf(YANG elk)
{
	m_self.elk = elk;
}
...
void CPythonExchange::SetItemToTarget(DWORD pos, DWORD vnum, ITEM_COUNT count)
{
...
void CPythonExchange::SetItemToSelf(DWORD pos, DWORD vnum, ITEM_COUNT count)
{
	if (pos >= EXCHANGE_ITEM_MAX_NUM)
		return;
...
ITEM_COUNT CPythonExchange::GetItemCountFromTarget(BYTE pos)
{
	if (pos >= EXCHANGE_ITEM_MAX_NUM)
		return 0;
...
ITEM_COUNT CPythonExchange::GetItemCountFromSelf(BYTE pos)
{
	if (pos >= EXCHANGE_ITEM_MAX_NUM)
		return 0;
