...
void CPythonNetworkStream::ServerCommand(const char * c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "cube"))
	{
		...
		else if ("info" == TokenVector[1])
		{
			...
			YANG gold = atoi(TokenVector[2].c_str());
	...
	else if (!strcmpi(szCmd, "sprint"))
	{
		int iFlag = atoi(TokenVector[1].c_str());
		IAbstractPlayer& rPlayer = IAbstractPlayer::GetSingleton();
		rPlayer.SetSprintSkillFlag(iFlag);
		m_bSprintSkillFlag = iFlag ? true : false;
	}
	...
	else if (!strcmpi(szCmd, "pickup"))
	{
		...
		if (pInstance1 && pInstance2)
		{
			pInstance1->NEW_LookAtDestInstance(*pInstance2);
			pInstance1->ActEmotion(CRaceMotionData::NAME_PICKUP);
		}
	}
	// Emotion End
	else
