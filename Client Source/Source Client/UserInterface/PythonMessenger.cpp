...
void CPythonMessenger::AddBlock(const char* c_szKey)
{
	m_BlockNameMap.insert(c_szKey);

	if (m_poMessengerHandler)
		PyCallClassMemberFunc(m_poMessengerHandler, "OnLogout", Py_BuildValue("(is)", MESSENGER_GRUOP_INDEX_BLOCK, c_szKey));
}
...
void CPythonMessenger::RemoveBlock(const char* c_szKey)
{
	...
	if (m_poMessengerHandler)
		PyCallClassMemberFunc(m_poMessengerHandler, "OnRemoveList", Py_BuildValue("(is)", MESSENGER_GRUOP_INDEX_BLOCK, c_szKey));
}

BOOL CPythonMessenger::IsBlock(const char* c_szKey)
{
...
PyObject* messengerIsBlock(PyObject* poSelf, PyObject* poArgs)
{
	char* szName;
	if (!PyTuple_GetString(poArgs, 0, &szName))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonMessenger::Instance().IsBlock(szName));
}
...
void initMessenger()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsBlock",					messengerIsBlock,					METH_VARARGS } ,
	...
	PyModule_AddIntConstant(poModule, "GROUP_INDEX_FRIEND", CPythonMessenger::EMessengerGroupIndex::MESSENGER_GRUOP_INDEX_FRIEND);
	PyModule_AddIntConstant(poModule, "GROUP_INDEX_GUILD", CPythonMessenger::EMessengerGroupIndex::MESSENGER_GRUOP_INDEX_GUILD);
	PyModule_AddIntConstant(poModule, "GROUP_INDEX_BLOCK", CPythonMessenger::EMessengerGroupIndex::MESSENGER_GRUOP_INDEX_BLOCK);
}
