...
void CInstanceBase::StartWalking()
{
	...
	if (IsAffect(AFF_SKILL_FEATHER_WALK))
	{
		m_adwCRCAffectEffect[AFF_SKILL_FEATHER_WALK] = __EffectContainer_AttachEffect(EFFECT_AFFECT_FEATHER_WALK);
	}
	else if (IsAffect(AFF_SKILL_SWIFTNESS))
	{
		m_adwCRCAffectEffect[AFF_SKILL_SWIFTNESS] = __EffectContainer_AttachEffect(EFFECT_AFFECT_SWIFTNESS);
	}

	if (IsAffect(AFF_SPRINT))
	{
		DWORD dwEID = __EffectContainer_AttachEffect(IsMountingHorse() ? EFFECT_AFFECT_SPRINT_HORSE : EFFECT_AFFECT_SPRINT);
		m_adwCRCAffectEffect[AFF_SPRINT] = dwEID;
	}
}
...
void CInstanceBase::EndWalking(float fBlendingTime)
{
	...
	if (IsWalking()||!IsAttacked())
	{
		...
		if (IsAffect(AFF_SKILL_FEATHER_WALK))
		{
			__EffectContainer_DetachEffect(EFFECT_AFFECT_FEATHER_WALK);
		}
		else if (IsAffect(AFF_SKILL_SWIFTNESS))
		{
			__EffectContainer_DetachEffect(EFFECT_AFFECT_SWIFTNESS);
		}

		if (IsAffect(AFF_SPRINT))
		{
			__EffectContainer_DetachEffect(IsMountingHorse() ? EFFECT_AFFECT_SPRINT_HORSE : EFFECT_AFFECT_SPRINT);
		}
	}
}
