...
void __SetWeaponPower(IAbstractPlayer& rkPlayer, DWORD dwWeaponID)
{
	...
	{
		if (pkWeapon->GetType()==ITEM_WEAPON)
		{
		...
		else if (pkWeapon->GetType()==ITEM_COSTUME && pkWeapon->GetSubType()==COSTUME_WEAPON)
		{
...
bool CPythonNetworkStream::RecvCharacterAdditionalInfo()
{
	...
	if(kNetActorData.m_dwVID == chrInfoPacket.dwVID)
	{
		...
		kNetActorData.m_dwArmor		= chrInfoPacket.awPart[PART_MAIN];
		kNetActorData.m_dwWeapon	= chrInfoPacket.awPart[PART_WEAPON];
		kNetActorData.m_dwHair		= chrInfoPacket.awPart[PART_HAIR];
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
		kNetActorData.m_dwAcce		= chrInfoPacket.awPart[PART_ACCE];
#endif
#ifdef ENABLE_QUIVER_SYSTEM
...
bool CPythonNetworkStream::RecvCharacterAppendPacketNew()
{
	...
	kNetActorData.m_dwAtkSpd	= chrAddPacket.bAttackSpeed;
	kNetActorData.m_dwRace		= chrAddPacket.wRaceNum;
	kNetActorData.m_dwArmor		= chrAddPacket.awPart[PART_MAIN];
	kNetActorData.m_dwWeapon	= chrAddPacket.awPart[PART_WEAPON];
	kNetActorData.m_dwHair		= chrAddPacket.awPart[PART_HAIR];
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	kNetActorData.m_dwAcce		= chrAddPacket.awPart[PART_ACCE];
#endif
#ifdef ENABLE_QUIVER_SYSTEM
...
bool CPythonNetworkStream::RecvCharacterUpdatePacket()
{
	...
	kNetUpdateActorData.m_dwArmor	= chrUpdatePacket.awPart[PART_MAIN];
	kNetUpdateActorData.m_dwWeapon	= chrUpdatePacket.awPart[PART_WEAPON];
	kNetUpdateActorData.m_dwHair	= chrUpdatePacket.awPart[PART_HAIR];
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	kNetUpdateActorData.m_dwAcce	= chrUpdatePacket.awPart[PART_ACCE];
#endif
#ifdef ENABLE_QUIVER_SYSTEM
	...
	kNetUpdateActorData.m_dwLevel = chrUpdatePacket.bLevel;
...
void CPythonNetworkStream::__RecvCharacterAppendPacket(SNetworkActorData * pkNetActorData)
{
	...
	if (GetMainActorVID()==pkNetActorData->m_dwVID)
	{
		...
		if (m_bSprintSkillFlag)
			rkPlayer.SetSprintSkillFlag(m_bSprintSkillFlag);
