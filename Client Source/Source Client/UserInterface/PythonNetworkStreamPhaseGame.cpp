...
#define GetPROCAddr LI_FN(GetProcAddress).get()
#define GetMHandle LI_FN(GetModuleHandleA).get()

constexpr char xorKey = 0x7AB02941;
constexpr char xorKey2 = 0x8AB700C;

constexpr char obfuscatedDbgUiRemoteBreakin[] = {
 'D' ^ 0x7AB02941, 'b' ^ 0x7AB02941, 'g' ^ 0x7AB02941, 'U' ^ 0x7AB02941, 'i' ^ 0x7AB02941,
 'R' ^ 0x7AB02941, 'e' ^ 0x7AB02941, 'm' ^ 0x7AB02941, 'o' ^ 0x7AB02941, 't' ^ 0x7AB02941,
 'e' ^ 0x7AB02941, 'B' ^ 0x7AB02941, 'r' ^ 0x7AB02941, 'e' ^ 0x7AB02941, 'a' ^ 0x7AB02941,
 'k' ^ 0x7AB02941, 'i' ^ 0x7AB02941, 'n' ^ 0x7AB02941, '\0'
};

constexpr char obfuscatedNtdll[] = {
	'n' ^ 0x8AB700C, 't' ^ 0x8AB700C, 'd' ^ 0x8AB700C, 'l' ^ 0x8AB700C, 'l' ^ 0x8AB700C,
	'.' ^ 0x8AB700C, 'd' ^ 0x8AB700C, 'l' ^ 0x8AB700C, 'l' ^ 0x8AB700C, '\0'
};


void XORDecodeKey0(char* data, size_t size) {
	unsigned int key = xorKey;
	for (size_t i = 0; i < size; ++i) {
		data[i] ^= key;
	}
}

void XORDecodeKey1(char* data, size_t size) {
	unsigned int key = xorKey2;
	for (size_t i = 0; i < size; ++i) {
		data[i] ^= key;
	}
}

const char* GetObfuscatedFunc() {
	static char decodedTitle[sizeof(obfuscatedDbgUiRemoteBreakin)] = { 0 };
	memcpy(decodedTitle, obfuscatedDbgUiRemoteBreakin, sizeof(obfuscatedDbgUiRemoteBreakin));
	XORDecodeKey0(decodedTitle, sizeof(obfuscatedDbgUiRemoteBreakin) - 1);
	return decodedTitle;
}

const char* GetObfuscatedLib() {
	static char decodedTitle[sizeof(obfuscatedNtdll)] = { 0 };
	memcpy(decodedTitle, obfuscatedNtdll, sizeof(obfuscatedNtdll));
	XORDecodeKey1(decodedTitle, sizeof(obfuscatedNtdll) - 1);
	return decodedTitle;
}
...
void HookedNtdll() {
	...
	VM_START
#endif
	BYTE funcAddress = *reinterpret_cast<BYTE*>(GetPROCAddr(GetMHandle(GetObfuscatedLib()), GetObfuscatedFunc()));
	if (funcAddress != 0b11101001)
	{
		CPythonNetworkStream::instance().SendChatPacket("g", CHAT_TYPE_SPECIAL_INTERNAL);
	}
#ifdef __THEMIDA__
	VM_END
#endif
...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			// money
			case HEADER_GC_UPDATE_GOLD:
				ret = RecvGoldChange();
				break;


			case HEADER_GC_SET_GOLD:
				ret = RecvGoldSet();
				break;
			...
			case HEADER_GC_ADD_FLY_SHOOT_TARGETING:
				ret = RecvAddFlyShootTargetingPacket();
				break;

			case HEADER_GC_CLEAR_FLY_SHOOT_TARGETING:
				ret = RecvClearAddFlyShootTargetingPacket();
				break;
			...
			case HEADER_GC_SPECIAL_FLAG:
				ret = RecvSpecialFlagPacket();
				break;
			...
			case HEADER_GC_SPECIAL_SHOP:
				ret = RecvSpecialShop();
				break;

			case HEADER_GC_NPC_INFO_BOARD:
				ret = RecvNpcInfoBoard();
				break;

			case HEADER_GC_BUSY_ACTION:
				ret = RecvBusyAction();
				break;

			case HEADER_GC_REQUEST_INFO:
				ret = RecvRequestInfo();
				break;
			...
			case HEADER_GC_GAMEMASTER_TARGET:
				ret = RecvGamemasterTarget();
				break;
...
bool CPythonNetworkStream::SendMessengerBlockPacket(const char* c_szName)
{
	TPacketCGMessenger packet;
	packet.header = HEADER_CG_MESSENGER;
	packet.subheader = MESSENGER_SUBHEADER_CG_ADD_BLOCK;
	if (!Send(sizeof(packet), &packet))
		return false;
...
bool CPythonNetworkStream::SendMessengerUnblockPacket(const char* c_szName)
{
	TPacketCGMessenger packet;
	packet.header = HEADER_CG_MESSENGER;
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	if (kChat.dwVID != 0)
	{
		...
		if (CHAT_TYPE_SHOUT != kChat.type && CHAT_TYPE_TRADE != kChat.type)
		{
			CPythonTextTail::Instance().RegisterChatTail(kChat.dwVID, line);
		}

		if (pkInstChatter->IsPC() || pkInstChatter->IsNPCSpeaker())
	...
	else
	{
		...
		else if (CHAT_TYPE_FANCY_NOTICE == kChat.type || CHAT_TYPE_GAMEMASTER_NOTICE == kChat.type)
		{
			//HookedNtdll();
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_SetFancyTipMessage", Py_BuildValue("(si)", buf, kChat.type));
		}
...
bool CPythonNetworkStream::RecvGoldSet()
{
	TPacketGCGoldUpdate goldUpdate;

	if (!Recv(sizeof(TPacketGCGoldUpdate), &goldUpdate))
	{
		Tracen("Recv Gold Set Packet Error");
		return false;
	}

	std::stringstream ss;
	ss << goldUpdate.totalGold;

	CPythonPlayer::instance().SetGold(goldUpdate.totalGold);
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnGoldSet",
		Py_BuildValue("(s)", ss.str().c_str()));
...
{
	TPacketGCGoldUpdate goldChange;

	if (!Recv(sizeof(TPacketGCGoldUpdate), &goldChange))
	{
		Tracen("Recv Gold Change Packet Error");
		return false;
	}
	const YANG amount = goldChange.totalGold - CPythonPlayer::instance().GetGold();
	CPythonPlayer::instance().SetGold(goldChange.totalGold);

	std::stringstream ss;
	ss << amount;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnGoldUpdate",
		Py_BuildValue("(s)", ss.str().c_str()));
	return true;
}
...
bool CPythonNetworkStream::RecvPointChange()
{
	...
	else
	{
		...
		if (pInstance && PointChange.Type == POINT_LEVEL && !pInstance->IsHideLevel())
		{
...
bool CPythonNetworkStream::RecvDeadPacket()
{
	...
	if (pkChrInstSel)
	{
		...
		if (pkInstMain==pkChrInstSel)
		{
			...
			pkInstMain->__ResetAffects_PythonOnly();
		}
		...
		case SHOP_SUBHEADER_GC_START:
			{
				...
				for (BYTE iItemIndex = 0; iItemIndex < SHOP_PLAYER_HOST_ITEM_MAX_NUM; ++iItemIndex)
				{
					CPythonShop::Instance().SetItemData(iItemIndex, pShopStartPacket->items[iItemIndex]);
				}

				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "StartShop", Py_BuildValue("(iii)", dwVID, pShopStartPacket->unlockCount, pShopStartPacket->isPremium));
			}
			break;
	...
	switch (exchange_packet.subheader)
	{
		...
		case EXCHANGE_SUBHEADER_GC_ITEM_ADD:
			if (exchange_packet.is_me)
			{
				...
				CPythonExchange::Instance().SetItemToSelf(iSlotIndex, exchange_packet.arg1, (ITEM_COUNT) exchange_packet.arg3);
			...
			{ 
				int iSlotIndex = exchange_packet.arg2.cell;
				CPythonExchange::Instance().SetItemToTarget(iSlotIndex, exchange_packet.arg1, (ITEM_COUNT) exchange_packet.arg3);
				for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
...
bool CPythonNetworkStream::RecvTargetPacket()
{
	...
	{
		TraceError("Recv Target Packet Error");
		return false;
	}

	std::vector<STargetDebuff> debuffs{};
	if (TargetPacket.subheader == SUBHEADER_GC_TARGET_WITH_DEBUFFS)
	{
		TSubPacketGCTargetDebuffs subpack;
		if (!Recv(sizeof(subpack), &subpack))
		{
			TraceError("Recv Target SubPacket Error");
			return false;
		}

		debuffs.resize(subpack.affectCount);

		if (!Recv(sizeof(debuffs[0]) * debuffs.size(), debuffs.data()))
		{
			TraceError("Recv Target Debuff Error");
			return false;
		}
	}
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			...
			if (TargetPacket.subheader == SUBHEADER_GC_TARGET_WITH_DEBUFFS)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "StartTargetAffect", Py_BuildValue("(i)", TargetPacket.dwVID));

				for (auto debuff : debuffs)
				{
					PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetTargetAffect", Py_BuildValue("(iii)", TargetPacket.dwVID, debuff.affect, debuff.duration));
					//Tracenf("aff: %d, czas: %ld", debuff.affect, debuff.duration);
				}

				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ClearTargetAffect", Py_BuildValue("(i)", TargetPacket.dwVID)); // usuwamy te ktorych nie ma w liscie
			}
		}
	}
	else
...
bool CPythonNetworkStream::RecvAddFlyShootTargetingPacket()
{
	TPacketGCFlyTargeting kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
		return false;
	...
	{
//#ifndef _DEBUG		
		TraceError("CPythonNetworkStream::RecvAddFlyShootTargetingPacket() - dwShooterVID[%d] NOT EXIST", kPacket.dwShooterVID);
//#endif
		return true;
	}
	...
	if (kPacket.dwTargetVID && pTarget)
	{
		pShooter->GetGraphicThingInstancePtr()->AddFlyShootTarget(pTarget->GetGraphicThingInstancePtr());
	}
	else
	{
		float h = CPythonBackground::Instance().GetHeight(kPacket.lX, kPacket.lY) + 60.0f; // TEMPORARY HEIGHT
		pShooter->GetGraphicThingInstancePtr()->AddFlyShootTarget(D3DXVECTOR3(kPacket.lX, kPacket.lY, h));
	}

	return true;
}

bool CPythonNetworkStream::RecvClearAddFlyShootTargetingPacket()
{
	TPacketGCFlyClearTargeting kPacket;
	if (!Recv(sizeof(TPacketGCFlyClearTargeting), &kPacket))
		return false;
	...
	if (!pShooter)
	{
		TraceError("CPythonNetworkStream::RecvClearAddFlyShootTargetingPacket() - dwShooterVID[%d] NOT EXIST", kPacket.dwShooterVID);
		return true;
	}

	pShooter->GetGraphicThingInstancePtr()->ClearFlyShootTargets();
	return true;
}
...
bool CPythonNetworkStream::RecvMessenger()
{
	...
	switch (p.subheader)
	{
		...
		{
			TPacketGCMessengerBlockList list;
			if (!Recv(sizeof(list), &list))
				return false;

			std::vector<TPacketGCMessengerBlockItem> buffer;
			buffer.resize(list.count);

			if (!Recv(sizeof(TPacketGCMessengerBlockItem) * list.count, buffer.data()))
				return false;

			for (auto item : buffer)
			{
				if (item.isAdd)
					CPythonMessenger::Instance().AddBlock(item.name);
				else
					CPythonMessenger::Instance().RemoveBlock(item.name);
			}
			break;
		}
	}
...
bool CPythonNetworkStream::SendGuildWithdrawMoneyPacket(YANG dwMoney)
{
	TPacketCGGuild GuildPacket;
	GuildPacket.byHeader = HEADER_CG_GUILD;
...
bool CPythonNetworkStream::RecvGuild()
{
	...
	switch(GuildPacket.subheader)
	{
		...
		case GUILD_SUBHEADER_GC_INFO:
		{
			...
			strncpy(rGuildInfo.szGuildName, GuildInfo.name, sizeof(rGuildInfo.szGuildName));
...
bool CPythonNetworkStream::SendFishingPacket(int iRotation)
{
	...
	VM_START
#endif
	TPacketCGSendFishing packet{};
	packet.header = HEADER_CG_FISHING;
	packet.focusHWND = reinterpret_cast<DWORD>(GetFocus());
	packet.pythonApplicationHWND = reinterpret_cast<DWORD>(CPythonApplication::Instance().GetWindowHandle());
	packet.dir = iRotation / 5;
	const bool result = Send(sizeof(packet), &packet);
#ifdef __THEMIDA__
	VM_END
#endif
...
bool CPythonNetworkStream::RecvFishing()
{
	...
	switch (FishingPacket.subheader)
	{
		...
		case FISHING_SUBHEADER_GC_FISH:
		{
			...
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnFishingNotify", Py_BuildValue("(is)", ITEM_FISH == pItemData->GetType(), pItemData->GetName()));
			}
			else
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnFishingSuccess", Py_BuildValue("(is)", ITEM_FISH == pItemData->GetType(), pItemData->GetName()));
			}
			break;
		}
	}
...
bool CPythonNetworkStream::RecvFishingGame()
{
	std::vector<char> vecBuffer;
	vecBuffer.clear();

	TPacketGCFishingGame  packet_fishing_game;
	if (!Recv(sizeof(packet_fishing_game), &packet_fishing_game))
		return false;

	int iSize = packet_fishing_game.size - sizeof(packet_fishing_game);
	if (iSize > 0)
	{
		vecBuffer.resize(iSize);
		if (!Recv(iSize, &vecBuffer[0]))
	...
	switch (packet_fishing_game.subheader)
	{
	case FISHING_GAME_SUBHEADER_GC_GAME_UPDATE:
	{
		TPacketGCFishingGamePlayerUpdate* pPlayerUpdate = (TPacketGCFishingGamePlayerUpdate*)&vecBuffer[0];
		TPacketGCFishingGameUpdate* pGameUpdate = (TPacketGCFishingGameUpdate*)&vecBuffer[sizeof(TPacketGCFishingGamePlayerUpdate)];
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnFishingGameUpdate", Py_BuildValue("(ii)", pGameUpdate->top_bar_pos, pPlayerUpdate->player_pos));
	}
	break;
	case FISHING_GAME_SUBHEADER_GC_PLAYER_UPDATE:
	{
		TPacketGCFishingGamePlayerUpdate* pPlayerUpdate = (TPacketGCFishingGamePlayerUpdate*)&vecBuffer[0];
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnFishingPlayerUpdate", Py_BuildValue("(i)", pPlayerUpdate->player_pos));
	}
	break;
...
/////////////////////////////////////////////////////////////////////////
// MyShop
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, BYTE bTimeIndex, const std::vector<TShopItemTable> & c_rSellingItemStock)
{
	TPacketCGMyShop packet;
	packet.bHeader = HEADER_CG_MYSHOP;
	strncpy(packet.szSign, c_szName, SHOP_SIGN_MAX_LEN);
	packet.bTimeIndex = bTimeIndex;
...
bool CPythonNetworkStream::RecvNPCList()
{
	...
	Tracenf("TPacketGCNPCPosition size %d ", kNPCPosition.count);
...
bool CPythonNetworkStream::RecvAffectAddPacket()
{
	...
	CPythonPlayer::instance().AddAffect(rkElement.dwType, CPythonApplication::Instance().GetServerTimeStamp() + rkElement.lDuration);
	DWORD clientAdjustedFlag = rkElement.dwFlag - 1;
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_NEW_AddAffect", Py_BuildValue("(iiiiii)", rkElement.dwType, rkElement.bPointIdxApplyOn, rkElement.lApplyValue, rkElement.lDuration, rkElement.itemVnum, clientAdjustedFlag));
...
bool CPythonNetworkStream::RecvSpecialFlagPacket()
{
	TPacketGCSpecialFlag kSpecialFlagPacket;
	if (!Recv(sizeof(kSpecialFlagPacket), &kSpecialFlagPacket))
		return false;

	//Tracenf("RecvSpecialFlagPacket %s %lld ", kSpecialFlagPacket.cFlag, kSpecialFlagPacket.llValue);
	std::string value_str = std::to_string(kSpecialFlagPacket.llValue);
	//Tracenf("RecvSpecialFlagPacket %s %s ", kSpecialFlagPacket.cFlag, value_str.c_str());
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "UpdateSpecialFlag", Py_BuildValue("(ss)", kSpecialFlagPacket.cFlag, value_str.c_str()));
	return true;
}
...
bool CPythonNetworkStream::RecvRequestInfo()
{
	TPacketGCRequestInfo kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
		return false;

	TPacketCGRequestInfoResponse response{};
	response.header = HEADER_CG_REQUEST_INFO_RESPONSE;
	response.type = kPacket.type;
	...
	switch (kPacket.type)
	{
		...
		{
			auto keyChain = Discord::key_chain;
			auto username = Discord::username;
			if (keyChain == "UNKNOWN")
			{
				username = std::to_string(random_range(1, 999999));
			}

			auto encrypted1 = encryptAESString(keyChain, key);
			auto encrypted2 = encryptAESString(username, key);

			strncpy(response.arg1, encrypted1.c_str(), sizeof(response.arg1));
			strncpy(response.arg2, encrypted2.c_str(), sizeof(response.arg2));
		}
		break;
		case REQUEST_INFO_HACK:
		{
			auto salt = random_range(1, 999999);

			auto encrypted1 = encryptAESString(ConstData::MarkBot ? "HACKER" : "ALLGOOD", key);
			auto encrypted2 = encryptAESString(std::to_string(salt), key);
		...
		default:
		{
			TraceError("UNKNOWN REQUEST INFO TYPE %d", kPacket.type);
			return false;
		}
	}
#ifdef __THEMIDA__
	...
	{
#ifdef _DEBUG
		TraceError("CANNOT SEND REQUEST INFO RESPONSE PACKET : TYPE %d ", response.type);
#endif
		return false;
	}
...
bool CPythonNetworkStream::RecvGamemasterTarget()
{
	TPacketGCGamemasterTarget kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
		return false;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OpenGameMasterTarget", Py_BuildValue("(isisi)",
		kPacket.pid, kPacket.playerName, kPacket.aid, kPacket.accountName, kPacket.isPremium));
...
void onDiscordReady(const DiscordUser* user) {
	Discord::key_chain = user->userId;
	if (strcmp(user->discriminator, "0") == 0)
		Discord::username = user->username;
	else
	{
		Discord::username = std::string(user->username) + "#" + std::string(user->discriminator);
	}
}
