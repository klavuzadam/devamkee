...
bool CPythonNonPlayer::LoadNonPlayerNames(const char* c_szFileName)
{
	const VOID* pvData;
	CMappedFile kFile;
	if (!CEterPackManager::Instance().Get(kFile, c_szFileName, &pvData))
	{
		Tracenf("CPythonNonPlayer::LoadNonPlayerNames(c_szFileName=%s) - Load Error", c_szFileName);
		return false;
	}
	...
	for (DWORD i = 0; i < kTextFileLoader.GetLineCount(); ++i)
	{
		...
		while (kTokenVector.size() < MOBNAME_COL_NUM)
			kTokenVector.push_back("");

		DWORD dwVnum = atoi(kTokenVector[MOBNAME_COL_VNUM].c_str());
		const std::string& c_rstrName = kTokenVector[MOBNAME_COL_NAME];
		auto p = GetTable(dwVnum);
		if (!p)
			continue;

		m_NonPlayerLocaleNameMap[dwVnum] = c_rstrName;
	}
	return true;
}
...
bool CPythonNonPlayer::GetName(DWORD dwVnum, const char ** c_pszName)
{
	...
	auto it = m_NonPlayerLocaleNameMap.find(dwVnum);
	if (it != m_NonPlayerLocaleNameMap.end()) {
		*c_pszName = it->second.c_str();
	}
	else
	{
