...
PyObject * questGetQuestData(PyObject * poSelf, PyObject * poArgs)
{
	...
	{
		std::string strIconFileName = "d:/ymir work/ui/game/quest/questicon/scroll_open.tga";
		pImage = (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(strIconFileName.c_str());
	}
...
PyObject* questIsExist(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BadArgument();
...
void initquest()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsExist",					questIsExist,					METH_VARARGS },
		{ "Clear",						questClear,						METH_VARARGS },
		{ NULL,							NULL,							NULL },
	};

	PyObject * poModule = Py_InitModule("quest", s_methods);
	PyModule_AddIntConstant(poModule, "QUEST_MAX_NUM", 7);
}
