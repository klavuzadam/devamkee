...
PyObject* playerGetCoolTimeReduction(PyObject* poSelf, PyObject* poArgs)
{
	float iSpd = 100 - CPythonPlayer::Instance().GetStatus(POINT_CASTING_SPEED);
	if (iSpd > 0)
		iSpd = 100 + iSpd;
	else if (iSpd < 0)
		iSpd = 10000 / (100 - iSpd);
	...
	return Py_BuildValue("f", iSpd / 100);
}

PyObject* playerGetMagicDamageBonusFromSP(PyObject* poSelf, PyObject* poArgs)
{
	float MgaicBonus = CPythonPlayer::Instance().GetMagicDamageBonusFromSP();
	return Py_BuildValue("f", MgaicBonus);
}

PyObject* playerGetBonusSP(PyObject* poSelf, PyObject* poArgs)
{
	float bonusSP = CPythonPlayer::Instance().GetBonusSP();
	return Py_BuildValue("f", bonusSP);
}
...
PyObject* playerGetGold(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 1:
		{
			...
			ITEM_COUNT ItemNum = CPythonPlayer::Instance().GetItemCount(TItemPos (INVENTORY, iSlotIndex));
	...
	case 2:
		{
			...
			ITEM_COUNT ItemNum = CPythonPlayer::Instance().GetItemCount(Cell);
			return Py_BuildValue("i", ItemNum);
		}
	default:
		return Py_BuildException();
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		for (int i = 0; i < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++i)
		{
			if (pPlayerItem->aAttr[i].bType != 0) // @fixme009 (this line must be commented)
			{
				...
				if (pPlayerItem->aAttr[i].bType >0 && i < ITEM_ATTRIBUTE_NORM_NUM)
					isAttr = true;
			}
		}

		if (isAttr)
			snprintf(buf, sizeof(buf), "|cffffc700|H%s|h[%s]|h|r", itemlink, pItemData->GetOriginalName());
		else
			snprintf(buf, sizeof(buf), "|cfff1e6c0|H%s|h[%s]|h|r", itemlink, pItemData->GetOriginalName());
	}
	else
		buf[0] = '\0';
...
PyObject * playerGetISellItemPrice(PyObject * poSelf, PyObject * poArgs)
{
	...
	DWORD itemVnum = pItemData->GetTable()->dwVnum;
	bool isGoldBar = itemVnum >= 80003 && itemVnum <= 80008;
	if (!isGoldBar)
		iPrice /= 5;
#endif
...
PyObject * playerIsValuableItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	YANG dwValue = pItemData->GetISellItemPrice();
	if (dwValue > 5000)
		isHighPrice = TRUE;
...
PyObject * playerCanAttachMetin(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (ITEM_ROD == pTargetItemData->GetType())
...
PyObject * playerCanDetach(PyObject * poSelf, PyObject * poArgs)
{
	...
	{
		if ((pTargetItemData->GetType() != ITEM_COSTUME) || (pTargetItemData->GetSubType() != COSTUME_ACCE))
			return Py_BuildValue("i", DETACH_METIN_CANT);
...
PyObject* playerGetEmpire(PyObject* poSelf, PyObject* poArgs)
{
	...
	return Py_BuildValue("i", EmpireID);
}
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetCoolTimeReduction",		playerGetCoolTimeReduction,		    METH_VARARGS },
		...
		{ "GetElk",						playerGetGold,						METH_VARARGS },
		{ "GetMoney",					playerGetGold,						METH_VARARGS },
		{ "GetGold",					playerGetGold,						METH_VARARGS },
#ifdef ENABLE_CHEQUE_SYSTEM
...
#endif
#endif
		{ "GetEmpire",			playerGetEmpire,				METH_VARARGS },
		{ "GetMagicDamageBonusFromSP",playerGetMagicDamageBonusFromSP,	METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "APPLY_ATTBONUS_WOLFMAN", APPLY_ATTBONUS_WOLFMAN);
...
#ifdef ENABLE_WOLFMAN_CHARACTER
	PyModule_AddIntConstant(poModule, "APPLY_ATTBONUS_CLAW", APPLY_ATTBONUS_CLAW);
#endif
	...
	PyModule_AddIntConstant(poModule, "INVENTORY_MAX_NUM",	INVENTORY_MAX_NUM);
	PyModule_AddIntConstant(poModule, "INVENTORY_DEFAULT_MAX_NUM", INVENTORY_DEFAULT_MAX_NUM);
	...
	PyModule_AddIntConstant(poModule, "POINT_HT", POINT_HT);
	PyModule_AddIntConstant(poModule, "POINT_IQ", POINT_IQ);
	PyModule_AddIntConstant(poModule, "POINT_ST", POINT_ST);
	PyModule_AddIntConstant(poModule, "POINT_DX", POINT_DX);
	PyModule_AddIntConstant(poModule, "POINT_ATT_SPEED", POINT_ATT_SPEED);
	PyModule_AddIntConstant(poModule, "POINT_MOV_SPEED", POINT_MOV_SPEED);
	PyModule_AddIntConstant(poModule, "POINT_CASTING_SPEED", POINT_CASTING_SPEED);
	...
	PyModule_AddIntConstant(poModule, "POINT_ROOT_PCT", POINT_ROOT_PCT);
	PyModule_AddIntConstant(poModule, "POINT_CRITICAL_PCT", POINT_CRITICAL_PCT);
	PyModule_AddIntConstant(poModule, "POINT_PENETRATE_PCT", POINT_PENETRATE_PCT);
	...
	PyModule_AddIntConstant(poModule, "POINT_REFLECT_ARROW", POINT_REFLECT_ARROW);
	...
	PyModule_AddIntConstant(poModule, "POINT_KILL_HP_RECOVERY", POINT_KILL_HP_RECOVERY);
	...
	PyModule_AddIntConstant(poModule, "POINT_IMMUNE_STUN", POINT_IMMUNE_STUN);
	PyModule_AddIntConstant(poModule, "POINT_IMMUNE_SLOW", POINT_IMMUNE_SLOW);
	PyModule_AddIntConstant(poModule, "POINT_IMMUNE_FALL", POINT_IMMUNE_FALL);
	PyModule_AddIntConstant(poModule, "POINT_BOW_DISTANCE", POINT_BOW_DISTANCE);
	...
	PyModule_AddIntConstant(poModule, "POINT_MAX_STAMINA", POINT_MAX_STAMINA);
	PyModule_AddIntConstant(poModule, "POINT_MALL_ATTBONUS", POINT_MALL_ATTBONUS);
	PyModule_AddIntConstant(poModule, "POINT_MALL_DEFBONUS", POINT_MALL_DEFBONUS);
	PyModule_AddIntConstant(poModule, "POINT_MALL_EXPBONUS", POINT_MALL_EXPBONUS);
	PyModule_AddIntConstant(poModule, "POINT_MALL_ITEMBONUS", POINT_MALL_ITEMBONUS);
	...
	PyModule_AddIntConstant(poModule, "POINT_MAX_HP_PCT", POINT_MAX_HP_PCT);
	PyModule_AddIntConstant(poModule, "POINT_MAX_SP_PCT", POINT_MAX_SP_PCT);
	PyModule_AddIntConstant(poModule, "POINT_ENERGY", POINT_ENERGY);
	...
	PyModule_AddIntConstant(poModule, "POINT_MANASHIELD", POINT_MANASHIELD);
	PyModule_AddIntConstant(poModule, "POINT_HIT_HP_RECOVERY", POINT_HIT_HP_RECOVERY);
	PyModule_AddIntConstant(poModule, "POINT_TERROR", POINT_TERROR);
	PyModule_AddIntConstant(poModule, "POINT_ST_REGEN", POINT_ST_REGEN);
	PyModule_AddIntConstant(poModule, "POINT_RESIST_NORMAL_DAMAGE", POINT_RESIST_NORMAL_DAMAGE);
	PyModule_AddIntConstant(poModule, "POINT_POLYMORPH", POINT_POLYMORPH);
	...
	PyModule_AddIntConstant(poModule, "POINT_DAGGER_ATT_GRADE_MONSTER", POINT_DAGGER_ATT_GRADE_MONSTER);
	PyModule_AddIntConstant(poModule, "POINT_ATT_GRADE_MONSTER", POINT_ATT_GRADE_MONSTER);
	PyModule_AddIntConstant(poModule, "POINT_RESIST_MONSTER_1000PCT", POINT_RESIST_MONSTER_1000PCT);
	PyModule_AddIntConstant(poModule, "POINT_ABSORB_DAMAGE", POINT_ABSORB_DAMAGE);
	PyModule_AddIntConstant(poModule, "POINT_ABSORB_DAMAGE_MONSTER", POINT_ABSORB_DAMAGE_MONSTER);
	PyModule_AddIntConstant(poModule, "POINT_IMMUNE_STUN_BREAK", POINT_IMMUNE_STUN_BREAK);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_TEMPLE_CURSE", POINT_BREAK_TEMPLE_CURSE);

	PyModule_AddIntConstant(poModule, "POINT_PARTY_BUFFER_BONUS", POINT_PARTY_BUFFER_BONUS);
	PyModule_AddIntConstant(poModule, "POINT_SKILL_DURATION", POINT_SKILL_DURATION);
	PyModule_AddIntConstant(poModule, "POINT_ATTBONUS_ORC_VALLEY", POINT_ATTBONUS_ORC_VALLEY);
	...
	PyModule_AddIntConstant(poModule, "POINT_MAGIC_ATT_MONSTER", POINT_MAGIC_ATT_MONSTER);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_RESIST_SWORD", POINT_BREAK_RESIST_SWORD);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_RESIST_TWOHAND", POINT_BREAK_RESIST_TWOHAND);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_RESIST_DAGGER", POINT_BREAK_RESIST_DAGGER);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_RESIST_BELL", POINT_BREAK_RESIST_BELL);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_RESIST_FAN", POINT_BREAK_RESIST_FAN);
	PyModule_AddIntConstant(poModule, "POINT_BREAK_RESIST_BOW", POINT_BREAK_RESIST_BOW);
	PyModule_AddIntConstant(poModule, "POINT_COLLECT_CHANCE", POINT_COLLECT_CHANCE);
	PyModule_AddIntConstant(poModule, "POINT_LEARN_CHANCE", POINT_LEARN_CHANCE);
	PyModule_AddIntConstant(poModule, "POINT_RESIST_HUMAN", POINT_RESIST_HUMAN);
	PyModule_AddIntConstant(poModule, "POINT_MAGIC_ATT", POINT_MAGIC_ATT);
	PyModule_AddIntConstant(poModule, "POINT_FIRE_PCT", POINT_FIRE_PCT);
	PyModule_AddIntConstant(poModule, "POINT_CONVERT_DAMAGE_TO_SP", POINT_CONVERT_DAMAGE_TO_SP);
	PyModule_AddIntConstant(poModule, "POINT_DROP_RARE", POINT_DROP_RARE);
	PyModule_AddIntConstant(poModule, "POINT_CURSE_PCT", POINT_CURSE_PCT);
	PyModule_AddIntConstant(poModule, "POINT_MAGIC_ATT_GRADE_BONUS_MONSTER", POINT_MAGIC_ATT_GRADE_BONUS_MONSTER);
	PyModule_AddIntConstant(poModule, "POINT_ATT_SPECIAL", POINT_ATT_SPECIAL);


	PyModule_AddIntConstant(poModule, "PREMIUM_EXP", PREMIUM_EXP);
	PyModule_AddIntConstant(poModule, "PREMIUM_ITEM", PREMIUM_ITEM);
	PyModule_AddIntConstant(poModule, "PREMIUM_SAFEBOX", PREMIUM_SAFEBOX);
	PyModule_AddIntConstant(poModule, "PREMIUM_AUTOLOOT", PREMIUM_AUTOLOOT);
	PyModule_AddIntConstant(poModule, "PREMIUM_FISH_MIND", PREMIUM_FISH_MIND);
	PyModule_AddIntConstant(poModule, "PREMIUM_MARRIAGE_FAST", PREMIUM_MARRIAGE_FAST);
	PyModule_AddIntConstant(poModule, "PREMIUM_GOLD", PREMIUM_GOLD);
	PyModule_AddIntConstant(poModule, "PREMIUM_SHOP", PREMIUM_SHOP);
	PyModule_AddIntConstant(poModule, "PREMIUM_MAX_NUM", PREMIUM_MAX_NUM);
}
