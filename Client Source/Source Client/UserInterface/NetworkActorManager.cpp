...
CInstanceBase* CNetworkActorManager::__FindActor(SNetworkActorData& rkNetActorData, LONG lDstX, LONG lDstY)
{
	...
	if (!pkInstFind)
	{
		if (__IsVisiblePos(lDstX, lDstY, rkNetActorData.m_dwRace))
...
bool CNetworkActorManager::__IsVisibleActor(const SNetworkActorData& c_rkNetActorData)
{
	...
	if (c_rkNetActorData.m_kAffectFlags.IsSet(AFF_SHOW_ALWAYS))
		return true;

	if (__IsVisiblePos(c_rkNetActorData.m_lCurX, c_rkNetActorData.m_lCurY, c_rkNetActorData.m_dwRace))
		return true;
...
bool CNetworkActorManager::__IsVisiblePos(LONG lPosX, LONG lPosY, DWORD dwRace)
{
	...
	int max_distance = CHAR_STAGE_VIEW_BOUND;

	if (dwRace == 30000) // shop
	{
		max_distance = CPythonSystem::Instance().GetShopRangeView();
	}
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (pkInstFind)
	{
		...
		if (pkInstFind->GetLevel() != c_rkNetUpdateActorData.m_dwLevel)
		{
			pkInstFind->SetHideLevel(c_rkNetUpdateActorData.m_dwLevel == 0);
			pkInstFind->SetLevel(c_rkNetUpdateActorData.m_dwLevel);
			pkInstFind->UpdateTextTailLevel(pkInstFind->GetLevel());
		}
	}
