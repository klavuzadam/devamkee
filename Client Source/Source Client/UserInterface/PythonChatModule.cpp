...
PyObject* chatDestroyWhisperMap(PyObject* poSelf, PyObject* poArgs)
{
	CPythonChat::Instance().DestroyWhisperMap();
	return Py_BuildNone();
}
	...
	if (0 == results[0].compare("item"))
	{
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData))
		{
			...
			if (results.size() >= 8)
			{
				...
				{
					int real_index = (i - 6)/2;
					int type = htoi(results[i].c_str());
					int value = atoi(results[i + 1].c_str());
					len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%d",
						type,
						value);

					if (type > 0 && real_index < ITEM_ATTRIBUTE_NORM_NUM)
						isAttr = true;
				}
			}
...
void initChat()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "DestroyWhisperMap",		chatDestroyWhisperMap,			METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_TRADE", CHAT_TYPE_TRADE);
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_FANCY_NOTICE", CHAT_TYPE_FANCY_NOTICE);
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_GAMEMASTER_NOTICE", CHAT_TYPE_GAMEMASTER_NOTICE);
#ifdef ENABLE_DICE_SYSTEM
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_DICE_INFO",	CHAT_TYPE_DICE_INFO);
#endif
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_NORMAL", WHISPER_TYPE_NORMAL);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_SYSTEM", WHISPER_TYPE_SYSTEM);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_GM", WHISPER_TYPE_GM);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_TUTOR", WHISPER_TYPE_TUTOR);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_MYSHOP", WHISPER_TYPE_MYSHOP);
