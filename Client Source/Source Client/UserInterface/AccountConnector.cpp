...
bool CAccountConnector::Connect(const char * c_szAddr, int iPort, const char * c_szAccountAddr, int iAccountPort, DWORD dwClientVersion)
{
...
bool CAccountConnector::__AuthState_RecvPhase()
{
	...
	else if (kPacketPhase.phase == PHASE_AUTH)
	{
		...
		TPacketCGCheckVersion checkVersionPacket;
		checkVersionPacket.header = HEADER_CG_CHECK_VERSION;
		checkVersionPacket.dwVersion = m_dwClientVersion;
		if (!Send(sizeof(checkVersionPacket), &checkVersionPacket))
		{
			Tracen(" CAccountConnector::__AuthState_RecvPhase - CheckVersionPacket Error");
			return false;
		}
		...
		strncpy_s(LoginPacket.hwid, &GetHardwareID()[0], HWID_MAX_LEN);
