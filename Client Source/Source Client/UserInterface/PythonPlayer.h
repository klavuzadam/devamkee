...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	...
	public:
		...
		float GetBonusSP();
		float GetMagicDamageBonusFromSP();
		...
		// Money
		YANG	GetGold() { return m_llGold; }
		void	SetGold(YANG gold) { m_llGold = gold; }
		...
		void	SetItemCount(TItemPos Cell, ITEM_COUNT byCount);
		...
		ITEM_COUNT	GetItemCount(TItemPos Cell);
		ITEM_COUNT	GetItemCountByVnum(DWORD dwVnum);
		...
		// Affect
		void	AddAffect(DWORD affectType, DWORD affectDuration);
		bool	HasAffect(DWORD affectType); // not affect flag!!!
		...
		// Sprint
		void	SetSprintSkillFlag(BOOL bFlag);
	...
	protected:
		...
		bool	__CanUseSkill(DWORD dwSkillIndex=0);
		...
		bool	__CheckShortStamina(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData);
	...
	protected:
		...
		std::map<DWORD, DWORD>	m_affectDict;
		std::string				m_stName;
