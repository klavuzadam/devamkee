...
std::string g_strShopEffectName = "d:/ymir work/effect/etc/direction/offline_shop_mark.mse";
std::string g_strShopViewedEffectName = "d:/ymir work/effect/etc/direction/offline_shop_mark_viewed.mse";
...
bool CPythonBackground::SetShadowLevel(int eLevel)
{
	...
	switch (m_eShadowLevel)
	{
		...
		case SHADOW_ALL_MAX_4k:
			rkMap.SetDrawShadow(true);
			rkMap.SetDrawCharacterShadow(true);
			rkMap.SetShadowTextureSize(4096);
			break;
		case SHADOW_ALL_MAX_8k:
			rkMap.SetDrawShadow(true);
			rkMap.SetDrawCharacterShadow(true);
...
{
	std::string stAtlasInfoFileName("gamedata/AtlasInfo.txt");
...
void CPythonBackground::Update(float fCenterX, float fCenterY, float fCenterZ)
{
	...
	// SHOP Target Effect Process
	{
		std::map<DWORD, DWORD>::iterator itor = m_kMap_dwTargetID_dwShopID.begin();
		for (; itor != m_kMap_dwTargetID_dwShopID.end(); ++itor)
		{
			DWORD dwTargetID = itor->first;
			DWORD dwShopID = itor->second;

			auto pShopInst = CPythonIkarusShop::instance().GetShopInstancePtr(dwShopID);
			...
			auto kPixelPosition = pShopInst->GetPixelPos();

			auto effectName = g_strEffectName;
			if (dwTargetID >= OFFLINESHOP_START_TARGET_INDEX)
			{
				effectName = g_strShopEffectName;

				if (CPythonIkarusShop::instance().IsFoundShopFromSearchViewed(dwShopID))
					effectName = g_strShopViewedEffectName;
			}
...
void CPythonBackground::RenderCharacterShadowToTexture()
{
	...
	if (m_eShadowLevel == SHADOW_ALL ||
		...
		m_eShadowLevel == SHADOW_ALL_MAX_4k ||
		m_eShadowLevel == SHADOW_ALL_MAX_8k ||
...
void CPythonBackground::Warp(DWORD dwX, DWORD dwY)
{
	...
	if (!LoadMap(rMapInfo.m_strName, rMapInfo.m_strPath, float(dwX - rMapInfo.m_dwBaseX), float(dwY - rMapInfo.m_dwBaseY), 0))
	{
	...
	m_kMap_dwTargetID_dwShopID.clear();
...
void CPythonBackground::CreateShopTargetEffect(DWORD dwID, DWORD dwShopVID)
{
	m_kMap_dwTargetID_dwShopID.emplace(dwID, dwShopVID);
}
...
void CPythonBackground::CreateTargetEffect(DWORD dwID, long lx, long ly)
{
	...
	if (m_kMap_dwTargetID_dwShopID.end() != m_kMap_dwTargetID_dwShopID.find(dwID))
		return;
...
void CPythonBackground::DeleteTargetEffect(DWORD dwID)
{
	...
	if (m_kMap_dwTargetID_dwShopID.end() != m_kMap_dwTargetID_dwShopID.find(dwID))
	{
		m_kMap_dwTargetID_dwShopID.erase(dwID);
	}
