...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		{
			if (pkInstEach->IsHerb() || pkInstEach->IsChest())
				continue;
...
void CPythonMiniMap::CreateTarget(int iID, const char* c_szName, long x, long y)
{
	CreateTarget(iID, c_szName);
	UpdateTarget(iID, x, y);
}
...
{
	D3DXVECTOR2 oldScale = m_AtlasImageInstance.GetScale();
	...
	UpdateAtlasMarks(oldScale);
}
void CPythonMiniMap::UpdateAtlasMarks(D3DXVECTOR2 oldScale)
{
	float scaleX = m_AtlasImageInstance.GetScale().x / oldScale.x;
	float scaleY = m_AtlasImageInstance.GetScale().y / oldScale.y;
	m_AtlasMarkInfoVectorIterator = m_AtlasNPCInfoVector.begin();
	while (m_AtlasMarkInfoVectorIterator != m_AtlasNPCInfoVector.end())
	{
		TAtlasMarkInfo& rAtlasMarkInfo = *m_AtlasMarkInfoVectorIterator;
		rAtlasMarkInfo.m_fScreenX *= scaleX;
		rAtlasMarkInfo.m_fScreenY *= scaleY;
...
bool CPythonMiniMap::GetAtlasMaxSize(float* pfSizeX, float* pfSizeY)
{
	CPythonBackground& rkBG = CPythonBackground::Instance();
	if (!rkBG.IsMapOutdoor())
		return false;

	*pfSizeX = m_fAtlasMaxX;
	*pfSizeY = m_fAtlasMaxY;
