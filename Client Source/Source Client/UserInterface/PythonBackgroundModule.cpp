...
PyObject* backgroundGetMapSize(PyObject* poSelf, PyObject* poArgs)
{
	int iX;
	if (!PyTuple_GetInteger(poArgs, 0, &iX))
		return Py_BadArgument();
	...
	if (pkMapInfo)
		return Py_BuildValue("ii", pkMapInfo->m_dwSizeX, pkMapInfo->m_dwSizeY);
	else
...
void initBackground()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetMapSize",							backgroundGetMapSize,						METH_VARARGS },
