...
const D3DXCOLOR c_TextTail_AttrItem_Color = D3DXCOLOR(1.0f, 0.53f, 0.0f, 1.0f);
...
CGraphicText* ms_pLargeFont = NULL;
...
void CPythonTextTail::UpdateTextTail(TTextTail * pTextTail)
{
	...
	auto scale = CPythonSystem::instance().GetScaleInterface();
	pTextTail->x = floorf(pTextTail->x / scale);
	pTextTail->y = floorf(pTextTail->y / scale);
...
void CPythonTextTail::AddToGroupedTextTailMap(std::unordered_map<std::string, GroupedTextTail>& groupedTextTailMap, TTextTailMap textTailMap, bool isCharacterTextTail)
{
	const bool isGroupShowName = CPythonSystem::instance().IsGroupShowName();

	TTextTailMap::iterator itor;
	for (itor = textTailMap.begin(); itor != textTailMap.end(); ++itor)
	{
		TTextTail* pTextTail = itor->second;
		pTextTail->groupCount = 1;
		if (pTextTail->fDistanceFromPlayer >= 3500.0f)
			continue;

		std::string characterName(pTextTail->pTextInstance->GetValueString());
		auto groupedIt = groupedTextTailMap.find(characterName);
		if (groupedIt != groupedTextTailMap.end())
		{
			auto& groupList = groupedIt->second;
			bool addNewGroup = !isGroupShowName;
			if (isGroupShowName)
			{
				for (const auto& groupedTextTail : groupList.textTailList) {
					const D3DXVECTOR3& groupedTextTailOwnerPosition = groupedTextTail->pOwner->GetPosition();
					const D3DXVECTOR3& pTextTailOwnerPosition = pTextTail->pOwner->GetPosition();

					D3DXVECTOR2 v2Distance(groupedTextTailOwnerPosition.x - pTextTailOwnerPosition.x, groupedTextTailOwnerPosition.y - pTextTailOwnerPosition.y);
					float distance = D3DXVec2Length(&v2Distance);
					//Tracenf("cze %f", distance);
					if (distance < (isCharacterTextTail ? 300.0f : 200.0f)) {
						groupedTextTail->groupCount++;
						addNewGroup = false;
						break;
					}
					addNewGroup = true;
			...
			if (addNewGroup) {
				groupList.textTailList.push_back(pTextTail);
			}
		}
		else
		{

			groupedTextTailMap[characterName] = { isCharacterTextTail, std::vector<TTextTail*>() };
			groupedTextTailMap[characterName].textTailList.push_back(pTextTail);
		}
	}
}
...
std::string GetCharacterTextTailFontName(int groupCount)
{
	std::string fontName(GetDefaultFontNameOnly());
	int fontSize = GetDefaultFontSizeNumber();
	...
	std::stringstream formattedFontName;
	formattedFontName << fontName << ":" << fontSize << ".fnt";
	return formattedFontName.str();
}
...
void CPythonTextTail::ShowAllTextTail()
{
	...
	std::unordered_map<std::string, GroupedTextTail> groupedTextTailMap;

	AddToGroupedTextTailMap(groupedTextTailMap, m_CharacterTextTailMap, true);
	AddToGroupedTextTailMap(groupedTextTailMap, m_ItemTextTailMap, false);

	for (const auto& pair : groupedTextTailMap) {
		auto groupedTextTail = pair.second;
		for (const auto& textTail : groupedTextTail.textTailList) {

			std::string fontName;

			if (textTail->pTextInstance->IsLarge())
				fontName = GetLargeFontName();
			else if (groupedTextTail.isCharacterTextTail)
				fontName = GetCharacterTextTailFontName(textTail->groupCount);
			else
				fontName = GetDefaultFontName();

			std::string name("");
			if (textTail->groupCount > 1)
				name = std::format("{} ({})", textTail->pTextInstance->GetValueString(), textTail->groupCount);

			textTail->pTextInstance->SetFormattedValue(name.c_str());

			if (textTail->groupFontName != fontName)
				textTail->pTextInstance->SetUpdate(false);

			if (!textTail->pTextInstance->IsUpdate())
			{
				CResource* pResource = CResourceManager::Instance().GetResourcePointer(fontName.c_str());
				textTail->pTextInstance->SetTextPointer(static_cast<CGraphicText*>(pResource));
				textTail->pTextInstance->Update();
				textTail->groupFontName = fontName;
			}

			if (groupedTextTail.isCharacterTextTail)
				ShowCharacterTextTail(textTail->dwVirtualID);
			else
				ShowItemTextTail(textTail->dwVirtualID);
		}
	}

	groupedTextTailMap.clear();
}
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight, bool bLarge)
{
	...
	if (0 != dwGuildID)
	{
		...
		prGuildNameInstance->SetTextPointer(bLarge ? ms_pLargeFont : ms_pFont);
...
void CPythonTextTail::RegisterItemTextTail(DWORD VirtualID, const char * c_szText, CGraphicObjectInstance * pOwner, ITEM_COUNT count, bool bIsAttr)
{
#ifdef __DEBUG
	char szName[256];
	...
	char szName[256];
	if (count > 1)
		sprintf(szName, "%dx %s", count, c_szText);
	else
		sprintf(szName, "%s", c_szText);

	TTextTail * pTextTail = RegisterTextTail(VirtualID, szName, pOwner, c_TextTail_Name_Position, bIsAttr ? c_TextTail_AttrItem_Color : c_TextTail_Item_Color);
	m_ItemTextTailMap.insert(TTextTailMap::value_type(VirtualID, pTextTail));
#endif
}
...
void CPythonTextTail::SetItemTextTailOwner(DWORD dwVID, const char * c_szName)
{
	...
	if (strlen(c_szName) > 0)
	{
		...
		bool isPlayerOwner = strcmp(c_szName, CPythonCharacterManager::instance().GetMainInstancePtr()->GetNameString()) == 0;
...
CPythonTextTail::TTextTail * CPythonTextTail::RegisterTextTail(DWORD dwVirtualID, const char * c_szText, CGraphicObjectInstance * pOwner, float fHeight, const D3DXCOLOR & c_rColor, bool bLarge)
{
	...
	pTextTail->pTextInstance->SetTextPointer(bLarge ? ms_pLargeFont : ms_pFont);
	pTextTail->pTextInstance->SetLarge(bLarge);
...
void CPythonTextTail::Initialize()
{
	...
	CGraphicText* pkLargeFont = static_cast<CGraphicText*>(LargeFont_GetResource());
	if (!pkLargeFont)
	{
		TraceError("CPythonTextTail::Initialize - CANNOT_FIND_LARGE_FONT");
		return;
	}
	ms_pLargeFont = pkLargeFont;
}
