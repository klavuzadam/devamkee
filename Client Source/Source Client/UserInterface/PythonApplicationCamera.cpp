...
void CPythonApplication::SetCamera(float Distance, float Pitch, float Rotation, float fDestinationHeight)
{
	...
	if (m_iCameraMode != CAMERA_MODE_FREE) {
		CCamera* pMainCamera = CCameraManager::Instance().GetCurrentCamera();
		if (pMainCamera)
...
void CPythonApplication::SetCamera(float x, float y, float z, float Distance, float Pitch, float Rotation)
{
	if (IsLockCurrentCamera())
		return;
	...
	D3DXVECTOR3 v3Target = pCurrentCamera->GetTarget();
	m_pyGraphic.SetPositionCamera(x, y, z, Distance, Pitch, Rotation);
...
void CPythonApplication::SetCameraFOV(float fov)
{
	m_cameraFov = fov;
}
...
void CPythonApplication::SetFreeCamera()
{
	...
	pCamera->SetTerrainCollision(false);
