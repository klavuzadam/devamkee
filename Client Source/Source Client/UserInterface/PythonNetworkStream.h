...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		void PingPort(const char* c_szAddr, UINT uPort);
		...
		bool SendItemMovePacket(TItemPos pos, TItemPos change_pos, ITEM_COUNT num);
		...
		bool SendGuildWithdrawMoneyPacket(YANG dwMoney);
		...
		void SendIkarusFindMyShop();
		void SendIkarusSearchItem(DWORD itemVnum, int socket0);
		void SendIkarusRequestEdit(bool state);
#ifdef EXTEND_IKASHOP_PRO
...
#endif
		bool RecvOfflineshopNotifyItemSold();
		bool RecvOfflineshopSearchItem();

		void AppendIkarusShopAdBoard(DWORD dwVID, const char * szShopName);
		void RemoveIkarusShopAdBoard(DWORD dwVID);
		void ToggleIkarusShopAdBoardVisibility(DWORD dwVID, bool state);

		void SendIkarusShopReopenShop(const char * name, BYTE bTimeIndex);
		...
		void SendIkarusShopRemoveAllItem();
		...
		void SendIkarusShopChangeName(const char* stName);
		...
		// Private Shop
		bool SendBuildPrivateShopPacket(const char * c_szName, BYTE bTimeIndex, const std::vector<TShopItemTable> & c_rSellingItemStock);
	...
	public:
		...
		bool SendMessengerBlockPacket(const char* c_szName);
		bool SendMessengerUnblockPacket(const char* c_szName);
	...
	protected:
		...
		bool RecvAddFlyShootTargetingPacket();
		bool RecvClearAddFlyShootTargetingPacket();
		...
		bool RecvAddItemShop();
		bool RecvSpecialShop();
		bool RecvNpcInfoBoard();
		bool RecvBusyAction();
		bool RecvRequestInfo();
		bool RecvGamemasterTarget();
		bool RecvCaptcha();
