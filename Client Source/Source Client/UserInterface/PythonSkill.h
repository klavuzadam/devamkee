...
class CPythonSkill : public CSingleton<CPythonSkill>
{
	public:
		...
		enum
		{
			...
			SKILL_ATTRIBUTE_USE_STAMINA				= (1 << 25),
		};
		...
		typedef struct SSkillData
		{
			...
			float ProcessFormula(CPoly * pPoly, float fSkillLevel = 0.0f, int iMinMaxType = VALUE_TYPE_FREE, int skillLevel = 0);
			bool CheckIfSkillIsMagic();
			const char * GetAffectDescription(DWORD dwIndex, float fSkillLevel, int skillLevel = 0);
