...
CPythonSkill::CPythonSkill()
{
	...
	m_SkillAttributeIndexMap.emplace("USE_STAMINA", SKILL_ATTRIBUTE_USE_STAMINA);
	...
	m_SkillWeaponTypeIndexMap.emplace("QUIVER",		WEAPON_QUIVER);
#endif
...
BOOL CPythonSkill::SSkillData::IsUseStaminaSkill()
{
	return 0 != (dwSkillAttribute & SKILL_ATTRIBUTE_USE_STAMINA);
}
...
float CPythonSkill::SSkillData::ProcessFormula(CPoly * pPoly, float fSkillLevel, int iMinMaxType, int skillLevel)
{
	if (pPoly->Analyze())
	{
		for (DWORD i = 0; i < pPoly->GetVarCount(); ++i)
		{
			...
			else if (!strcmp("sl", c_szVarName))
			{
				fState = skillLevel;
			}
			else if (!strcmp("isgraden", c_szVarName))
			{
				fState = skillLevel < 20 ? 1 : 0;
			}
			else if (!strcmp("isgradem", c_szVarName))
			{
				fState = skillLevel >= 20 && skillLevel < 30 ? 1 : 0;
			}
			else if (!strcmp("isgradeg", c_szVarName))
			{
				fState = skillLevel >= 30 && skillLevel < 40 ? 1 : 0;
			}
			else if (!strcmp("isgradep", c_szVarName))
			{
				fState = skillLevel >= 40 ? 1 : 0;
			}
			else if (!strcmp("gr", c_szVarName))
			{
				if (skillLevel < 20)
					fState = 0;
...
bool CPythonSkill::SSkillData::CheckIfSkillIsMagic() {
	const std::set<int> skillSet = {
		SKILL_POISON_CLOUD,
		SKILL_DARK_STRIKE,
		SKILL_FLAME_STRIKE,
		SKILL_FLAME_SPIRIT,
		SKILL_SPIRIT_ORB,
		SKILL_DARK_ORB,
		SKILL_FLYING_TALISMAN,
		SKILL_SHOOTING_DRAGON,
		SKILL_DRAGON_ROAR,
		SKILL_LIGHTNING_THROW,
		SKILL_SUMMON_LIGHTNING,
		SKILL_LIGHTNING_CLAW,
	};
	if (skillSet.find(this->dwSkillIndex) != skillSet.end()) {
		return true;
	}
	else {
		return false;
...
const char * CPythonSkill::SSkillData::GetAffectDescription(DWORD dwIndex, float fSkillLevel, int skillLevel)
{
	...
	// OVERWRITE_SKILLPROTO_POLY
	float fMinValue = ProcessFormula(&minPoly, fSkillLevel, VALUE_TYPE_FREE, skillLevel);
	float fMaxValue = ProcessFormula(&maxPoly, fSkillLevel, VALUE_TYPE_FREE, skillLevel);
	...
	else
	{
		...
		{
			if (CheckIfSkillIsMagic())
			{
				float bonus;
				if (this->dwSkillIndex == SKILL_FLAME_SPIRIT) {
					bonus = (1 + (CPythonPlayer::Instance().GetMagicDamageBonusFromSP() + CPythonPlayer::Instance().GetStatus(POINT_MAGIC_ATT)) / 200);
				}
				else {
					bonus = (1 + (CPythonPlayer::Instance().GetMagicDamageBonusFromSP() + CPythonPlayer::Instance().GetStatus(POINT_MAGIC_ATT)) / 100);
				}
				fMinValue *= bonus;
				fMaxValue *= bonus;
			}
...
PyObject * skillGetSkillAffectDescription(PyObject * poSelf, PyObject * poArgs)
{
	...
	int skillLevel;
	if (!PyTuple_GetInteger(poArgs, 2, &skillLevel))
		return Py_BadArgument();

	float fSkillPoint;
	if (!PyTuple_GetFloat(poArgs, 3, &fSkillPoint))
		return Py_BadArgument();
...
PyObject* skillIsUseStaminaSkill(PyObject* poSelf, PyObject* poArgs)
{
	int iSkillIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
		return Py_BadArgument();
	...
	CPythonSkill::SSkillData* c_pSkillData;
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
		return Py_BuildException("skill.IsUseStaminaSkill - Failed to find skill by %d", iSkillIndex);

	return Py_BuildValue("i", c_pSkillData->IsUseStaminaSkill());
}
...
void initskill()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsUseStaminaSkill",						skillIsUseStaminaSkill,						METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "SKILL_THREE_WAY_CUT", SKILL_THREE_WAY_CUT);
	PyModule_AddIntConstant(poModule, "SKILL_SWORD_SPIN", SKILL_SWORD_SPIN);
	PyModule_AddIntConstant(poModule, "SKILL_BERSERK", SKILL_BERSERK);
	PyModule_AddIntConstant(poModule, "SKILL_SWORD_AURA", SKILL_SWORD_AURA);
	PyModule_AddIntConstant(poModule, "SKILL_CHARGE", SKILL_CHARGE);
	PyModule_AddIntConstant(poModule, "SKILL_SPIRIT_STRIKE", SKILL_SPIRIT_STRIKE);
	PyModule_AddIntConstant(poModule, "SKILL_BASH", SKILL_BASH);
	PyModule_AddIntConstant(poModule, "SKILL_STUMP", SKILL_STUMP);
	PyModule_AddIntConstant(poModule, "SKILL_STRONG_BODY", SKILL_STRONG_BODY);
	PyModule_AddIntConstant(poModule, "SKILL_SWORD_STRIKE", SKILL_SWORD_STRIKE);
	PyModule_AddIntConstant(poModule, "SKILL_AMBUSH", SKILL_AMBUSH);
	PyModule_AddIntConstant(poModule, "SKILL_FAST_ATTACK", SKILL_FAST_ATTACK);
	PyModule_AddIntConstant(poModule, "SKILL_ROLLING_DAGGER", SKILL_ROLLING_DAGGER);
	PyModule_AddIntConstant(poModule, "SKILL_STEALTH", SKILL_STEALTH);
	PyModule_AddIntConstant(poModule, "SKILL_POISON_CLOUD", SKILL_POISON_CLOUD);
	PyModule_AddIntConstant(poModule, "SKILL_REPETITIVE_SHOT", SKILL_REPETITIVE_SHOT);
	PyModule_AddIntConstant(poModule, "SKILL_ARROW_SHOWER", SKILL_ARROW_SHOWER);
	PyModule_AddIntConstant(poModule, "SKILL_SPARK", SKILL_SPARK);
	PyModule_AddIntConstant(poModule, "SKILL_FIRE_ARROW", SKILL_FIRE_ARROW);
	PyModule_AddIntConstant(poModule, "SKILL_FEATHER_WALK", SKILL_FEATHER_WALK);
	PyModule_AddIntConstant(poModule, "SKILL_POISON_ARROW", SKILL_POISON_ARROW);
	PyModule_AddIntConstant(poModule, "SKILL_FINGER_STRIKE", SKILL_FINGER_STRIKE);
	PyModule_AddIntConstant(poModule, "SKILL_DRAGON_SWIRL", SKILL_DRAGON_SWIRL);
	PyModule_AddIntConstant(poModule, "SKILL_ENCHANTED_BLADE", SKILL_ENCHANTED_BLADE);
	PyModule_AddIntConstant(poModule, "SKILL_TERROR", SKILL_TERROR);
	PyModule_AddIntConstant(poModule, "SKILL_ENCHANTED_ARMOR", SKILL_ENCHANTED_ARMOR);
	PyModule_AddIntConstant(poModule, "SKILL_DISPEL", SKILL_DISPEL);
	PyModule_AddIntConstant(poModule, "SKILL_DARK_STRIKE", SKILL_DARK_STRIKE);
	PyModule_AddIntConstant(poModule, "SKILL_FLAME_STRIKE", SKILL_FLAME_STRIKE);
	PyModule_AddIntConstant(poModule, "SKILL_FLAME_SPIRIT", SKILL_FLAME_SPIRIT);
	PyModule_AddIntConstant(poModule, "SKILL_MANASHIELD", SKILL_MANASHIELD);
	PyModule_AddIntConstant(poModule, "SKILL_SPIRIT_ORB", SKILL_SPIRIT_ORB);
	PyModule_AddIntConstant(poModule, "SKILL_DARK_ORB", SKILL_DARK_ORB);
	PyModule_AddIntConstant(poModule, "SKILL_FLYING_TALISMAN", SKILL_FLYING_TALISMAN);
	PyModule_AddIntConstant(poModule, "SKILL_SHOOTING_DRAGON", SKILL_SHOOTING_DRAGON);
	PyModule_AddIntConstant(poModule, "SKILL_DRAGON_ROAR", SKILL_DRAGON_ROAR);
	PyModule_AddIntConstant(poModule, "SKILL_BLESSING", SKILL_BLESSING);
	PyModule_AddIntConstant(poModule, "SKILL_REFLECT", SKILL_REFLECT);
	PyModule_AddIntConstant(poModule, "SKILL_DRAGON_HELP", SKILL_DRAGON_HELP);
	PyModule_AddIntConstant(poModule, "SKILL_LIGHTNING_THROW", SKILL_LIGHTNING_THROW);
	PyModule_AddIntConstant(poModule, "SKILL_SUMMON_LIGHTNING", SKILL_SUMMON_LIGHTNING);
	PyModule_AddIntConstant(poModule, "SKILL_LIGHTNING_CLAW", SKILL_LIGHTNING_CLAW);
	PyModule_AddIntConstant(poModule, "SKILL_CURE", SKILL_CURE);
	PyModule_AddIntConstant(poModule, "SKILL_SWIFTNESS", SKILL_SWIFTNESS);
	PyModule_AddIntConstant(poModule, "SKILL_ATTACK_UP", SKILL_ATTACK_UP);
	PyModule_AddIntConstant(poModule, "SKILL_PLAYER_MAX", SKILL_PLAYER_MAX);

	PyModule_AddIntConstant(poModule, "SKILL_7_A_ANTI_TANHWAN", SKILL_7_A_ANTI_TANHWAN);
	PyModule_AddIntConstant(poModule, "SKILL_7_B_ANTI_AMSEOP", SKILL_7_B_ANTI_AMSEOP);
	PyModule_AddIntConstant(poModule, "SKILL_7_C_ANTI_SWAERYUNG", SKILL_7_C_ANTI_SWAERYUNG);
	PyModule_AddIntConstant(poModule, "SKILL_7_D_ANTI_YONGBI", SKILL_7_D_ANTI_YONGBI);
	PyModule_AddIntConstant(poModule, "SKILL_8_A_ANTI_GIGONGCHAM", SKILL_8_A_ANTI_GIGONGCHAM);
	PyModule_AddIntConstant(poModule, "SKILL_8_B_ANTI_YEONSA", SKILL_8_B_ANTI_YEONSA);
	PyModule_AddIntConstant(poModule, "SKILL_8_C_ANTI_MAHWAN", SKILL_8_C_ANTI_MAHWAN);
	PyModule_AddIntConstant(poModule, "SKILL_8_D_ANTI_BYEURAK", SKILL_8_D_ANTI_BYEURAK);

	PyModule_AddIntConstant(poModule, "SKILL_LEADERSHIP", SKILL_LEADERSHIP);
	PyModule_AddIntConstant(poModule, "SKILL_COMBO", SKILL_COMBO);
	PyModule_AddIntConstant(poModule, "SKILL_CREATE", SKILL_CREATE);
	PyModule_AddIntConstant(poModule, "SKILL_MINING", SKILL_MINING);
	PyModule_AddIntConstant(poModule, "SKILL_LANGUAGE1", SKILL_LANGUAGE1);
	PyModule_AddIntConstant(poModule, "SKILL_LANGUAGE2", SKILL_LANGUAGE2);
	PyModule_AddIntConstant(poModule, "SKILL_LANGUAGE3", SKILL_LANGUAGE3);
	PyModule_AddIntConstant(poModule, "SKILL_POLYMORPH", SKILL_POLYMORPH);
	PyModule_AddIntConstant(poModule, "SKILL_SPRINT", SKILL_SPRINT);
	PyModule_AddIntConstant(poModule, "SKILL_HORSE", SKILL_HORSE);
	PyModule_AddIntConstant(poModule, "SKILL_HORSE_SUMMON", SKILL_HORSE_SUMMON);
	PyModule_AddIntConstant(poModule, "SKILL_HORSE_WILDATTACK", SKILL_HORSE_WILDATTACK);
	PyModule_AddIntConstant(poModule, "SKILL_HORSE_CHARGE", SKILL_HORSE_CHARGE);
	PyModule_AddIntConstant(poModule, "SKILL_HORSE_ESCAPE", SKILL_HORSE_ESCAPE);
	PyModule_AddIntConstant(poModule, "SKILL_HORSE_WILDATTACK_RANGE", SKILL_HORSE_WILDATTACK_RANGE);
	PyModule_AddIntConstant(poModule, "SKILL_ADD_HP", SKILL_ADD_HP);
	PyModule_AddIntConstant(poModule, "SKILL_RESIST_PENETRATE", SKILL_RESIST_PENETRATE);
	PyModule_AddIntConstant(poModule, "SKILL_PASSIVE_MAX", SKILL_PASSIVE_MAX);
}
