...
DWORD CPythonPlayer::__GetStatAtk()
{
	switch (GetRace())
	{
		case MAIN_RACE_WARRIOR_M:
		case MAIN_RACE_WARRIOR_W:
			return GetStatus(POINT_ST) * 2;
			break;
		case MAIN_RACE_ASSASSIN_M:
		case MAIN_RACE_ASSASSIN_W:
			return GetStatus(POINT_DX) + GetStatus(POINT_ST);
			break;
		case MAIN_RACE_SURA_M:
		case MAIN_RACE_SURA_W:
			return GetStatus(POINT_ST) +  GetStatus(POINT_IQ);
			break;
		case MAIN_RACE_SHAMAN_M:
		case MAIN_RACE_SHAMAN_W:
			return (5 * GetStatus(POINT_IQ) + GetStatus(POINT_ST)) / 3;
			break;
	#ifdef ENABLE_WOLFMAN_CHARACTER
		case MAIN_RACE_WOLFMAN_M:
			return GetStatus(POINT_DX);
...
{
	int level = GetStatus(POINT_LEVEL);
	int src = (GetStatus(POINT_DX) * 4 + level * 2) / 6;
...
ITEM_COUNT CPythonPlayer::GetItemCount(TItemPos Cell)
{
	if (!Cell.IsValidItemPosition())
		return 0;
	const TItemData * pItem = GetItemData(Cell);
...
ITEM_COUNT CPythonPlayer::GetItemCountByVnum(DWORD dwVnum)
{
	DWORD dwCount = 0;
...
void CPythonPlayer::SetItemCount(TItemPos Cell, ITEM_COUNT byCount)
{
	if (!Cell.IsValidItemPosition())
		return;
...
void CPythonPlayer::SendClickItemPacket(DWORD dwIID)
{
	...
	{
#ifndef ENABLE_NO_PICKUP_LIMIT
		s_dwNextTCPTime=dwCurTime + (HasAffect(AFFECT_FAST_PICKUP) ? 100 : 500);
#endif
...
size_t CPythonPlayer::GetPartyMemberCount()
{
	return m_PartyMemberMap.size();
}
DWORD CPythonPlayer::GetPartyMemberPIDByIndex(size_t index)
{
	if (index >= m_PartyMemberMap.size())
		return 0;
...
void CPythonPlayer::AddAffect(DWORD affectType, DWORD affectDuration)
{
	m_affectDict[affectType] = affectDuration;
}

bool CPythonPlayer::HasAffect(DWORD affectType)
{
	auto it = m_affectDict.find(affectType);
	if (it == m_affectDict.end())
		return false;

	return CPythonApplication::Instance().GetServerTimeStamp() <= it->second;
}
...
float CPythonPlayer::GetMagicDamageBonusFromSP()
{
	float bonusManaPow = pow(GetBonusSP(), 1.22);
	return (55.0 * bonusManaPow / (bonusManaPow + 7000));
}

float CPythonPlayer::GetBonusSP()
{
	float maxMana = float(CPythonPlayer::Instance().GetStatus(POINT_MAX_SP));
	return MAX((maxMana - 180 - 20 * (CPythonPlayer::Instance().GetStatus(POINT_IQ) + CPythonPlayer::Instance().GetStatus(POINT_LEVEL))), 0);
}
...
CPythonPlayer::CPythonPlayer(void)
{
	...
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_BERSERK), SKILL_BERSERK));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_SWORD_AURA), SKILL_SWORD_AURA));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_STRONG_BODY), SKILL_STRONG_BODY));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_FEATHER_WALK), SKILL_FEATHER_WALK));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_STEALTH), SKILL_STEALTH));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_TERROR), SKILL_TERROR));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_ENCHANTED_ARMOR), SKILL_ENCHANTED_ARMOR));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_BLESSING), SKILL_BLESSING));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_REFLECT), SKILL_REFLECT));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_SWIFTNESS), SKILL_SWIFTNESS));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_DRAGON_HELP), SKILL_DRAGON_HELP));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_ATTACK_UP), SKILL_ATTACK_UP));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_DISPEL), SKILL_DISPEL));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_STRONG_BODY_WITH_FALL), SKILL_STRONG_BODY));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_HEAVEN_PROTECTION), SKILL_CURE));
	/////
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_ENCHANTED_BLADE), SKILL_ENCHANTED_BLADE));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_FLAME_SPIRIT), SKILL_FLAME_SPIRIT));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(AFF_SKILL_MANASHIELD), SKILL_MANASHIELD));
