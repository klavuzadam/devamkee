...
//		return false;
	...
	else
	{
		...
		if (__CheckShortStamina(rkSkillInst, *pSkillData))
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_ENOUGH_ST"));
			return false;
		}
...
bool CPythonPlayer::__CheckShortStamina(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData)
{
	if (!rkSkillData.IsUseStaminaSkill())
		return false;

	DWORD dwNeedStamina = rkSkillData.GetNeedSP(rkSkillInst.fcurEfficientPercentage);
	if (dwNeedStamina <= GetStatus(POINT_STAMINA))
		return false;
...
{
	return rkInstMain.IsAffect(AFF_CHARGE_DASH);
}
...
{
	DWORD range = rkSkillData.GetTargetRange();
	//if (rkSkillData.dwSkillIndex >= SKILL_ARCHER_START && rkSkillData.dwSkillIndex <= SKILL_ARCHER_END)
	//	range += GetStatus(POINT_BOW_DISTANCE)*100;
	return range;
}
...
bool CPythonPlayer::__CanUseSkill(DWORD dwSkillIndex)
{
	...
	// Fix me
	if (pkInstMain->IsMountingHorse() && (GetSkillGrade(109) < 1 && GetSkillLevel(109) < 20) && dwSkillIndex != SKILL_SPRINT)
	{
		return false;
	}
...
bool CPythonPlayer::__UseSkill(DWORD dwSlotIndex)
{
	...
	if (pSkillData->CanChangeDirection())
	{
		...
		if (!pSkillData->IsOnlyForAlliance() ||
			(pkInstTarget && pSkillData->IsOnlyForAlliance() &&
				(!pkInstMain ||
					!pkInstTarget->IsNPC() ||
					!pkInstTarget->IsEnemy() ||
					!pkInstTarget->IsStone() ||
...
bool CPythonPlayer::__HasEnoughArrow()
{
	...
	if (CItemManager::Instance().GetItemDataPointer(GetItemIndex(TItemPos(INVENTORY, c_Equipment_Arrow)), &pItemData)) {
		if (ITEM_WEAPON == pItemData->GetType()) {
			if (WEAPON_ARROW == pItemData->GetSubType()
				#ifdef ENABLE_QUIVER_SYSTEM
				|| WEAPON_QUIVER == pItemData->GetSubType()
				#endif
			) {
				return true;
			}
...
void CPythonPlayer::UseGuildSkill(DWORD dwSkillSlotIndex)
{
	...
	if (pSkillData->IsOnlyForGuildWar())
	{
		...
		auto mapName = rkBG.GetWarpMapName();
		Tracenf(mapName);
		if (strcmp(mapName, "metin2_map_t3") != 0 && strcmp(mapName, "metin2_map_t4") != 0)
		{
			Tracenf("blokada!");
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "ONLY_FOR_GUILD_WAR"));
			return;
...
void CPythonPlayer::SetSprintSkillFlag(BOOL bFlag)
{
	DWORD dwSlotIndex;
	if (!GetSkillSlotIndex(SKILL_SPRINT, &dwSlotIndex))
	{
		Tracef("CPythonPlayer::SetSprintSkillFlag(killIndex=%d) - Can't Find Slot Index\n", SKILL_SPRINT);
		return;
	}
...
bool CPythonPlayer::__CheckSpecialSkill(DWORD dwSkillIndex)
{
	...
	// sprint
	else if (SKILL_SPRINT == dwSkillIndex)
	{
		...
		if (!pkInstMain->IsAffect(AFF_SPRINT) && __CheckShortStamina(rkSkillInst, *pSkillData))
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NOT_ENOUGH_ST"));
			return true;
		}
