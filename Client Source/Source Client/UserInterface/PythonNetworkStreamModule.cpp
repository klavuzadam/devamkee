...
PyObject* netConnectToAccountServer(PyObject* poSelf, PyObject* poArgs)
{
	...
	int client_version;
	if (!PyTuple_GetInteger(poArgs, 4, &client_version))
		return Py_BuildException();

	CAccountConnector & rkAccountConnector = CAccountConnector::Instance();
	rkAccountConnector.Connect(addr, port, account_addr, account_port, client_version);
	return Py_BuildNone();
}
...
PyObject* netSendSafeboxItemMovePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxItemMovePacket(iSourcePos, iTargetPos);
...
PyObject* netSendMessengerBlockPacket(PyObject* poSelf, PyObject* poArgs)
{
	char* szName;
	if (!PyTuple_GetString(poArgs, 0, &szName))
		return Py_BuildException();
	...
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendMessengerBlockPacket(szName);

	return Py_BuildNone();
}

PyObject* netSendMessengerUnblockPacket(PyObject* poSelf, PyObject* poArgs)
{
	char* szName;
	if (!PyTuple_GetString(poArgs, 0, &szName))
		return Py_BuildException();
	...
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendMessengerUnblockPacket(szName);

	return Py_BuildNone();
}

PyObject* netSendPingPort(PyObject* poSelf, PyObject* poArgs)
{
	char* szHost;
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendPingPort",						netSendPingPort,						METH_VARARGS },
		...
		{ "SendMessengerBlockPacket",			netSendMessengerBlockPacket,			METH_VARARGS },
		{ "SendMessengerUnblockPacket",			netSendMessengerUnblockPacket,			METH_VARARGS },
